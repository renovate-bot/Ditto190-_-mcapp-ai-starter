...
if not AWESOME_COPILOT_PATH.exists():
    _fallback = (
        Path(__file__).resolve().parents[2] / "consolidated_sources" / "awesome-copilot"
    )
    if _fallback.exists():
...
def test_parse_agents_from_awesome_copilot():
    ...
    assert Path(
        agent["file_path"]
    ).exists(), f"Agent file should exist: {agent['file_path']}"
...
def test_parse_skills_from_awesome_copilot():
    ...
    assert Path(
        skill["file_path"]
    ).exists(), f"Skill file should exist: {skill['file_path']}"
...
def test_generate_agentspec_from_real_data():
    ...
    print(
        f"✓ Generated AgentSpec with {result['agent_count']} agents and {result['skill_count']} skills"
    )
...
def test_validate_valid_agentspec():
    ...
    ), f"Should be valid. Errors: {validation['errors']}"
...
def test_roundtrip_generate_save_load_validate(tmp_path: Path):
    ...
    print(
        f"✓ Round-trip complete: {result['agent_count']} agents, {result['skill_count']} skills"
    )
...
def test_agentspec_schema_compliance():
    ...
    for agent_id, agent in spec["agents"].items():
        ...
        assert Path(
            agent["source_file"]
        ).exists(), f"Agent source file should exist: {agent['source_file']}"
    ...
    for tool_id, tool in spec["tools"].items():
        ...
        assert Path(
            tool["source_file"]
        ).exists(), f"Tool source file should exist: {tool['source_file']}"

    print(
        f"✓ Schema compliance verified for {len(spec['agents'])} agents and {len(spec['tools'])} tools"
    )
