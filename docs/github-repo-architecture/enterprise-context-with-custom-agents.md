source: <https://wellarchitected.github.com/library/architecture/recommendations/expanding-enterprise-custom-agents-context/>
...
Provide a descriptive note (e.g., “Custom agents MCP access”)
Select the repo scope (full control of private repositories)
Set an appropriate expiration date based on your organization’s security policies
...
Navigate to the repository Settings > Environments
Create a new environment named copilot (if it doesn’t exist)
Click Add secret under environment secrets
Name the secret: COPILOT_MCP_GITHUB_PERSONAL_ACCESS_TOKEN
Paste the PAT value generated in the previous step
Click Add secret
...
Navigate to a repository where you configured the copilot environment secret
Assign a task to your custom agent using the GitHub Copilot interface
Monitor the agent’s activity in the agent panel
Look for log messages indicating “Get file contents from GitHub” or similar
Verify the agent’s responses reflect the guidance from your knowledge files
