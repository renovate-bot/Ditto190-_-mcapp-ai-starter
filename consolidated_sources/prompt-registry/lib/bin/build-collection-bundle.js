...
function runNodeScript(scriptPath, args, cwd) {
  ...
  if (res.status !== 0) {
    throw new Error(
      res.stderr || res.stdout || `Command failed: node ${scriptPath}`,
    );
  }
  return res;
}
...
async function main() {
  ...
  const repoSlug =
    args.repoSlug || (process.env.GITHUB_REPOSITORY || "").replace(/\//g, "-");
  if (!repoSlug)
    throw new Error("Missing --repo-slug (or set GITHUB_REPOSITORY)");
