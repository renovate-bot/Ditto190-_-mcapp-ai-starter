...
async function runInteractive(args) {
  ...
  if (result.success) {
    ...
    console.log("\nNext steps:");
    console.log(
      `  1. Edit ${args.skillsDir}/${name}/SKILL.md to add detailed instructions`,
    );
...
function runNonInteractive(args) {
  if (!args.skillName) {
    console.error(
      "Usage: create-skill <skill-name> [--description <desc>] [--skills-dir <dir>]",
    );
    console.error("");
  ...
  if (!args.description) {
    console.error(
      "❌ Description is required in non-interactive mode. Use --description <desc>",
    );
    process.exit(1);
  }
