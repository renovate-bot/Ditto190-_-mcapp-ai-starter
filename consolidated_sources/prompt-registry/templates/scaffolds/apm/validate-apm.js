...
function validateManifest(filePath) {
  ...
  try {
    ...
    return {
      valid: false,
      errors: [`Failed to parse apm.yml: ${e.message}`],
      warnings: [],
    };
  }
}
...
function validatePrompts(projectRoot) {
  ...
  files.forEach((file) => {
    ...
    if (!hasValidExt) {
      warnings.push(
        `File ${relativePath} has unknown extension. Supported: ${PROMPT_EXTENSIONS.join(", ")}`,
      );
    } else {
      validFiles++;
...
function main() {
  console.log(
    `${colors.cyan}${colors.bold}🔍 APM Package Validation${colors.reset}\n`,
  );
  ...
  if (manifestResult.errors.length > 0) {
    manifestResult.errors.forEach((e) =>
  ...
  if (manifestResult.warnings.length > 0) {
    manifestResult.warnings.forEach((w) =>
      console.warn(`  ${colors.yellow}⚠️  ${w}${colors.reset}`),
    );
  }
  ...
  if (promptsResult.errors.length > 0) {
    promptsResult.errors.forEach((e) =>
      console.error(`  ${colors.red}❌ ${e}${colors.reset}`),
    );
    exitCode = 1;
  }
  ...
  if (promptsResult.warnings.length > 0) {
    promptsResult.warnings.forEach((w) =>
      console.warn(`  ${colors.yellow}⚠️  ${w}${colors.reset}`),
    );
  }
  ...
  if (
    promptsResult.valid &&
    promptsResult.errors.length === 0 &&
    promptsResult.warnings.length === 0
  ) {
