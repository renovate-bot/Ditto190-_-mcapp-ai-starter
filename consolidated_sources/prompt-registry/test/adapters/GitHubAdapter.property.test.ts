      ...
      fc.asyncProperty(
        ...
        async (config: {
          ...
          try {
            ...
            if (
              config.hasExplicitToken &&
              config.explicitToken.trim().length > 0
            ) {
              // Explicit token should be used first
              if (
                token !== config.explicitToken.trim() ||
                method !== "explicit"
              ) {
                console.log(
    ...
    await fc.assert(
      fc.asyncProperty(
        ...
        async (config) => {
          ...
          const attemptedMethods = (adapter as any)
            .attemptedMethods as Set<string>;
          assert.ok(
      ...
      fc.asyncProperty(
        fc.record({
          ...
          htmlTemplate: fc.constantFrom(
            ...
            (text: string) =>
              `<html><body><p class="error">${text}</p></body></html>`,
      ...
      fc.asyncProperty(
        ...
        async (config) => {
          ...
          try {
            ...
            if (fullTokenInLogs) {
              console.log(
                "Token sanitization test failed: Full token found in logs",
              );
            ...
            if (!hasTokenPreview) {
              console.log(
                "Token sanitization test: No token preview found in logs",
              );
            }
          } finally {
            iterationSandbox.restore();
