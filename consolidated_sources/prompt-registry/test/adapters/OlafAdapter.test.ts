...
suite("OlafAdapter Integration Tests", () => {
  ...
  setup(() => {
    ...
    sinon
      .stub(OlafRuntimeManager, "getInstance")
      .returns(runtimeManagerStub as any);
  ...
  suite("Bundle Discovery", () => {
    test("should discover bundles from bundles/ directory", async () => {
      ...
      assert.strictEqual(
        bundles[0].id,
        "olaf-test-owner-test-olaf-repo-developer",
      );
    ...
    test("should discover multiple bundles with multiple skills", async () => {
      ...
      const developerBundle = bundles.find(
        (b) => b.name === "Developer Bundle",
      );
  ...
  suite("Bundle Packaging", () => {
    test("should generate deployment manifest for bundle", async () => {
      ...
      assert.strictEqual(bundles[0].version, "1.0.0");
      assert.ok(
        bundles[0].description.includes("Advanced data analysis capabilities"),
      );
      assert.deepStrictEqual(bundles[0].tags, [
        "olaf",
        "skill",
  ...
  suite("Bundle Validation", () => {
    test("should validate OLAF repository structure", async () => {
      ...
      nock("https://api.github.com")
        ...
        .get(
          "/repos/test-owner/test-olaf-repo/contents/skills/code-review/manifest.json",
        )
        .reply(200, {
          name: "manifest.json",
          type: "file",
        ...
        .get(
          "/repos/test-owner/test-olaf-repo/contents/skills/data-analysis/manifest.json",
        )
        .reply(200, {
          name: "manifest.json",
          type: "file",
      ...
      nock("https://raw.githubusercontent.com")
        ...
        .get(
          "/test-owner/test-olaf-repo/main/skills/data-analysis/manifest.json",
        )
        .reply(
          200,
          JSON.stringify({
  ...
  suite("Post-Installation", () => {
    test("should register skill in competency index after installation", async () => {
      ...
      nock("https://api.github.com")
        ...
        .get(
          "/repos/test-owner/test-olaf-repo/contents/skills/test-skill/manifest.json",
        )
        .reply(200, {
          download_url:
            "https://raw.githubusercontent.com/test-owner/test-olaf-repo/main/skills/test-skill/manifest.json",
      ...
      existsSyncStub
        .withArgs(sinon.match(/competency-index\.json$/))
        .returns(false);
      existsSyncStub.withArgs(sinon.match(/reference$/)).returns(false);

      const adapter = new OlafAdapter(mockSource);
      await adapter.postInstall(
        "olaf-test-owner-test-olaf-repo-test-skill",
        installPath,
      );

      // Verify competency index was created
      assert.ok(
        mkdirSyncStub.calledWith(sinon.match(/reference$/), {
          recursive: true,
        }),
      );
      ...
      assert.ok(
        Array.isArray(writtenData),
        "Competency index should be an array",
      );
      assert.strictEqual(writtenData.length, 1);
      assert.deepStrictEqual(writtenData[0].patterns, [
        "test-pattern",
        "test skill",
      ]);
      assert.strictEqual(
  ...
  suite("Post-Uninstallation", () => {
    test("should remove skill from competency index after uninstallation", async () => {
      ...
      const adapter = new OlafAdapter(mockSource);
      await adapter.postUninstall(
        "olaf-test-owner-test-repo-test-skill",
        installPath,
      );
      ...
      assert.strictEqual(
        writtenData.length,
        1,
        "Should have one skill remaining",
      );
      assert.strictEqual(
        writtenData[0].file,
    ...
    test("should handle skill not found in competency index", async () => {
      ...
      assert.ok(
        writeFileSyncStub.notCalled,
        "Should not write when skill is not found",
      );
    });
  });
});
