...
suite("LocalOlafAdapter", () => {
  ...
  suite("constructor", () => {
    ...
    test("should throw error with invalid path", () => {
      ...
      assert.throws(
        () => new LocalOlafAdapter(invalidSource),
        /Invalid local OLAF path/,
      );
    });
  });
  ...
  suite("competency index path consistency", () => {
    ...
    test("should use consistent paths for installation and uninstallation", async () => {
      ...
      const competencyIndexDir = path.join(
        mockWorkspace,
        ".olaf",
        "olaf-core",
        "reference",
      ...
      const originalWorkspaceFolders =
        require("vscode").workspace.workspaceFolders;
      require("vscode").workspace.workspaceFolders = [
      ...
      const originalCreateSkillSymbolicLinks =
        adapter["createSkillSymbolicLinks"];
      const originalRemoveSkillSymbolicLinks =
        adapter["removeSkillSymbolicLinks"];
      ...
      try {
        ...
        assert.ok(
          fs.existsSync(competencyIndexPath),
          "Competency index should be created",
        );
