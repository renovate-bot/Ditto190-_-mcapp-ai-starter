...
suite("ApmAdapter", () => {
  ...
  suite("Authentication", () => {
    ...
    test("should fallback to config token if VS Code auth fails", async () => {
      ...
      sandbox
        .stub(adapter as any, "execShell")
        .rejects(new Error("gh not found"));
      ...
      assert.ok(
        scope.isDone(),
        "Request with config token auth header was not made",
      );
    });
  });
  ...
  suite("Error Handling", () => {
    ...
    test("should provide helpful error messages when runtime not installed", async () => {
      ...
      try {
        ...
        assert.ok(
          error.message.includes("APM") || error.message.includes("install"),
        );
      }
    });
  });
