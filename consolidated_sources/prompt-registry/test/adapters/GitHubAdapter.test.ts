...
suite("GitHubAdapter", () => {
  ...
  suite("fetchBundles", () => {
    test("should fetch bundles from releases", async () => {
      ...
      assert.strictEqual(
        bundles[0].id,
        "test-owner-test-repo-test-bundle-1.0.0",
      );
...
`;
  ...
  suite("validate", () => {
    ...
    test("should follow HTTP 301 redirects when validating repository", async () => {
      ...
      nock("https://api.github.com")
        .get("/repos/test-owner/test-repo")
        .reply(301, "", {
          location: "https://api.github.com/repos/new-owner/new-repo",
        });
      ...
      nock("https://api.github.com")
        .get("/repos/test-owner/test-repo/releases")
        .reply(301, "", {
          location: "https://api.github.com/repos/new-owner/new-repo/releases",
        });
      ...
      assert.strictEqual(
        result.valid,
        true,
        "Should successfully validate after following redirect",
      );
      assert.strictEqual(result.errors.length, 0);
    });
    ...
    test("should follow HTTP 302 redirects when validating repository", async () => {
      ...
      nock("https://api.github.com")
        .get("/repos/test-owner/test-repo")
        .reply(302, "", {
          location: "https://api.github.com/repos/test-owner/test-repo-v2",
        });
      ...
      nock("https://api.github.com")
        ...
        .reply(302, "", {
          location:
            "https://api.github.com/repos/test-owner/test-repo-v2/releases",
        });
    ...
    test("should produce clear error message for 401 authentication failure", async () => {
      ...
      try {
        ...
        assert.ok(
          ...
          "Error should include status code 401",
        );
        assert.ok(
    ...
    test("should produce clear error message for 403 access forbidden", async () => {
      ...
      try {
        ...
        assert.ok(
          ...
          "Error should include status code 403",
        );
        assert.ok(
    ...
    test("should produce clear error message for 404 repository not found", async () => {
      ...
      try {
        ...
        assert.ok(
          ...
          "Error should include status code 404",
        );
        assert.ok(
  ...
  suite("Logging Behavior", () => {
    ...
    test("should log URL and auth method before download", async () => {
      ...
      const downloadLogs = debugStub
        ...
        .filter(
          (call) =>
            call.args[0].includes("Downloading") &&
            call.args[0].includes("bundle.zip"),
        );
    ...
    test("should log redirect URL when following redirects", async () => {
      ...
      nock("https://github.com")
        .get("/test-owner/test-repo/releases/download/v1.0.0/bundle.zip")
        .reply(302, "", {
          location: "https://objects.githubusercontent.com/bundle.zip",
        })
        .get("/bundle.zip")
        .reply(200, bundleContent);
  ...
  suite("Manifest Caching", () => {
    test("should make only one HTTP request when same manifest URL is fetched multiple times", async () => {
      ...
      assert.strictEqual(
        manifestDownloadCount,
        1,
        "Should make only one HTTP request for manifest",
      );
    });
    ...
    test("should fetch fresh manifest after cache is cleared", async () => {
      ...
      assert.strictEqual(
        manifestDownloadCount,
        1,
        "First fetch should make one HTTP request",
      );
      ...
      assert.strictEqual(
        manifestDownloadCount,
        2,
        "After cache clear, should make new HTTP request",
      );
    });
  });
  ...
  suite("Multiple Releases Processing", () => {
    ...
    test("should skip releases without manifest and continue processing others", async () => {
      ...
      assert.ok(
        bundles.some((b) => b.version === "1.0.0"),
        "Should include v1.0.0",
      );
      assert.ok(
        bundles.some((b) => b.version === "0.8.0"),
        "Should include v0.8.0",
      );
    });
  });
});
