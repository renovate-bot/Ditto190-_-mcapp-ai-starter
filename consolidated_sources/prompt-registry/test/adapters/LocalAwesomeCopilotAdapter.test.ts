...
suite("LocalAwesomeCopilotAdapter", () => {
  const fixturesPath = path.join(
    __dirname,
    "../fixtures/local-awesome-collections",
  );
  ...
  suite("Constructor and Validation", () => {
    ...
    test("should throw error for invalid path format", () => {
      ...
      assert.throws(
        () => new LocalAwesomeCopilotAdapter(source),
        /Invalid local path/,
      );
    });
  ...
  suite("validate", () => {
    ...
    test("should fail validation for non-existent directory", async () => {
      ...
      assert.ok(
        result.errors[0].includes("Collections directory does not exist"),
      );
    ...
    test("should fail validation for directory without collections", async () => {
      ...
      const source = {
        ...mockSource,
        url: path.join(__dirname, "../fixtures"),
      };
      const adapter = new LocalAwesomeCopilotAdapter(source);
      const result = await adapter.validate();
  ...
  suite("Error Handling", () => {
    ...
    test("should provide helpful error messages", async () => {
      ...
      try {
        ...
        assert.ok(
          error.message.includes("Collections directory does not exist"),
        );
      }
    });
  });
