  ...
  suite("Diagnostics", () => {
    test("should log directory scanning details", async () => {
      ...
      try {
        ...
        assert.ok(
          logs.some((log) => log.includes("[LocalAdapter] Scanning directory")),
        );
        assert.ok(
          logs.some(
            (log) =>
              log.includes("[LocalAdapter] Found") && log.includes("entries"),
          ),
        );
        assert.ok(
          logs.some(
  ...
  suite("downloadBundle", () => {
    ...
    test("should handle binary file handling (ZIP files)", async () => {
      ...
      assert.strictEqual(
        buffer[0],
        0x50,
        "Should start with ZIP magic number (P)",
      );
      assert.strictEqual(
        buffer[1],
        0x4b,
        "Should start with ZIP magic number (K)",
      );
    ...
    test("should handle file:// URL format", async () => {
      ...
      assert.ok(
        buffer.length > 0,
        "Should successfully download from file:// URL",
      );
    });
    ...
    test("should preserve binary data integrity", async () => {
      ...
      assert.ok(
        ...
        "Buffers should be byte-for-byte identical",
      );
    });
  });
});
