...
suite("VersionManager Unit Tests", () => {
  suite("compareVersions", () => {
    ...
    test("should handle versions with v prefix", () => {
      assert.strictEqual(
        VersionManager.compareVersions("v1.0.0", "v2.0.0"),
        -1,
      );
    ...
    test("should handle pre-release versions", () => {
      assert.strictEqual(
        VersionManager.compareVersions("1.0.0-alpha", "1.0.0"),
        -1,
      );
      assert.strictEqual(
        VersionManager.compareVersions("1.0.0", "1.0.0-alpha"),
        1,
      );
      assert.strictEqual(
        VersionManager.compareVersions("1.0.0-alpha", "1.0.0-beta"),
        -1,
      );
    });
    ...
    test("should throw error for empty strings", () => {
      ...
      assert.throws(
        () => VersionManager.compareVersions("", "1.0.0"),
        /cannot be empty/,
      );
      assert.throws(
        () => VersionManager.compareVersions("1.0.0", ""),
    ...
    test("should handle build metadata correctly", () => {
      ...
      assert.strictEqual(
        VersionManager.compareVersions("1.0.0+build", "1.0.0"),
        0,
      );
    });
  });
  ...
  suite("isUpdateAvailable", () => {
    test("should return true when latest version is higher", () => {
      assert.strictEqual(
        VersionManager.isUpdateAvailable("1.0.0", "2.0.0"),
        true,
      );
      assert.strictEqual(
        VersionManager.isUpdateAvailable("1.0.0", "1.1.0"),
        true,
      );
      assert.strictEqual(
        VersionManager.isUpdateAvailable("1.0.0", "1.0.1"),
        true,
      );
    });
    ...
    test("should return false when versions are equal", () => {
      assert.strictEqual(
        VersionManager.isUpdateAvailable("1.0.0", "1.0.0"),
        false,
      );
      assert.strictEqual(
        VersionManager.isUpdateAvailable("v1.0.0", "v1.0.0"),
        false,
      );
    });
    ...
    test("should return false when installed version is higher", () => {
      assert.strictEqual(
        VersionManager.isUpdateAvailable("2.0.0", "1.0.0"),
        false,
      );
      assert.strictEqual(
        VersionManager.isUpdateAvailable("1.1.0", "1.0.0"),
        false,
      );
    });
    ...
    test("should handle versions with v prefix", () => {
      assert.strictEqual(
        VersionManager.isUpdateAvailable("v1.0.0", "v2.0.0"),
        true,
      );
      assert.strictEqual(
        VersionManager.isUpdateAvailable("v2.0.0", "v1.0.0"),
        false,
      );
    });
    ...
    test("should handle mixed format versions", () => {
      assert.strictEqual(
        VersionManager.isUpdateAvailable("v1.0.0", "2.0.0"),
        true,
      );
      assert.strictEqual(
        VersionManager.isUpdateAvailable("1.0.0", "v2.0.0"),
        true,
      );
    });
    ...
    test("should throw error for empty versions", () => {
      assert.throws(
        () => VersionManager.isUpdateAvailable("", "1.0.0"),
        /cannot be empty/,
      );
      assert.throws(
        () => VersionManager.isUpdateAvailable("1.0.0", ""),
  ...
  suite("extractBundleIdentity", () => {
    test("should extract identity from GitHub bundle IDs with version", () => {
      ...
      assert.strictEqual(
        VersionManager.extractBundleIdentity(
  ...
  suite("parseVersion", () => {
    ...
    test("should handle pre-release versions", () => {
      assert.strictEqual(
        VersionManager.parseVersion("1.0.0-alpha"),
        "1.0.0-alpha",
      );
      assert.strictEqual(
        VersionManager.parseVersion("v1.0.0-beta"),
        "1.0.0-beta",
      );
    });
    ...
    test("should handle build metadata", () => {
      ...
      assert.strictEqual(
        VersionManager.parseVersion("1.0.0+build.123"),
        "1.0.0",
      );
    });
  });
