...
suite("CollectionValidator", () => {
  ...
  const fixturesDir = path.join(
    __dirname,
    "../../../test/fixtures/collections-validator",
  );
  ...
  suite("validateCollection", () => {
    ...
    suite("Required Fields", () => {
      test("should fail if id is missing", () => {
        const collectionPath = path.join(
          fixturesDir,
          "invalid/missing-id.collection.yml",
        );
        const result = validator.validateCollection(
          collectionPath,
          fixturesDir,
    ...
    suite("ID Validation", () => {
      ...
      test("should accept valid id with lowercase, numbers, and hyphens", () => {
        ...
        assert.strictEqual(
          idErrors.length,
          0,
          "Should have no ID format errors",
        );
      });
    });
    ...
    suite("File Reference Validation", () => {
      ...
      test("should pass if all referenced files exist", () => {
        ...
        assert.strictEqual(
          fileErrors.length,
          0,
          "Should have no file reference errors",
        );
      });
    });
    ...
    suite("Tags Validation", () => {
      test("should warn if more than 10 tags", () => {
        const collectionPath = path.join(
          fixturesDir,
          "warnings/many-tags.collection.yml",
        );
        const result = validator.validateCollection(
          collectionPath,
          fixturesDir,
    ...
    suite("Error Structure", () => {
      test("should include file name in errors", () => {
        ...
        assert.ok(
          result.errors[0].file.includes("missing-id"),
