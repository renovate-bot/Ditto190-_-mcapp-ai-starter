...
suite("sourceIdUtils", () => {
  suite("generateHubSourceId()", () => {
    test("should produce correct format: {sourceType}-{12-char-hash}", () => {
      const result = generateHubSourceId(
        "github",
    ...
    test("should normalize URL case", () => {
      const result1 = generateHubSourceId(
        "github",
        "https://GitHub.com/Owner/Repo",
      );
      const result2 = generateHubSourceId(
        "github",
        "https://github.com/Owner/Repo",
      );
    ...
    test("should normalize path case (case-insensitive)", () => {
      ...
      assert.strictEqual(
        result1,
        result2,
        "Path case should be normalized (case-insensitive)",
      );
    });

    test("should normalize full URL case", () => {
      const resultMixedHost = generateHubSourceId(
        "github",
        "https://GitHub.COM/Owner/Repo",
      );
      const resultLowerHost = generateHubSourceId(
        "github",
        "https://github.com/Owner/Repo",
      );
      const resultDiffPath = generateHubSourceId(
        "github",
        "https://github.com/owner/repo",
      );
      ...
      assert.strictEqual(
        resultMixedHost,
        resultLowerHost,
        "Host case should be normalized",
      );
      ...
      assert.strictEqual(
        resultLowerHost,
        resultDiffPath,
        "Path case should be normalized",
      );
    });
    ...
    test("should normalize trailing slashes", () => {
      ...
      const result3 = generateHubSourceId(
        "github",
        "https://github.com/owner/repo///",
      );
    ...
    test("should produce different IDs for different URLs", () => {
      ...
      const id1 = generateHubSourceId(
        sourceType,
        "https://github.com/owner1/repo",
      );
      const id2 = generateHubSourceId(
        sourceType,
    ...
    test("should produce different IDs for same URL but different collectionsPath", () => {
      ...
      const id1 = generateHubSourceId(sourceType, url, {
        collectionsPath: "collections",
      });
      const id2 = generateHubSourceId(sourceType, url, {
      ...
      assert.notStrictEqual(
        id1,
        id2,
        "Different collectionsPath should produce different IDs",
      );
    });
    ...
    test("should produce same ID when defaults are explicit vs omitted", () => {
      ...
      assert.strictEqual(
        idNoConfig,
        idExplicitDefaults,
        "Omitted config should equal explicit defaults",
      );
    });
    ...
    test("should produce same ID when branch default varies (main vs master)", () => {
      ...
      const idMaster = generateHubSourceId(sourceType, url, {
        branch: "master",
      });
      ...
      assert.strictEqual(
        idMain,
        idMaster,
        "main and master should produce same ID",
      );
    });
  });
  ...
  suite("isLegacyHubSourceId()", () => {
    ...
    test("should return true for legacy format with more than 3 segments", () => {
      assert.strictEqual(
        isLegacyHubSourceId("hub-test-hub-github-source"),
        true,
      );
  ...
  suite("generateHubKey()", () => {
    ...
    test("should handle various branch names", () => {
      ...
      assert.match(
        generateHubKey(url, "feature/test"),
        /^[a-f0-9]{12}-feature\/test$/,
      );
      assert.match(
        generateHubKey(url, "release-1.0"),
        /^[a-f0-9]{12}-release-1\.0$/,
      );
  ...
  suite("generateLegacyHubSourceId()", () => {
    test("should return undefined when URL path is all lowercase", () => {
      const result = generateLegacyHubSourceId(
        "github",
        "https://github.com/owner/repo",
      );
    ...
    test("should respect branch and collectionsPath config", () => {
      ...
      const id2 = generateLegacyHubSourceId("github", url, {
        branch: "develop",
      });
      ...
      assert.notStrictEqual(
        id1,
        id2,
        "Different branches should produce different legacy IDs",
      );
    });
  });
  ...
  suite("generateLegacyHubKey()", () => {
    ...
    test("should append branch for non-main branches", () => {
      const result = generateLegacyHubKey(
        "https://example.com/Hub.json",
        "develop",
      );
      assert.ok(result);
      assert.match(result!, /^[a-f0-9]{12}-develop$/);
