...
suite("regexUtils", () => {
  suite("escapeRegex", () => {
    test("should escape all special regex characters", () => {
      ...
      assert.ok(
        regex.test(input),
        "Escaped regex should match original string",
      );
    });
    ...
    test("should handle string with no special chars", () => {
      ...
      assert.strictEqual(
        escaped,
        input,
        "Should not modify string without special chars",
      );
    });
  ...
  suite("createSafeRegex", () => {
    test("should create regex from string with special chars", () => {
      ...
      assert.ok(
        !regex.test("pathXwithXdots"),
        "Should not match with different chars",
      );
    });
    ...
    test("should support global flag", () => {
      ...
      assert.strictEqual(
        matches?.length,
        3,
        "Should match all occurrences with g flag",
      );
    });
  });
