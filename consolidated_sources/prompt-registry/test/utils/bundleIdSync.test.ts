...
suite("Bundle ID Synchronization", () => {
  ...
  const testCases = [
    ...
    {
      repoSlug: "company/product-prompts",
  ...
  test("TypeScript and JavaScript implementations should produce identical bundle IDs", () => {
    for (const tc of testCases) {
      const tsResult = generateBuildScriptBundleId(
        tc.repoSlug,
        tc.collectionId,
        tc.version,
      );
      const jsResult = bundleIdJs.generateBundleId(
        tc.repoSlug,
        tc.collectionId,
        tc.version,
      );
  ...
  test("Bundle ID format should be {owner}-{repo}-{collectionId}-v{version}", () => {
    ...
    assert.strictEqual(
      tsResult,
      expected,
      `TS result should match expected format`,
    );
    assert.strictEqual(
      jsResult,
      expected,
      `JS result should match expected format`,
    );
  });
