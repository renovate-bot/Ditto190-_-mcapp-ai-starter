...
suite("bundleNameUtils", () => {
  suite("generateSanitizedId", () => {
    ...
    test("should replace spaces with hyphens", () => {
      ...
      assert.strictEqual(
        generateSanitizedId("hello world test"),
        "hello-world-test",
      );
    });
    ...
    test("should handle complex cases", () => {
      assert.strictEqual(
        generateSanitizedId("  My  Project!!  "),
        "my-project",
      );
      assert.strictEqual(
        generateSanitizedId("Hello, World! 123"),
        "hello-world-123",
      );
      assert.strictEqual(
        generateSanitizedId("test_name-here"),
        "test-name-here",
      );
    });
  ...
  suite("isManifestIdMatch", () => {
    test("should match exact IDs", () => {
      ...
      assert.ok(
        isManifestIdMatch("owner-repo-v1.0.0", "1.0.0", "owner-repo-v1.0.0"),
      );
    });
    ...
    test("should match suffix pattern without v prefix", () => {
      assert.ok(
        isManifestIdMatch("collection", "1.0.0", "owner-repo-collection-1.0.0"),
      );
    ...
    test("should not match mismatched IDs", () => {
      assert.ok(
        !isManifestIdMatch("wrong", "1.0.0", "owner-repo-collection-v1.0.0"),
      );
      assert.ok(
        !isManifestIdMatch(
          "collection",
    ...
    test("should handle special characters in repo names", () => {
      assert.ok(
        isManifestIdMatch("test2", "1.0.2", "org-repo.name-test2-1.0.2"),
      );
    });
  });
  ...
  suite("generateGitHubBundleId", () => {
    test("should generate ID with manifest info", () => {
      const id = generateGitHubBundleId(
        "owner",
        "repo",
        "v1.0.0",
        "collection",
  ...
  suite("generateBuildScriptBundleId", () => {
    test("should generate ID with v prefix", () => {
      const id = generateBuildScriptBundleId(
        "owner/repo",
        "collection",
        "1.0.0",
      );
