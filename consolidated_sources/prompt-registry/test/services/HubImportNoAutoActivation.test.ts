...
suite("Hub Import - No Auto-Activation", () => {
  ...
  suite("Hub Import Behavior", () => {
    ...
    test("profiles should remain inactive until explicitly activated by user", async () => {
      ...
      assert.ok(
        profiles.every((p) => !p.active),
        "All profiles should be inactive initially",
      );

      // Act: Explicitly activate a profile (simulating user action)
      await hubManager.activateProfile(hubId, "profile-1", {
      ...
      assert.strictEqual(
        activeProfile?.id,
        "profile-1",
        "The explicitly activated profile should be active",
      );
      ...
      assert.strictEqual(
        ...
        1,
        "Other profiles should remain inactive",
      );
    });
  });
