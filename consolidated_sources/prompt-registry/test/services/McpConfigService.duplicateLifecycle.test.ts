...
suite("McpConfigService - Duplicate Server Lifecycle", () => {
  ...
  const installBundleServers = async (
    ...
    for (const [serverName, serverConfig] of Object.entries(servers)) {
      const prefixedName = configService.generatePrefixedServerName(
        bundleId,
        serverName,
      );
  ...
  suite("Stdio Server Duplicate Lifecycle", () => {
    ...
    test("should keep exactly one active server when multiple bundles install the same server", async () => {
      ...
      ({ config, tracking } = await installBundleServers(
        "bundle-a",
        { shared: sharedServer },
        config,
        tracking,
      ...
      const activeCount = countActiveServersWithIdentity(
        config,
        sharedIdentity,
      );
      ...
      assert.strictEqual(
        allServers.filter((s) => s.disabled).length,
        2,
        "Should have 2 disabled servers",
      );
    });
    ...
    test("should re-enable a duplicate when the active server is removed", async () => {
      ...
      let config: McpConfiguration = {
        servers: {
          ...
          "prompt-registry:bundle-b:shared": {
            ...sharedServer,
            disabled: true,
            description: "Duplicate of prompt-registry:bundle-a:shared",
          },
      ...
      ({ config, tracking } = await uninstallBundleServers(
        "bundle-a",
        config,
        tracking,
      ));
      ...
      for (const serverConfig of Object.values(config.servers)) {
        if (
          ...
          serverConfig.description?.includes("Duplicate")
        ) {
      ...
      assert.strictEqual(
        activeCount,
        1,
        "Should have exactly 1 active server after removing the original active",
      );
      ...
      assert.strictEqual(
        allServers.length,
        2,
        "Should have 2 total servers remaining",
      );
    });
    ...
    test("should maintain at least one active server until all bundles are removed", async () => {
      ...
      assert.strictEqual(
        countActiveServersWithIdentity(config, sharedIdentity),
        1,
        "Initial: 1 active",
      );
      assert.strictEqual(
        getServersWithIdentity(config, sharedIdentity).length,
        3,
        "Initial: 3 total",
      );
      ...
      assert.strictEqual(
        countActiveServersWithIdentity(config, sharedIdentity),
        1,
        "After removing bundle-a: 1 active",
      );
      assert.strictEqual(
        getServersWithIdentity(config, sharedIdentity).length,
        2,
        "After removing bundle-a: 2 total",
      );
      ...
      assert.strictEqual(
        countActiveServersWithIdentity(config, sharedIdentity),
        1,
        "After removing bundle-b: 1 active",
      );
      assert.strictEqual(
        getServersWithIdentity(config, sharedIdentity).length,
        1,
        "After removing bundle-b: 1 total",
      );
      ...
      assert.strictEqual(
        countActiveServersWithIdentity(config, sharedIdentity),
        0,
        "After removing all: 0 active",
      );
      assert.strictEqual(
        getServersWithIdentity(config, sharedIdentity).length,
        0,
        "After removing all: 0 total",
      );
    });
  });
  ...
  suite("Remote Server Duplicate Lifecycle", () => {
    ...
    test("should handle remote server duplicates the same as stdio", async () => {
      ...
      assert.strictEqual(
        activeCount,
        1,
        "Should have exactly 1 active remote server",
      );
      ...
      assert.strictEqual(
        allServers.length,
        3,
        "Should have 3 total remote servers",
      );
      assert.strictEqual(
        allServers.filter((s) => s.disabled).length,
        2,
        "Should have 2 disabled remote servers",
      );
    });
  });
  ...
  suite("Mixed Server Types", () => {
    test("should not cross-disable stdio and remote servers with similar identifiers", async () => {
      ...
      assert.strictEqual(
        countActiveServersWithIdentity(config, stdioIdentity),
        1,
        "Stdio server should be active",
      );
      assert.strictEqual(
        countActiveServersWithIdentity(config, remoteIdentity),
        1,
        "Remote server should be active",
      );
    });
  });
});
