...
suite("LocalModificationWarningService - Property Tests", () => {
        ...
        async (bundleId, modifiedFiles, bundleRepoUrl, userResponse) => {
          ...
          if (userResponse === "Contribute Changes") {
            assert.strictEqual(
              result,
              "contribute",
              'Should return "contribute" for Contribute Changes',
            );
            ...
            assert.strictEqual(
              result,
              "override",
              'Should return "override" for Override',
            );
            assert.ok(
              !openExternalStub.called,
              "Should not open URL for Override",
            );
          } else {
            // Cancel or dismissed
            assert.strictEqual(
              result,
              "cancel",
              'Should return "cancel" for Cancel or dismiss',
            );
            assert.ok(
              !openExternalStub.called,
              "Should not open URL for Cancel",
            );
          }
        },
      ),
  ...
  test("Property: checkAndWarn returns null when no modifications detected", async () => {
    await fc.assert(
      fc.asyncProperty(LockfileGenerators.bundleId(), async (bundleId) => {
        // Reset stubs
        showWarningMessageStub.reset();
    ...
    await fc.assert(
      fc.asyncProperty(
        ...
        async (bundleId, modifiedFiles, userResponse) => {
          ...
          assert.ok(
            result !== null,
            "Should return a result when modifications exist",
          );
          assert.ok(
      ...
      fc.asyncProperty(
        ...
        async (bundleId, modifiedFiles) => {
          ...
          assert.strictEqual(
            buttons[0],
            "Contribute Changes",
            "First button should be Contribute Changes",
          );
          assert.strictEqual(
            buttons[1],
            "Override",
            "Second button should be Override",
          );
          assert.strictEqual(
            buttons[2],
            "Cancel",
            "Third button should be Cancel",
          );
        },
      ),
      { numRuns: 100 },
