...
suite("RegistryManager.listInstalledBundles Property Tests", () => {
  ...
  const createMockUserBundle = (
    ...
    manifest: {
      ...
      bundle_settings: {
        ...
        compression: "none",
        naming: { environment_bundle: id },
      },
      metadata: {
        manifest_version: "1.0.0",
        description: `User bundle: ${id}`,
      },
    },
  });
  ...
  setup(() => {
    ...
    getWorkspaceRootStub = sandbox
      .stub(scopeSelectionUI, "getWorkspaceRoot")
      .returns(tempDir);
      ...
      await fc.assert(
        fc.asyncProperty(
          ...
          async (bundleCount: number) => {
            ...
            const userBundles = [
              createMockUserBundle("user-bundle-1", "1.0.0"),
            ];
            ...
            const hasUserBundle = result.some(
              (b) => b.bundleId === "user-bundle-1",
            );
            assert.strictEqual(
              hasUserBundle,
              false,
        ...
        fc.asyncProperty(
          ...
          async (lockfileBundleCount: number, userBundleCount: number) => {
            ...
            for (let i = 0; i < userBundleCount; i++) {
              userBundles.push(
                createMockUserBundle(`user-bundle-${i}`, `${i + 1}.0.0`),
              );
            }
            mockStorage.getInstalledBundles.resolves(userBundles);
            ...
            const workspaceBundles: InstalledBundle[] = [
              {
                ...
                manifest: {
                  ...
                  bundle_settings: {
                    ...
                    create_common_bundle: false,
                    compression: "none",
                    naming: { environment_bundle: "workspace-bundle-1" },
                  },
                  metadata: {
                    manifest_version: "1.0.0",
