...
suite("UserScopeService", () => {
  ...
  suite("Path Resolution", () => {
    ...
    const pathTestCases = [
      {
        ...
        expectedPath:
          "/Users/testuser/Library/Application Support/Kiro/User/prompts",
      },
      {
        name: "macOS - VS Code",
        globalStoragePath:
        ...
        expectedPath:
          "/Users/testuser/Library/Application Support/Code/User/prompts",
      },
      {
      ...
      },
      {
        ...
        expectedPath:
          "/Users/testuser/Library/Application Support/Windsurf/User/prompts",
      },
      {
        name: "macOS - Cursor",
        globalStoragePath:
        ...
        expectedPath:
          "/Users/testuser/Library/Application Support/Cursor/User/prompts",
      },
      {
        name: "Linux - VS Code",
        globalStoragePath:
    ...
    const profileTestCases = [
      ...
      },
      {
        ...
        expectedPath:
          "/home/testuser/.config/Code/User/profiles/xyz789/prompts",
      },
    ];
  ...
  suite("Windows Path Regex Handling (Backslash Escaping)", () => {
    ...
    test("should handle Windows-style path with backslashes - standard profile", async () => {
      ...
      assert.strictEqual(
        status.copilotDir,
        expectedPath,
        "Should resolve to User/prompts",
      );
    });
    ...
    test("should handle Windows-style path with backslashes - profile-based", async () => {
      ...
      assert.ok(status.copilotDir, "Should return a valid path");
      assert.ok(
        status.copilotDir.includes("profiles"),
        "Should include profiles directory",
      );
      assert.ok(
        status.copilotDir.includes(profileId),
        "Should include profile ID",
      );
      assert.ok(
        status.copilotDir.endsWith("prompts"),
        "Should end with prompts",
  ...
  suite("getStatus", () => {
    test("should return status information", async () => {
      ...
      assert.ok(
        typeof status.copilotDir === "string",
        "Should include copilot directory",
      );
      assert.ok(
      ...
      assert.ok(
        typeof status.syncedFiles === "number",
        "Should include synced files count",
      );
    });
...
`,
      ...
      try {
        ...
        assert.ok(
          error.message || true,
          "Error is expected in unit test environment",
        );
      }
    });
...
`,
  ...
  suite("Behavior Validation", () => {
    ...
    test("should create directory structure when syncing", async () => {
      ...
      assert.ok(
        typeof status.dirExists === "boolean",
        "Should report directory existence",
      );
      assert.ok(
        typeof status.syncedFiles === "number",
        "Should report synced file count",
      );
  ...
  suite("Profile Detection Workarounds", () => {
    test("should use filesystem heuristic when storage.json not available", async () => {
      ...
      const profileGlobalStorage = path.join(
        profilesDir,
        profileId,
        "globalStorage",
      );
  ...
  suite("WSL Support", () => {
    test("should detect WSL remote and return Windows mount path when globalStorage is on /mnt/c", async () => {
      ...
      const wslMountContext: any = {
        globalStorageUri: {
          fsPath:
            "/mnt/c/Users/testuser/AppData/Roaming/Code/User/globalStorage/publisher.extension",
        },
    ...
    test("should handle WSL with D: drive mount", async () => {
      const wslDriveContext: any = {
        globalStorageUri: {
          fsPath:
            "/mnt/d/Users/testuser/AppData/Roaming/Code/User/globalStorage/publisher.extension",
        },
        storageUri: { fsPath: "/home/testuser/workspace" },
        extensionPath: __dirname,
        subscriptions: [],
    ...
    test("should handle WSL with Code Insiders flavor", async () => {
      const wslInsidersContext: any = {
        globalStorageUri: {
          fsPath:
            "/mnt/c/Users/testuser/AppData/Roaming/Code - Insiders/User/globalStorage/publisher.extension",
        },
        storageUri: { fsPath: "/home/testuser/workspace" },
        extensionPath: __dirname,
        subscriptions: [],
    ...
    test("should handle WSL with profile path", async () => {
      const wslProfileContext: any = {
        globalStorageUri: {
          fsPath:
            "/mnt/c/Users/testuser/AppData/Roaming/Code/User/profiles/abc123/globalStorage/publisher.extension",
        },
        storageUri: { fsPath: "/home/testuser/workspace" },
        extensionPath: __dirname,
        subscriptions: [],
    ...
    test("should handle SSH remote (not WSL) with existing logic", async () => {
      const sshContext: any = {
        globalStorageUri: {
          fsPath:
            "/home/remoteuser/.vscode-server/data/User/globalStorage/publisher.extension",
        },
  ...
  suite("Broken Symlink Handling", () => {
    test("should detect and remove broken symlinks when creating new symlinks", async () => {
      ...
      try {
        ...
        const newBundlePath = createSimpleMockBundle(
          tempDir,
          bundleId,
          "2.0.0",
        );
        ...
        assert.ok(
          newStatus.syncedFiles >= initialSyncedFiles,
    ...
    test("should correctly identify broken vs valid symlinks", async () => {
      ...
      try {
        ...
        assert.strictEqual(
          fs.existsSync(validSymlink),
          true,
          "fs.existsSync should return true for valid symlink",
        );
        assert.strictEqual(
          fs.existsSync(brokenSymlink),
          false,
