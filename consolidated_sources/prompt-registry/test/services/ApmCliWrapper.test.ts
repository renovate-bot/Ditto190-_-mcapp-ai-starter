...
suite("ApmCliWrapper", () => {
  ...
  suite("validatePackageRef", () => {
    ...
    test("should reject strings with special characters", () => {
      assert.strictEqual(
        wrapper.validatePackageRef("owner/repo;rm -rf"),
        false,
      );
    });
    ...
    test("should reject strings with shell metacharacters", () => {
      assert.strictEqual(
        wrapper.validatePackageRef("owner/repo$(whoami)"),
        false,
      );
    });
  });
  ...
  suite("install", () => {
    ...
    test("should reject path traversal in target directory", async () => {
      ...
      assert.strictEqual(result.success, false);
      assert.ok(
        result.error?.includes("Invalid") || result.error?.includes("path"),
      );
    });
  });
  ...
  suite("Security", () => {
    ...
    test("should not allow absolute paths as package refs", () => {
      ...
      assert.strictEqual(
        wrapper.validatePackageRef("C:\\Windows\\System32"),
        false,
      );
    });
    ...
    test("should not allow URLs as package refs", () => {
      assert.strictEqual(
        wrapper.validatePackageRef("http://evil.com/malware"),
        false,
      );
      assert.strictEqual(
        wrapper.validatePackageRef("https://evil.com/malware"),
        false,
      );
    });
  });
