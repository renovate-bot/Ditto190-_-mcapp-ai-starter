...
suite("LocalModificationWarningService", () => {
  ...
  suite("checkForModifications()", () => {
    test("should return empty array when no modifications detected", async () => {
      ...
      assert.deepStrictEqual(
        result,
        [],
        "Should return empty array when no modifications",
      );
      assert.ok(
        mockLockfileManager.detectModifiedFiles.calledOnceWith(bundleId),
      );
    });
  ...
  suite("showWarningDialog()", () => {
    test("should display warning dialog with modified file list", async () => {
      ...
      assert.ok(
        ...
        "Message should mention modifications",
      );
      assert.ok(
        message.includes(".github/prompts/test.prompt.md"),
        "Message should list modified files",
      );
    });
    ...
    test("should include three action buttons: Contribute Changes, Override, Cancel", async () => {
      ...
      assert.ok(
        ...
        "Should have Contribute Changes button",
      );
    ...
    test('should return "contribute" when user clicks Contribute Changes', async () => {
      ...
      assert.strictEqual(
        result,
        "contribute",
        'Should return "contribute" action',
      );
    });
    ...
    test('should return "cancel" when user dismisses dialog', async () => {
      ...
      assert.strictEqual(
        result,
        "cancel",
        'Should return "cancel" when dismissed',
      );
    });
    ...
    test("should list all modified files in dialog message", async () => {
      ...
      assert.ok(
        message.includes("file3.instructions.md"),
        "Should list third file",
      );
    });
    ...
    test("should indicate file modification type in message", async () => {
      ...
      const message = showWarningMessageStub.firstCall.args[0] as string;
      assert.ok(
        message.includes("modified") || message.includes("changed"),
  ...
  suite("checkAndWarn()", () => {
    test("should return null when no modifications detected", async () => {
      ...
      assert.strictEqual(
        result,
        null,
        "Should return null when no modifications",
      );
      assert.ok(
    ...
    test("should pass bundle repo URL to dialog", async () => {
      ...
      assert.ok(
        ...
        "Should open URL when Contribute Changes clicked",
      );
      const uri = openExternalStub.firstCall.args[0] as vscode.Uri;
    ...
    test("should combine check and warn in single call", async () => {
      ...
      // Assert
      assert.ok(
        mockLockfileManager.detectModifiedFiles.calledOnce,
        "Should check for modifications",
      );
  ...
  suite("Edge cases", () => {
    ...
    test("should handle LockfileManager errors gracefully", async () => {
      // Arrange
      const bundleId = "test-bundle";
      mockLockfileManager.detectModifiedFiles.rejects(
        new Error("Lockfile error"),
      );
    ...
    test("should handle openExternal errors gracefully", async () => {
      ...
      // Act
      const result = await service.showWarningDialog(
        bundleId,
        modifiedFiles,
        bundleRepoUrl,
      );
      ...
      assert.strictEqual(
        result,
        "contribute",
        "Should return contribute action even if URL fails",
      );
    });
  });
});
