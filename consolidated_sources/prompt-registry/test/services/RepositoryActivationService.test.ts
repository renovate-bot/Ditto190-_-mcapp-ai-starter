...
suite("RepositoryActivationService", () => {
  ...
  suite("getInstance()", () => {
    ...
    test("should return same instance for same workspace", () => {
      ...
      const instance2 =
        RepositoryActivationService.getInstance(testWorkspaceRoot);
  ...
  suite("getExistingInstance()", () => {
    test("should return existing instance", () => {
      ...
      // Act
      const existing =
        RepositoryActivationService.getExistingInstance(testWorkspaceRoot);
    ...
    test("should return undefined for non-existent workspace", () => {
      // Act
      const existing =
        RepositoryActivationService.getExistingInstance("/non/existent");
  ...
  suite("checkAndPromptActivation()", () => {
    test("should not prompt when no lockfile exists", async () => {
      ...
      assert.ok(
        ...
        "Should not show prompt when no lockfile",
      );
    });
    ...
    test("should not prompt when repository was previously declined", async () => {
      ...
      assert.ok(
        !showInformationMessageStub.called,
        "Should not prompt when previously declined",
      );
    });
    ...
    test("should prompt when lockfile exists and not previously declined", async () => {
      ...
      const customContext = {
        globalState: {
          get: sandbox
            .stub()
            .withArgs("repositoryActivation.declined")
            .returns([]),
        },
      } as any;
    ...
    test("should check for missing sources when lockfile exists", async () => {
      ...
      if (showInformationMessageStub.called) {
        ...
        assert.ok(
          !message.includes("enable") && !message.includes("Enable"),
    ...
    test("should call checkAndOfferMissingSources when lockfile exists", async () => {
      ...
      assert.ok(
        checkSpy.calledWith(lockfile),
        "Should pass lockfile to check method",
      );
    });
    ...
    test("should skip detection for declined repositories", async () => {
      ...
      assert.ok(
        !checkSpy.called,
        "Should not check for missing sources when declined",
      );
      assert.ok(
        !showInformationMessageStub.called,
        "Should not show any prompt when declined",
      );
    });
  });
  ...
  suite("checkAndOfferMissingSources()", () => {
    ...
    test("should offer to add missing hubs", async () => {
      ...
      const message = showInformationMessageStub.firstCall.args[0] as string;
      assert.ok(
        message.toLowerCase().includes("hub") ||
...
suite("RepositoryActivationService - Workspace Switching Scenarios", () => {
  ...
  test("should allow independent source detection per workspace", async () => {
    ...
    mockLockfileManager1.getLockfilePath.returns(
      `${workspace1}/prompt-registry.lock.json`,
    );

    mockLockfileManager2.read.resolves(lockfile2);
    mockLockfileManager2.getLockfilePath.returns(
      `${workspace2}/prompt-registry.lock.json`,
    );
    ...
    assert.ok(
      mockLockfileManager1.read.calledOnce,
      "Should read lockfile for workspace 1",
    );
    assert.ok(
      mockLockfileManager2.read.calledOnce,
      "Should read lockfile for workspace 2",
    );
  });
  ...
  test("should normalize paths for consistent instance lookup", () => {
    ...
    const instance2 =
      RepositoryActivationService.getExistingInstance(workspace);

    // Assert
    assert.strictEqual(
      instance1,
      instance2,
      "Should find same instance with normalized path",
    );
  });

  test("should return undefined for non-existent workspace in getExistingInstance", () => {
    // Act
    const instance = RepositoryActivationService.getExistingInstance(
    ...
    assert.strictEqual(
      instance,
      undefined,
      "Should return undefined for non-existent workspace",
    );
  });
  ...
  test("should preserve other workspace instances when resetting one", () => {
    ...
    const existingService2 =
      RepositoryActivationService.getExistingInstance(workspace2);
    assert.strictEqual(
      existingService2,
      service2,
      "Should preserve other workspace instances",
    );

    // workspace1 should be gone
    const existingService1 =
      RepositoryActivationService.getExistingInstance(workspace1);
    assert.strictEqual(
      existingService1,
      undefined,
      "Should have removed workspace1 instance",
    );
  });
});
...
suite("RepositoryActivationService - Missing Bundle Installation", () => {
  ...
  suite("installMissingBundles()", () => {
    test("should install missing bundles when user accepts", async () => {
      ...
      // Act
      const result = await service.installMissingBundles(
        lockfile,
        missingBundleIds,
      );
      ...
      assert.strictEqual(
        ...
        2,
        "Should report 2 successful installations",
      );
    ...
    test("should handle partial failure when some bundles fail to install", async () => {
      ...
      mockRegistryManager.installBundle
        ...
        .onSecondCall()
        .rejects(new Error("Installation failed"))
        .onThirdCall()
        .resolves({
          bundleId: "bundle-2",
          version: "3.0.0",
      ...
      assert.strictEqual(
        result.succeeded.length,
        2,
        "Should have 2 successful installations",
      );
      assert.strictEqual(result.failed.length, 1, "Should have 1 failure");
      assert.strictEqual(
        result.failed[0].bundleId,
        "bundle-1",
        "Should identify failed bundle",
      );
      assert.ok(
        result.failed[0].error.includes("Installation failed"),
    ...
    test("should show progress notification during batch installation", async () => {
      ...
      assert.ok(
        progressOptions.title.includes("Installing"),
        "Progress title should mention installing",
      );
    ...
    test("should use source information from lockfile for installation", async () => {
      ...
      // Assert
      assert.ok(
        mockRegistryManager.installBundle.calledOnce,
        "Should call installBundle",
      );
    ...
    test("should use repository scope with correct commitMode from lockfile", async () => {
      ...
      assert.strictEqual(
        options.scope,
        "repository",
        "Should use repository scope",
      );
      assert.strictEqual(
        options.commitMode,
        "local-only",
        "Should use commitMode from lockfile",
      );
    });
    ...
    test("should handle cancellation during batch installation", async () => {
      ...
      withProgressStub.callsFake(async (_options: any, task: any) => {
        ...
        mockRegistryManager.installBundle.callsFake(async () => {
          ...
          return {
            bundleId: `bundle-${installCount - 1}`,
            version: "1.0.0",
            scope: "repository",
          };
        });
      ...
      // Assert
      assert.ok(
        result.succeeded.length < 3,
        "Should stop before installing all bundles",
      );
    ...
    test("should return empty result when no bundles to install", async () => {
      ...
      assert.ok(
        !mockRegistryManager.installBundle.called,
        "Should not call installBundle",
      );
    });
    ...
    test("should use version from lockfile for installation", async () => {
      ...
      assert.strictEqual(
        options.version,
        "2.5.0",
        "Should use version from lockfile",
      );
    });
    ...
    test("should skip bundles not found in lockfile", async () => {
      ...
      assert.strictEqual(
        ...
        1,
        "Should report 1 skipped bundle",
      );
...
suite("RepositoryActivationService - Setup Timing Behavior", () => {
  ...
  suite("checkAndPromptActivation() with SetupStateManager", () => {
    test("should skip detection when SetupStateManager.isComplete() returns false", async () => {
      ...
      mockLockfileManager.read.resolves(lockfile);
      mockLockfileManager.getLockfilePath.returns(
        `${testWorkspaceRoot}/prompt-registry.lock.json`,
      );
      ...
      // Assert
      assert.ok(
        mockSetupStateManager.isComplete.calledOnce,
        "Should check if setup is complete",
      );
      assert.ok(
        !mockLockfileManager.read.called,
    ...
    test("should proceed with detection when SetupStateManager.isComplete() returns true", async () => {
      ...
      assert.ok(
        mockLockfileManager.read.calledOnce,
    ...
    test("should log appropriate message when deferring due to incomplete setup", async () => {
      ...
      assert.ok(
        !mockStorage.getSources.called,
        "Should not check sources when deferring",
      );
      assert.ok(
        !mockHubManager.listHubs.called,
