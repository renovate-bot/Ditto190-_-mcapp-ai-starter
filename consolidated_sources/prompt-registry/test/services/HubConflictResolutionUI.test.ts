...
suite("Hub Conflict Resolution UI", () => {
  ...
  suite("Change Summary Formatting", () => {
    test("should format bundle additions", async () => {
      ...
      await storage.saveHub("test-hub", hub, {
        type: "github",
        location: "test/repo",
      });
      await hubManager.activateProfile("test-hub", "profile-1", {
      ...
      const changes = await hubManager.getProfileChanges(
        "test-hub",
        "profile-1",
      );
  ...
  suite("QuickPick Item Formatting", () => {
    ...
    test("should mark required bundles", async () => {
      ...
      const requiredItem = items.find((item) =>
        item.label.includes("bundle-2"),
      );
      assert.ok(requiredItem);
      assert.ok(
        requiredItem.description?.includes("required") ||
          requiredItem.label.includes("*"),
      );
    });
  ...
  suite("Conflict Resolution Dialog", () => {
    ...
    test("should include sync action option", async () => {
      ...
      const syncOption = dialog.options.find((opt) =>
        opt.label.includes("Sync"),
      );
    ...
    test("should include review option", async () => {
      ...
      const reviewOption = dialog.options.find((opt) =>
        opt.label.includes("Review"),
      );
  ...
  suite("Change Detail Formatting", () => {
    test("should format detailed bundle addition info", async () => {
      ...
      const detail = hubManager.formatBundleAdditionDetail(
        changes!.bundlesAdded![0],
      );
    ...
    test("should format detailed bundle removal info", async () => {
      ...
      const detail = hubManager.formatBundleRemovalDetail(
        changes!.bundlesRemoved![0],
      );
    ...
    test("should format detailed bundle update info", async () => {
      ...
      const detail = hubManager.formatBundleUpdateDetail(
        changes!.bundlesUpdated![0],
      );
