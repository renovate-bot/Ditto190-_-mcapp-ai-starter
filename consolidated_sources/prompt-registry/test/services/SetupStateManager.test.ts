...
suite("SetupStateManager - Unit Tests", () => {
  ...
  suite("getInstance()", () => {
    test("should return singleton instance", () => {
      const instance1 = SetupStateManager.getInstance(
        mockContext,
        mockHubManager as any,
      );
      const instance2 = SetupStateManager.getInstance(
        mockContext,
        mockHubManager as any,
      );
    ...
    test("should return existing instance without parameters after first call", () => {
      ...
      assert.strictEqual(
        instance1,
        instance2,
        "Should return same instance without parameters",
      );
    });
  });
  ...
  suite("reset()", () => {
    ...
    test("should reset resume prompt shown flag (in-memory)", async () => {
      ...
      assert.strictEqual(
        shouldShow,
        false,
        "Should be false after marking shown",
      );
  ...
  suite("markResumePromptShown()", () => {
    test("should set resume prompt shown flag in-memory only", async () => {
      ...
      const promptShown = globalStateData.get(
        "promptregistry.resumePromptShown",
      );
      assert.strictEqual(
        promptShown,
        undefined,
        "Should not persist to globalState",
      );
      ...
      assert.strictEqual(
        shouldShow,
        false,
        "Should remember within same instance",
      );
    });
    ...
    test("should reset flag when instance is reset (session-scoped)", async () => {
      ...
      assert.strictEqual(
        shouldShow,
        true,
        "Should be true in new instance (flag reset)",
      );
    });
  });
