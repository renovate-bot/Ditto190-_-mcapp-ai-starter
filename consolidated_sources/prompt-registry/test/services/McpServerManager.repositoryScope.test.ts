...
suite("McpServerManager Repository Scope Test Suite", () => {
  ...
  const readTrackingMetadata =
    async (): Promise<McpTrackingMetadata | null> => {
  ...
  suite("installServersToWorkspace()", () => {
    ...
    test("should preserve existing MCP servers in .vscode/mcp.json", async () => {
      ...
      assert.ok(config);
      assert.ok(
        config.servers["user-server-1"],
        "user-server-1 should be preserved",
      );
      assert.ok(
        config.servers["user-server-2"],
        "user-server-2 should be preserved",
      );
    });
    ...
    test("should track bundle-specific MCP servers for uninstallation", async () => {
      ...
      const trackedServers = Object.entries(tracking.managedServers).filter(
        ([_, meta]) => meta.bundleId === "test-bundle",
      );
      assert.strictEqual(
        trackedServers.length,
        2,
    ...
    test("should overwrite conflicting server when overwrite option is true", async () => {
      ...
      assert.ok(config);
      const serverConfig =
        config.servers["prompt-registry:test-bundle:my-server"];
      assert.ok(serverConfig);
      assert.ok(
        !serverConfig.type || serverConfig.type === "stdio",
        "should be stdio server",
      );
      assert.strictEqual(
        (serverConfig as any).command,
        "node",
        "server should be overwritten",
      );
    });
    ...
    test("should create .vscode directory if it does not exist", async () => {
      ...
      assert.strictEqual(result.success, true);
      assert.ok(
        await fs.pathExists(vscodeDir),
        ".vscode directory should be created",
      );
      assert.ok(
        await fs.pathExists(getMcpConfigPath()),
  ...
  suite("uninstallServersFromWorkspace()", () => {
    test("should remove MCP servers from .vscode/mcp.json on uninstall", async () => {
      ...
      assert.ok(config);
      const serverKey = Object.keys(config.servers).find((k) =>
        k.includes("server-to-remove"),
      );
      assert.ok(serverKey, "server should be installed");

      // Act
      const result = await manager.uninstallServersFromWorkspace(
        "test-bundle",
        testWorkspaceRoot,
      );
      ...
      assert.ok(config);
      assert.ok(
        !Object.keys(config.servers).some((k) =>
          k.includes("server-to-remove"),
        ),
    ...
    test("should clean up git exclude entries on uninstall for local-only mode", async () => {
      ...
      assert.ok(
        excludeContent?.includes(".vscode/mcp.json"),
        "mcp.json should be in git exclude",
      );

      // Act
      await manager.uninstallServersFromWorkspace(
        "test-bundle",
        testWorkspaceRoot,
      );
  ...
  suite("getServersForBundleInWorkspace()", () => {
    test("should return servers for a specific bundle", async () => {
      ...
      // Act
      const servers = await manager.getServersForBundleInWorkspace(
        "test-bundle",
        testWorkspaceRoot,
      );
