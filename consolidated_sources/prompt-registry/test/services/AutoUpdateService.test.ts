...
suite("AutoUpdateService", () => {
  ...
  suite("autoUpdateBundle()", () => {
    test("should call updateBundle with correct parameters", async () => {
      ...
      assert.strictEqual(
        mockRegistryManager.updateBundle.firstCall.args[0],
        bundleId,
      );
      assert.strictEqual(
        mockRegistryManager.updateBundle.firstCall.args[1],
        targetVersion,
      );
    });
    ...
    test("should show failure notification on error", async () => {
      ...
      const failureMessage =
        mockBundleNotifications.showUpdateFailure.firstCall.args[1];
      assert.ok(
        failureMessage.includes(errorMessage) ||
          failureMessage.includes("Rollback failed"),
  ...
  suite("isAutoUpdateEnabled()", () => {
    test("should return storage preference", async () => {
      ...
      assert.strictEqual(
        mockStorage.getUpdatePreference.firstCall.args[0],
        bundleId,
      );
    });
  });
  ...
  suite("setAutoUpdate()", () => {
    test("should update storage preference", async () => {
      ...
      assert.strictEqual(
        mockStorage.setUpdatePreference.firstCall.args[0],
        bundleId,
      );
      assert.strictEqual(
        mockStorage.setUpdatePreference.firstCall.args[1],
        true,
      );
    });
  });
