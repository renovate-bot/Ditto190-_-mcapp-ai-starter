      ...
      await fc.assert(
        fc.asyncProperty(
          ...
          async (bundleId, version, existingScope, targetScope) => {
            ...
            const conflict = await resolver.checkConflict(
              bundleId,
              targetScope,
            );
        ...
        fc.asyncProperty(
          ...
          async (bundleId, version, fromScope, toScope) => {
            ...
            const mockInstall = async () => {
              ...
              const newBundle = createMockInstalledBundle(bundleId, version, {
                scope: toScope,
              });
              mockStorage.getInstalledBundle
                .withArgs(bundleId, toScope)
            ...
            const conflictingScopes =
              await resolver.getConflictingScopes(bundleId);
            assert.strictEqual(
              conflictingScopes.length,
              1,
          ...
          async (bundleId, version, installedScopes) => {
            ...
            for (const scope of ALL_SCOPES) {
              if (installedScopes.includes(scope)) {
                const bundle = createMockInstalledBundle(bundleId, version, {
                  scope,
                });
                mockStorage.getInstalledBundle
                  .withArgs(bundleId, scope)
