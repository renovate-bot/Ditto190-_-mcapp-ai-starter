...
suite("Bundle State Management - Integration Tests", () => {
  ...
  suite(
    "Workflow 1: Install GitHub bundle → Uninstall → Verify UI shows Install button",
    () => {
      test("should show Install button after uninstalling GitHub bundle", async () => {
        ...
        const mockAdapter = {
          ...
          downloadBundle: sandbox
            .stub()
            .resolves(Buffer.from("mock-bundle-data")),
        };
        sandbox
        ...
        assert.ok(
          mockStorage.recordInstallation.calledOnce,
  ...
  suite(
    ...
    () => {
      test("should show Update button when newer version available after sync", async () => {
        ...
        assert.ok(
          mockStorage.cacheSourceBundles.calledOnce,
  ...
  suite(
    ...
    () => {
      test("should auto-update installed bundles when syncing Awesome Copilot source", async () => {
        // Setup: Bundle v1.0.0 is installed, v1.1.0 becomes available
        const bundleV1 = BundleBuilder.fromSource(
          "awesome-bundle",
          "AWESOME_COPILOT",
        )
          .withVersion("1.0.0")
        ...
        const bundleV1_1 = BundleBuilder.fromSource(
          "awesome-bundle",
          "AWESOME_COPILOT",
        )
          .withVersion("1.1.0")
        ...
        assert.ok(
          updateOccurred,
          "Should auto-update bundles from Awesome Copilot source",
        );
      });
      ...
      test("should NOT auto-update bundles from other sources when syncing Awesome Copilot source", async () => {
        ...
        const awesomeBundle = BundleBuilder.fromSource(
          "awesome-bundle",
          "AWESOME_COPILOT",
        )
          .withVersion("1.0.0")
        ...
        const installed = await mockStorage.getInstalledBundles();
        const githubInstalled = installed.find(
          (i) => i.sourceId === "github-source",
        );
  ...
  suite("Bug Fix: Downgrade removes old version from display", () => {
    test("should show bug: v1.0.18 and v1.0.17 both installed after downgrade", async () => {
      ...
      assert.strictEqual(
        installed.length,
        1,
        "Should have v1.0.18 installed initially",
      );
      assert.strictEqual(
        installed[0].version,
        "1.0.18",
        "Initial version should be 1.0.18",
      );
    ...
    test("should cleanup old version when downgrading through RegistryManager", async () => {
      ...
      assert.strictEqual(
        installed.length,
        1,
        "Initially v1.0.18 should be installed",
      );
      assert.strictEqual(
        installed[0].version,
        "1.0.18",
        "Should be version 1.0.18",
      );
  ...
  suite("Rollback & Failure Recovery - Integration Tests", () => {
    test("should FAIL if cleanup happens BEFORE recording (catches the bug)", async () => {
      ...
      assert.ok(
        installedBundles.has(`${bundleIdBase}-1.0.18`),
        "v1.0.18 should be recorded",
      );
    ...
    test("should preserve old version when update fails during storage recording", async () => {
      ...
      assert.strictEqual(
        installed.length,
        1,
        "Should have v1.0.17 installed initially",
      );
      assert.strictEqual(
        installed[0].version,
        "1.0.17",
        "Version should be 1.0.17",
      );
    ...
    test("should allow retry of failed update by checking if old version still exists", async () => {
      ...
      assert.ok(
        v1_0_17_exists,
        "Old version should still be available for rollback",
      );
  ...
  suite(
    "Bundle Details View: Opening details for installed versioned bundles",
    () => {
      test("should find bundle details when opening installed GitHub bundle with versioned ID (e.g., bundle-1.0.17)", async () => {
        ...
        const bundleDetails =
          await registryManager.getBundleDetails(versionedBundleId);
        ...
        assert.strictEqual(
          bundleDetails.id,
          bundleIdentity,
          "Bundle ID should match identity",
        );
        assert.strictEqual(
          bundleDetails.name,
          bundle.name,
