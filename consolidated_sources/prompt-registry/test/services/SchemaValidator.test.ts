...
suite("SchemaValidator", () => {
  ...
  setup(() => {
    ...
    testCollectionSchemaPath = path.join(
      process.cwd(),
      "schemas",
  ...
  suite("Collection Validation", () => {
    ...
    test("should detect invalid id format", async function () {
      ...
      assert.strictEqual(result.valid, false);
      assert.ok(
        result.errors.some((e) => e.includes("pattern") || e.includes("id")),
      );
    });
    ...
    test("should detect invalid item kind", async function () {
      ...
      assert.ok(
        result.errors.some(
          (e) => e.includes("allowed values") || e.includes("enum"),
        ),
      );
    });
    ...
    test("should detect description too long", async function () {
      ...
      assert.strictEqual(result.valid, false);
      assert.ok(
        result.errors.some((e) => e.includes("maximum") || e.includes("500")),
      );
    });
  });
