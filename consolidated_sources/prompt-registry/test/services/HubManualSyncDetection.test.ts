...
suite("Hub Manual Sync Detection", () => {
  ...
  suite("Profile Change Detection", () => {
    test("should detect when active profile has changed in hub", async () => {
      ...
      updated.config.profiles[0].updatedAt = new Date(
        Date.now() + 1000,
      ).toISOString();
      await storage.saveHub("test-hub", updated.config, updated.reference);

      const hasChanges = await hubManager.hasProfileChanges(
        "test-hub",
        "profile-1",
      );
    ...
    test("should detect bundle removals", async () => {
      ...
      updated.config.profiles[0].bundles =
        updated.config.profiles[0].bundles.filter((b) => b.id !== "bundle-2");
      updated.config.profiles[0].updatedAt = new Date(
        Date.now() + 1000,
      ).toISOString();
      await storage.saveHub("test-hub", updated.config, updated.reference);
  ...
  suite("Last Sync Tracking", () => {
    ...
    test("should update last sync on profile sync", async () => {
      ...
      const state1 = await storage.getProfileActivationState(
        "test-hub",
        "profile-1",
      );
      assert.ok(state1);
      ...
      const state2 = await storage.getProfileActivationState(
        "test-hub",
        "profile-1",
      );
      assert.ok(state2);
    ...
    test("should get time since last sync", async () => {
      ...
      const timeSince = await hubManager.getTimeSinceLastSync(
        "test-hub",
        "profile-1",
      );
      assert.ok(timeSince);
  ...
  suite("Hub Update Detection", () => {
    ...
    test("should list profiles with pending updates", async () => {
      ...
      const profilesWithUpdates =
        await hubManager.getProfilesWithUpdates("test-hub");
