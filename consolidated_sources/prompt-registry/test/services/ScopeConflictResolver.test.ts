...
suite("ScopeConflictResolver", () => {
    ...
    return {
      globalState: {
        get: (key: string, defaultValue?: any) =>
          globalStateData.get(key) ?? defaultValue,
      ...
      asAbsolutePath: (relativePath: string) =>
        path.join("/mock/extension", relativePath),
  ...
  suite("checkConflict()", () => {
    test("should return null when bundle is not installed anywhere", async () => {
      ...
      assert.strictEqual(
        result,
        null,
        "Should return null when no conflict exists",
      );
    });
    ...
    test("should return null when bundle is only installed at target scope", async () => {
      ...
      mockStorage.getInstalledBundle
        .withArgs(bundleId, "user")
        .resolves(undefined);
      mockStorage.getInstalledBundle
        .withArgs(bundleId, "workspace")
        .resolves(undefined);
      mockStorage.getInstalledBundle
        .withArgs(bundleId, "repository")
      ...
      assert.strictEqual(
        result,
        null,
        "Should return null when bundle is only at target scope",
      );
    });
    ...
    test("should detect conflict when bundle is at repository scope and target is user", async () => {
      // Arrange
      const bundleId = "test-bundle";
      const installedBundle = createMockInstalledBundle(bundleId, "2.0.0", {
        scope: "repository",
      });
    ...
    test("should detect conflict when bundle is at workspace scope and target is repository", async () => {
      // Arrange
      const bundleId = "test-bundle";
      const installedBundle = createMockInstalledBundle(bundleId, "1.5.0", {
        scope: "workspace",
      });
    ...
    test("should check all scopes except target scope", async () => {
      ...
      assert.ok(
        mockStorage.getInstalledBundle.calledWith(bundleId, "user"),
        "Should check user scope",
      );
      assert.ok(
        mockStorage.getInstalledBundle.calledWith(bundleId, "workspace"),
        "Should check workspace scope",
      );
    });
  });
  ...
  suite("migrateBundle()", () => {
    ...
    test("should successfully migrate bundle from user to repository scope", async () => {
      ...
      assert.ok(
        mockUninstallCallback.calledOnce,
        "Uninstall should be called once",
      );
      assert.ok(
        mockInstallCallback.calledOnce,
        "Install should be called once",
      );
      assert.ok(
        mockUninstallCallback.calledBefore(mockInstallCallback),
        "Uninstall should be called before install",
      );
    });
    ...
    test("should fail migration when bundle is not installed at source scope", async () => {
      ...
      assert.ok(
        ...
        "Error should mention bundle not installed",
      );
      assert.ok(
        !mockUninstallCallback.called,
    ...
    test("should fail migration when uninstall fails", async () => {
      ...
      assert.ok(result.error, "Should have error message");
      assert.ok(
        result.error!.includes("Uninstall failed"),
        "Error should contain original error",
      );
      assert.ok(
        !mockInstallCallback.called,
        "Install should not be called after uninstall failure",
      );
    });
    ...
    test("should fail migration when install fails", async () => {
      ...
      assert.ok(result.error, "Should have error message");
      assert.ok(
        result.error!.includes("Install failed"),
        "Error should contain original error",
      );
    });
    ...
    test("should pass installed bundle info to callbacks", async () => {
      ...
      // Assert
      assert.ok(
        mockUninstallCallback.calledWith(installedBundle),
        "Uninstall should receive installed bundle",
      );
      assert.ok(
        mockInstallCallback.calledWith(installedBundle, "repository"),
        "Install should receive bundle and target scope",
      );
    });
    ...
    suite("rollback behavior", () => {
      test("should attempt rollback when install fails after successful uninstall", async () => {
        ...
        assert.ok(
          result.error!.includes("Rollback successful"),
          "Error should indicate rollback success",
        );
        assert.ok(
          result.error!.includes("restored at user"),
          "Error should mention original scope",
        );
        assert.strictEqual(
          mockInstallCallback.callCount,
          2,
          "Install should be called twice (target + rollback)",
        );
      });
      ...
      test("should report rollback failure when both install and rollback fail", async () => {
        ...
        mockInstallCallback
          .onSecondCall()
          .rejects(new Error("Rollback install failed"));
        ...
        assert.ok(
          result.error!.includes("Rollback also failed"),
          "Error should indicate rollback failure",
        );
        assert.ok(
          result.error!.includes("inconsistent state"),
          "Error should warn about inconsistent state",
        );
        assert.strictEqual(
          mockInstallCallback.callCount,
          2,
          "Install should be called twice",
        );
      });
      ...
      test("should call rollback with original scope", async () => {
        ...
        assert.strictEqual(
          secondCall.args[1],
          fromScope,
          "Rollback should use original scope",
        );
      });
      ...
      test("should preserve original bundle state on successful rollback", async () => {
        ...
        assert.deepStrictEqual(
          rollbackCall.args[0],
          installedBundle,
          "Rollback should use original bundle info",
        );
      });
      ...
      test("should not attempt rollback when uninstall fails", async () => {
        ...
        assert.strictEqual(
          result.rollbackAttempted,
          undefined,
          "Rollback should not be attempted",
        );
        assert.strictEqual(
          result.rollbackSucceeded,
          undefined,
          "Rollback succeeded should not be set",
        );
        assert.ok(!mockInstallCallback.called, "Install should not be called");
      });
      ...
      test("should not set rollback fields on successful migration", async () => {
        ...
        assert.strictEqual(
          result.rollbackAttempted,
          undefined,
          "Rollback attempted should not be set",
        );
        assert.strictEqual(
          result.rollbackSucceeded,
          undefined,
  ...
  suite("hasConflict()", () => {
    test("should return true when conflict exists", async () => {
      ...
      assert.strictEqual(
        result,
        true,
        "Should return true when conflict exists",
      );
    });
  ...
  suite("getConflictingScopes()", () => {
    test("should return all scopes where bundle is installed", async () => {
      // Arrange
      const bundleId = "test-bundle";
      const userBundle = createMockInstalledBundle(bundleId, "1.0.0", {
        scope: "user",
      });
      const workspaceBundle = createMockInstalledBundle(bundleId, "1.0.0", {
        scope: "workspace",
      });
      ...
      assert.deepStrictEqual(
        result.sort(),
        ["user", "workspace"].sort(),
        "Should return all installed scopes",
      );
    });
    ...
    test("should return single scope when bundle is installed at one scope", async () => {
      // Arrange
      const bundleId = "test-bundle";
      const repoBundle = createMockInstalledBundle(bundleId, "1.0.0", {
        scope: "repository",
      });
  ...
  suite("Edge cases", () => {
    test("should handle empty bundle ID gracefully", async () => {
      ...
      assert.strictEqual(
        result,
        null,
        "Should return null for empty bundle ID",
      );
    });
