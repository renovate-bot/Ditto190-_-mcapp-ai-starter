...
suite("HubSchemaValidator - TDD", () => {
  ...
  suite("Format validation", () => {
    test("should validate version format (semver)", async () => {
      ...
      assert.ok(
        result.errors.some(
          (e) => e.includes("version") || e.includes("pattern"),
        ),
      );
    });
    ...
    test("should validate checksum format", async () => {
      ...
      assert.ok(
        result.errors.some(
          (e) => e.includes("checksum") || e.includes("pattern"),
        ),
      );
    });
    ...
    test("should accept sha256 and sha512 checksums", async () => {
      ...
      for (const checksum of checksums) {
        ...
        assert.strictEqual(
          result.valid,
          true,
          `Checksum ${checksum} should be valid`,
        );
      }
    });
    ...
    test("should validate source type enum", async () => {
      ...
      assert.strictEqual(result.valid, false);
      assert.ok(
        result.errors.some((e) => e.includes("type") || e.includes("enum")),
      );
    });
    ...
    test("should accept valid source types", async () => {
      ...
      for (const type of types) {
        ...
        assert.strictEqual(
          result.valid,
          true,
          `Source type ${type} should be valid`,
        );
      }
    });
  });
  ...
  suite("Type validation", () => {
    test("should reject non-string version", async () => {
      ...
      assert.ok(
        result.errors.some(
          (e) => e.includes("version") || e.includes("string"),
        ),
      );
    });
    ...
    test("should reject non-object metadata", async () => {
      ...
      assert.ok(
        result.errors.some(
          (e) => e.includes("metadata") || e.includes("object"),
        ),
      );
    });
    ...
    test("should reject non-array sources", async () => {
      ...
      assert.strictEqual(result.valid, false);
      assert.ok(
        result.errors.some((e) => e.includes("sources") || e.includes("array")),
      );
    });
    ...
    test("should reject non-boolean enabled field", async () => {
      ...
      assert.ok(
        result.errors.some(
          (e) => e.includes("enabled") || e.includes("boolean"),
        ),
      );
    });
    ...
    test("should reject non-number priority", async () => {
      ...
      assert.ok(
        result.errors.some(
          (e) => e.includes("priority") || e.includes("number"),
        ),
      );
    });
  });
  ...
  suite("Additional properties", () => {
    test("should handle additional properties based on schema config", async () => {
      const config = JSON.parse(JSON.stringify(validHubConfig));
      config.extraField =
        "should be handled based on additionalProperties setting";
  ...
  suite("Profile path validation", () => {
    ...
    test("should accept path with spaces", async () => {
      ...
      assert.strictEqual(
        result.valid,
        true,
        "Profile path with spaces should be valid",
      );
    });
    ...
    test("should accept path with dots", async () => {
      ...
      assert.strictEqual(
        result.valid,
        true,
        "Profile path with dots should be valid",
      );
    });
    ...
    test("should reject path with invalid characters like slashes", async () => {
      ...
      assert.strictEqual(result.valid, false);
      assert.ok(
        result.errors.some((e) => e.includes("path") || e.includes("pattern")),
      );
    });
    ...
    test("should reject path that is not an array", async () => {
      ...
      assert.strictEqual(result.valid, false);
      assert.ok(
        result.errors.some((e) => e.includes("path") || e.includes("array")),
      );
    });
  });
  ...
  suite("Engagement configuration validation", () => {
    ...
    test("should accept all backend types", async () => {
      ...
      for (const backendType of backendTypes) {
        ...
        assert.strictEqual(
          result.valid,
          true,
          `Backend type ${backendType} should be valid`,
        );
      }
    });
    ...
    test("should reject invalid repository format in backend", async () => {
      ...
      assert.ok(
        result.errors.some(
          (e) => e.includes("repository") || e.includes("pattern"),
        ),
      );
    });
    ...
    test("should validate feedback maxLength constraints", async () => {
      ...
      assert.strictEqual(
        result.valid,
        false,
        "maxLength below minimum should fail",
      );
      assert.ok(
        result.errors.some(
          (e) => e.includes("maxLength") || e.includes("minimum"),
        ),
      );
    });
    ...
    test("should accept hub config without engagement section", async () => {
      ...
      assert.strictEqual(
        result.valid,
        true,
        "Hub config without engagement should be valid",
      );
    });
  });
});
