...
suite("SetupStateManager - Property Tests", () => {
    ...
    await fc.assert(
      fc.asyncProperty(
        ...
        async (operations) => {
          ...
          SetupStateManager.resetInstance();
          const manager = SetupStateManager.getInstance(
            mockContext,
          ...
          const validTransitions: Record<SetupState, SetupState[]> = {
            ...
            [SetupState.IN_PROGRESS]: [
              ...
              SetupState.INCOMPLETE,
              SetupState.NOT_STARTED,
            ], // Can reset from in_progress
            [SetupState.COMPLETE]: [
              SetupState.NOT_STARTED,
              SetupState.IN_PROGRESS,
              SetupState.INCOMPLETE,
            ], // Reset, re-setup, or error
            [SetupState.INCOMPLETE]: [
              SetupState.IN_PROGRESS,
              SetupState.NOT_STARTED,
              SetupState.COMPLETE,
            ], // Resume, reset, or direct complete
          };
  ...
  test("Property 6: Setup completion is idempotent (Req 5.1, 5.2, 5.3)", async () => {
    await fc.assert(
      fc.asyncProperty(fc.integer({ min: 1, max: 10 }), async (repeatCount) => {
        globalStateData.clear();
        SetupStateManager.resetInstance();
        const manager = SetupStateManager.getInstance(
          mockContext,
  ...
  test("Property 9: State persists across manager instances (Req 1.4, 2.6)", async () => {
    ...
    await fc.assert(
      fc.asyncProperty(stateArbitrary, async (targetState) => {
        globalStateData.clear();
        SetupStateManager.resetInstance();
        const manager1 = SetupStateManager.getInstance(
          mockContext,
          mockHubManager as any,
        );
        ...
        SetupStateManager.resetInstance();
        const manager2 = SetupStateManager.getInstance(
          mockContext,
          mockHubManager as any,
        );
    ...
    await fc.assert(
      fc.asyncProperty(
        ...
        async (firstRun, hubInitialized, hasHubs, hasActiveHub) => {
          ...
          const { mockHubs, mockActiveHub } = createMockHubData(
            hasHubs,
            hasActiveHub,
          );
    ...
    await fc.assert(
      fc.asyncProperty(
        ...
        async (checkCount) => {
          ...
          const promptShownFlag = globalStateData.get(
            "promptregistry.resumePromptShown",
          );
          assert.strictEqual(
            promptShownFlag,
            undefined,
          ...
          SetupStateManager.resetInstance();
          const newSessionManager = SetupStateManager.getInstance(
            mockContext,
            mockHubManager as any,
          );
          await newSessionManager.markIncomplete();
          const shouldShowInNewSession =
            await newSessionManager.shouldShowResumePrompt();
          assert.strictEqual(
