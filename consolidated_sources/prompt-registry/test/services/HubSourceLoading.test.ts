...
class MockRegistryManager {
  ...
  public updateSourceCalls: Array<{
    id: string;
...
suite("Hub Source Loading", () => {
  ...
  suite("Source Loading Basics", () => {
    test("should load enabled sources from hub into registry", async () => {
      ...
      const expectedSource1Id = generateHubSourceId(
        "awesome-copilot",
        "https://github.com/github/awesome-copilot",
      );
      const expectedSource2Id = generateHubSourceId(
        "awesome-copilot",
      ...
      assert.ok(
        sources.some((s) => s.id === expectedSource1Id),
        `Should have source with id ${expectedSource1Id}`,
      );
      assert.ok(
        sources.some((s) => s.id === expectedSource2Id),
        `Should have source with id ${expectedSource2Id}`,
      );
    ...
    test("should skip disabled sources", async () => {
      ...
      assert.strictEqual(
        sources.length,
        1,
        "Should have only 1 enabled source loaded",
      );

      // Compute expected sourceIds using the new format
      const expectedEnabledSourceId = generateHubSourceId(
        "awesome-copilot",
        "https://github.com/github/awesome-copilot",
      );
      const expectedDisabledSourceId = generateHubSourceId(
        "awesome-copilot",
        "https://github.com/disabled/repo",
      );

      // Check correct source was loaded
      assert.ok(
        mockRegistry.hasSource(expectedEnabledSourceId),
        "Should have enabled source",
      );
      assert.ok(
        !mockRegistry.hasSource(expectedDisabledSourceId),
  ...
  suite("Duplicate Detection", () => {
    test("should skip duplicate when URL and type match exactly", async () => {
      ...
      assert.strictEqual(
        hubSource1,
        undefined,
        "Duplicate source-1 should be skipped",
      );
    ...
    test("should allow same URL with different branch", async () => {
      ...
      assert.strictEqual(
        sources.length,
        2,
        "Should have 2 sources (different branches)",
      );
    });
    ...
    test("should skip duplicate across multiple hubs", async () => {
      ...
      assert.ok(
        mockRegistry.hasSource(expectedSource1Id),
        `Should have source with id ${expectedSource1Id}`,
      );
      assert.ok(
        mockRegistry.hasSource(expectedSource2Id),
        `Should have source with id ${expectedSource2Id}`,
      );
    });
  });
  ...
  suite("Integration with Hub Operations", () => {
    test("should load sources automatically when importing hub", async () => {
      ...
      assert.ok(
        mockRegistry.addSourceCalls.length > 0,
        "addSource should have been called",
      );
    });
    ...
    test("should reload sources when syncing hub", async () => {
      ...
      assert.strictEqual(
        sourcesAfterImport.length,
        2,
        "Should have 2 sources after import",
      );

      // Modify the hub config to add a 3rd source
      const yaml = require("js-yaml");
      const hubConfig = yaml.load(
        fs.readFileSync(tempHubPath, "utf-8"),
      ) as HubConfig;
      ...
      assert.strictEqual(
        sourcesAfterSync.length,
        3,
        "Should have 3 sources after sync",
      );

      // Compute expected sourceId for the new source using the new format
      const expectedSource3Id = generateHubSourceId(
        "awesome-copilot",
        "https://github.com/org/new-repo",
      );
      assert.ok(
        mockRegistry.hasSource(expectedSource3Id),
        `Should have the newly added source with id ${expectedSource3Id}`,
      );
    });
  });
});
