...
suite("RepositoryScopeService", () => {
  ...
  setup(() => {
    ...
    // Create service
    service = new RepositoryScopeService(
      workspaceRoot,
      mockStorage as unknown as RegistryStorage,
    );
  });
  ...
  suite("Service Initialization", () => {
    ...
    test("should have IScopeService methods", () => {
      assert.ok(
        typeof service.syncBundle === "function",
        "Should have syncBundle method",
      );
      assert.ok(
      ...
      assert.ok(
        typeof service.getTargetPath === "function",
        "Should have getTargetPath method",
      );
      assert.ok(
    ...
    test("should have switchCommitMode method", () => {
      assert.ok(
        typeof service.switchCommitMode === "function",
        "Should have switchCommitMode method",
      );
    });
  });
  ...
  suite("getTargetPath", () => {
    test("should return correct path for prompt type", () => {
      ...
      assert.ok(
        targetPath.includes(".github/prompts/"),
        "Should include .github/prompts/",
      );
      assert.ok(
        targetPath.endsWith("my-prompt.prompt.md"),
        "Should end with correct filename",
      );
    });

    test("should return correct path for instructions type", () => {
      const targetPath = service.getTargetPath(
        "instructions",
        "coding-standards",
      );
      assert.ok(
        targetPath.includes(".github/instructions/"),
        "Should include .github/instructions/",
      );
      assert.ok(
        targetPath.endsWith("coding-standards.instructions.md"),
        "Should end with correct filename",
      );
    });
    ...
    test("should return correct path for agent type", () => {
      ...
      assert.ok(
        targetPath.includes(".github/agents/"),
        "Should include .github/agents/",
      );
      assert.ok(
        targetPath.endsWith("code-reviewer.agent.md"),
        "Should end with correct filename",
      );
    });
    ...
    test("should return correct path for skill type", () => {
      ...
      assert.ok(
        targetPath.includes(".github/skills/"),
        "Should include .github/skills/",
      );
    });
    ...
    test("should return correct path for chatmode type", () => {
      ...
      assert.ok(
        targetPath.includes(".github/prompts/"),
        "Chatmodes should go to prompts directory",
      );
      assert.ok(
        targetPath.endsWith("expert-mode.chatmode.md"),
        "Should end with correct filename",
      );
    });
    ...
    test("should return absolute path within workspace", () => {
      ...
      assert.ok(
        ...
        "Should be within workspace root",
      );
    });
  });
  ...
  suite("getStatus", () => {
    test("should return status with baseDirectory", async () => {
      ...
      assert.ok(
        status.baseDirectory.includes(".github"),
        "baseDirectory should include .github",
      );
    });
    ...
    test("should count synced files", async () => {
      ...
      assert.ok(
        status.files.includes("test.prompt.md"),
        "Should list synced files",
      );
    });
  ...
  suite("syncBundle - File Placement", () => {
    test("should place prompt files in .github/prompts/", async () => {
      ...
      mockStorage.getInstalledBundle.resolves(
        createMockInstalledBundle(bundleId, "commit"),
      );
      ...
      assert.ok(
        ...
        "Prompt file should be placed in .github/prompts/",
      );
    });
    ...
    test("should place instruction files in .github/instructions/", async () => {
      ...
      assert.ok(
        fs.existsSync(targetFile),
        "Instructions file should be placed in .github/instructions/",
      );
    });
    ...
    test("should place agent files in .github/agents/", async () => {
      ...
      assert.ok(
        fs.existsSync(targetFile),
        "Agent file should be placed in .github/agents/",
      );
    });
    ...
    test("should create parent directories if they do not exist", async () => {
      ...
      assert.ok(
        ...
        ".github should not exist initially",
      );

      await service.syncBundle(bundleId, bundlePath);

      assert.ok(
        fs.existsSync(path.join(workspaceRoot, ".github", "prompts")),
        "Should create .github/prompts/",
      );
    });
    ...
    test("should handle bundles with mixed file types", async () => {
      ...
      assert.ok(
        fs.existsSync(
          path.join(workspaceRoot, ".github", "prompts", "prompt1.prompt.md"),
        ),
      );
      assert.ok(
        fs.existsSync(
  ...
  suite("syncBundle - Git Exclude Management", () => {
    ...
    test("should add paths to git exclude for local-only mode", async () => {
      ...
      mockStorage.getInstalledBundle.resolves(
        createMockInstalledBundle(bundleId, "local-only"),
      );
    ...
    test("should create .git/info/exclude if it does not exist", async () => {
      ...
      assert.ok(
        ...
        "Exclude file should not exist initially",
      );
      ...
      assert.ok(
        fs.existsSync(excludePath),
        "Git exclude file should be created",
      );
    });
    ...
    test("should preserve existing git exclude content", async () => {
      ...
      const excludeContent = readGitExclude();
      assert.ok(
        excludeContent!.includes("# Existing content"),
        "Should preserve existing content",
      );
      assert.ok(
        excludeContent!.includes("*.log"),
  ...
  suite("syncBundle - commitMode from Storage", () => {
    test("should retrieve commitMode from RegistryStorage", async () => {
      ...
      assert.ok(
        mockStorage.getInstalledBundle.calledWith(bundleId, "repository"),
  ...
  suite("unsyncBundle", () => {
    test("should remove files from .github/ directories", async () => {
      ...
      assert.ok(
        !fs.existsSync(path.join(promptsDir, "test.prompt.md")),
  ...
  suite("Error Handling", () => {
    test("should proceed without git integration when .git directory is missing", async () => {
      ...
      assert.ok(
        fs.existsSync(targetFile),
        "File should be placed even without .git",
      );
    });
  ...
  suite("Git Exclude Section Management", () => {
    ...
    test("should keep section header when entries remain", async () => {
      ...
      fs.writeFileSync(
        excludePath,
        content.replace(".github/prompts/test1.prompt.md\n", ""),
      );
...
prompts:
  - id: ${skillName}
    ...
    test("should copy skill directories to .github/skills/<skill-name>/", async () => {
      ...
      assert.ok(
        fs.existsSync(path.join(targetSkillDir, "SKILL.md")),
        "SKILL.md should be copied",
      );
      assert.ok(
        fs.existsSync(path.join(targetSkillDir, "index.js")),
        "index.js should be copied",
      );
    });
    ...
    test("should copy all files within skill directory recursively", async () => {
      ...
      assert.ok(
        fs.existsSync(path.join(targetSkillDir, "src", "main.js")),
        "src/main.js should be copied",
      );
      assert.ok(
        fs.existsSync(path.join(targetSkillDir, "src", "utils", "helper.js")),
        "src/utils/helper.js should be copied",
      );
      assert.ok(
        fs.existsSync(path.join(targetSkillDir, "config", "settings.json")),
        "config/settings.json should be copied",
      );
    });
    ...
    test("should preserve skill directory structure", async () => {
      ...
      const bundlePath = createMockBundleWithSkill(bundleId, skillName, [
        ...
        {
          relativePath: "lib/utils/format.js",
      ...
      assert.ok(
        fs.statSync(path.join(targetSkillDir, "lib")).isDirectory(),
        "lib should be a directory",
      );
      assert.ok(
        fs.statSync(path.join(targetSkillDir, "lib", "utils")).isDirectory(),
        "lib/utils should be a directory",
      );

      // Verify file contents are preserved
      const coreContent = fs.readFileSync(
        path.join(targetSkillDir, "lib", "core.js"),
        "utf-8",
      );
      assert.strictEqual(
        coreContent,
    ...
    test("should create parent .github/skills/ directory if it does not exist", async () => {
      ...
      assert.ok(
        ...
        ".github/skills should not exist initially",
      );
    ...
    test("should NOT add skill files to git exclude for commit mode", async () => {
      ...
      assert.ok(
        excludeContent === null ||
          !excludeContent.includes(".github/skills/commit-skill"),
    ...
    test("should handle bundles with mixed skills and prompts", async () => {
      ...
      fs.writeFileSync(
        path.join(bundlePath, "my-prompt.prompt.md"),
        "# My Prompt",
      );
...
prompts:
  ...
  - id: my-skill
    ...
    type: skill`;
      ...
      assert.ok(
        fs.existsSync(
          path.join(workspaceRoot, ".github", "prompts", "my-prompt.prompt.md"),
        ),
    ...
    test("should handle skill manifest with file path (skills/name/SKILL.md) instead of directory path", async () => {
      ...
      fs.writeFileSync(
        path.join(skillDir, "resources", "helper.md"),
        "# Helper Resource",
      );
...
prompts:
  - id: ${skillName}
    ...
    type: skill`;
      ...
      assert.ok(
        fs.existsSync(path.join(targetSkillDir, "resources", "helper.md")),
        "resources/helper.md should be copied",
      );
    });
  });
...
prompts:
  - id: ${skillName}
    ...
    test("should remove entire skill directory on unsync", async () => {
      ...
      createLockfile(bundleId, "commit", [
        {
          path: `.github/skills/${skillName}/SKILL.md`,
          checksum: skillMdChecksum,
        },
        {
          path: `.github/skills/${skillName}/index.js`,
          checksum: indexJsChecksum,
        },
      ]);
