    ...
    fc.assert(
      fc.property(
        ...
        ({ owner, repo, versions }) => {
          ...
          return (
            consolidated.length === 1 && consolidated[0].isConsolidated === true
          );
        },
      ),
    ...
    fc.assert(
      fc.property(
        ...
        (repoConfigs) => {
          ...
          const allFromGitHub = consolidated.every(
            (b) => b.sourceId === "github-source",
          );

          // Verify all entries are marked as consolidated (since we have multiple versions)
          const allConsolidated = consolidated.every(
            (b) => b.isConsolidated === true,
          );
