...
suite("RepositoryScopeService Property Tests", () => {
   ...
   */
  const uniquePathsGen = (
    minLength: number = 1,
    maxLength: number = 5,
  ): fc.Arbitrary<string[]> => {
    return fc
      .array(relativePathGen(), { minLength, maxLength })
  ...
  suite("Property 1: File Type to Directory Mapping", function () {
    ...
    test("each file type maps to a distinct directory (except chatmode)", async () => {
      ...
      assert.strictEqual(
        uniqueDirs.size,
        4,
        "Should have 4 distinct directories",
      );
    });
  });
      ...
      await fc.assert(
        fc.asyncProperty(uniquePathsGen(1, 3), async (paths) => {
          ...
          for (const p of paths) {
            assert.ok(excludeContent!.includes(p), `Should contain path: ${p}`);
          }
      ...
      await fc.assert(
        fc.asyncProperty(
          fc
            ...
            .filter((s) => !s.includes("Prompt Registry")),
          uniquePathsGen(1, 2),
  ...
  suite("Property: Skill Directory Preservation", function () {
     ...
     */
    const skillStructureGen = (): fc.Arbitrary<{
      skillName: string;
      ...
      return fc
        ...
        .map(([skillName, files]) => {
          ...
          const hasSkillMd = files.some(
            (f) => f.relativePath.toLowerCase() === "skill.md",
          );
          if (!hasSkillMd) {
            files.unshift({
      ...
      await fc.assert(
        fc.asyncProperty(skillStructureGen(), async ({ skillName, files }) => {
          ...
          mockStorage.getInstalledBundle.resolves(
            createMockInstalledBundle(bundleId, bundlePath, "commit"),
          );
    ...
    test("skill files are placed in correct .github/skills/<skill-name>/ directory", async () => {
      await fc.assert(
        fc.asyncProperty(skillNameGen(), async (skillName) => {
          ...
          const bundleId = `skill-bundle-${skillName}`;
          const files = [
            { relativePath: "SKILL.md", content: `# ${skillName}` },
          ];
          const bundlePath = createSkillBundle(bundleId, skillName, files);
