      ...
      fc.asyncProperty(
        ...
        async (lockfile) => {
          ...
          const mockLockfileManager =
            sandbox.createStubInstance(LockfileManager);
    ...
    return fc.assert(
      fc.asyncProperty(
        ...
        fc
          ...
          .map((s) => `/repo/${s.replace(/[^a-zA-Z0-9-]/g, "a")}`),
        async (lockfile, repositoryPath) => {
          ...
          mockLockfileManager.read.resolves(lockfile);
          mockLockfileManager.getLockfilePath.returns(
            `${repositoryPath}/prompt-registry.lock.json`,
          );
      ...
      fc.asyncProperty(
        ...
        async (lockfile) => {
          ...
          if (showInformationMessageStub.called) {
            const message = showInformationMessageStub.firstCall
              .args[0] as string;
            assert.ok(
 ...
 */
suite(
  "RepositoryActivationService - Property Tests (Missing Sources/Hubs)",
  () => {
    let sandbox: sinon.SinonSandbox;
    let showInformationMessageStub: sinon.SinonStub;
    ...
    test("Property 14: Missing Source/Hub Detection - does not report configured sources", () => {
      return fc.assert(
        fc.asyncProperty(
          LockfileGenerators.consistentLockfile({
            minBundles: 1,
            maxBundles: 5,
          }),
          async (lockfile) => {
            ...
            const configuredSources = Object.entries(lockfile.sources).map(
              ([id, source]) => ({
                id,
                type: source.type,
                url: source.url,
        ...
        fc.asyncProperty(
          ...
          async (lockfile) => {
            ...
            assert.deepStrictEqual(
              result1.missingSources.sort(),
              result2.missingSources.sort(),
              "Missing sources detection should be deterministic",
            );
            assert.deepStrictEqual(
              result1.missingHubs.sort(),
              result2.missingHubs.sort(),
      ...
      fc.asyncProperty(
        ...
        async (lockfile, setupState) => {
          ...
          const mockSetupStateManager =
            sandbox.createStubInstance(SetupStateManager);
    ...
    return fc.assert(
      fc.asyncProperty(
        ...
        async (lockfile, setupManagerState) => {
          ...
          let mockSetupStateManager:
            | sinon.SinonStubbedInstance<SetupStateManager>
            | undefined;
          if (setupManagerState !== "undefined") {
            mockSetupStateManager =
              sandbox.createStubInstance(SetupStateManager);
            mockSetupStateManager.isComplete.resolves(
              setupManagerState === "complete",
            );
          }
