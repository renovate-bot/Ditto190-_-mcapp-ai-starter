...
suite("UpdateScheduler Property Tests", () => {
  ...
  const originalAllowTimersEnv =
    process.env.UPDATE_SCHEDULER_ALLOW_TIMERS_IN_TESTS;
    ...
    await fc.assert(
      fc.asyncProperty(
        fc.record({
          ...
          frequency: fc.constantFrom<UpdateCheckFrequency>(
            "daily",
            "weekly",
            "manual",
          ),
        ...
        async ({ numUpdates, enabled, frequency }) => {
          ...
          try {
            ...
            try {
              ...
              const testUpdateChecker =
                testSandbox.createStubInstance(UpdateChecker);
              const mockUpdates = Array(numUpdates)
                .fill(null)
                .map((_, i) => ({
                  bundleId: `bundle-${i}`,
              ...
              const callArgs =
                testUpdateChecker.checkForUpdates.getCall(0).args;
              if (callArgs[0] === true) {
  ...
  test("Property 28: Frequency change immediate application", async () => {
    await fc.assert(
      fc.asyncProperty(
        fc
          .record({
            initialFrequency: fc.constantFrom<UpdateCheckFrequency>(
              "daily",
              "weekly",
            ),
            newFrequency: fc.constantFrom<UpdateCheckFrequency>(
              "daily",
              "weekly",
            ),
          })
