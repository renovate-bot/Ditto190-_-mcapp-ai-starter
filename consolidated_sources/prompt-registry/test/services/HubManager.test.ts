  ...
  async validate(data: any, schemaPath: string): Promise<ValidationResult> {
    if (this.shouldFail) {
      return {
        valid: false,
        errors:
          this.errors.length > 0 ? this.errors : ["Schema validation failed"],
        warnings: [],
      };
    }
    return {
...
suite("HubManager", () => {
  ...
  suite("Import Hub from Local", () => {
    test("should import hub from local file", async () => {
      ...
      assert.strictEqual(
        loaded.config.metadata.name,
        "Official Prompt Registry Hub",
      );
    });
  ...
  suite("Hub Validation", () => {
    test("should validate hub config", async () => {
      ...
      const config = yaml.load(
        fs.readFileSync(localRef.location, "utf-8"),
      ) as HubConfig;
  ...
  suite("Active Hub Management", () => {
    test("should return null when no active hub is set", async () => {
      ...
      assert.strictEqual(
        activeHub,
        null,
        "Should return null when no active hub exists",
      );
    });
    ...
    test("should list profiles from active hub only", async () => {
      const hubId1 = await hubManager.importHub(
        localRef,
        "test-profiles-hub-1",
      );
      const hubId2 = await hubManager.importHub(
        localRef,
        "test-profiles-hub-2",
      );
      ...
      for (const profile of profiles) {
        assert.strictEqual(
          profile.hubId,
          hubId1,
          "Profile should belong to active hub",
        );
    ...
    test("should return empty array when no active hub is set", async () => {
      ...
      assert.strictEqual(
        profiles.length,
        0,
        "Should return empty array when no active hub",
      );
    });
    ...
    test("should auto-clear invalid active hub ID", async () => {
      ...
      assert.strictEqual(
        activeHubId,
        null,
        "Storage should have cleared active hub ID",
      );
    });
  ...
  suite("Duplicate Detection with URL Matching", () => {
    test("should detect duplicate sources by URL matching (not by ID)", async () => {
      ...
      assert.strictEqual(
        sources.length,
        2,
        "Should have 2 sources (1 existing + 1 new, duplicate skipped)",
      );
    ...
    test("should add new source when same URL but different branch is imported (different sourceId)", async () => {
      ...
      assert.strictEqual(
        sources.length,
        2,
        "Should have 2 sources (original + new with different branch)",
      );
      ...
      assert.strictEqual(
        ...
        1,
        "Should have 1 add call",
      );
      assert.strictEqual(
        mockRegistry.updateSourceCalls.length,
        0,
        "Should have 0 update calls",
      );
      ...
      assert.notStrictEqual(
        mainSource!.id,
        developSource!.id,
        "Different branches should have different sourceIds",
      );
    });
...
`;
    ...
    nock("https://raw.githubusercontent.com")
      ...
      .reply(301, "", {
        location:
          "https://raw.githubusercontent.com/new-owner/new-repo/main/hub-config.yml",
      });
...
`;
    ...
    await storage.saveHub(
      hubId,
      yaml.load(fs.readFileSync(fixturePath, "utf-8")) as HubConfig,
      {
        type: "url",
        location:
          "https://raw.githubusercontent.com/test-owner/test-repo/main/hub-config.yml",
      },
    );
    ...
    nock("https://raw.githubusercontent.com")
      ...
      .reply(302, "", {
        location:
          "https://raw.githubusercontent.com/test-owner/test-repo-v2/main/hub-config.yml",
      });
