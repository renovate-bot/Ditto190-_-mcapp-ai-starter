      ...
      fc.asyncProperty(
        ...
        async (bundleConfigs) => {
          ...
          const expectedUpdates = bundles.filter((b) => {
            const [iMajor, iMinor, iPatch] = b.installedVersion
              .split(".")
              .map(Number);
            const [lMajor, lMinor, lPatch] = b.latestVersion
              .split(".")
              .map(Number);
          ...
          for (const bundle of bundles) {
            registryManager.getBundleDetails
      ...
      fc.asyncProperty(
        ...
        async ({ bundles, subsequentCalls }) => {
          ...
          const testMemento: vscode.Memento = {
            ...
            update: testSandbox
              .stub()
              .callsFake(async (key: string, value: any) => {
                if (value === undefined) {
                  storage.delete(key);
                } else {
                  storage.set(key, value);
          ...
          const testRegistryManager =
            testSandbox.createStubInstance(RegistryManager);
          const testRegistryStorage =
            testSandbox.createStubInstance(RegistryStorage);
          ...
          try {
            ...
            for (let i = 0; i < subsequentCalls; i++) {
              ...
              for (let j = 0; j < cachedResults.length; j++) {
                ...
                if (
                  cachedResults[j].currentVersion !==
                  firstResults[j].currentVersion
                ) {
                  return false;
                }
                if (
                  cachedResults[j].latestVersion !==
                  firstResults[j].latestVersion
                ) {
                  return false;
                }
              }
      ...
      fc.asyncProperty(
        ...
        async ({ bundles, hasNewUpdates }) => {
          ...
          try {
            ...
            await testUpdateChecker.checkForUpdates(false);
            const afterFirstCallCount =
              testRegistryManager.checkUpdates.callCount;
            ...
            if (hasNewUpdates && bundles.length > 0) {
              ...
              testRegistryManager.getBundleDetails
                .withArgs(newUpdates[newUpdates.length - 1].bundleId)
                .resolves({
        ...
        async ({ githubSources, nonGithubSources }) => {
          ...
          try {
            ...
            for (const githubSource of githubSources) {
              const wasCalledForGithub =
                testRegistryManager.syncSource.calledWith(githubSource.id);
            ...
            for (const nonGithubSource of nonGithubSources) {
              const wasCalledForNonGithub =
                testRegistryManager.syncSource.calledWith(nonGithubSource.id);
