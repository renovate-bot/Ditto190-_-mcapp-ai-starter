...
suite("BundleInstaller Property Tests", () => {
  ...
  let lockfileCreateOrUpdateCalls: Array<{
    scope: InstallationScope;
    bundleId: string;
  }>;
  let lockfileRemoveCalls: Array<{
    scope: InstallationScope;
    bundleId: string;
  }>;
  ...
  const createTempDir = (): string => {
    const dir = path.join(
      ...
      "..",
      "..",
      "test-temp-bundleinstaller-prop-" + Date.now(),
    );
  ...
  setup(() => {
    ...
    mockLockfileManager = {
      ...
      validate: sandbox
        .stub()
        .resolves({ valid: true, errors: [], warnings: [] }),
      detectModifiedFiles: sandbox.stub().resolves([]),
      getLockfilePath: sandbox
        .stub()
        .returns(path.join(tempDir, "prompt-registry.lock.json")),
    ...
    sandbox
      .stub(LockfileManager, "getInstance")
      .returns(mockLockfileManager as unknown as LockfileManager);
  ...
  suite(
    ...
    () => {
      ...
      let recordInstallationCalls: Array<{
        bundleId: string;
        scope: InstallationScope;
      }>;
      let removeInstallationCalls: Array<{
        bundleId: string;
        scope: InstallationScope;
      }>;
       ...
       */
      const bundleDataGenerator = (): fc.Arbitrary<{
        bundleId: string;
        version: string;
      }> => {
        return fc.record({
      ...
      setup(() => {
        ...
        mockStorage.recordInstallation.callsFake(
        ...
        mockStorage.removeInstallation.callsFake(
          async (bundleId: string, scope: InstallationScope) => {
        ...
        await fc.assert(
          fc.asyncProperty(
            ...
            async ({ bundleId, version }) => {
              ...
              if (installed.scope !== "repository") {
                await mockStorage.removeInstallation(
                  installed.bundleId,
                  installed.scope,
                );
              }
        ...
        await fc.assert(
          fc.asyncProperty(
            fc.constantFrom(
              "user",
              "workspace",
            ) as fc.Arbitrary<InstallationScope>,
            bundleDataGenerator(),
          ...
          fc.asyncProperty(
            ...
            async (scope, { bundleId, version }) => {
              ...
              if (shouldModifyStorage) {
                ...
                assert.ok(
                  recordInstallationCalls.length > 0 ||
                    removeInstallationCalls.length > 0,
                  `${scope} scope should modify RegistryStorage`,
                );
              } else {
                // Repository scope should NOT have storage calls
                assert.strictEqual(
                  recordInstallationCalls.length +
                    removeInstallationCalls.length,
                  0,
  ...
  suite("Property 5: Repository Scope Operations Update Lockfile", () => {
    ...
    test("repository scope uninstall should remove from lockfile via LockfileManager", async () => {
       ...
       */
      await fc.assert(
        fc.asyncProperty(bundleDataGenerator(), async ({ bundleId }) => {
          // Reset tracking
          lockfileRemoveCalls = [];
      ...
      await fc.assert(
        fc.asyncProperty(
          ...
          async ({ bundleId, version }, sourceId, commitMode) => {
            ...
            const call = lockfileCreateOrUpdateCalls.find(
              (c) => c.bundleId === bundleId,
            );
            assert.ok(
              call,
              `Lockfile should be updated for bundle ${bundleId}`,
            );
      ...
      await fc.assert(
        fc.asyncProperty(
          ...
          async ({ bundleId, version }) => {
            ...
            assert.strictEqual(
              lockfileCreateOrUpdateCalls.filter(
                (call) => call.bundleId === bundleId,
              ).length,
              0,
            ...
            assert.strictEqual(
              lockfileRemoveCalls.filter((call) => call.bundleId === bundleId)
                .length,
              0,
      ...
      await fc.assert(
        fc.asyncProperty(
          ...
          async ({ bundleId, version }, sourceId, commitMode) => {
            ...
            assert.ok(
              bundleEntry,
              `Bundle ${bundleId} should exist in lockfile`,
            );
            assert.strictEqual(
              bundleEntry.version,
              version,
            ...
            assert.strictEqual(
              bundleEntry.commitMode,
              commitMode,
              "Commit mode should match",
            );
