  ...
  setup(() => {
    ...
    const mockLockfileManager = {
      ...
      getLockfilePath: sandbox
        .stub()
        .returns(path.join(workspaceDir, "prompt-registry.lock.json")),
...
`),
      ...
      const promptRegistryInWorkspace = path.join(
        workspaceDir,
        ".prompt-registry",
      );
...
`),
      ...
      let installedBundle;
      try {
        installedBundle = await installer.installFromBuffer(
          testBundle,
          bundleBuffer,
          options,
          "github",
...
`),
      ...
      const manifestInWorkspace = path.join(
        workspaceDir,
        "deployment-manifest.yml",
      );
      const manifestInGithub = path.join(
        workspaceDir,
        ".github",
        "deployment-manifest.yml",
      );
