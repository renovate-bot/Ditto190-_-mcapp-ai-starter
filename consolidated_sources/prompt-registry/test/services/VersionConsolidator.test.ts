...
suite("VersionConsolidator Unit Tests", () => {
  ...
  suite("consolidateBundles", () => {
    test("should consolidate 3 versions (1.0.0, 2.0.0, 1.5.0) into single entry with latest (2.0.0)", () => {
      const bundles = [
        BundleBuilder.github("microsoft", "vscode")
          .withVersion("1.0.0")
          .build(),
        BundleBuilder.github("microsoft", "vscode")
          .withVersion("2.0.0")
      ...
      assert.strictEqual(
        consolidated.length,
        1,
        "Should have one consolidated entry",
      );
      assert.strictEqual(
        consolidated[0].version,
        "2.0.0",
        "Should select latest version",
      );
      assert.strictEqual(
    ...
    test("should handle mixed source types (GitHub consolidated, others unchanged)", () => {
      const bundles = [
        BundleBuilder.github("owner", "repo").withVersion("1.0.0").build(),
        BundleBuilder.github("owner", "repo").withVersion("2.0.0").build(),
        BundleBuilder.fromSource("gitlab-bundle", "GITLAB")
          .withVersion("1.0.0")
          .build(),
        BundleBuilder.fromSource("local-bundle", "LOCAL")
          .withVersion("1.0.0")
          .build(),
      ];
      ...
      const githubEntry = consolidated.find(
        (b) => b.sourceId === TEST_SOURCE_IDS.GITHUB,
      );
    ...
    test("should consolidate each GitHub repo separately", () => {
      ...
      assert.strictEqual(
        consolidated.length,
        2,
        "Should have 2 consolidated entries",
      );
  ...
  suite("getAllVersions", () => {
    ...
    test("should return same results as getAvailableVersions", () => {
      ...
      const versionsFromGetAvailable =
        consolidator.getAllVersions("owner-repo");

      assert.strictEqual(
        versionsFromGetAll.length,
        versionsFromGetAvailable.length,
      );
  ...
  suite("LRU Cache Mechanism", () => {
    ...
    test("should evict least recently used entry when cache is full", async () => {
      ...
      const bundle3 = [
        BundleBuilder.github("owner3", "repo3").withVersion("1.0.0").build(),
      ];
      ...
      await new Promise((resolve) => setTimeout(resolve, 10));
      const bundle4 = [
        BundleBuilder.github("owner4", "repo4").withVersion("1.0.0").build(),
      ];
