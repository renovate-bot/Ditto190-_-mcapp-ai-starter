...
suite("RegistryManager - Event Emission Behavior", () => {
  ...
  test("should fire onBundleUpdated event when bundle is updated", async () => {
    ...
    assert.strictEqual(
      firedUpdate.version,
      "2.0.0",
      "Event should contain new version",
    );
  ...
  test("should pass source metadata to installer during update", async () => {
    ...
    assert.strictEqual(
      callArgs[3],
      "skills",
      "source type should be forwarded",
    );
    assert.strictEqual(
      callArgs[4],
      "Remote Skills Source",
      "source name should be forwarded",
    );
  });
  ...
  test("should refresh local skill installations on sync", async () => {
    ...
    mockStorage.recordInstallation.callsFake(
      async (installation: InstalledBundle) => {
    ...
    assert.strictEqual(
      mockAdapter.fetchBundles.callCount,
      1,
      "Local skills adapter should fetch bundles",
    );
    assert.strictEqual(
      updatedInstallations.length,
      1,
      "Installation record should be updated to latest hash",
    );
    const refreshed = updatedInstallations[0];
    assert.strictEqual(
      refreshed.version,
      "hash:new",
      "Version should match latest content hash",
    );
    assert.ok(
      updatedEventPayload,
      "Should emit onBundleUpdated for refreshed skill",
    );
...
suite("RegistryManager - Installation Record Structure", () => {
  ...
  test("should list installed bundles", async () => {
    ...
    assert.ok(
      ...
      "Should return array of installed bundles",
    );
  });
...
suite("RegistryManager - Source Management", () => {
  ...
  test("should add a new source and make it available in source list", async () => {
    ...
    assert.ok(
      mockAdapter.validate.called,
      "Adapter validation should be called",
    );
  });
});
...
suite("RegistryManager - Version Change Installation", () => {
  ...
  test("should allow installing different version when bundle already installed", async () => {
    ...
    sandbox
      .stub(manager as any, "resolveInstallationBundle")
      .resolves(mockBundle);
    sandbox
    ...
    assert.ok(
      mockStorage.recordInstallation.called,
      "Installation should be recorded",
    );
    const recordedInstallation =
      mockStorage.recordInstallation.firstCall.args[0];
    assert.strictEqual(
...
suite("RegistryManager - Bundle Resolution", () => {
  ...
  test("should resolve bundle by versioned ID via identity matching", async () => {
    ...
    const versionedBundleId =
      "amadeus-airlines-solutions-workflow-instructions-1.0.17";
    ...
    assert.ok(
      result.id.includes("amadeus-airlines-solutions-workflow-instructions"),
    );
  });
