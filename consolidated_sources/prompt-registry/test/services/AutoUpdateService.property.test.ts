...
suite("AutoUpdateService - Property Tests", () => {
   ...
   */
  const setupBatchUpdates = (
    updates: UpdateCheckResult[],
    failureFlags: boolean[],
  ): void => {
    const bundles: InstalledBundle[] = updates.map((u) =>
      createInstalledBundle(u),
    );
  ...
  setup(() => {
    ...
    mockBundleNotifications = sandbox.createStubInstance(
      ...
      await fc.assert(
        fc.asyncProperty(
          ...
          async (bundleId, version, errorMessage) => {
            ...
            assert.strictEqual(
              mockBundleNotifications.showUpdateFailure.callCount,
              1,
            );
      ...
      await fc.assert(
        fc.asyncProperty(
          ...
          async (bundleId, oldVersion, newVersion) => {
            ...
            const installedBundle = createMockInstalledBundle(
              bundleId,
              oldVersion,
            );
            ...
            const rollbackCall = mockRegistryManager.updateBundle
              ...
              .find(
                (call) =>
                  call.args[0] === bundleId && call.args[1] === oldVersion,
              );
      ...
      await fc.assert(
        fc.asyncProperty(
          ...
          async (bundleId, oldVersion, newVersion) => {
            ...
            mockRegistryManager.updateBundle.rejects(
      ...
      await fc.assert(
        fc.asyncProperty(
          ...
          async (bundleId1, bundleId2, version) => {
            ...
            const updatedBundle1 = createMockInstalledBundle(
              bundleId1,
              version,
              { installPath: "/mock/path1" },
            );
            const updatedBundle2 = createMockInstalledBundle(
              bundleId2,
              version,
              { installPath: "/mock/path2" },
            );
      ...
      await fc.assert(
        fc.asyncProperty(
          ...
          async (numUpdates, failureFlags) => {
            ...
            const [successful, failed] =
              mockBundleNotifications.showBatchUpdateSummary.firstCall.args;
            const expectedSuccesses = mappedFailureFlags.filter(
              (f) => !f,
            ).length;
        ...
        fc.asyncProperty(
          ...
          async (numUpdates) => {
            ...
            mockRegistryManager.listInstalledBundles.callsFake(async () => {
              ...
              return Array.from(bundleVersions.entries()).map(
                ([bundleId, version]) =>
        ...
        fc.asyncProperty(
          ...
          async (numUpdates, failureFlags) => {
            ...
            const expectedUpdateCalls =
              expectedSuccesses + expectedFailures * 2; // Failures trigger rollback
        ...
        await fc.assert(
          fc.asyncProperty(
            ...
            async (bundleId, currentVersion, latestVersion, sourceType) => {
              ...
              const updatedBundle = createMockInstalledBundle(
                bundleId,
                latestVersion,
              );
