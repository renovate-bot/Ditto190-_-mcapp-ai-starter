...
suite("LockfileManager", () => {
  ...
  suite("createOrUpdate()", () => {
    suite("Lockfile Creation", () => {
      ...
      test("should include generatedAt ISO timestamp", async () => {
        ...
        const lockfile = readLockfileFromDisk();
        assert.ok(
          new Date(lockfile!.generatedAt).toISOString() ===
    ...
    suite("Bundle Entry Management", () => {
      ...
      test("should include sourceId in bundle entry", async () => {
        ...
        const lockfile = readLockfileFromDisk();
        assert.strictEqual(
          lockfile!.bundles["my-bundle"].sourceId,
          "test-source",
        );
      });
      ...
      test("should NOT include commitMode in bundle entry (implicit based on file)", async () => {
        ...
        assert.strictEqual(
          lockfile!.bundles["my-bundle"].commitMode,
          undefined,
        );
      });
    ...
    suite("Source Recording", () => {
      ...
      test("should include source URL", async () => {
        ...
        const lockfile = readLockfileFromDisk();
        assert.strictEqual(
          lockfile!.sources["test-source"].url,
      ...
      test("should include optional branch for git sources", async () => {
        ...
        const manager = LockfileManager.getInstance(tempDir);
        const options = createTestOptions("test-bundle");
        options.source = createMockSourceEntry(
          "github",
          "https://github.com/owner/repo",
          "main",
        );
    ...
    suite("Hub Recording", () => {
      test("should record hub configuration when bundle comes from hub", async () => {
        ...
        options.hub = {
          id: "hub-1",
          entry: createMockHubEntry(
            "My Hub",
            "https://hub.example.com/config.yml",
          ),
        };
        await manager.createOrUpdate(options);
        const lockfile = readLockfileFromDisk();
    ...
    suite("Profile Recording", () => {
      ...
      test("should include profile bundleIds", async () => {
        ...
        await manager.createOrUpdate(options);
        const lockfile = readLockfileFromDisk();
        assert.deepStrictEqual(lockfile!.profiles!["profile-1"].bundleIds, [
          "bundle-1",
          "bundle-2",
        ]);
      });
    ...
    suite("Dual-Lockfile Write Operations", () => {
      const localLockfilePath = () =>
        path.join(tempDir, "prompt-registry.local.lock.json");
      ...
      test("should write commit mode bundles to main lockfile", async () => {
        ...
        assert.ok(
          mainLockfile!.bundles["commit-bundle"],
      ...
      test("should write local-only mode bundles to local lockfile", async () => {
        ...
        assert.ok(
          localLockfile!.bundles["local-bundle"],
      ...
      test("should keep commit and local-only bundles in separate lockfiles", async () => {
        ...
        assert.ok(
          mainLockfile!.bundles["commit-bundle"],
          "Commit bundle should be in main lockfile",
        );
        assert.strictEqual(
          mainLockfile!.bundles["local-bundle"],
          undefined,
          "Local bundle should NOT be in main lockfile",
        );
        ...
        assert.ok(
          localLockfile!.bundles["local-bundle"],
          "Local bundle should be in local lockfile",
        );
        assert.strictEqual(
          localLockfile!.bundles["commit-bundle"],
          undefined,
          "Commit bundle should NOT be in local lockfile",
        );
      });
      ...
      test("should not duplicate git exclude entry on subsequent local-only bundle creations", async () => {
        ...
        const matches = excludeContent.match(
          /prompt-registry\.local\.lock\.json/g,
        );
        assert.strictEqual(
          matches?.length,
          1,
          "Local lockfile should appear only once in git exclude",
        );
      });
    });
  });
  ...
  suite("remove()", () => {
    ...
    test("should clean up orphaned sources when bundle removed", async () => {
      ...
      options2.sourceId = "source-2";
      options2.source = createMockSourceEntry(
        "gitlab",
      ...
      const updated = readLockfileFromDisk();
      assert.ok(
        !updated!.sources["source-1"],
        "Orphaned source should be removed",
      );
      assert.ok(
        updated!.sources["source-2"],
        "Referenced source should remain",
      );
    });
    ...
    suite("Dual-Lockfile Remove Operations", () => {
      ...
      test("should remove local-only bundle from local lockfile", async () => {
        ...
        assert.ok(
          !updatedLocal!.bundles["bundle-0"],
          "bundle-0 should be removed",
        );
      ...
      test("should remove from correct lockfile when both exist", async () => {
        ...
        assert.ok(updatedMain, "Main lockfile should still exist");
        assert.ok(
          updatedMain!.bundles["main-bundle"],
          "main-bundle should remain",
        );
      });
      ...
      test("should handle removing non-existent bundle from both lockfiles gracefully", async () => {
        ...
        assert.ok(
          updatedMain!.bundles["bundle-0"],
          "Main lockfile bundle should remain",
        );
        assert.ok(
          updatedLocal!.bundles["bundle-0"],
          "Local lockfile bundle should remain",
        );
      });
    });
  });
  ...
  suite("updateCommitMode()", () => {
    ...
    test("should move bundle from main lockfile to local lockfile when switching to local-only", async () => {
      ...
      assert.ok(localLockfile, "Local lockfile should exist");
      assert.ok(
        localLockfile!.bundles["bundle-0"],
        "Bundle should be in local lockfile",
      );
      ...
      assert.strictEqual(
        mainLockfile,
        null,
        "Main lockfile should be deleted when empty",
      );
    });
    ...
    test("should emit onLockfileUpdated event with target lockfile", async () => {
      ...
      assert.ok(
        eventLockfile!.bundles["bundle-0"],
        "Event lockfile should contain the moved bundle",
      );
    });
    ...
    test("should preserve all bundle metadata during move", async () => {
      ...
      assert.ok(localLockfile, "Local lockfile should exist");
      assert.strictEqual(
        localLockfile!.bundles["bundle-0"].version,
        originalVersion,
        "Version should be preserved",
      );
      assert.strictEqual(
        localLockfile!.bundles["bundle-0"].sourceId,
        originalSourceId,
        "SourceId should be preserved",
      );
      assert.strictEqual(
        localLockfile!.bundles["bundle-0"].sourceType,
        originalSourceType,
        "SourceType should be preserved",
      );
      assert.strictEqual(
        localLockfile!.bundles["bundle-0"].installedAt,
        originalInstalledAt,
        "InstalledAt should be preserved",
      );
      assert.deepStrictEqual(
        localLockfile!.bundles["bundle-0"].files,
        originalFiles,
    ...
    test("should copy source entry to target lockfile", async () => {
      ...
      assert.ok(localLockfile, "Local lockfile should exist");
      assert.ok(
        localLockfile!.sources[sourceId],
        "Source should be copied to local lockfile",
      );
      assert.strictEqual(
        localLockfile!.sources[sourceId].type,
        originalSource.type,
        "Source type should be preserved",
      );
      assert.strictEqual(
        localLockfile!.sources[sourceId].url,
        originalSource.url,
    ...
    test("should preserve other bundles in source lockfile when moving one", async () => {
      ...
      assert.ok(
        mainLockfile!.bundles["bundle-1"],
        "bundle-1 should still be in main lockfile",
      );
      assert.ok(
        !mainLockfile!.bundles["bundle-0"],
        "bundle-0 should NOT be in main lockfile",
      );
    });
  });
  ...
  suite("detectModifiedFiles()", () => {
    ...
    test("should include original and current checksums in result", async () => {
      ...
      const options = createTestOptions("test-bundle");
      options.files = [
        { path: ".github/prompts/test.prompt.md", checksum: originalChecksum },
      ];
      await manager.createOrUpdate(options);
      ...
      assert.notStrictEqual(
        result[0].originalChecksum,
        result[0].currentChecksum,
      );
    });
  ...
  suite("File Watcher Initialization and Disposal", () => {
    ...
    test("should initialize file watcher on construction", () => {
      ...
      assert.ok(
        createFileSystemWatcherStub.calledOnce,
      ...
      assert.ok(
        mockFileWatcher.onDidChange.calledOnce,
        "onDidChange handler should be registered",
      );
      assert.ok(
        mockFileWatcher.onDidCreate.calledOnce,
        "onDidCreate handler should be registered",
      );
      assert.ok(
        mockFileWatcher.onDidDelete.calledOnce,
        "onDidDelete handler should be registered",
      );
    ...
    test("should dispose file watcher on dispose() call", () => {
      ...
      assert.ok(
        mockFileWatcher.dispose.calledOnce,
        "File watcher dispose should be called",
      );
    });
    ...
    test("should not fire events after disposal", async () => {
      ...
      if (changeHandler) {
        try {
          ...
          /* handler may fail after dispose */
        }
      }
      if (createHandler) {
        try {
    ...
    test("should handle file watcher initialization failure gracefully", () => {
      ...
      createFileSystemWatcherStub.throws(
        new Error("Mock watcher creation failed"),
      );
  ...
  suite("getInstalledBundles() - Dual Lockfile Support", () => {
    ...
    test("should return bundles from main lockfile only when local lockfile does not exist", async () => {
      ...
      assert.ok(
        bundles.every((b) => b.commitMode === "commit"),
        "All bundles from main lockfile should have commitMode: commit",
      );
    });
    ...
    test("should return bundles from local lockfile only when main lockfile does not exist", async () => {
      ...
      assert.strictEqual(bundles.length, 2);
      assert.ok(
        bundles.every((b) => b.commitMode === "local-only"),
        "All bundles from local lockfile should have commitMode: local-only",
      );
    });
    ...
    test("should merge bundles from both lockfiles", async () => {
      ...
      const mainLockfile = LockfileBuilder.create()
        .withSource("main-source", "github", "https://github.com/main/repo")
        .withBundle("main-bundle-1", "1.0.0", "main-source", {
          commitMode: "commit",
        })
        .withBundle("main-bundle-2", "2.0.0", "main-source", {
          commitMode: "commit",
        })
        .build();
      writeLockfile(mainLockfile);
      ...
      const localLockfile = LockfileBuilder.create()
        .withSource("local-source", "github", "https://github.com/local/repo")
        .withBundle("local-bundle-1", "1.0.0", "local-source", {
      ...
      assert.strictEqual(
        mainBundles.length,
        2,
        "Should have 2 bundles from main lockfile",
      );
      assert.strictEqual(
        localBundles.length,
        1,
        "Should have 1 bundle from local lockfile",
      );
    });
    ...
    test("should annotate bundles from main lockfile with commitMode: commit", async () => {
      ...
      assert.strictEqual(
        bundles[0].commitMode,
        "commit",
        "Bundle from main lockfile should have commitMode: commit regardless of entry value",
      );
    });
    ...
    test("should annotate bundles from local lockfile with commitMode: local-only", async () => {
      ...
      assert.strictEqual(
        bundles[0].commitMode,
        "local-only",
        "Bundle from local lockfile should have commitMode: local-only regardless of entry value",
      );
    });
    ...
    test("should detect conflict when bundle ID exists in both lockfiles", async () => {
      ...
      assert.strictEqual(
        bundles.length,
        1,
        "Should only return 1 bundle (conflict skips local)",
      );
      assert.strictEqual(bundles[0].bundleId, conflictingBundleId);
      assert.strictEqual(
        bundles[0].commitMode,
        "commit",
        "Should be from main lockfile",
      );

      // Should display error message
      assert.ok(
        showErrorMessageStub.calledOnce,
        "Should display error message for conflict",
      );
      assert.ok(
    ...
    test("should handle multiple conflicts correctly", async () => {
      ...
      assert.strictEqual(
        bundles.length,
        4,
        "Should return 4 bundles (3 main + 1 unique local)",
      );
      ...
      assert.strictEqual(
        ...
        2,
        "Should display 2 error messages for 2 conflicts",
      );
    });
  ...
  suite("Backward Compatibility - Legacy SourceId Format", () => {
    ...
    test("should read lockfile with multiple legacy sourceIds correctly", async () => {
      ...
      const lockfile = LockfileBuilder.create()
        ...
        .withBundle("bundle-1", "1.0.0", legacySourceId1, {
          sourceType: "github",
        })
        .withBundle("bundle-2", "2.0.0", legacySourceId2, {
          sourceType: "gitlab",
        })
        .build();
      writeLockfile(lockfile);
    ...
    test("should read lockfile with mixed legacy and new sourceId formats", async () => {
      ...
      const lockfile = LockfileBuilder.create()
        ...
        .withBundle("legacy-bundle", "1.0.0", legacySourceId, {
          sourceType: "github",
        })
        .withBundle("new-bundle", "2.0.0", newSourceId, {
          sourceType: "github",
        })
        .build();
      writeLockfile(lockfile);
      ...
      assert.strictEqual(
        legacyBundle!.sourceId,
        legacySourceId,
        "Legacy sourceId preserved",
      );
    ...
    test("should preserve legacy sourceId when bundle is not updated", async () => {
      ...
      const lockfile = LockfileBuilder.create()
        .withSource(legacySourceId, "github", "https://github.com/owner/repo")
        .withBundle("preserved-bundle", "1.0.0", legacySourceId, {
          sourceType: "github",
        })
        .build();
      writeLockfile(lockfile);
    ...
    test("should handle legacy sourceId with many segments correctly", async () => {
      ...
      const lockfile = LockfileBuilder.create()
        ...
        .withBundle("enterprise-bundle", "1.0.0", legacySourceId, {
          sourceType: "github",
        })
        .build();
      writeLockfile(lockfile);
      ...
      assert.strictEqual(
        bundles[0].sourceId,
        legacySourceId,
        "Multi-segment legacy sourceId preserved",
      );
    });
  });
});
