...
suite("LockfileManager Property Tests", () => {
  ...
  const lockfileSchemaPath = path.join(
    process.cwd(),
    "schemas",
  ...
  const createTempDir = (): string => {
    const dir = path.join(
      ...
      "..",
      "..",
      "test-temp-lockfile-prop-" + Date.now(),
    );
        ...
        fc.asyncProperty(
          ...
          async (lockfile: Lockfile) => {
            ...
            for (const [bundleId, entry] of Object.entries(lockfile.bundles)) {
              ...
              assert.ok(
                entry.sourceType,
                `Bundle ${bundleId} missing sourceType`,
              );
              assert.ok(
                entry.installedAt,
                `Bundle ${bundleId} missing installedAt`,
              );
              assert.ok(
                entry.commitMode,
                `Bundle ${bundleId} missing commitMode`,
              );
              assert.ok(
                Array.isArray(entry.files),
                `Bundle ${bundleId} files should be array`,
              );
            }
            return true;
          },
        ...
        fc.asyncProperty(
          ...
          async (lockfile: Lockfile) => {
            ...
            if (
              ...
              Object.keys(lockfile.sources).length > 0
            ) {
              assert.ok(
      ...
      await fc.assert(
        fc.asyncProperty(
          ...
          async (originalLockfile: Lockfile, newLockfile: Lockfile) => {
            ...
            fs.writeFileSync(
              lockfilePath,
              JSON.stringify(originalLockfile, null, 2),
            );
        ...
        fc.asyncProperty(
          ...
          async (lockfile: Lockfile) => {
            ...
            for (const [sourceId, source] of Object.entries(lockfile.sources)) {
              assert.ok(source.type, `Source ${sourceId} missing type field`);
              assert.ok(source.url, `Source ${sourceId} missing url field`);
            }
            return true;
          },
        ),
          ...
          async (lockfile: Lockfile) => {
            ...
            if (lockfile.hubs) {
              for (const [hubId, hub] of Object.entries(lockfile.hubs)) {
                assert.ok(hub.name, `Hub ${hubId} missing name field`);
                assert.ok(hub.url, `Hub ${hubId} missing url field`);
              }
            }
            return true;
          },
        ...
        fc.asyncProperty(
          ...
          async (lockfile: Lockfile) => {
            ...
            const { LockfileManager } =
              await import("../../src/services/LockfileManager");
        ...
        fc.asyncProperty(
          ...
          async (lockfile: Lockfile) => {
            ...
            for (const bundle of installedBundles) {
              ...
              assert.ok(
                lockfileEntry,
                `Bundle ${bundle.bundleId} not found in lockfile`,
              );
              assert.strictEqual(
                bundle.version,
        ...
        fc.asyncProperty(
          ...
          async (
            ...
            const uniqueDir = path.join(
              tempDir,
              `commit-${Date.now()}-${Math.random().toString(36).substring(7)}`,
            );
            fs.mkdirSync(uniqueDir, { recursive: true });
      ...
      await fc.assert(
        fc.asyncProperty(
          ...
          async (
            ...
            const uniqueDir = path.join(
              tempDir,
              `nodupe-${Date.now()}-${Math.random().toString(36).substring(7)}`,
            );
            fs.mkdirSync(uniqueDir, { recursive: true });
            createGitDir(uniqueDir);
      ...
      await fc.assert(
        fc.asyncProperty(
          ...
          async (
            ...
            const uniqueDir = path.join(
              tempDir,
              `main-mode-${Date.now()}-${Math.random().toString(36).substring(7)}`,
            );
            fs.mkdirSync(uniqueDir, { recursive: true });
            createGitDir(uniqueDir);
            ...
            assert.ok(
              bundle,
              `Bundle ${bundleId} should be in installed bundles`,
            );
            assert.strictEqual(
              bundle!.commitMode,
              "commit",
      ...
      await fc.assert(
        fc.asyncProperty(
          ...
          async (
            ...
            const uniqueDir = path.join(
              tempDir,
              `local-mode-${Date.now()}-${Math.random().toString(36).substring(7)}`,
            );
            fs.mkdirSync(uniqueDir, { recursive: true });
            createGitDir(uniqueDir);
    ...
    test("mode switching preserves all bundle metadata", async () => {
      await fc.assert(
        fc.asyncProperty(
          ...
          LockfileGenerators.sourceId(),
          LockfileGenerators.sourceType(),
          fc.array(LockfileGenerators.fileEntry(), {
            minLength: 0,
            maxLength: 3,
          }),
          LockfileGenerators.commitMode(),
          async (
            ...
            const uniqueDir = path.join(
              tempDir,
              `preserve-${Date.now()}-${Math.random().toString(36).substring(7)}`,
            );
            fs.mkdirSync(uniqueDir, { recursive: true });
            createGitDir(uniqueDir);
            ...
            const originalBundle = originalBundles.find(
              (b) => b.bundleId === bundleId,
            );
      ...
      await fc.assert(
        fc.asyncProperty(
          ...
          async (
            ...
            const uniqueDir = path.join(
              tempDir,
              `empty-${Date.now()}-${Math.random().toString(36).substring(7)}`,
            );
            fs.mkdirSync(uniqueDir, { recursive: true });
            createGitDir(uniqueDir);
      ...
      await fc.assert(
        fc.asyncProperty(
          ...
          async (
            ...
            const uniqueDir = path.join(
              tempDir,
              `remain-${Date.now()}-${Math.random().toString(36).substring(7)}`,
            );
            fs.mkdirSync(uniqueDir, { recursive: true });
            createGitDir(uniqueDir);
