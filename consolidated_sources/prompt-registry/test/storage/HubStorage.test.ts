  ...
  suite("Save Hub Configuration", () => {
    ...
    test("should reject invalid hub IDs", async () => {
      await assert.rejects(
        async () =>
          await storage.saveHub("../invalid", testHubConfig, testHubReference),
        /Invalid hub ID/,
      );
    });
  ...
  suite("Security and Validation", () => {
    test("should reject path traversal in hub IDs", async () => {
      ...
      for (const id of invalidIds) {
        await assert.rejects(
          async () =>
            await storage.saveHub(id, testHubConfig, testHubReference),
          /Invalid hub ID/,
        );
      }
    });
  ...
  suite("Active Hub Management", () => {
    ...
    test("should set and retrieve active hub ID", async () => {
      ...
      assert.strictEqual(
        retrievedHubId,
        hubId,
        "Should return the correct active hub ID",
      );
    });
