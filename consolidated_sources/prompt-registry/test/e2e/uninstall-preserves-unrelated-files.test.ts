...
`,
      ...
      assert.ok(
        fs.existsSync(ciWorkflowFile),
        "CI workflow should exist before installation",
      );
      assert.ok(
        fs.existsSync(releaseWorkflowFile),
        "Release workflow should exist before installation",
      );
      assert.ok(
        fs.existsSync(codeownersFile),
        "CODEOWNERS should exist before installation",
      );
      assert.ok(
        fs.existsSync(issueTemplateFile),
        "Issue template should exist before installation",
      );
      ...
      assert.ok(
        ...
        ".github/prompts should be removed when empty",
      );

      // === CRITICAL ASSERTIONS: Unrelated files should be preserved ===
      assert.ok(
        fs.existsSync(githubDir),
        ".github folder should NOT be removed when unrelated files exist",
      );
      assert.ok(
        fs.existsSync(workflowsDir),
        ".github/workflows should NOT be removed",
      );
      assert.ok(
        fs.existsSync(ciWorkflowFile),
        "CI workflow file should be preserved",
      );
      assert.ok(
        fs.existsSync(releaseWorkflowFile),
        "Release workflow file should be preserved",
      );
      assert.ok(
        fs.existsSync(codeownersFile),
        "CODEOWNERS file should be preserved",
      );
      assert.ok(
        fs.existsSync(issueTemplateFile),
        "Issue template file should be preserved",
      );
      ...
      assert.ok(
        ...
        "CI workflow content should be intact",
      );

      const codeownersContent = fs.readFileSync(codeownersFile, "utf-8");
      assert.ok(
        codeownersContent.includes("@test-owner"),
        "CODEOWNERS content should be intact",
      );
    });
...
`,
      ...
      assert.ok(
        fs.existsSync(dependabotFile),
        "Dependabot config should exist before installation",
      );
      ...
      assert.ok(
        fs.existsSync(promptsDir),
        ".github/prompts should exist after installation",
      );
      ...
      assert.ok(
        fs.existsSync(dependabotFile),
        "Dependabot config should be preserved",
      );
      ...
      assert.ok(
        ...
        "Dependabot content should be intact",
      );
    });
    ...
    test("Uninstalling bundle should preserve user-modified prompt files", async function () {
      ...
      assert.ok(
        fs.existsSync(promptFile),
        "User-modified prompt file should be preserved",
      );

      // Verify the user's modifications are intact
      const preservedContent = fs.readFileSync(promptFile, "utf-8");
      assert.ok(
        preservedContent.includes("User Customization"),
        "User modifications should be preserved",
      );
      assert.ok(
        preservedContent.includes("This was added by the user"),
      ...
      assert.ok(
        fs.existsSync(promptsDir),
        ".github/prompts should be preserved when containing user-modified files",
      );
    });
    ...
    test("Uninstalling bundle should preserve user-created prompt files in same directory", async function () {
      ...
      assert.ok(
        fs.existsSync(bundlePromptFile),
        "Bundle prompt file should exist after installation",
      );

      // === ACT: Create a user's own prompt file in the same directory ===
      const userPromptFile = path.join(
        promptsDir,
        "my-custom-prompt.prompt.md",
      );
      fs.writeFileSync(
        userPromptFile,
        `# My Custom Prompt
...
`,
      ...
      assert.ok(
        fs.existsSync(userPromptFile),
        "User-created prompt should exist",
      );
      ...
      assert.ok(
        !fs.existsSync(bundlePromptFile),
        "Bundle prompt file should be removed",
      );

      // User-created prompt file should be preserved (not tracked in lockfile)
      assert.ok(
        fs.existsSync(userPromptFile),
        "User-created prompt file should be preserved",
      );

      // Verify user's content is intact
      const userContent = fs.readFileSync(userPromptFile, "utf-8");
      assert.ok(
        userContent.includes("My Custom Prompt"),
        "User prompt content should be intact",
      );
      assert.ok(
        userContent.includes("not from any bundle"),
      ...
      assert.ok(
        fs.existsSync(promptsDir),
        ".github/prompts should be preserved when containing user-created files",
      );
    });
  });
});
