  ...
  suite("Awesome Copilot Update Workflow", () => {
    test("Example 1.1: Update command downloads from configured branch", async function () {
      ...
      nock("https://api.github.com")
        .persist()
        .get(
          "/repos/test-owner/awesome-copilot-test/contents/collections?ref=main",
        )
        .reply(200, [{ name: "test-collection.collection.yml", type: "file" }]);
      ...
      assert.strictEqual(
        ...
        1,
        "Should still have one installed bundle",
      );
      assert.strictEqual(
