...
suite("E2E: Lockfile as Single Source of Truth Tests", () => {
  ...
  suite("11.1: Repository Bundle Listing from Lockfile", () => {
    ...
    test("Requirement 1.1, 1.3: listInstalledBundles(repository) returns bundles from lockfile", async function () {
      ...
      const installedBundles =
        await testContext.registryManager.listInstalledBundles("repository");

      // Verify bundle is returned
      assert.ok(
        installedBundles.length > 0,
        "listInstalledBundles should return at least one bundle",
      );

      const installedBundle = installedBundles.find(
        (b) => b.bundleId === actualBundleId,
      );
      assert.ok(
        installedBundle,
        `Should find bundle ${actualBundleId} in installed bundles`,
      );
      ...
      assert.strictEqual(
        ...
        "Version should match lockfile entry",
      );
      assert.strictEqual(
        installedBundle!.sourceId,
        lockfileEntry.sourceId,
        "SourceId should match lockfile entry",
      );
      assert.strictEqual(
        installedBundle!.sourceType,
        lockfileEntry.sourceType,
        "SourceType should match lockfile entry",
      );
      assert.strictEqual(
        installedBundle!.scope,
        "repository",
        "Scope should be repository",
      );
      assert.ok(installedBundle!.installPath, "InstallPath should be set");
      assert.ok(
        installedBundle!.installPath!.includes(".github"),
        "InstallPath should include .github",
      );
    });
    ...
    test("Requirement 1.1: listInstalledBundles without scope includes repository bundles", async function () {
      ...
      const allBundles =
        await testContext.registryManager.listInstalledBundles();

      // Verify repository bundle is included
      const repoBundle = allBundles.find((b) => b.bundleId === actualBundleId);
      assert.ok(
        repoBundle,
        "Repository bundle should be included when querying without scope filter",
      );
      assert.strictEqual(
        repoBundle!.scope,
        "repository",
        "Bundle scope should be repository",
      );
    });
  ...
  suite("11.2: Stale Record Handling", () => {
    ...
    test("Requirement 1.1, 2.1: Lockfile takes precedence over stale RegistryStorage records", async function () {
      ...
      assert.strictEqual(
        installedBundles.length,
        1,
        "Should return exactly one bundle from lockfile",
      );
      assert.strictEqual(
        installedBundles[0].bundleId,
        lockfileBundleId,
        "Should return bundle from lockfile",
      );
      assert.strictEqual(
        installedBundles[0].version,
        "2.0.0",
        "Version should match lockfile",
      );

      // Verify stale record is NOT returned
      const staleBundle = installedBundles.find(
        (b) => b.bundleId === staleBundleId,
      );
      assert.ok(
        !staleBundle,
        "Stale RegistryStorage record should NOT be returned",
      );
    });
    ...
    test("Requirement 2.1: Repository scope installation does not create RegistryStorage record", async function () {
      ...
      const workspaceBundles =
        await testContext.storage.getInstalledBundles("workspace");
      const repoScopeInWorkspace = workspaceBundles.find(
        (b) => b.scope === "repository",
      );
      assert.ok(
        !repoScopeInWorkspace,
        "Repository scope bundle should NOT be in workspace storage",
      );
    });
  });
  ...
  suite("11.3: Cleanup Command for Stale Lockfile Entries", () => {
    ...
    test("Requirement 3.4: Cleanup command removes stale entries with missing files", async function () {
      ...
      const mockLockfile = {
        ...
        bundles: {
          [staleBundleId]: {
            ...
            commitMode: "commit",
            files: [
              { path: ".github/prompts/missing.prompt.md", checksum: "abc123" },
            ],
          },
          [validBundleId]: {
            version: "1.0.0",
            ...
            files: [
              {
                path: ".github/prompts/existing.prompt.md",
                checksum: "def456",
              },
            ],
          },
      ...
      fs.mkdirSync(promptsDir, { recursive: true });
      fs.writeFileSync(
        path.join(promptsDir, "existing.prompt.md"),
        "# Existing Prompt",
      );
      ...
      assert.strictEqual(
        bundlesBefore.length,
        2,
        "Should have 2 bundles initially",
      );

      const staleBundle = bundlesBefore.find(
        (b) => b.bundleId === staleBundleId,
      );
      assert.ok(staleBundle, "Should find stale bundle");
      assert.ok(
        staleBundle!.filesMissing,
        "Stale bundle should have filesMissing flag set",
      );

      const validBundle = bundlesBefore.find(
        (b) => b.bundleId === validBundleId,
      );
      assert.ok(validBundle, "Should find valid bundle");
      assert.ok(
        !validBundle!.filesMissing,
        "Valid bundle should NOT have filesMissing flag set",
      );
      ...
      const showInfoMessageStub = sandbox.stub(
        vscode.window,
      ...
      assert.strictEqual(
        bundlesAfter.length,
        1,
        "Should have 1 bundle after cleanup",
      );
      assert.strictEqual(
        bundlesAfter[0].bundleId,
        validBundleId,
        "Valid bundle should remain",
      );

      const staleBundleAfter = bundlesAfter.find(
        (b) => b.bundleId === staleBundleId,
      );
      ...
      const mockLockfile = {
        ...
        bundles: {
          [validBundleId]: {
            ...
            commitMode: "commit",
            files: [
              { path: ".github/prompts/valid.prompt.md", checksum: "abc123" },
            ],
          },
        },
        sources: {
    ...
    test("Requirement 3.4: Cleanup command respects user cancellation", async function () {
      ...
      assert.strictEqual(
        bundlesAfter.length,
        1,
        "Bundle should still exist after cancellation",
      );
      assert.strictEqual(
        bundlesAfter[0].bundleId,
        staleBundleId,
        "Stale bundle should remain",
      );
    });
  });
  ...
  suite("11.4: Uninstall Scenarios", () => {
    ...
    test("Requirement 3.1, 3.2, 3.4: Uninstalling last bundle deletes lockfile and fires event with null", async function () {
      ...
      try {
        ...
        assert.strictEqual(
          eventPayload,
          null,
          "onLockfileUpdated event should fire with null when lockfile is deleted",
        );
      } finally {
        disposable.dispose();
      }
    ...
    test("Requirement 3.3: Uninstalling one bundle preserves other bundles in lockfile", async function () {
      ...
      const deps = {
        registryManager: testContext.registryManager,
      ...
      const { sourceId: sourceId1, bundle: bundle1 } =
        await setupSourceWithCustomConfig(
          deps,
          testId,
          "partial-source-1",
          config1,
      ...
      await installBundleOrSkip(this, bundle1.id as string, {
        scope: "repository",
        commitMode: "commit",
        version: "1.0.0",
      });
      ...
      const { sourceId: sourceId2, bundle: bundle2 } =
        await setupSourceWithCustomConfig(
          deps,
          testId,
          "partial-source-2",
      ...
      await installBundleOrSkip(this, bundle2.id as string, {
        scope: "repository",
        commitMode: "commit",
        version: "1.0.0",
      });
      ...
      assert.strictEqual(
        bundleIdsBefore.length,
        2,
        "Should have two bundles in lockfile",
      );

      // Find the actual bundle IDs from lockfile
      const actualBundleId1 = bundleIdsBefore.find((id) =>
        id.includes("bundle-1"),
      );
      const actualBundleId2 = bundleIdsBefore.find((id) =>
        id.includes("bundle-2"),
      );
      ...
      assert.ok(
        fs.existsSync(lockfilePath),
        "Lockfile should still exist after partial uninstall",
      );
      ...
      assert.strictEqual(
        bundleIdsAfter.length,
        1,
        "Should have one bundle remaining in lockfile",
      );
      assert.ok(
        bundleIdsAfter.includes(actualBundleId2!),
        "Bundle-2 should remain in lockfile",
      );
      assert.ok(
        !bundleIdsAfter.includes(actualBundleId1!),
        "Bundle-1 should be removed from lockfile",
      );

      // Verify bundle-2 data is intact
      const remainingBundle = lockfileAfter!.bundles[actualBundleId2!];
      assert.strictEqual(
        remainingBundle.version,
        "1.0.0",
        "Remaining bundle version should be intact",
      );
      assert.strictEqual(
        remainingBundle.sourceId,
        sourceId2,
        "Remaining bundle sourceId should be intact",
      );
    });
  });
  ...
  suite("12.2: Lockfile Portability - SourceId Format", () => {
    ...
    test("Requirement 2.1, 2.3: Lockfile with new sourceId format is portable across hub configurations", async function () {
      ...
      fs.mkdirSync(promptsDir, { recursive: true });
      fs.writeFileSync(
        path.join(promptsDir, "portable.prompt.md"),
        "# Portable Prompt",
      );
      ...
      assert.strictEqual(
        installedBundles[0].sourceId,
        newFormatSourceId,
        "SourceId should use new format",
      );
      assert.strictEqual(
        installedBundles[0].sourceType,
        sourceType,
        "SourceType should match",
      );
      assert.strictEqual(
        installedBundles[0].scope,
        "repository",
        "Scope should be repository",
      );
    });
    ...
    test("Requirement 2.2, 2.3: Same source URL always produces same sourceId (deterministic)", async function () {
      ...
      assert.strictEqual(
        sourceId1,
        sourceId2,
        "SourceId should be deterministic (1 vs 2)",
      );
      assert.strictEqual(
        sourceId2,
        sourceId3,
        "SourceId should be deterministic (2 vs 3)",
      );
    });
    ...
    test("Requirement 2.3: SourceId is URL-normalized (case-insensitive, protocol-agnostic)", async function () {
      ...
      assert.strictEqual(
        sourceId1,
        sourceId2,
        "SourceId should be case-insensitive",
      );
      assert.strictEqual(
        sourceId2,
        sourceId3,
        "SourceId should be protocol-agnostic",
      );
      assert.strictEqual(
        sourceId3,
        sourceId4,
        "SourceId should ignore trailing slashes",
      );
    });
    ...
    test("Requirement 3.1, 3.4: Legacy hub-prefixed sourceId still resolves correctly", async function () {
      ...
      const mockLockfile = {
        ...
        bundles: {
          [bundleId]: {
            ...
            commitMode: "commit",
            files: [
              { path: ".github/prompts/legacy.prompt.md", checksum: "def456" },
            ],
          },
        },
        sources: {
      ...
      fs.mkdirSync(promptsDir, { recursive: true });
      fs.writeFileSync(
        path.join(promptsDir, "legacy.prompt.md"),
        "# Legacy Prompt",
      );
      ...
      assert.strictEqual(
        installedBundles[0].sourceId,
        legacySourceId,
        "Legacy sourceId should be preserved",
      );
      assert.strictEqual(
        installedBundles[0].sourceType,
        "github",
    ...
    test("Requirement 2.5: Different source types with same URL produce different sourceIds", async function () {
      ...
      assert.notStrictEqual(
        githubSourceId,
        gitlabSourceId,
        "Different types should produce different sourceIds",
      );
      assert.notStrictEqual(
        gitlabSourceId,
        httpSourceId,
      ...
      assert.ok(
        githubSourceId.startsWith("github-"),
        "GitHub sourceId should start with github-",
      );
      assert.ok(
        gitlabSourceId.startsWith("gitlab-"),
        "GitLab sourceId should start with gitlab-",
      );
      assert.ok(
        httpSourceId.startsWith("http-"),
        "HTTP sourceId should start with http-",
      );
    });
    ...
    test("Requirement 2: Lockfile with multiple bundles from different sources works correctly", async function () {
      ...
      const githubSourceId = generateHubSourceId(
        "github",
        "https://github.com/owner1/repo1",
      );
      const gitlabSourceId = generateHubSourceId(
        "gitlab",
      ...
      const mockLockfile = {
        ...
        bundles: {
          [bundle1Id]: {
            ...
            files: [
              {
                path: ".github/prompts/github-bundle.prompt.md",
                checksum: "gh123",
              },
            ],
          },
          [bundle2Id]: {
            ...
            files: [
              {
                path: ".github/prompts/gitlab-bundle.prompt.md",
                checksum: "gl456",
              },
            ],
          },
      ...
      fs.mkdirSync(promptsDir, { recursive: true });
      fs.writeFileSync(
        path.join(promptsDir, "github-bundle.prompt.md"),
        "# GitHub Bundle",
      );
      fs.writeFileSync(
        path.join(promptsDir, "gitlab-bundle.prompt.md"),
        "# GitLab Bundle",
      );
      ...
      assert.strictEqual(
        installedBundles.length,
        2,
        "Should return both bundles",
      );

      const githubBundle = installedBundles.find(
        (b) => b.bundleId === bundle1Id,
      );
      const gitlabBundle = installedBundles.find(
        (b) => b.bundleId === bundle2Id,
      );

      assert.ok(githubBundle, "Should find GitHub bundle");
      assert.strictEqual(
        githubBundle!.sourceId,
        githubSourceId,
        "GitHub bundle sourceId should match",
      );
      assert.strictEqual(
        githubBundle!.sourceType,
        "github",
        "GitHub bundle sourceType should match",
      );

      assert.ok(gitlabBundle, "Should find GitLab bundle");
      assert.strictEqual(
        gitlabBundle!.sourceId,
        gitlabSourceId,
        "GitLab bundle sourceId should match",
      );
      assert.strictEqual(
        gitlabBundle!.sourceType,
        "gitlab",
        "GitLab bundle sourceType should match",
      );
    });
  });
});
