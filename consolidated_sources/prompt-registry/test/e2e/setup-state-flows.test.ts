...
suite("E2E: Setup State Flows", () => {
    ...
    return {
      globalState: {
        get: (key: string, defaultValue?: any) => {
          return globalStateStorage.has(key)
            ? globalStateStorage.get(key)
            : defaultValue;
        },
  ...
  setup(() => {
    ...
    setupStateManager = SetupStateManager.getInstance(
      mockContext,
  ...
  suite("14.1: Fresh Install Flow", () => {
    ...
    test("should complete setup successfully: not_started → in_progress → complete", async () => {
      ...
      assert.strictEqual(
        ...
        "State should be IN_PROGRESS after markStarted()",
      );
      ...
      assert.strictEqual(
        finalState,
        SetupState.COMPLETE,
        "State should be COMPLETE after successful setup",
      );
    });
    ...
    test("should not show prompts after successful completion", async () => {
      ...
      assert.strictEqual(
        shouldShowPrompt,
        false,
        "Should not show resume prompt after completion",
      );
    ...
    test("should persist complete state across manager instances", async () => {
      ...
      SetupStateManager.resetInstance();
      const newManager = SetupStateManager.getInstance(
        mockContext,
        mockHubManager as any,
      );
      ...
      assert.strictEqual(
        state,
        SetupState.COMPLETE,
        "State should persist as COMPLETE after reload",
      );
    });
  });
  ...
  suite("14.2: Cancellation and Resume Flow", () => {
    ...
    test("should handle cancellation and resume: not_started → in_progress → incomplete → in_progress → complete", async () => {
      ...
      assert.strictEqual(
        ...
        "State should be INCOMPLETE after cancellation",
      );
      ...
      assert.strictEqual(
        shouldShowPrompt,
        true,
        "Should show resume prompt when incomplete",
      );
      ...
      assert.strictEqual(
        state,
        SetupState.COMPLETE,
        "State should be COMPLETE after successful resume",
      );
    });
    ...
    test("should show resume prompt on next activation after cancellation", async () => {
      ...
      assert.strictEqual(
        shouldShow,
        true,
        "Should show resume prompt on next activation",
      );
    });
  });
  ...
  suite("14.3: Skip Resume Flow", () => {
    ...
    test("should handle skip and remain incomplete", async () => {
      ...
      assert.strictEqual(
        state,
        SetupState.INCOMPLETE,
        "State should remain INCOMPLETE after skip",
      );
      ...
      assert.strictEqual(
        shouldShow,
        false,
        "Should not show prompt again after skip",
      );
    });
    ...
    test("should allow manual reset after skip: incomplete → not_started → complete", async () => {
      ...
      assert.strictEqual(
        state,
        SetupState.NOT_STARTED,
        "Should be NOT_STARTED after reset",
      );
      ...
      assert.strictEqual(
        shouldShow,
        false,
        "Resume prompt flag should be cleared",
      );
      ...
      assert.strictEqual(
        state,
        SetupState.COMPLETE,
        "Should be COMPLETE after reset and setup",
      );
    });
    ...
    test("should track skip action via prompt shown flag", async () => {
      ...
      assert.strictEqual(
        shouldShow,
        false,
        "Should not show prompt after skip",
      );
      ...
      assert.strictEqual(
        state,
        SetupState.INCOMPLETE,
        "State should remain INCOMPLETE",
      );
    });
  });
  ...
  suite("14.4: Backward Compatibility", () => {
    ...
    test("should not show prompts for existing install with hub", async () => {
      ...
      assert.strictEqual(
        isIncomplete,
        false,
        "Should not detect incomplete when hub is configured",
      );
      ...
      assert.strictEqual(
        shouldShow,
        false,
        "Should not show resume prompt for existing install with hub",
      );
    });
    ...
    test("should show resume prompt for existing install without hub", async () => {
      ...
      assert.strictEqual(
        isIncomplete,
        true,
        "Should detect incomplete when no hub configured",
      );
      ...
      assert.strictEqual(
        state,
        SetupState.INCOMPLETE,
        "Should migrate to INCOMPLETE state",
      );
      ...
      assert.strictEqual(
        shouldShow,
        true,
        "Should show resume prompt for existing install without hub",
      );
    });
    ...
    test("should migrate old flags to new state system", async () => {
      ...
      assert.strictEqual(
        state,
        SetupState.INCOMPLETE,
        "State should be migrated to INCOMPLETE",
      );
    });
  });
  ...
  suite("14.5: Test Environment", () => {
    ...
    test("should skip setup when VSCODE_TEST=1", async () => {
      ...
      try {
        ...
        SetupStateManager.resetInstance();
        const testManager = SetupStateManager.getInstance(
          mockContext,
          mockHubManager as any,
        );
        ...
        assert.strictEqual(
          state,
          SetupState.COMPLETE,
          "State should be COMPLETE in test environment",
        );
        ...
        assert.strictEqual(
          shouldShow,
          false,
          "Should not show prompts in test environment",
        );
      } finally {
    ...
    test("should skip setup when ExtensionMode.Test", async () => {
      ...
      assert.strictEqual(
        state,
        SetupState.COMPLETE,
        "State should be COMPLETE in Test mode",
      );
    });
    ...
    test("should set state to complete immediately in test environment", async () => {
      ...
      assert.strictEqual(
        state,
        SetupState.COMPLETE,
        "Should be able to mark complete in test environment",
      );
      ...
      assert.strictEqual(
        shouldShow,
        false,
        "Should not show prompts after marking complete",
      );
    });
    ...
    test("should detect test environment via both VSCODE_TEST and ExtensionMode", async () => {
      ...
      try {
        ...
        assert.strictEqual(
          isTestViaEnv,
          true,
          "Should detect test via VSCODE_TEST",
        );
        ...
        assert.strictEqual(
          isTestViaMode,
          true,
          "Should detect test via ExtensionMode.Test",
        );
      } finally {
        if (originalEnv === undefined) {
          delete process.env.VSCODE_TEST;
