...
suite("E2E: Repository-Level Installation Tests", () => {
  ...
  async function installBundleOrSkip(
    ...
    try {
      ...
      if (error.message.includes("not yet implemented")) {
        console.log(
          `[Test Skipped] Feature not implemented: ${options.scope} scope installation`,
        );
        context.skip();
        return false;
      }
  ...
  async function getBundleFromSource(sourceId: string): Promise<any> {
    ...
    if (!bundle) {
      throw new Error(
        `Should find bundle ${BUNDLE_ID}, found: ${rawBundles.map((b) => b.id).join(", ")}`,
      );
    }
  ...
  suite("Repository-Level Installation Workflow", () => {
    test("Requirement 1.2-1.7: Install bundle at repository scope places files in .github folder", async function () {
      ...
      assert.ok(
        ...
        ".github/prompts directory should exist",
      );
      ...
      assert.ok(
        ...
        "Prompt file should exist in .github/prompts",
      );
      const content = fs.readFileSync(promptFile, "utf-8");
      assert.ok(
        content.includes(TEST_CONTENT.BASIC_INSTALL),
    ...
    test("Requirement 4.1-4.5: Lockfile is generated with proper structure", async function () {
      ...
      assert.ok(
        ...
        "Lockfile should exist at repository root",
      );
      ...
      assert.ok(
        lockfileContent.generatedAt,
        "Lockfile should have generatedAt timestamp",
      );
      assert.ok(
      ...
      assert.ok(
        lockfileContent.generatedBy,
        "Lockfile should have generatedBy field",
      );

      // Req 4.5: bundles object
      assert.ok(lockfileContent.bundles, "Lockfile should have bundles object");
      assert.ok(
        Object.keys(lockfileContent.bundles).length > 0,
        "Bundles object should not be empty",
      );
      ...
      assert.ok(
        bundleEntry.sourceId,
        "Bundle entry should have sourceId field",
      );
      assert.ok(
        bundleEntry.sourceType,
        "Bundle entry should have sourceType field",
      );
      assert.ok(
        bundleEntry.installedAt,
        "Bundle entry should have installedAt field",
      );
  ...
  suite("Lockfile-Based Reinstallation Workflow", () => {
    test("Requirement 13.1-13.4: LockfileManager detects lockfile and identifies missing bundles", async function () {
      ...
      assert.strictEqual(
        Object.keys(lockfile!.bundles).length,
        1,
        "Should have one bundle",
      );

      const bundleCount = Object.keys(lockfile!.bundles).length;
      const profileCount = lockfile!.profiles
        ? Object.keys(lockfile!.profiles).length
        : 0;
      ...
      const installedBundles =
        await testContext.storage.getInstalledBundles("repository");
      const installedBundleIds = new Set(
        installedBundles.map((b) => b.bundleId),
      );
      const lockfileBundleIds = Object.keys(lockfile!.bundles);
      const missingBundleIds = lockfileBundleIds.filter(
        (id) => !installedBundleIds.has(id),
      );

      assert.strictEqual(
        missingBundleIds.length,
        1,
        "Should detect 1 missing bundle",
      );
      assert.strictEqual(
        missingBundleIds[0],
        "test-bundle-v1.0.0",
        "Missing bundle ID should match",
      );
    });
    ...
    test("Requirement 5.5-5.6, 13.6: Missing bundles detected and reinstalled from lockfile", async function () {
      ...
      const { sourceId, bundle } = await setupSourceAndGetBundle(
        "reinstall-source",
        TEST_CONTENT.REINSTALL,
      );
      ...
      assert.ok(
        fs.existsSync(lockfilePath),
        "Lockfile should exist after installation",
      );
      ...
      assert.ok(lockfile, "Lockfile should be readable");
      assert.ok(
        Object.keys(lockfile!.bundles).length > 0,
        "Lockfile should have bundle entries",
      );
    });
  });
  ...
  suite("Complete Repository Installation Lifecycle", () => {
    test("Full lifecycle: Install → Delete files → Fresh context → Detect → Reinstall", async function () {
      ...
      assert.deepStrictEqual(
        Object.keys(lockfile!.bundles),
        originalBundleIds,
        "Lockfile should have same bundles",
      );

      // === PHASE 5: Reinstall bundles from lockfile ===
      // Expected: Bundles reinstalled based on lockfile entries
      const reinstallReleases: ReleaseConfig[] = [
        { tag: "v1.0.0", version: "1.0.0", content: TEST_CONTENT.LIFECYCLE },
      ];
      ...
      for (const lockfileBundleId of Object.keys(lockfile!.bundles)) {
        ...
        try {
          await testContext.registryManager.installBundle(lockfileBundleId, {
            scope: "repository",
            commitMode:
              (bundleEntry.commitMode as RepositoryCommitMode) || "commit",
  ...
  suite("Git Integration for Repository Installations (Requirement 3)", () => {
    ...
    test("Requirement 3.2-3.4: Local-only mode should add paths to .git/info/exclude with section header", async function () {
      ...
      if (!fs.existsSync(promptsDir)) {
        ...
        console.log(
          "[Test] Skipping: Repository scope installation did not create files",
        );
        this.skip();
      }
      ...
      if (!fs.existsSync(excludePath)) {
        console.log(
          "[Test] Skipping: local-only mode did not create .git/info/exclude - feature may not be implemented",
        );
        this.skip();
      }
      ...
      assert.ok(
        isPathExcluded(".github/prompts") ||
          isPathExcluded("test-prompt.prompt.md"),
    ...
    test("Requirement 3.3: Local-only mode should create .git/info/exclude if it does not exist", async function () {
      ...
      assert.ok(
        !fs.existsSync(excludePath),
        ".git/info/exclude should not exist initially",
      );

      const { bundle } = await setupSourceAndGetBundle(
        "create-exclude-source",
        TEST_CONTENT.CREATE_EXCLUDE,
      );
    ...
    test("Requirement 3.6: Should use .git/info/exclude instead of .gitignore", async function () {
      ...
      const { bundle } = await setupSourceAndGetBundle(
        "gitignore-check-source",
        TEST_CONTENT.GITIGNORE_CHECK,
      );
      ...
      if (!excludeContent.includes(GIT_EXCLUDE_SECTION_HEADER)) {
        console.log(
          "[Test] Skipping: local-only mode did not modify .git/info/exclude - feature may not be implemented",
        );
        this.skip();
      }
  ...
  suite("Scope Conflict Prevention (Requirement 6)", () => {
     ...
     */
    async function installAtUserScope(
      context: { skip: () => void },
      bundleId: string,
      version: string,
    ): Promise<boolean> {
    ...
    test("Requirement 6.1: Should check if bundle exists at other scope before installation", async function () {
      ...
      const { bundle } = await setupSourceAndGetBundle(
        ...
        TEST_CONTENT.CONFLICT_CHECK,
      );

      // First, install at user scope
      const userInstallSuccess = await installAtUserScope(
        this,
        bundle!.id,
        "1.0.0",
      );
      ...
      try {
        ...
        const userBundlesAfter =
          await testContext.storage.getInstalledBundles("user");
        const repoBundlesAfter =
          await testContext.storage.getInstalledBundles("repository");

        const atUserScope = userBundlesAfter.some(
          (b) => b.bundleId === bundle!.id,
        );
        const atRepoScope = repoBundlesAfter.some(
          (b) => b.bundleId === bundle!.id,
        );
    ...
    test("Requirement 6.6: Same bundle should NOT exist at both scopes simultaneously", async function () {
      ...
      const finalUserBundles =
        await testContext.storage.getInstalledBundles("user");
      const finalRepoBundles =
        await testContext.storage.getInstalledBundles("repository");

      const atUserScope = finalUserBundles.some(
        (b) => b.bundleId === bundle!.id,
      );
      const atRepoScope = finalRepoBundles.some(
        (b) => b.bundleId === bundle!.id,
      );
    ...
    test("Requirement 6.2-6.3: Should detect when bundle exists at different scope", async function () {
      ...
      const { bundle } = await setupSourceAndGetBundle(
        "warn-conflict-source",
        TEST_CONTENT.WARN_CONFLICT,
      );
  ...
  suite("Repository Scope Uninstallation (Requirement 4.8-4.9)", () => {
    test("Requirement 4.8: Uninstalling repository-scoped bundle removes files from .github/", async function () {
      ...
      const { bundle } = await setupSourceAndGetBundle(
        "uninstall-files-source",
        TEST_CONTENT.UNINSTALL_FILES,
      );
      ...
      assert.ok(
        fs.existsSync(promptFile),
        "Prompt file should exist after installation",
      );
      ...
      assert.ok(
        lockfileBundleIds.length > 0,
        "Lockfile should contain at least one bundle",
      );
      ...
      await testContext.registryManager.uninstallBundle(
        actualBundleId,
        "repository",
      );
      ...
      assert.ok(
        !fs.existsSync(promptFile),
        "Prompt file should be removed after uninstallation",
      );
    });
    ...
    test("Requirement 4.8: Uninstalling repository-scoped bundle updates lockfile", async function () {
      ...
      const { bundle } = await setupSourceAndGetBundle(
        "uninstall-lockfile-source",
        TEST_CONTENT.UNINSTALL_LOCKFILE,
      );
      ...
      if (lockfileAfter) {
        assert.ok(
          !lockfileAfter.bundles[actualBundleId],
    ...
    test("Requirement 4.9: Lockfile is deleted when last bundle is uninstalled", async function () {
      ...
      const { bundle } = await setupSourceAndGetBundle(
        "uninstall-last-source",
        TEST_CONTENT.UNINSTALL_LAST,
      );
      ...
      assert.strictEqual(
        bundleCount,
        1,
        "Should have exactly one bundle in lockfile",
      );
      ...
      assert.ok(
        !fs.existsSync(lockfilePath),
        "Lockfile should be deleted when last bundle is uninstalled",
      );
    });
    ...
    test("Uninstalling local-only bundle removes entries from .git/info/exclude", async function () {
      ...
      const { bundle } = await setupSourceAndGetBundle(
        "uninstall-local-only-source",
        TEST_CONTENT.UNINSTALL_LOCAL_ONLY,
      );
      ...
      if (!fs.existsSync(excludePath)) {
        console.log(
          "[Test] Skipping: local-only mode did not create .git/info/exclude",
        );
        this.skip();
      }
  ...
  suite("Context Menu Scope Operations (Requirement 7)", () => {
     ...
     */
    async function installAtUserScopeForMigration(
      context: { skip: () => void },
      bundleId: string,
      version: string,
    ): Promise<boolean> {
    ...
    function createBundleScopeCommands(): BundleScopeCommands {
      const scopeConflictResolver = new ScopeConflictResolver(
        testContext.storage,
      );
      const bundleInstaller = testContext.registryManager.getBundleInstaller();
      const repositoryScopeService =
        bundleInstaller.createRepositoryScopeService();

      if (!repositoryScopeService) {
        throw new Error(
          "RepositoryScopeService not available - workspace may not be open",
        );
      }
    ...
    test("Requirement 7.2-7.3: Move to Repository (Commit) moves bundle from user to repository scope", async function () {
      ...
      const { bundle } = await setupSourceAndGetBundle(
        "move-to-repo-commit-source",
        TEST_CONTENT.MOVE_TO_REPO_COMMIT,
      );

      // First, install at user scope
      const userInstallSuccess = await installAtUserScopeForMigration(
        this,
        bundle.id,
        "1.0.0",
      );
      ...
      // Verify bundle is installed at user scope
      const userBundlesBefore =
        await testContext.storage.getInstalledBundles("user");
      const userBundle = userBundlesBefore.find(
        (b) => b.bundleId === bundle.id,
      );
      assert.ok(
        userBundle,
        "Bundle should be installed at user scope initially",
      );
      ...
      assert.ok(
        repoBundle || filesExist,
        "Bundle should be at repository scope after migration",
      );

      // Verify bundle is no longer at user scope
      const userBundlesAfter =
        await testContext.storage.getInstalledBundles("user");
      const userBundleAfter = userBundlesAfter.find(
        (b) => b.bundleId === bundle.id,
      );
      assert.ok(
        !userBundleAfter,
        "Bundle should NOT be at user scope after migration",
      );

      // Verify commit mode is 'commit' (not in .git/info/exclude)
      const excludePath = path.join(workspaceRoot, ".git/info/exclude");
      const excludeContent = fs.existsSync(excludePath)
        ? fs.readFileSync(excludePath, "utf-8")
        : "";
      assert.ok(
    ...
    test("Requirement 7.4, 7.6: Move to User moves bundle from repository to user scope", async function () {
      ...
      assert.ok(
        lockfileBundleIds.length > 0,
        "Lockfile should have at least one bundle",
      );
      ...
      const userBundlesAfter =
        await testContext.storage.getInstalledBundles("user");
      const userBundle = userBundlesAfter.find(
        (b) => b.bundleId === actualBundleId,
      );
    ...
    test("Requirement 7.5, 7.8: Switch to Local Only adds paths to .git/info/exclude", async function () {
      ...
      const { bundle } = await setupSourceAndGetBundle(
        "switch-local-only-source",
        TEST_CONTENT.SWITCH_LOCAL_ONLY,
      );
      ...
      assert.ok(
        ...
        "Local lockfile should exist after switching to local-only mode",
      );
      const updatedLockfile = JSON.parse(
        fs.readFileSync(localLockfilePath, "utf-8"),
      );
      assert.ok(
    ...
    test("Requirement 7.7, 7.9: Switch to Commit removes paths from .git/info/exclude", async function () {
      ...
      assert.ok(
        fs.existsSync(localLockfilePath),
        "Local lockfile should exist for local-only bundle",
      );
      const lockfile = JSON.parse(fs.readFileSync(localLockfilePath, "utf-8"));
      const actualBundleId = Object.keys(lockfile.bundles)[0];
      ...
      assert.ok(
        bundleEntry,
        `Bundle ${actualBundleId} should be in local lockfile`,
      );
      ...
      assert.ok(
        ...
        "Main lockfile should exist after switching to commit mode",
      );
      const updatedLockfile = JSON.parse(
        fs.readFileSync(mainLockfilePath, "utf-8"),
      );
      assert.ok(
        updatedLockfile.bundles[actualBundleId],
  ...
  suite("Local Modification Warning on Update (Requirement 14)", () => {
    ...
    test("Requirement 14.1-14.3: Should detect local file modifications before update", async function () {
      ...
      const { bundleId, promptFilePath } = await installAndGetBundleId(
        this,
        "mod-detect-source",
        TEST_CONTENT.MOD_DETECT,
      );

      // Verify prompt file exists
      assert.ok(
        fs.existsSync(promptFilePath),
        "Prompt file should exist after installation",
      );
      ...
      assert.ok(
        readBack.includes("Local modification"),
        "File should contain local modification",
      );
    ...
    test("Requirement 14.6: Override proceeds with update despite local modifications", async function () {
      ...
      try {
        ...
        console.log(
          "[Test] Warning dialog was shown, but update failed:",
          error.message,
        );
      }
    ...
    test("Requirement 14.7: Cancel aborts update and preserves local modifications", async function () {
      ...
      try {
        ...
        if (
          ...
          error.message.includes("Update cancelled")
        ) {
          updateAborted = true;
        } else {
          throw error;
    ...
    test("Requirement 14.4-14.5: Warning dialog lists modified files", async function () {
      ...
      assert.ok(
        dialogMessage.includes("test-prompt.prompt.md") ||
          dialogMessage.includes(".github/prompts"),
    ...
    test("Requirement 14.5, 14.9: Contribute Changes opens repository URL and aborts update", async function () {
      ...
      const openExternalStub = sandbox
      ...
      try {
        ...
        if (
          error.message.includes("contribute") ||
          error.message.includes("Update cancelled")
        ) {
          updateAborted = true;
        } else {
      ...
      assert.ok(
        updateAborted,
        "Update should have been aborted when user chose Contribute Changes",
      );
  ...
  suite("Local-Only Lockfile Separation (Requirements 1-5)", () => {
    ...
    test("11.1: Installing local-only bundle creates local lockfile and git exclude entry", async function () {
      ...
      const { bundle } = await setupSourceAndGetBundle(
        "local-lockfile-create-source",
        "local-lockfile-create",
      );
      ...
      assert.ok(
        !fs.existsSync(localLockfilePath),
        "Local lockfile should NOT exist initially",
      );

      // Verify local lockfile is NOT in git exclude initially
      assert.ok(
        !isLocalLockfileExcluded(),
        "Local lockfile should NOT be in git exclude initially",
      );
      ...
      assert.ok(
        fs.existsSync(localLockfilePath),
        "Local lockfile should be created for local-only bundle",
      );

      // Verify local lockfile has correct structure
      const localLockfile = JSON.parse(
        fs.readFileSync(localLockfilePath, "utf-8"),
      );
      assert.ok(
        localLockfile.bundles,
        "Local lockfile should have bundles object",
      );
      assert.ok(
        Object.keys(localLockfile.bundles).length > 0,
        "Local lockfile should contain the bundle",
      );
      ...
      assert.ok(
        !fs.existsSync(mainLockfilePath),
        "Main lockfile should NOT be created for local-only bundle",
      );

      // Verify local lockfile is in git exclude (Requirement 2.1)
      assert.ok(
        isLocalLockfileExcluded(),
        "Local lockfile should be added to git exclude",
      );
    });
    ...
    test("11.2: Installing committed bundle creates main lockfile only", async function () {
      ...
      assert.ok(
        !fs.existsSync(mainLockfilePath),
        "Main lockfile should NOT exist initially",
      );
      assert.ok(
        !fs.existsSync(localLockfilePath),
        "Local lockfile should NOT exist initially",
      ...
      assert.ok(
        fs.existsSync(mainLockfilePath),
        "Main lockfile should be created for committed bundle",
      );

      // Verify main lockfile has correct structure
      const mainLockfile = JSON.parse(
        fs.readFileSync(mainLockfilePath, "utf-8"),
      );
      assert.ok(
        mainLockfile.bundles,
        "Main lockfile should have bundles object",
      );
      assert.ok(
        Object.keys(mainLockfile.bundles).length > 0,
        "Main lockfile should contain the bundle",
      );
      ...
      assert.ok(
        !fs.existsSync(localLockfilePath),
        "Local lockfile should NOT be created for committed bundle",
      );

      // Verify local lockfile is NOT in git exclude
      assert.ok(
        !isLocalLockfileExcluded(),
        "Local lockfile should NOT be in git exclude for committed bundle",
      );
    });
    ...
    test("11.3: Switching commit mode moves bundle and updates git exclude", async function () {
      ...
      const mainLockfileBefore = JSON.parse(
        fs.readFileSync(mainLockfilePath, "utf-8"),
      );
      ...
      assert.ok(
        fs.existsSync(localLockfilePath),
        "Local lockfile should be created after switching to local-only",
      );
      const localLockfile = JSON.parse(
        fs.readFileSync(localLockfilePath, "utf-8"),
      );
      assert.ok(
        localLockfile.bundles[actualBundleId],
        "Bundle should be in local lockfile",
      );

      // Verify bundle was removed from main lockfile
      if (fs.existsSync(mainLockfilePath)) {
        const mainLockfileAfter = JSON.parse(
          fs.readFileSync(mainLockfilePath, "utf-8"),
        );
        assert.ok(
          !mainLockfileAfter.bundles[actualBundleId],
          "Bundle should NOT be in main lockfile",
        );
      }
      ...
      assert.strictEqual(
        ...
        "SourceId should be preserved",
      );

      // Verify local lockfile is in git exclude (Requirement 4.4)
      assert.ok(
        isLocalLockfileExcluded(),
        "Local lockfile should be in git exclude after switching to local-only",
      );
      ...
      assert.ok(
        fs.existsSync(mainLockfilePath),
        "Main lockfile should exist after switching to commit",
      );
      const mainLockfileAfterSwitch = JSON.parse(
        fs.readFileSync(mainLockfilePath, "utf-8"),
      );
      assert.ok(
        mainLockfileAfterSwitch.bundles[actualBundleId],
      ...
      assert.ok(
        !fs.existsSync(localLockfilePath),
        "Local lockfile should be deleted when empty",
      );

      // Verify local lockfile is removed from git exclude (Requirement 4.5)
      assert.ok(
        !isLocalLockfileExcluded(),
        "Local lockfile should be removed from git exclude when empty",
      );
    });
    ...
    test("11.4: Removing last local-only bundle deletes local lockfile and git exclude entry", async function () {
      ...
      assert.ok(
        fs.existsSync(localLockfilePath),
        "Local lockfile should exist after installation",
      );
      ...
      assert.strictEqual(
        Object.keys(localLockfile.bundles).length,
        1,
        "Should have exactly one bundle",
      );
      ...
      assert.ok(
        !fs.existsSync(localLockfilePath),
        "Local lockfile should be deleted when last bundle is removed",
      );
    ...
    test("11.5: Mixed bundles (some commit, some local-only) in same repository", async function () {
      ...
      const commitReleases: ReleaseConfig[] = [
        { tag: "v1.0.0", version: "1.0.0", content: "commit-bundle-content" },
      ];
      ...
      const commitBundles =
        await testContext.storage.getCachedSourceBundles(commitSourceId);
      ...
      const localReleases: ReleaseConfig[] = [
        { tag: "v1.0.0", version: "1.0.0", content: "local-bundle-content" },
      ];
      ...
      const localBundles =
        await testContext.storage.getCachedSourceBundles(localSourceId);
      ...
      assert.ok(
        fs.existsSync(mainLockfilePath),
        "Main lockfile should exist for committed bundle",
      );
      assert.ok(
        fs.existsSync(localLockfilePath),
        "Local lockfile should exist for local-only bundle",
      ...
      assert.ok(
        mainBundleIds.length > 0,
        "Main lockfile should have at least one bundle",
      );
      ...
      assert.ok(
        localBundleIds.length > 0,
        "Local lockfile should have at least one bundle",
      );
      ...
      assert.strictEqual(
        overlap.length,
        0,
        "No bundle should exist in both lockfiles",
      );
      ...
      assert.ok(
        allBundles.length >= 2,
        "Should list bundles from both lockfiles",
      );

      // Verify commit modes are correctly annotated
      const commitBundles2 = allBundles.filter(
        (b) => b.commitMode === "commit",
      );
      const localOnlyBundles = allBundles.filter(
        (b) => b.commitMode === "local-only",
      );

      assert.ok(
        commitBundles2.length > 0,
        "Should have at least one committed bundle",
      );
      assert.ok(
        localOnlyBundles.length > 0,
        "Should have at least one local-only bundle",
      );
      ...
      assert.ok(
        !fs.existsSync(localLockfilePath),
        "Local lockfile should be deleted after removing last local-only bundle",
      );

      // Verify main lockfile still exists with committed bundle
      assert.ok(
        fs.existsSync(mainLockfilePath),
        "Main lockfile should still exist",
      );
      const mainLockfileAfter = JSON.parse(
        fs.readFileSync(mainLockfilePath, "utf-8"),
      );
      assert.ok(
        Object.keys(mainLockfileAfter.bundles).length > 0,
        "Main lockfile should still have committed bundle",
      );
