...
`;
  ...
  suite("Simultaneous Installation", () => {
    test("should maintain separate installation records", async function () {
      ...
      const recordA = installed.find((b) =>
        b.bundleId.includes("collection-a"),
      );
      const recordB = installed.find((b) =>
        b.bundleId.includes("collection-b"),
      );
      ...
      assert.strictEqual(
        recordA!.version,
        "1.0.0",
        "Collection-a should be v1.0.0",
      );
      assert.strictEqual(
        recordB!.version,
        "2.0.0",
        "Collection-b should be v2.0.0",
      );
    });
    ...
    test("should maintain separate file structures", async function () {
      ...
      assert.ok(
        contentA.includes("collection-a"),
        "Should have collection-a content",
      );

      // Verify collection-b files
      const promptB = path.join(
        recordB!.installPath,
        "prompts",
      ...
      assert.ok(
        contentB.includes("collection-b"),
        "Should have collection-b content",
      );
  ...
  suite("Independent Uninstallation", () => {
    test("should uninstall collection-a without affecting collection-b", async function () {
      ...
      assert.strictEqual(
        installed.length,
        1,
        "Should have one bundle remaining",
      );
      assert.ok(
      ...
      assert.ok(
        fs.existsSync(promptB),
        "Collection-b files should still exist",
      );
    });
  });
});
