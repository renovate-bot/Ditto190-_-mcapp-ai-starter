  ...
  suite("Scaffold with Skills", () => {
    test("Scaffold awesome-copilot project includes skills directory", async function () {
      ...
      await scaffoldCommand.execute(projectDir, {
        projectName: "test-skills-project",
      });
      ...
      assert.ok(
        fs.existsSync(exampleSkillDir),
        "Example skill directory should exist",
      );

      const skillMdPath = path.join(exampleSkillDir, "SKILL.md");
      assert.ok(
        fs.existsSync(skillMdPath),
        "SKILL.md should exist in example skill",
      );
    ...
    test("Scaffold creates skill validation script", async function () {
      ...
      assert.ok(
        readmeContent.includes("validate-skills") ||
          readmeContent.includes("@prompt-registry/collection-scripts"),
  ...
  suite("Collection with Skills - Install Workflow", () => {
    test("Install bundle containing skill copies skill to correct location", async function () {
      ...
      nock("https://api.github.com")
        .persist()
        .get(
          "/repos/test-owner/awesome-copilot-skills/contents/collections?ref=main",
        )
        .reply(200, [
      ...
      nock("https://raw.githubusercontent.com")
        .persist()
        .get(
          "/test-owner/awesome-copilot-skills/main/prompts/example.prompt.md",
        )
      ...
      nock("https://raw.githubusercontent.com")
        ...
        .reply(
          200,
          createSkillMd("Test Skill", "A test skill for e2e testing"),
        );
      ...
      // Search for bundles
      const bundles = await testContext.registryManager.searchBundles({
        sourceId,
      });
      assert.ok(bundles.length > 0, "Should find bundles with skills");

      // Find the skill collection bundle
      const skillBundle = bundles.find((b) =>
        b.id.includes("skill-collection"),
      );
      assert.ok(skillBundle, "Should find skill-collection bundle");

      // Install the bundle
      await testContext.registryManager.installBundle(skillBundle!.id, {
        scope: "user",
      });
...
`;
  ...
  suite("UserScopeService with Skills", () => {
    test("Sync recognizes skill files by pattern", async function () {
      ...
      fs.writeFileSync(
        skillMdPath,
        createSkillMd("Sync Test Skill", "Testing sync"),
      );
  ...
  suite("Skill Content Validation", () => {
    ...
    test("SKILL.md can include allowed-tools field", async function () {
      ...
      assert.ok(
        ...
        "Should have allowed-tools field",
      );
      assert.ok(
...
`;
      ...
      nock("https://raw.githubusercontent.com")
        .persist()
        .get(
          "/test-owner/mixed-content/main/instructions/example.instructions.md",
        )
