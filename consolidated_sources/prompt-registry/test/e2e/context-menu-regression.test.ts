...
suite("E2E: Context Menu Regression Prevention", () => {
   ...
   */
  async function setupBundleAndGetTreeItem(
    ...
    registryManagerStub.getBundleDetails
      ...
      .resolves(createMockBundleDetails(bundle.bundleId));
  ...
  suite("Property 2: User Scope Menu Options", () => {
    ...
    test("Requirement 7.2: user-scoped bundle with update available should have valid context value", async () => {
      const userBundle = createMockInstalledBundle(
        "user-bundle-updatable",
        "user",
      );

      registryManagerStub.listInstalledBundles.resolves([userBundle]);
      registryManagerStub.getBundleDetails
        .withArgs("user-bundle-updatable")
        .resolves(createMockBundleDetails("user-bundle-updatable"));
  ...
  suite("Property 3: Repository Scope Menu Options", () => {
    ...
    test("Requirement 7.3: repository-scoped bundle (commit mode) should have valid context value", async () => {
      const repoBundle = createMockInstalledBundle(
        "repo-bundle-commit",
        "repository",
        "commit",
      );
    ...
    test("Requirement 7.3: repository-scoped bundles should match combined repository pattern", async () => {
      const commitBundle = createMockInstalledBundle(
        "repo-commit",
        "repository",
        "commit",
      );
      const localOnlyBundle = createMockInstalledBundle(
        "repo-local",
        "repository",
        "local-only",
      );
      ...
      registryManagerStub.getBundleDetails
        .withArgs("repo-commit")
        .resolves(createMockBundleDetails("repo-commit"));
      registryManagerStub.getBundleDetails
        .withArgs("repo-local")
        .resolves(createMockBundleDetails("repo-local"));
      ...
      const combinedRepoPattern =
        /^installed_bundle.*_repository_(commit|local_only)$/;
  ...
  suite("Regression Detection", () => {
    ...
    test("Requirement 7.4: all scope combinations should produce valid context values", async () => {
      ...
      for (const testCase of testCases) {
        ...
        const bundle = createMockInstalledBundle(
          bundleId,
          testCase.scope,
          testCase.commitMode,
        );
    ...
    test("Requirement 7.4: valid patterns array should include all necessary patterns", () => {
      ...
      const hasBasePatterns = VALID_CONTEXT_PATTERNS_FOR_MENUS.some(
        (p) =>
          p.source.startsWith("^installed_bundle_auto_") &&
          !p.source.endsWith("$"),
      );
      ...
      const hasRepoLocalOnlyPattern = VALID_CONTEXT_PATTERNS_FOR_MENUS.some(
        (p) => p.source.includes("_repository_local_only$"),
      );

      assert.ok(
        hasBasePatterns,
        "Should have base context value patterns (for Update, Check Updates, Auto-Update, Uninstall menus)",
      );
      assert.ok(
        hasUserPattern,
        "Should have user scope pattern (for Move to Repository menus)",
      );
      assert.ok(
        hasRepoCommitPattern,
        "Should have repository commit pattern (for Switch to Local Only menu)",
      );
      assert.ok(
        hasRepoLocalOnlyPattern,
        "Should have repository local-only pattern (for Switch to Commit menu)",
      );
    });
  });
