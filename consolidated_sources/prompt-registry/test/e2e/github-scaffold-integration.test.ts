...
suite("E2E: Script Execution Tests", () => {
    ...
    try {
      ...
      assert.ok(
        !output.toLowerCase().includes("error") ||
          output.toLowerCase().includes("0 error"),
  ...
  test("E2E: Build script creates bundle with correct structure", async function () {
    ...
    try {
      ...
      execSync(
        `npm run build-collection-bundle -- --collection-file collections/example.collection.yml --version 1.0.0 --repo-slug test-repo --out-dir ${outputDir}`,
        {
          cwd: testDir,
          stdio: "pipe",
          timeout: 30000,
  ...
  test("E2E: Compute version script returns valid version", async function () {
    ...
    execSync(
      'git init && git config user.email "test@test.com" && git config user.name "Test" && git add -A && git commit -m "initial"',
      {
        cwd: testDir,
        stdio: "pipe",
        timeout: 10000,
    ...
    try {
      const result = execSync(
        "npm run compute-collection-version -- --collection-file collections/example.collection.yml",
        {
          cwd: testDir,
          stdio: "pipe",
          timeout: 30000,
  ...
  test("E2E: Package.json references collection-scripts npm package", async function () {
    ...
    for (const [scriptName, command] of Object.entries(
      ...
      assert.ok(
        packageJson.scripts &&
          packageJson.scripts[scriptName]?.includes(command),
  ...
  test("E2E: Skill helper script is properly configured", async function () {
    ...
    assert.ok(fs.existsSync(skillScript), "Skill helper script should exist");
  ...
  test("E2E: Scripts README documents available npm commands", async function () {
    ...
    assert.ok(fs.existsSync(readmePath), "Scripts README should exist");
