...
`;
    ...
    zip.addFile(
      ...
      Buffer.from(createPromptContent(content)),
    );
  ...
  suite("Test Setup Validation", () => {
    test("should create isolated test context with unique storage path", async function () {
      ...
      assert.ok(
        fs.existsSync(testContext.tempStoragePath),
  ...
  suite("GitHub Bundle Update Workflow", () => {
    test("Example 2.1: Specific version installation records correct version", async function () {
      ...
      assert.strictEqual(
        installed.length,
        1,
        "Should have one installed bundle",
      );
      assert.strictEqual(
        installed[0].version,
        "1.0.0",
    ...
    test("Example 2.2: Update check compares installed vs latest version", async function () {
      ...
      const update = updates.find((u) =>
        u.bundleId.includes("test-owner-test-repo"),
      );
    ...
    test("Example 2.3: Update information displays both versions", async function () {
      ...
      assert.ok(
        update!.currentVersion,
        "Update info should have currentVersion",
      );
    ...
    test("Example 2.6: Previous version files are removed", async function () {
      ...
      assert.ok(
        v1Content.includes("INITIAL_CONTENT"),
        "v1.0.0 should have initial content",
      );
      ...
      assert.ok(
        v2Content.includes("UPDATED_CONTENT"),
        "v2.0.0 should have updated content",
      );
      assert.ok(
    ...
    test("Example 2.7: New installation record is created", async function () {
      ...
      assert.strictEqual(
        installedAfter.length,
        1,
        "Should have one installation record",
      );
      ...
      assert.ok(
        newRecord.installedAt,
        "New record should have installedAt timestamp",
      );
    });
  });
});
