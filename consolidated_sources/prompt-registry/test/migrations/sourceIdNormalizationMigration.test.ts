...
suite("sourceIdNormalizationMigration", () => {
  ...
  setup(async () => {
    ...
    tmpDir = path.join(
      ...
      "..",
      "..",
      `.test-tmp-migration-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`,
    );
  ...
  test("should skip migration if already completed", async () => {
    ...
    assert.strictEqual(
      await migrationRegistry.isMigrationComplete(MIGRATION_NAME),
      true,
    );
  });
  ...
  test("should migrate a source with legacy ID to new ID", async () => {
    ...
    assert.strictEqual(
      migratedSource!.id,
      newId,
      "Source ID should be updated to new format",
    );
  });
  ...
  test("should not migrate non-hub source IDs", async () => {
    ...
    assert.strictEqual(
      source!.id,
      "my-custom-source",
      "Non-hub ID should remain unchanged",
    );
  });
  ...
  test("should rename source cache files during migration", async () => {
    ...
    assert.strictEqual(
      ...
      false,
      "Legacy cache file should be removed",
    );
    assert.strictEqual(
      fs.existsSync(newCacheFile),
      true,
  ...
  test("should update installation records referencing old sourceId", async () => {
    ...
    assert.strictEqual(
      updatedRecord.sourceId,
      newId,
      "Installation record sourceId should be updated",
    );
  });
  ...
  test("should be idempotent - second run is a no-op", async () => {
    ...
    const migratedFirst = sourcesAfterFirst.find(
      (s) => s.name === "Idempotent Test",
    );
    ...
    const migratedSecond = sourcesAfterSecond.find(
      (s) => s.name === "Idempotent Test",
    );
    ...
    assert.strictEqual(
      migratedSecond!.id,
      newId,
      "ID should still be new format after second run",
    );
  });
});
