      ...
      await fc.assert(
        fc.asyncProperty(
          ...
          async (bundleId, initialState) => {
            ...
            AutoUpdateTestHelpers.assertPreferenceStored(
              mockStorage,
              bundleId,
              finalState,
            );
    ...
    test("should retrieve auto-update status correctly", async () => {
      await fc.assert(
        fc.asyncProperty(bundleIdArb, booleanArb, async (bundleId, enabled) => {
          ...
          AutoUpdateTestHelpers.setupStorageMock(
            mockStorage,
            bundleId,
            enabled,
          );
          ...
          const result =
            await mockAutoUpdateService.isAutoUpdateEnabled(bundleId);

          // Verify correct status was returned
          assert.strictEqual(result, enabled);
          AutoUpdateTestHelpers.assertPreferenceRetrieved(
            mockStorage,
            bundleId,
          );
