      ...
      fc.asyncProperty(
        ...
        async (setupState, bundleCount) => {
          ...
          const isSetupIncomplete =
            setupState === SetupState.INCOMPLETE ||
      ...
      fc.asyncProperty(
        ...
        async (checkSetupPrompt) => {
          ...
          if (fs.existsSync(cssPath)) {
            ...
            assert.ok(
              css.includes(".primary-button"),
              "CSS should include primary-button class",
            );
            assert.ok(
              css.includes(".empty-state"),
              "CSS should include empty-state class",
            );
            assert.ok(
              css.includes(".empty-state-icon"),
              "CSS should include empty-state-icon class",
            );
            assert.ok(
              css.includes(".empty-state-title"),
              "CSS should include empty-state-title class",
            );
          }
          ...
          if (fs.existsSync(jsPath)) {
            ...
            assert.ok(
              js.includes("completeSetup"),
              "JS should include completeSetup function",
            );
            assert.ok(
              js.includes("setupState"),
              "JS should include setupState variable",
            );
            assert.ok(
              js.includes("Setup Not Complete") ||
                js.includes("No hub is configured"),
              "JS should include setup prompt message",
            );
            assert.ok(
              js.includes("Syncing sources..."),
              "JS should include syncing message",
            );
          }
