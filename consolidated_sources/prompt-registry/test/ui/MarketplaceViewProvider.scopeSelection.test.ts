...
suite("MarketplaceViewProvider - Scope Selection Bug", () => {
  suite("Scope selection dialog options", () => {
    test("should provide three scope options", () => {
      ...
      assert.strictEqual(
        items.length,
        3,
        "Should have exactly 3 scope options",
      );

      const scopes = items.map((i) => ({
        scope: i._scope,
        commitMode: i._commitMode,
      }));
      assert.ok(
        scopes.some(
          (s) => s.scope === "repository" && s.commitMode === "commit",
        ),
        "Should have repository commit option",
      );
      assert.ok(
        scopes.some(
          (s) => s.scope === "repository" && s.commitMode === "local-only",
        ),
        "Should have repository local-only option",
      );
      assert.ok(
        scopes.some((s) => s.scope === "user"),
    ...
    test("should disable repository options when no workspace is open", () => {
      ...
      const repoCommit = items.find(
        (i) => i._scope === "repository" && i._commitMode === "commit",
      );
      const repoLocal = items.find(
        (i) => i._scope === "repository" && i._commitMode === "local-only",
      );
      const userOption = items.find((i) => i._scope === "user");

      assert.ok(
        repoCommit?._disabled,
        "Repository commit option should be disabled without workspace",
      );
      assert.ok(
        repoLocal?._disabled,
        "Repository local-only option should be disabled without workspace",
      );
    ...
    test("should enable all options when workspace is open", () => {
      ...
      assert.ok(
        !repoCommit?._disabled,
        "Repository commit option should be enabled with workspace",
      );
      assert.ok(
        !repoLocal?._disabled,
        "Repository local-only option should be enabled with workspace",
      );
    ...
    test("should pre-select repository commit when workspace is open", () => {
      ...
      assert.ok(
        repoCommit?.picked,
        "Repository commit should be pre-selected when workspace is open",
      );
      assert.strictEqual(
        othersPicked.length,
        0,
  ...
  suite("Scope option labels and descriptions", () => {
    test("should have descriptive labels for each option", () => {
      ...
      assert.ok(
        repoCommit?.label.includes("Repository"),
        "Commit option should mention Repository",
      );
      assert.ok(
        repoCommit?.label.includes("Commit") ||
          repoCommit?.label.includes("Git"),
        "Commit option should mention Git/Commit",
      );

      assert.ok(
        repoLocal?.label.includes("Repository"),
        "Local option should mention Repository",
      );
      assert.ok(
        repoLocal?.label.includes("Local"),
        "Local option should mention Local",
      );

      assert.ok(
        userOption?.label.includes("User"),
        "User option should mention User",
      );
    });
    ...
    test("should show workspace requirement hint when disabled", () => {
      ...
      assert.ok(
        repoCommit?.detail?.includes("workspace"),
        "Disabled commit option should explain workspace requirement",
      );
      assert.ok(
        repoLocal?.detail?.includes("workspace"),
