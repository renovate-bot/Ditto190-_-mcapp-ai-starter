...
suite("RegistryTreeProvider - Hub Profiles", () => {
  ...
  setup(() => {
    ...
    provider = new RegistryTreeProvider(
      registryManagerStub as any,
      hubManagerStub as any,
    );
  });
  ...
  test("getFavoritesItems should return items with correct contextValue for menu actions", async () => {
    ...
    assert.ok(
      profileItem.contextValue &&
        profileItem.contextValue.includes("hub_profile"),
    );
  });
  ...
  test("Favorites view should show active local profile in Active Profile section", async () => {
    ...
    const activeItem = children.find(
      (i: RegistryTreeItem) => i.label === "Local Profile",
    );
  ...
  test("Favorites view should show active hub profile in Active Profile section", async () => {
    ...
    hubManagerStub.getHubProfile
      .withArgs(hubId, "active-profile")
      .resolves(activeProfile);
  ...
  test("should deduplicate items when local profile matches favorite Hub Profile", async () => {
    ...
    const hubItem = items.find(
      (i: RegistryTreeItem) => i.type === TreeItemType.HUB,
    );
  ...
  test("should organize profiles by path", async () => {
    ...
    hubManagerStub.listProfilesFromHub
      .withArgs(hubId)
      .resolves([rootProfile, nestedProfile]);
    hubManagerStub.getFavoriteProfiles.resolves({});
    ...
    const rootProfileItem = rootItems.find(
      (i) => i.label === "icon Root Profile",
    );
...
suite("RegistryTreeProvider - Dual-Scope Display", () => {
  ...
  test("should list bundles from both user and repository scopes", async () => {
    ...
    registryManagerStub.listInstalledBundles.resolves([
      userBundle,
      repositoryBundle,
    ]);
    ...
    assert.strictEqual(
      items.length,
      2,
      "Should display both user and repository bundles",
    );
  ...
  test("should show scope indicator for repository bundles", async () => {
    ...
    assert.ok(
      item.contextValue.includes("repository") ||
        item.data.scope === "repository",
    ...
    assert.strictEqual(
      item.data.scope,
      "repository",
      "Bundle data should contain repository scope",
    );
  });
  ...
  test("should show scope indicator for user bundles", async () => {
    ...
    assert.strictEqual(
      item.data.scope,
      "user",
      "Bundle data should contain user scope",
    );
  });
  ...
  test("should show update indicators for both user and repository scopes", async () => {
    ...
    assert.ok(
      userItem.label.includes("⬆️"),
      "User bundle should show update indicator",
    );
    assert.ok(
      repoItem.label.includes("⬆️"),
      "Repository bundle should show update indicator",
    );

    // Check version display shows both versions
    assert.ok(
      userItem.description &&
        typeof userItem.description === "string" &&
        userItem.description.includes("→"),
      "User bundle should show version arrow",
    );
    assert.ok(
      repoItem.description &&
        typeof repoItem.description === "string" &&
        repoItem.description.includes("→"),
      "Repository bundle should show version arrow",
    );
  });
  ...
  test("Favorites view should organize hub profiles by path", async () => {
    ...
    const folderItem = children.find(
      (i: RegistryTreeItem) => i.label === "Folder",
    );
    ...
    // Should find "Subfolder"
    const subfolderItem = folderChildren.find(
      (i: RegistryTreeItem) => i.label === "Subfolder",
    );
    assert.ok(subfolderItem, "Subfolder not found");
    assert.strictEqual(subfolderItem.type, TreeItemType.PROFILE_FOLDER);
...
suite("RegistryTreeProvider - Files Missing Warning Indicator", () => {
  ...
  test("should show warning indicator for bundle with filesMissing flag", async () => {
    ...
    registryManagerStub.getBundleDetails
      .withArgs("missing-files-bundle")
      .resolves({
    ...
    assert.ok(
      item.label.includes("⚠️"),
      "Bundle with missing files should show warning emoji",
    );
    ...
    assert.ok(
      item.tooltip &&
        ...
        item.tooltip.includes("Files missing"),
    ...
    assert.ok(
      item.iconPath,
      "Bundle with missing files should have warning icon",
    );
    assert.ok(
  ...
  test("should NOT show warning indicator for bundle without filesMissing flag", async () => {
    ...
    assert.ok(
      !item.label.includes("⚠️"),
      "Normal bundle should not show warning emoji",
    );
  ...
  test("should show warning indicator when bundle details are not available", async () => {
    ...
    registryManagerStub.listInstalledBundles.resolves([bundleWithMissingFiles]);
    registryManagerStub.getBundleDetails
      .withArgs("missing-files-bundle-no-details")
    ...
    assert.ok(
      item.label.includes("⚠️"),
      "Bundle with missing files should show warning emoji even without details",
    );
  ...
  test("should distinguish between bundles with and without missing files", async () => {
    ...
    const missingFilesItem = items.find((i) =>
      i.label.includes("Missing Files Bundle"),
    );
    ...
    assert.ok(
      missingFilesItem.label.includes("⚠️"),
      "Missing files bundle should show warning emoji",
    );
    assert.ok(
