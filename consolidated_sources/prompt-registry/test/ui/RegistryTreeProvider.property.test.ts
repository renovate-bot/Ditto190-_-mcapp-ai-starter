          ...
          fc
            ...
            .filter(({ updatableBundles, regularBundles }) => {
              ...
              const updatableIds = new Set(
                updatableBundles.map((b) => b.bundleId),
              );
      ...
      fc.property(
        ...
        ({ userBundles, repositoryBundles }) => {
          ...
          for (const userBundle of userBundles) {
            const found = allBundles.find(
              (b) => b.bundleId === userBundle.bundleId && b.scope === "user",
            );
            if (!found) {
    ...
    fc.assert(
      fc.property(
        ...
        ({
          ...
          const userIconPrefix = getBundleIconPrefix(
            userBundleId,
            availableUpdates,
          );
          if (userIconPrefix !== "⬆️") {
          ...
          const userVersionDisplay = getVersionDisplay(
            userBundleId,
            userVersion,
            availableUpdates,
          );
          ...
          const repoIconPrefix = getBundleIconPrefix(
            repoBundleId,
            availableUpdates,
          );
          if (repoIconPrefix !== "⬆️") {
          ...
          const repoVersionDisplay = getVersionDisplay(
            repoBundleId,
            repoVersion,
            availableUpdates,
          );
    ...
    fc.assert(
      fc.property(
        fc
          ...
          .filter(
            ({ userBundleId, repoBundleId }) => userBundleId !== repoBundleId,
          ),
        ({ userBundleId, userHasUpdate, repoBundleId, repoHasUpdate }) => {
          ...
          const userContextValue = getContextValue(
            userBundleId,
            availableUpdates,
          );
          const expectedUserContext = userHasUpdate
            ? "installed_bundle_updatable"
            : "installed_bundle";
          ...
          const repoContextValue = getContextValue(
            repoBundleId,
            availableUpdates,
          );
          const expectedRepoContext = repoHasUpdate
            ? "installed_bundle_updatable"
            : "installed_bundle";
