 ...
 */
export type LockfileSourceType =
  (typeof LOCKFILE_DEFAULTS.SOURCE_TYPES)[number];
...
export class LockfileBuilder {
  ...
  withBundleAndFiles(
    ...
    options?: Partial<
      Omit<LockfileBundleEntry, "version" | "sourceId" | "files">
    >,
  ): LockfileBuilder {
...
export const LockfileGenerators = {
  ...
  bundleEntry: (sourceId?: string): fc.Arbitrary<LockfileBundleEntry> => {
    return fc.record({
      ...
      files: fc.array(LockfileGenerators.fileEntry(), {
        minLength: 0,
        maxLength: 5,
      }),
    });
  ...
  lockfile: (options?: {
    ...
    return fc
      .array(LockfileGenerators.sourceId(), { minLength: 1, maxLength: 3 })
      .chain((sourceIds) => {
        // Create sources record
        const sourcesArb = fc
          .tuple(
            ...sourceIds.map((id) =>
              LockfileGenerators.sourceEntry().map(
        ...
        const bundlesArb = fc
          .array(
            fc.tuple(
              ...
              fc
                .constantFrom(...sourceIds)
                .chain((sourceId) => LockfileGenerators.bundleEntry(sourceId)),
            ),
          ...
          ? fc.option(
              fc
                .array(
                  fc.tuple(
                    LockfileGenerators.sourceId(),
                    LockfileGenerators.profileEntry(),
                  ),
                  { minLength: 1, maxLength: 3 },
                )
                .map((entries) => Object.fromEntries(entries)),
...
export function testLockfileBuilder() {
  ...
  suite("LockfileBuilder", () => {
    ...
    test("should add bundle with default values", () => {
      ...
      assert.strictEqual(
        lockfile.bundles["test-bundle"].sourceId,
        "test-source",
      );
    ...
    test("should add source entry", () => {
      ...
      assert.strictEqual(
        lockfile.sources["github-source"].url,
        "https://github.com/owner/repo",
      );
    ...
    test("should add profile entry", () => {
      ...
      assert.deepStrictEqual(lockfile.profiles!["my-profile"].bundleIds, [
        "bundle-1",
        "bundle-2",
      ]);
    });
  ...
  suite("createMockLockfile", () => {
    ...
    test("should include profiles when requested", () => {
      ...
      assert.deepStrictEqual(lockfile.profiles!["mock-profile"].bundleIds, [
        "bundle-0",
        "bundle-1",
      ]);
    });
