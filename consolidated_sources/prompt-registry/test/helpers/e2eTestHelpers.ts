...
export async function createE2ETestContext(): Promise<E2ETestContext> {
  ...
  const mockContext: vscode.ExtensionContext = {
    globalState: {
      get: <T>(key: string, defaultValue?: T): T | undefined => {
        return globalStateData.has(key)
          ? globalStateData.get(key)
          : defaultValue;
      },
    ...
    workspaceState: {
      get: <T>(key: string, defaultValue?: T): T | undefined => {
        return workspaceStateData.has(key)
          ? workspaceStateData.get(key)
          : defaultValue;
      },
      update: async (key: string, value: any): Promise<void> => {
  ...
  const BundleInstaller =
    require("../../src/services/BundleInstaller").BundleInstaller;
...
export interface BundleSetupOptions {
  ...
  setupReleaseMocks: (
    releases: Array<{ tag: string; version: string; content: string }>,
  ) => void;
...
export async function setupSourceAndGetBundleGeneric(
  ...
  if (!bundle) {
    throw new Error(
      `Should find bundle ${expectedBundleId}, found: ${rawBundles.map((b: any) => b.id).join(", ")}`,
    );
  }
