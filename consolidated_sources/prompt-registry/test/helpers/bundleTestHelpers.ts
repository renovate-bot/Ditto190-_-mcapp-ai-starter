...
export class BundleBuilder {
  ...
  static fromSource(
    bundleId: string,
    sourceType: keyof typeof TEST_SOURCE_IDS,
  ): BundleBuilder {
    const builder = new BundleBuilder();
  ...
  build(): Bundle {
    if (!this.bundle.id || !this.bundle.version) {
      throw new Error(
        "Bundle must have id and version. Call withVersion() before build()",
      );
    }
 ...
 */
export function createUniqueUpdateCheckResult(
  index: number,
): UpdateCheckResult {
  return {
...
export function setupUpdateAvailable(
  ...
  const update = createMockUpdateCheckResult(
    bundleId,
    currentVersion,
    latestVersion,
  );
...
export function setupProgressMock(sandbox: any): any {
  const mockWithProgress = sandbox.stub(
...
export function resetBundleCommandsMocks(
  ...
  mockWithProgress.callsFake(async (_options: any, callback: any) => {
    return await callback(
      ...
      {
        isCancellationRequested: false,
        onCancellationRequested: require("sinon").stub(),
      },
    );
  });
 ...
 */
export function generateBundlesWithUpdates(
  ...
  suite("BundleBuilder", () => {
    suite("withVersion", () => {
      test("should handle multiple version updates correctly", () => {
        ...
        const bundle2 = BundleBuilder.github("owner", "repo")
          .withVersion("2.0.0")
          .build();
 ...
 * ```
 */
export function createMockBundleDirectory(
  options: MockBundleDirectoryOptions,
): string {
 ...
 * ```
 */
export function createSimpleMockBundle(
  basePath: string,
  bundleId: string,
  version: string,
): string {
