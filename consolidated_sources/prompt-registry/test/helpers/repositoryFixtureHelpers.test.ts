...
suite("Repository Fixture Helpers", () => {
  ...
  suite("createDeploymentManifest", () => {
    test("should create manifest with all required fields (Requirement 1.3)", () => {
      ...
      assert.ok(
        ...
        "environments should be non-empty array",
      );
      assert.ok(
    ...
    test("should include content identifier in name and description", () => {
      ...
      assert.ok(
        manifest.name.includes("custom-content"),
        "name should include content identifier",
      );
      assert.ok(
        manifest.description.includes("custom-content"),
        "description should include content identifier",
      );
    });
    ...
    test("should use default content when not specified", () => {
      ...
      assert.ok(
        manifest.name.includes("initial"),
        "name should include default content",
      );
    });
  });
  ...
  suite("createBundleZip", () => {
    ...
    test("should contain deployment-manifest.yml (Requirement 1.5)", () => {
      ...
      const manifestEntry = entries.find(
        (e) => e.entryName === "deployment-manifest.yml",
      );
      ...
      assert.ok(
        manifestContent.includes("id: test-bundle"),
        "manifest should contain id",
      );
      assert.ok(
        manifestContent.includes("version: 1.0.0"),
    ...
    test("should contain prompts/test.prompt.md (Requirement 1.5)", () => {
      ...
      const promptEntry = entries.find(
        (e) => e.entryName === "prompts/test.prompt.md",
      );
      ...
      assert.ok(
        promptContent.includes("# Test Prompt"),
        "prompt should have header",
      );
      assert.ok(
        promptContent.includes("Content: test"),
        "prompt should include content identifier",
      );
    });
    ...
    test("should create ZIP that can be extracted and re-read (round-trip)", () => {
      ...
      const manifestContent = manifestEntry!.getData().toString("utf-8");
      assert.ok(
        manifestContent.includes(`id: ${config.manifestId}`),
        "manifest id should match",
      );
      assert.ok(
        manifestContent.includes(`version: ${version}`),
        "manifest version should match",
      );
      assert.ok(
        manifestContent.includes(`author: ${config.owner}`),
        "manifest author should match",
      );
    });
  });
  ...
  suite("setupReleaseMocks", () => {
    ...
    test("should configure repository metadata endpoint (Requirement 1.4)", async () => {
      const config = createTestConfig();
      setupReleaseMocks(config, [
        { tag: "v1.0.0", version: "1.0.0", content: "test" },
      ]);
    ...
    test("should configure bundle download with redirect (Requirement 1.4)", async () => {
      ...
      assert.ok(
        redirectResponse.headers.location.includes(
  ...
  suite("MochaTestContext", () => {
    test("should be compatible with Mocha test context", function () {
      ...
      assert.strictEqual(
        typeof context.retries,
        "function",
        "retries should be a function",
      );
    });
  });
