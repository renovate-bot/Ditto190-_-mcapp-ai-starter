...
export function isValidContextValue(contextValue: string): boolean {
  return VALID_CONTEXT_PATTERNS_FOR_MENUS.some((pattern) =>
...
export function extractPackageJsonContextPatterns(): RegExp[] {
  ...
  const menus: PackageJsonMenuItem[] =
...
export function validateContextPatterns(): PatternValidationResult {
  ...
  const missingInHelper = [...packageSources].filter(
    (s) => !helperSources.has(s),
  );
  const extraInHelper = [...helperSources].filter(
    (s) => !packageSources.has(s),
  );
