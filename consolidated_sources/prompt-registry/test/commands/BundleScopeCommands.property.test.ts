...
suite("BundleScopeCommands Property Tests", () => {
  ...
  suite("Property 9: Context Menu Scope Actions", function () {
    ...
    test("user-scoped bundles show repository move options", async () => {
      await fc.assert(
        fc.asyncProperty(BundleGenerators.bundleId(), async (bundleId) => {
          // Arrange
          ...
          // Assert
          const hasCommitOption = actions.some(
            (a: ContextMenuAction) => a.id === "moveToRepositoryCommit",
          );
          const hasLocalOnlyOption = actions.some(
            (a: ContextMenuAction) => a.id === "moveToRepositoryLocalOnly",
          );
          const hasMoveToUserOption = actions.some(
            (a: ContextMenuAction) => a.id === "moveToUser",
          );
    ...
    test("repository-scoped bundles with commit mode show correct options", async () => {
      await fc.assert(
        fc.asyncProperty(BundleGenerators.bundleId(), async (bundleId) => {
          // Arrange
          const repoBundle = createTestInstalledBundle(
            bundleId,
            "repository",
            "commit",
          );
          ...
          const hasSwitchToLocalOnlyOption = actions.some(
            (a: ContextMenuAction) => a.id === "switchToLocalOnly",
          );
          const hasSwitchToCommitOption = actions.some(
            (a: ContextMenuAction) => a.id === "switchToCommit",
          );

          return (
            hasMoveToUserOption &&
            hasSwitchToLocalOnlyOption &&
            !hasSwitchToCommitOption
          );
        }),
        {
          ...PropertyTestConfig.FAST_CHECK_OPTIONS,
      ...
      await fc.assert(
        fc.asyncProperty(BundleGenerators.bundleId(), async (bundleId) => {
          ...
          const repoActions = actions.filter(
            (a: ContextMenuAction) =>
              a.id === "moveToRepositoryCommit" ||
              a.id === "moveToRepositoryLocalOnly",
          );

          return repoActions.every(
            (a: ContextMenuAction) => a.disabled === true,
          );
        }),
        {
          ...PropertyTestConfig.FAST_CHECK_OPTIONS,
    ...
    test("context menu actions are mutually exclusive", async () => {
      await fc.assert(
        fc.asyncProperty(bundleConfigGenerator(), async (config) => {
          // Arrange
          const bundle = createTestInstalledBundle(
            config.bundleId,
            config.scope,
            config.commitMode,
          );
          ...
          if (config.scope === "user" || config.scope === "workspace") {
            mockStorage.getInstalledBundle
              .withArgs(config.bundleId, config.scope)
              .resolves(bundle);
            mockStorage.getInstalledBundle
              .withArgs(config.bundleId, "repository")
              .resolves(undefined);
          } else {
            mockStorage.getInstalledBundle
              .withArgs(config.bundleId, "user")
              .resolves(undefined);
            mockStorage.getInstalledBundle
              .withArgs(config.bundleId, "repository")
              .resolves(bundle);
          ...
          mockStorage.getInstalledBundle
            .withArgs(config.bundleId, "workspace")
            .resolves(undefined);
          ...
          const hasSwitchToCommit = actions.some(
            (a: ContextMenuAction) => a.id === "switchToCommit",
          );
          const hasSwitchToLocalOnly = actions.some(
            (a: ContextMenuAction) => a.id === "switchToLocalOnly",
          );
