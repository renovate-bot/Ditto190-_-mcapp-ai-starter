...
suite("ScopeSelectionUI - Property Tests", () => {
  ...
  suite("Property 8: Scope Selection UI Completeness", () => {
    ...
    test("should always present exactly three options regardless of workspace state", async () => {
      await fc.assert(
        fc.asyncProperty(workspaceStateArb, async (hasWorkspace) => {
          resetAllMocks();
    ...
    test("should present options in correct order for any workspace state", async () => {
      await fc.assert(
        fc.asyncProperty(workspaceStateArb, async (hasWorkspace) => {
          ...
          // Verify order
          assert.ok(
            items[0].label.includes("Repository") &&
              items[0].label.includes("Commit"),
            "First option should be Repository - Commit",
          );
          assert.ok(
            items[1].label.includes("Repository") &&
              items[1].label.includes("Local"),
          ...
          async (selectionIndex) => {
            ...
            if (selectionIndex === 0) {
              assert.strictEqual(
                result.scope,
                "repository",
                "Index 0 should return repository scope",
              );
              assert.strictEqual(
                result.commitMode,
                "commit",
                "Index 0 should return commit mode",
              );
            } else if (selectionIndex === 1) {
              assert.strictEqual(
                result.scope,
                "repository",
                "Index 1 should return repository scope",
              );
              assert.strictEqual(
                result.commitMode,
                "local-only",
                "Index 1 should return local-only mode",
              );
            } else {
              assert.strictEqual(
                result.scope,
                "user",
                "Index 2 should return user scope",
              );
              assert.strictEqual(
                result.commitMode,
                undefined,
                "Index 2 should not have commit mode",
              );
            }
