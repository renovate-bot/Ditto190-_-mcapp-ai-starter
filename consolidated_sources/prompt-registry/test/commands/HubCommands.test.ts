...
suite("HubCommands", () => {
  ...
  suite.skip(
    "Command Registration (skipped - commands not registered in test mode)",
    () => {
  ...
  suite.skip("Import Hub Command (requires vscode UI mocks)", () => {
    test("should import hub from GitHub URL", async () => {
      ...
      const showOpenDialogStub = () =>
        Promise.resolve([vscode.Uri.file(fixturePath)]);
    ...
    test("should show error message on import failure", async () => {
      ...
      const showQuickPickStub = () =>
        Promise.resolve({ label: "GitHub", value: "github" });
  ...
  suite.skip("Error Handling (requires vscode UI mocks)", () => {
    test("should handle hub manager errors gracefully", async () => {
      ...
      const badCommands = new HubCommands(
        badManager,
