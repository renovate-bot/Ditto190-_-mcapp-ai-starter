...
suite("BundleCommands Cleanup Stale Property Tests", () => {
  ...
  const createTempDir = (): string => {
    const dir = path.join(
      ...
      "..",
      "..",
      "test-temp-cleanup-prop-" + Date.now(),
    );
      ...
      await fc.assert(
        fc.asyncProperty(
          ...
          async (validBundleCount: number, staleBundleCount: number) => {
            ...
            const builder = LockfileBuilder.create().withSource(
              "test-source",
              "github",
            ...
            const installedBundles =
              await lockfileManager.getInstalledBundles();

            // Property: Valid bundles should NOT have filesMissing flag
            for (const bundleId of validBundleIds) {
              const bundle = installedBundles.find(
                (b) => b.bundleId === bundleId,
              );
              assert.ok(
                bundle,
                `Valid bundle ${bundleId} should be in installed bundles`,
              );
              assert.strictEqual(
                bundle.filesMissing,
                false,
            ...
            for (const bundleId of staleBundleIds) {
              ...
              assert.ok(
                bundle,
                `Stale bundle ${bundleId} should be in installed bundles`,
              );
              assert.strictEqual(
                bundle.filesMissing,
                true,
      ...
      await fc.assert(
        fc.asyncProperty(
          ...
          async (validBundleCount: number, staleBundleCount: number) => {
            ...
            const remainingBundleCount = Object.keys(
              updatedLockfile?.bundles || {},
            ).length;
            assert.strictEqual(
      ...
      await fc.assert(
        fc.asyncProperty(
          ...
          async (validBundleCount: number) => {
            ...
            const bundleCount = Object.keys(
              currentLockfile?.bundles || {},
            ).length;
            assert.strictEqual(
              bundleCount,
              validBundleCount,
        ...
        fc.asyncProperty(
          ...
          async (staleBundleCount: number) => {
            ...
            assert.ok(
              ...
              "Lockfile should exist before cleanup",
            );
