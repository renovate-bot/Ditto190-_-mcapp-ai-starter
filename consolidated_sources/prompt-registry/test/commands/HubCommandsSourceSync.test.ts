...
suite("HubCommands Source Sync", () => {
  ...
  test("should sync sources when importing a hub", async () => {
    ...
    const source1 = mockRegistryManager.sources.find(
      (s: any) => s.id === expectedSource1Id,
    );
    assert.ok(
      source1,
      `Source 1 should be present with new format ID: ${expectedSource1Id}`,
    );
    assert.strictEqual(
      source1.hubId,
      "test-hub-id",
      "Source should have hubId injected",
    );

    const source2 = mockRegistryManager.sources.find(
      (s: any) => s.id === expectedSource2Id,
    );
    assert.ok(
      source2,
      `Source 2 should be present with new format ID: ${expectedSource2Id}`,
    );
    assert.strictEqual(
      source2.hubId,
      "test-hub-id",
  ...
  test("should skip existing sources when importing a hub", async () => {
    ...
    assert.ok(
      source2,
      `Source 2 should be added with new format ID: ${expectedSource2Id}`,
    );
  });
