...
suite("Hub Sync History", () => {
    ...
    test("should format history entry with all change types", async () => {
      const entry: SyncHistoryEntry = {
        ...
        changes: {
          ...
          updated: [
            { id: "bundle-1", oldVersion: "1.0.0", newVersion: "2.0.0" },
          ],
          removed: [],
          metadataChanged: false,
        },
  ...
  suite("Rollback to History Entry", () => {
    ...
    test("should record rollback as new history entry", async () => {
      ...
      const historyBefore = await syncHistory.getHistory(
        "test-hub",
        "test-profile",
      );
      await syncHistory.rollbackToEntry("test-hub", "test-profile", entry, {
      ...
      const historyAfter = await syncHistory.getHistory(
        "test-hub",
        "test-profile",
      );
  ...
  suite("Clear History", () => {
    ...
    test("should clear history only for specified profile", async () => {
      ...
      const history2 = await syncHistory.getHistory(
        "test-hub",
        "test-profile-2",
      );
