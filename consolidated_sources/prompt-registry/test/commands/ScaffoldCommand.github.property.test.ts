...
suite("GitHub Scaffold Property-Based Tests", () => {
    ...
    await fc.assert(
      fc.asyncProperty(scaffoldContextGenerator(), async (config) => {
        const tempDir = fs.mkdtempSync(
          path.join(os.tmpdir(), "scaffold-prop-test-"),
        );
  ...
  test("Property 2: Agent-Only Item Kinds", async function () {
    ...
    await fc.assert(
      fc.asyncProperty(scaffoldContextGenerator(), async (config) => {
        const tempDir = fs.mkdtempSync(
          path.join(os.tmpdir(), "scaffold-agent-test-"),
        );
  ...
  test("Property 4: GitHub Runner Substitution", async function () {
    ...
    await fc.assert(
      fc.asyncProperty(scaffoldContextGenerator(), async (config) => {
        const tempDir = fs.mkdtempSync(
          path.join(os.tmpdir(), "scaffold-runner-test-"),
        );
    ...
    await fc.assert(
      fc.asyncProperty(scaffoldContextGenerator(), async (config) => {
        ...
        try {
          ...
          const publishWorkflowContent = fs.readFileSync(
            publishWorkflowPath,
            "utf8",
          );
          ...
          const publishCommonContent = fs.readFileSync(
            publishCommonPath,
            "utf8",
          );
          ...
          assert.ok(
            publishWorkflowContent.includes(
          ...
          const publishCommonIndex =
            publishWorkflowContent.indexOf("publish-common");
          ...
          const installDepsIndex = publishCommonContent.indexOf(
            "Install dependencies",
          );
          const validateIndex = publishCommonContent.indexOf(
...
`,
          ...
          const scenario =
            await ScaffoldCommand.detectMigrationScenario(tempDir);
          assert.ok(
          ...
          const recommendation =
            ScaffoldCommand.getMigrationRecommendation(scenario);
          assert.ok(
            recommendation &&
