...
suite("Hub Sync Commands", () => {
  ...
  suite("Sync Profile", () => {
    test("should sync profile and update activation state", async () => {
      ...
      const changesBefore = await hubManager.hasProfileChanges(
        "test-hub",
        "test-profile",
      );
      ...
      const changesAfter = await hubManager.hasProfileChanges(
        "test-hub",
        "test-profile",
      );
  ...
  suite("Check All Hubs For Updates", () => {
    test("should check all hubs for updates", async () => {
      ...
      await storage.saveHub("hub-1", hub1, {
        type: "github",
        location: "test/repo",
      });
      ...
      await storage.saveHub("hub-2", hub2, {
        type: "github",
        location: "test/repo",
      });
      await hubManager.activateProfile("hub-2", "profile-2", {
        installBundles: false,
      });
  ...
  suite("Command Registration", () => {
    test("should register all sync commands", () => {
      ...
      assert.ok(
        registeredCommands.includes("promptRegistry.hub.checkForUpdates"),
      );
      ...
      assert.ok(
        registeredCommands.includes("promptRegistry.hub.reviewAndSync"),
      );
      assert.ok(
        registeredCommands.includes("promptRegistry.hub.checkAllForUpdates"),
      );
    });
  });
});
