...
suite("BundleScopeCommands", () => {
  ...
  setup(() => {
    ...
    mockScopeConflictResolver = sandbox.createStubInstance(
      ScopeConflictResolver,
    );
    mockRepositoryScopeService = sandbox.createStubInstance(
    ...
    mockWorkspaceFolders = [
      { uri: vscode.Uri.file("/mock/workspace"), name: "workspace", index: 0 },
    ];
    sandbox
    ...
    sandbox
      ...
      .returns(mockLockfileManager as any);
  ...
  suite("moveToRepository()", () => {
    test("should move bundle from user scope to repository scope with commit mode", async () => {
      ...
      mockStorage.getInstalledBundle
        .withArgs(testBundleId, "user")
        .resolves(userBundle);
      mockStorage.getInstalledBundle
        .withArgs(testBundleId, "repository")
        .resolves(undefined);
      ...
      // Assert
      assert.ok(
        mockScopeConflictResolver.migrateBundle.calledOnce,
        "migrateBundle should be called",
      );
      assert.ok(
        mockShowInformationMessage.calledOnce,
    ...
    test("should abort move if user cancels confirmation", async () => {
      ...
      // Assert
      assert.ok(
        mockScopeConflictResolver.migrateBundle.notCalled,
        "migrateBundle should not be called",
      );
    });
    ...
    test("should show error if bundle is not installed at user scope", async () => {
      ...
      // Assert
      assert.ok(
        mockShowErrorMessage.calledOnce,
  ...
  suite("moveToUser()", () => {
    ...
    test("should pass repository scope to uninstallBundle — Validates: Requirement 3.1", async () => {
      ...
      assert.ok(
        mockRegistryManager.uninstallBundle.calledWith(
          testBundleId,
          "repository",
        ),
  ...
  suite("switchCommitMode()", () => {
    test("should switch from commit mode to local-only mode", async () => {
      ...
      // Assert
      assert.ok(
        mockRepositoryScopeService.switchCommitMode.calledOnceWith(
          testBundleId,
          "local-only",
        ),
        "switchCommitMode should be called with correct args",
      );
      assert.ok(
        mockShowInformationMessage.calledOnce,
        "Success message should be shown",
    ...
    test("should abort switch if user cancels confirmation", async () => {
      ...
      // Assert
      assert.ok(
        mockRepositoryScopeService.switchCommitMode.notCalled,
        "switchCommitMode should not be called",
      );
    });
  ...
  suite("getContextMenuActions()", () => {
    test("should return move to repository options for user-scoped bundle", async () => {
      ...
      // Assert
      assert.ok(
        actions.some((a: any) => a.id === "moveToRepositoryCommit"),
        "Should have move to repository (commit) option",
      );
      assert.ok(
        actions.some((a: any) => a.id === "moveToRepositoryLocalOnly"),
        "Should have move to repository (local-only) option",
      );
      assert.ok(
        !actions.some((a: any) => a.id === "moveToUser"),
        "Should not have move to user option",
      );
    });
    ...
    test("should return move to user and switch mode options for repository-scoped bundle with commit mode", async () => {
      ...
      assert.ok(
        actions.some((a: any) => a.id === "moveToUser"),
        "Should have move to user option",
      );
      assert.ok(
        actions.some((a: any) => a.id === "switchToLocalOnly"),
        "Should have switch to local-only option",
      );
      assert.ok(
        !actions.some((a: any) => a.id === "switchToCommit"),
        "Should not have switch to commit option",
      );
    });
    ...
    test("should return move to user and switch mode options for repository-scoped bundle with local-only mode", async () => {
      ...
      assert.ok(
        actions.some((a: any) => a.id === "switchToCommit"),
        "Should have switch to commit option",
      );
      assert.ok(
        !actions.some((a: any) => a.id === "switchToLocalOnly"),
        "Should not have switch to local-only option",
      );
    });
    ...
    test("should disable repository options when no workspace is open", async () => {
      ...
      // Assert
      const repoActions = actions.filter((a: any) =>
        a.id.startsWith("moveToRepository"),
      );
      assert.ok(
        repoActions.every((a: any) => a.disabled),
        "Repository options should be disabled",
      );
    });
  });
  ...
  suite("Error Handling", () => {
    ...
    test("should handle switchCommitMode failure gracefully", async () => {
      ...
      mockStorage.getInstalledBundle
        .withArgs(testBundleId, "repository")
        .resolves(repoBundle);
      mockRepositoryScopeService.switchCommitMode.rejects(
