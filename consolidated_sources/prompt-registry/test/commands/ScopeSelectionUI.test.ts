...
suite("ScopeSelectionUI", () => {
  ...
  let selectionChangeHandler:
    | ((selection: ScopeQuickPickItem[]) => void)
    | null = null;
  ...
  setup(() => {
    ...
    createMockQuickPick();
    mockCreateQuickPick = sandbox
      .stub(vscode.window, "createQuickPick")
      .returns(mockQuickPick as any);
  ...
  suite("createScopeQuickPickItems()", () => {
    ...
    test("should include _originalDetail for disabled items", () => {
      const items = createScopeQuickPickItems(false);
      assert.strictEqual(
        items[0]._originalDetail,
        "(Requires an open workspace)",
      );
      assert.strictEqual(
        items[1]._originalDetail,
        "(Requires an open workspace)",
      );
  ...
  suite("Dialog Options When Workspace Is Open", () => {
    ...
    test("should display exactly three options when workspace is open", async () => {
      ...
      assert.strictEqual(
        mockCreateQuickPick.callCount,
        1,
        "Should create one QuickPick dialog",
      );
      assert.strictEqual(
        mockQuickPick.items.length,
        3,
  ...
  suite("Dialog Options When No Workspace Is Open", () => {
    ...
    test("should show disabled message for repository options when no workspace", async () => {
      ...
      assert.ok(
        items[0].detail &&
          items[0].detail.includes("Requires an open workspace"),
        "Repository - Commit should show disabled message",
      );
      assert.ok(
        items[1].detail &&
          items[1].detail.includes("Requires an open workspace"),
  ...
  suite("Selection Handling", () => {
    ...
    test("should return repository scope with commit mode when first option selected", async () => {
      ...
      assert.strictEqual(
        result.scope,
        "repository",
        "Should return repository scope",
      );
      assert.strictEqual(
        result.commitMode,
        "commit",
        "Should return commit mode",
      );
    });
    ...
    test("should return user scope without commit mode when third option selected", async () => {
      ...
      assert.strictEqual(
        result.commitMode,
        undefined,
        "Should not have commit mode for user scope",
      );
    });
  ...
  suite("Disabled Option Handling (Improved UX)", () => {
    ...
    test("should show inline warning when disabled option is selected", async () => {
      ...
      const updatedItem = mockQuickPick.items.find(
        (item) => item._scope === "repository" && item._commitMode === "commit",
      );
      assert.ok(
        updatedItem?.detail?.includes("⚠️") ||
          updatedItem?.detail?.includes("Requires an open workspace"),
    ...
    test("should not accept disabled option when user presses Enter", async () => {
      ...
      assert.strictEqual(
        mockQuickPick.hide.callCount,
        0,
        "Dialog should not be hidden for disabled option",
      );
    ...
    test("should allow User Profile selection when no workspace is open", async () => {
      ...
      assert.strictEqual(
        mockQuickPick.hide.callCount,
        1,
        "Dialog should be hidden after valid selection",
      );
    });
  ...
  suite("QuickPick Configuration", () => {
    ...
    test("should include bundle name in title when provided", async () => {
      ...
      assert.ok(
        mockQuickPick.title.includes("my-bundle"),
        "Title should include bundle name",
      );
    });
    ...
    test("should set ignoreFocusOut to true", async () => {
      ...
      assert.strictEqual(
        mockQuickPick.ignoreFocusOut,
        true,
        "Should ignore focus out",
      );
    });
    ...
    test("should dispose QuickPick when dialog is hidden", async () => {
      ...
      assert.strictEqual(
        mockQuickPick.dispose.callCount,
        1,
        "Should dispose QuickPick",
      );
    });
  });
});
