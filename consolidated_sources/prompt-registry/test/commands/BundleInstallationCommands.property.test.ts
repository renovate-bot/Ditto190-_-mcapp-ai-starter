...
suite("BundleInstallationCommands - Property Tests", () => {
  ...
  const getScopeQuickPickItem = (
    ...
    return items.find(
      (item) => item._scope === scope && item._commitMode === commitMode,
    )!;
  };

  const createAutoUpdateQuickPickItem = (enabled: boolean) => ({
    label: enabled
      ? "$(sync) Enable auto-update"
      : "$(circle-slash) Manual updates only",
    description: enabled
  ...
  const setupSuccessfulInstallation = (
    ...
    mockCreateQuickPick.returns(
      createMockQuickPick("accept", scope, commitMode) as any,
    );
        ...
        fc.asyncProperty(
          ...
          async (bundleId, version, scope, autoUpdateChoice) => {
            ...
            const bundle = BundleBuilder.fromSource(bundleId, "GITHUB")
              ...
              .build();
            const commitMode =
              scope === "repository" ? ("commit" as const) : undefined;
            setupSuccessfulInstallation(
              bundleId,
              bundle,
              scope,
              autoUpdateChoice,
            ...
            assert.strictEqual(
              ...
              1,
              "Should show scope selection dialog",
            );
            ...
            assert.strictEqual(
              ...
              1,
              "Should show auto-update quick pick",
            );
            ...
            assert.ok(
              autoUpdateCall,
              "Should have auto-update quick pick call",
            );
      ...
      await fc.assert(
        fc.asyncProperty(bundleIdArb, versionArb, async (bundleId, version) => {
          ...
          assert.strictEqual(
            mockShowQuickPick.callCount,
            0,
            "Should not show auto-update dialog when user cancels scope",
          );
        ...
        fc.asyncProperty(
          ...
          async (bundleId, version, scope, autoUpdateChoice, errorMessage) => {
            ...
            mockShowQuickPick.resolves(
              createAutoUpdateQuickPickItem(autoUpdateChoice),
            );
