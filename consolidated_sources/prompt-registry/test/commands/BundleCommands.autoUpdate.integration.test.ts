...
suite("Auto-Update Preference Storage - Integration", () => {
  ...
  setup(async () => {
    ...
    sandbox
      .stub(storage as any, "loadConfig")
      .resolves({ sources: [], profiles: [] });
  ...
  suite("RegistryManager.getStorage() integration", () => {
    ...
    test("should integrate with AutoUpdateService", async () => {
      ...
      assert.strictEqual(
        isEnabled,
        true,
        "AutoUpdateService should read stored preference",
      );
    });
    ...
    test("should allow updating existing preference", async () => {
      ...
      assert.strictEqual(
        preference,
        false,
        "Updated preference should be false",
      );
    });
    ...
    test("should store preferences for multiple bundles independently", async () => {
      ...
      assert.strictEqual(
        await storageFromManager.getUpdatePreference("bundle-1"),
        true,
      );
      assert.strictEqual(
        await storageFromManager.getUpdatePreference("bundle-2"),
        false,
      );
      assert.strictEqual(
        await storageFromManager.getUpdatePreference("bundle-3"),
        true,
      );
    });
  });
  ...
  suite("Auto-Update Event Emission and Synchronization", () => {
    test("should emit onAutoUpdatePreferenceChanged event when enabling auto-update via RegistryManager", async () => {
      ...
      assert.strictEqual(
        eventData.enabled,
        true,
        "Event should indicate enabled = true",
      );
    });
    ...
    test("should emit onAutoUpdatePreferenceChanged event when disabling auto-update via RegistryManager", async () => {
      ...
      assert.strictEqual(
        eventData.enabled,
        false,
        "Event should indicate enabled = false",
      );
    });
    ...
    test("should allow multiple components to listen to auto-update changes independently", async () => {
      ...
      assert.strictEqual(
        treeViewEventFired,
        true,
        "Tree view should receive event",
      );
      assert.strictEqual(
        marketplaceEventFired,
        true,
        "Marketplace should receive event",
      );
    });
  });
});
