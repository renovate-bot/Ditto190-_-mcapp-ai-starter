...
suite("Profile Management Commands", () => {
  ...
  suite("createProfile", () => {
    ...
    test("should show expanded icon list with search keywords", async () => {
      ...
      sandbox
        .stub(profileCommands as any, "selectBundles")
        .resolves(["bundle-1"]);
      sandbox
        .stub(profileCommands as any, "generateProfileId")
        .returns("test-profile-id");
      ...
      assert.ok(
        rocketIcon.description &&
          rocketIcon.description.toLowerCase().includes("launch"),
  ...
  suite("activateProfile", () => {
    ...
    test("should deactivate other profiles", async () => {
      ...
      assert.strictEqual(
        updated.find((p) => p.id === "profile-2")?.active,
        true,
      );
    });
  ...
  suite("importProfile", () => {
    test("should prompt for profile file", async () => {
      ...
      showOpenDialogStub.resolves([
        { fsPath: "/path/to/profile.json" } as vscode.Uri,
      ]);
    ...
    test("should validate imported profile structure", async () => {
      ...
      const isValid =
        importedData.id &&
        importedData.name &&
        Array.isArray(importedData.bundles);
