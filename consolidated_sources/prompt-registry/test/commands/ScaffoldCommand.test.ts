...
suite("ScaffoldCommand", () => {
  ...
  suite("ScaffoldType Enum", () => {
    ...
    test("should only contain GitHub, Apm, and Skill types", () => {
      ...
      assert.ok(
        scaffoldTypes.includes(ScaffoldType.Apm),
        "ScaffoldType should contain Apm",
      );
      assert.ok(
    ...
    test("Apm type should have correct value", () => {
      assert.strictEqual(
        ScaffoldType.Apm,
        "apm",
        'Apm type should have value "apm"',
      );
    });
  });
  ...
  suite("InnerSource Documentation", () => {
    test("should create InnerSource documentation files", async () => {
      ...
      for (const file of innerSourceFiles) {
        ...
        assert.ok(
          ...
          `InnerSource file ${file} should be created`,
        );
      }
    });
    ...
    test("should include project name in documentation templates", async () => {
      ...
      assert.ok(
        contributingContent.includes(fixedProjectName),
      ...
      assert.ok(
        communicationContent.includes(fixedProjectName),
    ...
    test("should create comprehensive documentation with proper structure", async () => {
      ...
      for (const section of requiredSections) {
        assert.ok(
          contributingContent.includes(section),
    ...
    test("should include security best practices", async () => {
      ...
      for (const topic of securityTopics) {
        assert.ok(
          securityContent.includes(topic),
    ...
    test("should include internal use license", async () => {
      ...
      assert.ok(
        licenseContent.includes("Internal Use License"),
        "LICENSE should be Internal Use License",
      );
      assert.ok(
        licenseContent.includes("proprietary to"),
    ...
    test("should substitute organization details in LICENSE when provided", async () => {
      ...
      assert.ok(
        licenseContent.includes("Acme Corp"),
        "LICENSE should contain organization name",
      );
      assert.ok(
        licenseContent.includes("security@acme.com"),
        "LICENSE should contain internal contact",
      );
      assert.ok(
        licenseContent.includes("legal@acme.com"),
        "LICENSE should contain legal contact",
      );
      assert.ok(
        licenseContent.includes("https://acme.com/policies"),
    ...
    test("should substitute author and githubOrg in generated files", async () => {
      ...
      const communicationContent = fs.readFileSync(communicationPath, "utf8");
      assert.ok(
        communicationContent.includes("github.com/test-org/test-project"),
      ...
      const securityContent = fs.readFileSync(securityPath, "utf8");
      assert.ok(
        securityContent.includes("security@test.com"),
  ...
  suite("Collection File Validation", () => {
    ...
    test("collection items should have correct kinds", async () => {
      ...
      const promptItem = collection.items.find((item: any) =>
        item.path.includes("prompt"),
      );
      const instructionItem = collection.items.find((item: any) =>
        item.path.includes("instruction"),
      );
      const agentItem = collection.items.find((item: any) =>
  ...
  suite("Error Handling", () => {
    test("should throw error for invalid path", async () => {
      const invalidPath =
        "/invalid/path/that/does/not/exist/and/cannot/be/created/abc123xyz";
    ...
    test("should support custom project name in collection", async () => {
      await scaffoldCommand.execute(testDir, {
        projectName: "my-awesome-prompts",
      });
    ...
    test.skip("should support skipping example files", async () => {
      ...
      assert.ok(
        !fs.existsSync(path.join(testDir, "prompts", "example.prompt.md")),
      );
      assert.ok(
        !fs.existsSync(
          path.join(testDir, "instructions", "example.instructions.md"),
        ),
      );
    });
  });
  ...
  suite("Content Quality", () => {
    ...
    test("example skill should have rich structure with scripts, references, and assets", async () => {
      ...
      assert.ok(
        ...
        "SKILL.md should have name in frontmatter",
      );
      assert.ok(
        skillContent.includes("description:"),
        "SKILL.md should have description in frontmatter",
      );
      ...
      assert.ok(
        ...
        "Checklist reference should exist",
      );
      ...
      assert.ok(
        ...
        "Comment templates asset should exist",
      );
    });
    ...
    test("should create GitHub Issue and PR templates", async () => {
      ...
      const bugReportTemplate = path.join(
        testDir,
        ".github",
        "ISSUE_TEMPLATE",
        "bug_report.yml",
      ...
      assert.ok(
        fs.existsSync(bugReportTemplate),
        "Bug report template should exist",
      );

      const featureRequestTemplate = path.join(
        testDir,
        ".github",
        "ISSUE_TEMPLATE",
        "feature_request.yml",
      ...
      assert.ok(
        fs.existsSync(featureRequestTemplate),
        "Feature request template should exist",
      );

      const issueConfigTemplate = path.join(
        testDir,
        ".github",
        "ISSUE_TEMPLATE",
        "config.yml",
      ...
      assert.ok(
        fs.existsSync(issueConfigTemplate),
        "Issue config template should exist",
      );
...
suite("Skill Scaffold", () => {
  ...
  setup(() => {
    ...
    const skillTemplateRoot = path.join(
      process.cwd(),
      "templates/scaffolds/skill",
    );
    skillScaffoldCommand = new ScaffoldCommand(
      skillTemplateRoot,
      ScaffoldType.Skill,
    );
  });
  ...
  test("should create example script", async () => {
    ...
    assert.ok(
      content.includes("scripted-skill"),
      "Should reference skill name",
    );
    assert.ok(
