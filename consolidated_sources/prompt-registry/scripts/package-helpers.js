...
function showStatus() {
  ...
  const devContent = fileExists(DEV_IGNORE_FILE)
    ? fs.readFileSync(DEV_IGNORE_FILE, "utf8")
    : "";
  const prodContent = fileExists(PROD_IGNORE_FILE)
    ? fs.readFileSync(PROD_IGNORE_FILE, "utf8")
    : "";
  ...
  console.log(
    `   - Development: ${fileExists(DEV_IGNORE_FILE) ? "✅" : "❌"} ${DEV_IGNORE_FILE}`,
  );
  console.log(
    `   - Production:  ${fileExists(PROD_IGNORE_FILE) ? "✅" : "❌"} ${PROD_IGNORE_FILE}`,
  );
}
...
switch (command) {
  ...
  default:
    ...
    console.log(
      "  prod, production  - Switch to production .vscodeignore for packaging",
    );
    console.log(
      "  dev, development  - Switch to development .vscodeignore for coding",
    );
    console.log(
      "  restore          - Restore from backup (.vscodeignore.backup)",
    );
