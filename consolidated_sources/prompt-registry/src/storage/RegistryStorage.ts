...
export class RegistryStorage {
  ...
  async removeInstallation(
    ...
    const scopePath =
      scope === "user" ? this.paths.userInstalled : this.paths.installed;
    const sanitizedId = this.sanitizeFilename(bundleId);
  ...
  async getInstalledBundles(
    ...
    for (const s of scopes) {
      const scopePath =
        s === "user" ? this.paths.userInstalled : this.paths.installed;
   ...
   */
  private getSupportedScopes(
    scope?: InstallationScope,
  ): Array<"user" | "workspace"> {
    // Repository scope bundles are tracked via LockfileManager, not RegistryStorage
    if (scope === "repository") {
      return [];
    }
  ...
  private getInstalledBundlePath(bundle: InstalledBundle): string {
    const scopePath =
      bundle.scope === "user" ? this.paths.userInstalled : this.paths.installed;
