...
export abstract class CliWrapper {
  ...
  async isAvailable(): Promise<boolean> {
    return new Promise((resolve) => {
      const proc = spawn(this.getCommandName(), ["--version"], {
        shell: USE_SHELL,
      });
      ...
      vscode.window.withProgress(
        ...
        async (_progress, token) => {
          ...
          proc.on("error", (err) => {
            ...
            this.logger.error(
              `${cmdName} install process error: ${errorMessage}`,
            );
  ...
  async installInTerminal(cwd: string): Promise<CliInstallResult> {
    ...
    try {
      ...
      this.logger.error(
        `Failed to create terminal for ${cmdName} install: ${errorMessage}`,
      );
      vscode.window.showErrorMessage(
        `Failed to run ${cmdName} install: ${errorMessage}`,
      );
      return { success: false, error: errorMessage };
    }
  }
