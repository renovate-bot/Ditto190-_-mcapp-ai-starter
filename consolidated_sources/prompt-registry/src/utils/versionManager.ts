...
export class VersionManager {
  ...
  static compareVersions(v1: string, v2: string): number {
    ...
    if (
      v1.length > this.MAX_VERSION_LENGTH ||
      v2.length > this.MAX_VERSION_LENGTH
    ) {
      throw new Error(
        `Version string exceeds maximum length of ${this.MAX_VERSION_LENGTH}`,
      );
    }
    ...
    this.logger.warn(
      `Falling back to string comparison for invalid semver: "${v1}", "${v2}"`,
    );
  ...
  static isUpdateAvailable(
    ...
    if (
      this.isContentHashVersion(installedVersion) ||
      this.isContentHashVersion(latestVersion)
    ) {
    ...
    const cleanLatest =
      semver.clean(latestVersion) || semver.coerce(latestVersion)?.version;
  ...
  static extractBundleIdentity(
    ...
    if (bundleId.length > this.MAX_BUNDLE_ID_LENGTH) {
      throw new Error(
        `Bundle ID exceeds maximum length of ${this.MAX_BUNDLE_ID_LENGTH}`,
      );
    }
  ...
  static parseVersion(version: string): string | null {
    ...
    if (coerced) {
      this.logger.debug(
        `Coerced version: "${version}" -> "${coerced.version}"`,
      );
