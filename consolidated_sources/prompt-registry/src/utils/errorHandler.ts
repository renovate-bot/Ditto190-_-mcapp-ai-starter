 ...
 */
export interface ErrorHandlingOptionsWithFallback<
  T,
...
export class ErrorHandler {
   ...
   */
  static async handleCategorized(
    error: unknown,
    options: ErrorHandlingOptions,
  ): Promise<void> {
    const errorObj = toError(error);
    ...
    const enhancedOptions = {
      ...options,
      userMessagePrefix:
        options.userMessagePrefix || this.getUserMessage(errorObj, category),
    };
...
export class UpdateCancelledError extends RegistryError {
  constructor(
    ...
    public readonly reason:
      | typeof WARNING_RESULTS.CONTRIBUTE
      | typeof WARNING_RESULTS.CANCEL,
  ) {
    const message =
      reason === "contribute"
