...
export async function getInstalledBundleForScope(
  ...
  if (scope === "repository") {
    ...
    try {
      ...
      logger.warn(
        `[bundleScopeUtils] Failed to read lockfile for bundle ${bundleId}:`,
...
export function createInstalledBundleFromLockfile(
  ...
  const manifest =
    options?.manifest ?? createMinimalManifest(bundleId, bundleEntry.files);
  ...
  const commitMode =
    options?.commitModeOverride ??
    (bundleEntry.commitMode as RepositoryCommitMode);
