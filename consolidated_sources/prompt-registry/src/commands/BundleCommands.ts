...
export class BundleCommands {
  ...
  constructor(registryManager: RegistryManager) {
    ...
    this.updateCommands = new BundleUpdateCommands(
      registryManager,
      bundleNotifications,
    );
  ...
  async cleanupStaleLockfileEntries(): Promise<void> {
    ...
    if (!workspaceRoot) {
      vscode.window.showWarningMessage(
        "No workspace open. Please open a workspace to clean up stale repository bundles.",
      );
      return;
    }
    ...
    try {
      ...
      if (staleBundles.length === 0) {
        vscode.window.showInformationMessage(
          "No stale repository bundle entries found.",
        );
        return;
      }
      ...
      for (const bundle of staleBundles) {
        try {
          ...
          this.logger.error(
            `Failed to remove stale entry ${bundle.bundleId}:`,
      ...
      if (removedCount === staleBundles.length) {
        vscode.window.showInformationMessage(
          `Successfully removed ${removedCount} stale bundle entries from the lockfile.`,
        );
      } else {
        vscode.window.showWarningMessage(
          `Removed ${removedCount} of ${staleBundles.length} stale entries. Some entries could not be removed.`,
        );
      }
    } catch (error) {
      this.logger.error(
        "Failed to clean up stale lockfile entries:",
        error instanceof Error ? error : undefined,
      );
      vscode.window.showErrorMessage(
        `Failed to clean up stale entries: ${error instanceof Error ? error.message : String(error)}`,
      );
    }
  }
}
