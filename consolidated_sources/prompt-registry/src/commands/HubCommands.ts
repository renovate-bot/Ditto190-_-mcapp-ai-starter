...
export class HubCommands {
  ...
  registerCommands(): void {
    ...
    this.context.subscriptions.push(
      vscode.commands.registerCommand("promptregistry.importHub", () =>
        this.importHub(),
      ),
      vscode.commands.registerCommand("promptregistry.listHubs", () =>
        this.listHubs(),
      ),
      vscode.commands.registerCommand(
        "promptregistry.syncHub",
        (hubId?: string) => this.syncHub(hubId),
      ),
      vscode.commands.registerCommand(
        "promptregistry.deleteHub",
        (hubId?: string) => this.deleteHub(hubId),
      ),
      vscode.commands.registerCommand("promptregistry.switchHub", () =>
        this.switchHub(),
      ),
      ...
      vscode.commands.registerCommand(
        ...
        (item: any) => this.openItemRepository(item),
      ),
    );
  }
    ...
    try {
      ...
      await vscode.window.withProgress(
        ...
        async () => {
          ...
          vscode.window.showInformationMessage(
            `Successfully synced hub: ${targetHubId}`,
          );
        },
      );
    } catch (error) {
  ...
  async switchHub(): Promise<void> {
    try {
      ...
      const items: (vscode.QuickPickItem & {
        ...
        label: hub.name,
        description:
          hub.id === currentActiveId ? "$(check) Active" : hub.description,
        detail: `ID: ${hub.id}`,
        hubId: hub.id,
      }));
      ...
      if (selected.hubId) {
        ...
        vscode.window.showInformationMessage(
          `✅ Switched to hub: ${selected.label}`,
        );
        vscode.commands.executeCommand("promptRegistry.refresh");
      }
    } catch (error) {
  ...
  async exportHubConfig(): Promise<void> {
    try {
      ...
      const hubName = await vscode.window.showInputBox({
        ...
        validateInput: (value) =>
          value.trim() ? null : "Hub name is required",
        ignoreFocusOut: true,
      });
      if (!hubName) {
        return;
      ...
      const hubConfig: any = {
        ...
        profiles: profiles.map((p) => ({
          ...
          bundles: (Array.isArray(p.bundles)
            ? p.bundles
            : (Object.values(p.bundles || {}) as any[])
          ).map((bundle: any) => ({
            id: bundle.id,
   ...
   */
  private referenceToUrl(reference: {
    type: string;
    location: string;
  }): string | undefined {
    switch (reference.type) {
  ...
  /**
     ...
     */
  private async selectSourceType(): Promise<
    "github" | "url" | "local" | undefined
  > {
