...
export class InstallSpecificVersionCommand {
  ...
  public static getInstance(): InstallSpecificVersionCommand {
    if (!InstallSpecificVersionCommand.instance) {
      InstallSpecificVersionCommand.instance =
        new InstallSpecificVersionCommand();
    }
    ...
    try {
      ...
      if (versions.length === 0) {
        vscode.window.showInformationMessage(
          "No versions available for installation.",
        );
        return;
      }
      ...
      this.logger.error(
        "Install specific version command failed",
        error as Error,
      );
    ...
    await vscode.window.withProgress(
      ...
      async (progress) => {
        try {
          ...
          const release =
            await this.githubService.getReleaseByVersionPreference(version);
          ...
          const result = await this.installationManager.installBundle(
            ...
            (installProgress: number, message: string) => {
              progress.report({
                increment: 0,
                message:
                  message || `Installing... ${Math.round(installProgress)}%`,
              });
            },
          );
