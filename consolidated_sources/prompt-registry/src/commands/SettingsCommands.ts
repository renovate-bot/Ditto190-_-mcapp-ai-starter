...
export class SettingsCommands {
  ...
  async exportSettings(): Promise<void> {
    try {
      ...
      const data = await this.registryManager.exportSettings(
        formatChoice.value,
      );
  ...
  async importSettings(): Promise<void> {
    try {
      ...
      const format: ExportFormat = fileUri.fsPath.endsWith(".json")
        ? "json"
        : "yaml";
      this.logger.info(
        ...
        [
          {
            label: "Merge",
            description:
              "Add imported items to existing settings (recommended)",
          ...
          },
          {
            ...
            detail:
              "⚠️  Warning: All current sources, profiles, and settings will be lost",
