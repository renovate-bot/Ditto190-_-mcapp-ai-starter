...
export class AddResourceCommand {
  ...
  async execute(): Promise<void> {
    try {
      ...
      const fileName =
        generateSanitizedId(resourceName) + resourceInfo.extension;
  ...
  private async promptForDescription(): Promise<string | undefined> {
    return await vscode.window.showInputBox({
      ...
      placeHolder:
        "e.g., Helps review code for best practices and potential issues",
      ignoreFocusOut: true,
      validateInput: (value) => {
        if (!value || value.trim().length === 0) {
  ...
  private async addToCollection(
    ...
    try {
      ...
      const entries =
        await vscode.workspace.fs.readDirectory(collectionsDirUri);
      ...
      const collectionUri = vscode.Uri.file(
        path.join(collectionsDir, selected.label),
      );
      const collectionContentBytes =
        await vscode.workspace.fs.readFile(collectionUri);
      const collectionContent = Buffer.from(collectionContentBytes).toString(
        "utf-8",
      );
      ...
      vscode.window.showInformationMessage(
        `✓ Added resource to ${selected.label}`,
      );
    } catch (error) {
      vscode.window.showErrorMessage(
