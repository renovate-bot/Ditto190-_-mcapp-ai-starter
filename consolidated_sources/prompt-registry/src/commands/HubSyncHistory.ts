...
export class HubSyncHistory {
  ...
  createHistoryQuickPickItems(
    ...
    return entries.map((entry) => {
      ...
      const description =
        changeParts.length > 0 ? changeParts.join(", ") : "No changes";
  ...
  async rollbackToEntry(
    ...
    const currentState = await this.hubManager[
      "storage"
    ].getProfileActivationState(hubId, profileId);
    if (!currentState) {
    ...
    const currentProfile = hub.config.profiles.find(
    ...
    const currentBundleIds = new Set(
      currentBundles.map((b: HubProfileBundle) => b.id),
    );
    ...
    const rollbackChanges = {
      ...
      updated: entry.previousState.bundles
        .filter((b: HubProfileBundle) => {
          const current = currentBundles.find(
            (cb: HubProfileBundle) => cb.id === b.id,
          );
    ...
    const newState = {
      ...
      syncedBundles: entry.previousState.bundles.map(
        (b: HubProfileBundle) => b.id,
      ),
    };

    await this.hubManager["storage"].saveProfileActivationState(
      hubId,
      profileId,
      newState,
    );
