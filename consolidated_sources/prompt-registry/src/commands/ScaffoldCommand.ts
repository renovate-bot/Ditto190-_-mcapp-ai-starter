...
export class ScaffoldCommand {
    ...
    try {
      const targetUri =
        typeof targetPath === "string"
          ...
          : targetPath;
      this.logger.info(
        `Scaffolding ${this.scaffoldType} structure at: ${targetUri.fsPath}`,
      );
      ...
      const context: TemplateContext = {
        ...
        collectionId: generateSanitizedId(
          options?.projectName || projectDirName,
        ),
  ...
  private validateScaffoldTargetPath(targetPath: string): void {
    ...
    const workspaceRoots = (vscode.workspace.workspaceFolders || []).map(
      (folder) => path.resolve(folder.uri.fsPath),
    );
    const allowedRoots = [
    ...
    if (!isUnderAllowedRoot) {
      throw new Error(
        `Cannot create directory outside allowed locations: ${resolved}`,
      );
    }
    ...
    if (path.isAbsolute(resolved) && process.platform !== "win32") {
      ...
      if (parts.length > 0) {
        ...
        if (!fs.existsSync(topLevel)) {
          throw new Error(
            `Cannot create directory: top-level path does not exist (${topLevel})`,
          );
        }
      }
    }
   ...
   */
  static async detectMigrationScenario(
    targetPath: string,
    ...
    switch (scenario) {
      case MigrationScenario.MissingWorkflow:
        return {
          scenario,
          message:
            "This project has collections but no GitHub publish workflow.",
          documentationUrl:
  ...
  static async showMigrationRecommendation(
    ...
    if (action === "View Migration Guide") {
      await vscode.env.openExternal(
        vscode.Uri.parse(recommendation.documentationUrl),
      );
    }
  }
  ...
  static async runWithUI(): Promise<void> {
    ...
    try {
      ...
      const targetPath = await ScaffoldCommand.promptForTargetDirectory(
        scaffoldType.label,
      );
      if (!targetPath) {
        return;
      ...
      const options = await ScaffoldCommand.promptForProjectDetails(
        scaffoldType.value,
      );
      if (!options) {
        return;
      ...
      await ScaffoldCommand.handlePostScaffoldActions(
        scaffoldType.label,
        targetPath,
      );
    } catch (error) {
      ...
      vscode.window.showErrorMessage(
        `Scaffold failed: ${(error as Error).message}`,
      );
    }
  }
