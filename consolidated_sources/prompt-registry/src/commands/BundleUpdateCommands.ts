...
export class BundleUpdateCommands {
  ...
  async enableAutoUpdate(bundleId?: string): Promise<void> {
    await this.withErrorHandling(async () => {
      ...
      const currentStatus =
        await this.registryManager.isAutoUpdateEnabled(bundleId);
      ...
      vscode.window.showInformationMessage(
        `✅ Auto-update enabled for ${bundleId}`,
      );
    }, "enable auto-update");
  }
  ...
  async disableAutoUpdate(bundleId?: string): Promise<void> {
    await this.withErrorHandling(async () => {
      ...
      vscode.window.showInformationMessage(
        `✅ Auto-update disabled for ${bundleId}`,
      );
    }, "disable auto-update");
  }
    ...
    return await Promise.allSettled(
      batch.map(async (update) => {
        try {
          ...
          await this.registryManager.updateBundle(
            update.bundleId,
