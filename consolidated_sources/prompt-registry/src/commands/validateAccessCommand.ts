    ...
    try {
      ...
      await vscode.window.withProgress(
        ...
        async (progress) => {
          ...
          const owner =
            config.get<string>("repositoryOwner") || "AmadeusITGroup";
          const repo = config.get<string>("repositoryName") || "olaf";
          const usePrivateRepo =
            config.get<boolean>("usePrivateRepository") || false;
          const hasToken = !!config.get<string>("githubToken");

          progress.report({
            increment: 30,
            message: "Testing repository access...",
          });
      ...
      vscode.window.showErrorMessage(
        `Failed to validate GitHub access: ${error}`,
      );
    }
  }
