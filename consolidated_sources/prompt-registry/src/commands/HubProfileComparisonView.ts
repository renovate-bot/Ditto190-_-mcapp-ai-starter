...
export class HubProfileComparisonView {
  ...
  async getProfileComparisonData(
    ...
    const hasChanges = await this.hubManager.hasProfileChanges(
      hubId,
      profileId,
    );
    if (!hasChanges) {
    ...
    const availableProfile = hub.config.profiles.find(
      (p) => p.id === profileId,
    );
    if (!availableProfile) {
      return null;
    ...
    const updatedBundles: BundleUpdate[] = (changes.bundlesUpdated || []).map(
      (update) => ({
        id: update.id,
  ...
  formatBundleComparison(
    ...
    const bundleVersion =
      typeof bundle === "string" ? "unknown" : bundle.version;
  ...
  generateComparisonSummary(comparison: ProfileComparisonData): string {
    ...
    if (comparison.metadataChanged && comparison.availableMetadata) {
      ...
      if (comparison.availableMetadata.description) {
        lines.push(
          `  Description: ${comparison.availableMetadata.description}`,
        );
      }
      lines.push("");
    }
  ...
  createComparisonQuickPickItems(
    ...
    const unchangedBundles = comparison.currentBundles.filter((current) => {
      ...
      const isUpdated = comparison.updatedBundles.some(
        (u) => u.id === current.id,
      );
  ...
  getSideBySideComparison(comparison: ProfileComparisonData): string {
    ...
    // Display each bundle
    Array.from(allBundleIds)
      .sort()
      .forEach((bundleId) => {
        const current = comparison.currentBundles.find(
          (b) => b.id === bundleId,
        );
        const available = comparison.availableBundles.find(
          (b) => b.id === bundleId,
        );
