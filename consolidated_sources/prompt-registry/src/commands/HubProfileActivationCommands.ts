 ...
 */
export async function activateHubProfile(
  ...
  try {
    ...
    if (!activeHubResult) {
      vscode.window.showWarningMessage(
        "Failed to load active hub configuration.",
      );
      return;
    }
    ...
    if (profiles.length === 0) {
      vscode.window.showWarningMessage(
        `No profiles found in active hub: ${activeHubName}`,
      );
      return;
    }
    ...
    await hubManager.activateProfile(
    ...
    vscode.window.showInformationMessage(
      `Activated profile: ${selectedProfile.profileId}`,
    );
  } catch (error) {
    const errorMessage = error instanceof Error ? error.message : String(error);
    vscode.window.showErrorMessage(
      `Failed to activate profile: ${errorMessage}`,
    );
  }
}

export async function deactivateHubProfile(
  hubManager: HubManager,
  item?: any,
): Promise<void> {
  try {
    ...
    const profile = await hubManager.getHubProfile(
      activeHubId,
    ...
    if (!profile) {
      vscode.window.showErrorMessage(
        "Active profile not found in hub configuration.",
      );
      return;
    }
...
export async function showActiveProfiles(
  hubManager: HubManager,
): Promise<void> {
  try {
    // Get the active hub ID first
    ...
    if (!activeProfile) {
      vscode.window.showInformationMessage(
        `No active profile in ${activeHubName}.`,
      );
      return;
    }
