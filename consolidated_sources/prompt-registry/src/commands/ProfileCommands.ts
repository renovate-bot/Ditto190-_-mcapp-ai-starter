...
const PROFILE_ICONS = [
  ...
  { icon: "🚀", label: "Rocket", tags: "launch, deploy, speed, fast, startup" },
  {
    icon: "📦",
    label: "Package",
    tags: "bundle, build, delivery, box, container",
  },
  {
    icon: "💻",
    label: "Computer",
    tags: "code, dev, programming, tech, laptop, work",
  },
  ...
  { icon: "�", label: "Plug", tags: "api, connect, integration, power" },
  {
    icon: "📡",
    label: "Satellite",
    tags: "communication, signal, remote, broadcast",
  },
  ...
  { icon: "🔨", label: "Hammer", tags: "build, construct, fix" },
  {
    icon: "🛠️",
    label: "Tools",
    tags: "utility, settings, fix, repair, maintenance",
  },
  ...
  { icon: "🤖", label: "Robot", tags: "bot, ai, automation, smart, android" },
  {
    icon: "🧠",
    label: "Brain",
    tags: "intelligence, smart, logic, think, ai, ml",
  },
  ...
  { icon: "🔑", label: "Key", tags: "lock, access, secret, auth, password" },
  {
    icon: "🛡️",
    label: "Shield",
    tags: "security, protect, guard, safe, firewall",
  },
  ...
  // Business, Product & Analytics
  {
    icon: "📊",
    label: "Chart",
    tags: "data, analytics, stats, graph, report, bi",
  },
  {
    icon: "📈",
    label: "Chart Up",
    tags: "growth, profit, success, trend, increase",
  },
  { icon: "📉", label: "Chart Down", tags: "loss, decrease, trend, drop" },
  {
    icon: "📋",
    label: "Clipboard",
    tags: "plan, checklist, task, todo, audit",
  },
  ...
  { icon: "📌", label: "Pushpin", tags: "pin, sticky, note, location" },
  {
    icon: "🎨",
    label: "Palette",
    tags: "design, art, creative, ui, ux, color",
  },
  {
    icon: "�",
    label: "Bulb",
    tags: "idea, solution, light, think, innovation",
  },
  {
    icon: "📢",
    label: "Megaphone",
    tags: "announce, shout, news, marketing, promo",
  },
  {
    icon: "💰",
    label: "Money",
    tags: "finance, cash, dollar, price, cost, budget",
  },
  ...
  { icon: "🏢", label: "Office", tags: "building, work, company, enterprise" },
  {
    icon: "🏗️",
    label: "Building",
    tags: "architecture, construct, wip, structure",
  },
  ...
  // Science & QA
  {
    icon: "🔬",
    label: "Microscope",
    tags: "science, research, test, analysis, lab",
  },
  { icon: "🧪", label: "Test Tube", tags: "experiment, chemistry, lab, test" },
  {
    icon: "🌡️",
    label: "Thermometer",
    tags: "temperature, measure, heat, cold",
  },
  {
    icon: "🎯",
    label: "Target",
    tags: "goal, objective, focus, aim, accuracy",
  },
  ...
  { icon: "⚡", label: "Zap", tags: "power, energy, instant, fast, electric" },
  {
    icon: "🌟",
    label: "Star",
    tags: "favorite, special, featured, top, rating",
  },
  { icon: "🔥", label: "Fire", tags: "hot, trending, urgent, burn" },
  {
    icon: "🎓",
    label: "Cap",
    tags: "education, school, student, learn, degree",
  },
...
export class ProfileCommands {
  ...
  async editProfile(profileId?: string | any): Promise<void> {
    try {
      ...
      vscode.window.showErrorMessage(
        `Failed to edit profile: ${(error as Error).message}`,
      );
    }
  }
  ...
  async activateProfile(profileId?: string | any): Promise<void> {
    try {
      ...
      if (profileId && typeof profileId === "object" && "data" in profileId) {
        targetProfileId = profileId.data.id;
        this.logger.info(
          `Activating profile from tree item: ${targetProfileId}`,
        );
      }
  ...
  async deactivateProfile(profileIdOrItem?: string | any): Promise<void> {
    try {
      ...
      if (
        ...
        this.logger.info(
          `Deactivating profile from tree item: ${targetProfileId}`,
        );
      }
      // Handle direct profile ID
      ...
      // No profile specified - show picker
        ...
        if (activeProfiles.length === 0) {
          vscode.window.showInformationMessage(
            "No active profiles to deactivate",
          );
          return;
        }
    ...
    try {
      ...
      const uri = await vscode.window.showSaveDialog({
        defaultUri: vscode.Uri.file(
          `${generateSanitizedId(profile.name)}.json`,
        ),
        filters: {
          "JSON Files": ["json"],
          "All Files": ["*"],
      ...
      vscode.window.showErrorMessage(
        `Failed to export profile: ${(error as Error).message}`,
      );
    }
  }
  ...
  async importProfile(): Promise<void> {
    try {
      ...
      vscode.window.showErrorMessage(
        `Failed to import profile: ${(error as Error).message}`,
      );
    }
  }
    ...
    try {
      ...
      if (allBundles.length === 0) {
        vscode.window.showWarningMessage(
          "No bundles available. Add a source first.",
        );
        return [];
      }
    ...
    if (action.value === "add") {
      ...
      if (newBundles.length > 0) {
        ...
        if (profile) {
          ...
          vscode.window.showInformationMessage(
            `Added ${newBundles.length} bundle(s)`,
          );
        }
      }
    } else {
      ...
      if (profile && profile.bundles.length > 0) {
        ...
        if (toRemove && toRemove.length > 0) {
          ...
          const updatedBundles = profile.bundles.filter(
          ...
          vscode.window.showInformationMessage(
            `Removed ${toRemove.length} bundle(s)`,
          );
        }
      }
    }
