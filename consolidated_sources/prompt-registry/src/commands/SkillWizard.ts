...
export class SkillWizard {
  ...
  isAwesomeCopilotProject(workspaceRoot: string): boolean {
    ...
    if (hasCollections) {
      ...
      const hasCollectionFiles = files.some((f) =>
        f.endsWith(".collection.yml"),
      );
...
`;
  ...
  async addSkillToCollection(
    ...
    const collection = yaml.load(content) as {
    ...
    this.logger.info(
      `Added skill ${skillName} to collection ${path.basename(collectionPath)}`,
    );
  }
