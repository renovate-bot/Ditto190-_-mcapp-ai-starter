...
export class BundleScopeCommands {
   ...
   */
  private async getBundleForScope(
    bundleId: string,
    scope: InstallationScope,
  ): Promise<InstalledBundle | undefined> {
    return getInstalledBundleForScope(
   ...
   */
  async moveToRepository(
    bundleId: string,
    commitMode: RepositoryCommitMode,
  ): Promise<void> {
    try {
      this.logger.debug(
        `[BundleScopeCommands] Moving bundle ${bundleId} to repository scope (${commitMode})`,
      );

      // Check if workspace is open
      if (!hasOpenWorkspace()) {
        vscode.window.showErrorMessage(
          "Cannot move to repository scope: No workspace is open.",
        );
        return;
      }
      ...
      if (!userBundle) {
        vscode.window.showErrorMessage(
          `Bundle "${bundleId}" is not installed at user scope.`,
        );
        return;
      }
      ...
      const result = await this.scopeConflictResolver.migrateBundle(
        ...
        async (
          _installedBundle: InstalledBundle,
  ...
  async moveToUser(bundleId: string): Promise<void> {
    try {
      this.logger.debug(
        `[BundleScopeCommands] Moving bundle ${bundleId} to user scope`,
      );
      ...
      if (!repoBundle) {
        vscode.window.showErrorMessage(
          `Bundle "${bundleId}" is not installed at repository scope.`,
        );
        return;
      }
  ...
  async switchCommitMode(
    ...
    try {
      this.logger.debug(
        `[BundleScopeCommands] Switching commit mode for ${bundleId} to ${newMode}`,
      );
      ...
      if (currentMode === newMode) {
        vscode.window.showInformationMessage(
          `Bundle is already in ${newMode} mode.`,
        );
        return;
      }
