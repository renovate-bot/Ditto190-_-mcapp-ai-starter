...
export class HubProfileCommands {
  ...
  private registerCommands(context: vscode.ExtensionContext): void {
    context.subscriptions.push(
      ...
      vscode.commands.registerCommand(
        ...
        (hubId: string, profileId: string) =>
          this.viewHubProfile(hubId, profileId),
      ),
      vscode.commands.registerCommand(
        "promptRegistry.toggleProfileFavorite",
        (arg: any) => this.toggleProfileFavorite(arg),
      ),
    );
  }
  ...
  async toggleProfileFavorite(arg: any): Promise<void> {
    try {
      ...
      this.logger.info(
        `Toggling favorite for profile ${profileId} in hub ${hubId}`,
      );
      await this.hubManager.toggleProfileFavorite(hubId, profileId);
      const isFavorite = await this.hubManager.isProfileFavorite(
        hubId,
        profileId,
      );
  ...
  async browseHubProfiles(): Promise<void> {
    try {
      ...
      const profiles = await this.hubManager.listProfilesFromHub(
        selectedHub.hub.id,
      );
  ...
  async viewHubProfile(hubId: string, profileId: string): Promise<void> {
    try {
      ...
      vscode.window.showErrorMessage(
        `Failed to view profile: ${(error as Error).message}`,
      );
    }
  }
...
<body>
  ...
  private async copyProfileToLocal(
    ...
    try {
      ...
      vscode.window.showErrorMessage(
        `Failed to copy profile: ${(error as Error).message}`,
      );
    }
  }
}
