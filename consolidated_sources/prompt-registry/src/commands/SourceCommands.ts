...
export class SourceCommands {
    ...
    try {
      ...
      if (sourceType.value === "awesome-copilot") {
        ...
        const collectionsPath = await vscode.window.showInputBox({
          prompt:
            'Enter collections directory path (or press Enter for "collections")',
      ...
      vscode.window.showErrorMessage(
        `Failed to add source: ${(error as Error).message}`,
      );
    }
  }
  ...
  async editSource(sourceId?: string | any): Promise<void> {
    try {
      ...
      if (!extractedId) {
        ...
        if (sources.length === 0) {
          vscode.window.showInformationMessage(
            "No sources found. Add one first.",
          );
          return;
        }
      ...
      vscode.window.showErrorMessage(
        `Failed to edit source: ${(error as Error).message}`,
      );
    }
  }
  ...
  async removeSource(sourceId?: string | any): Promise<void> {
    try {
      ...
      vscode.window.showErrorMessage(
        `Failed to remove source: ${(error as Error).message}`,
      );
    }
  }
  ...
  async syncSource(sourceId?: string | any): Promise<void> {
    try {
      ...
      vscode.window.showErrorMessage(
        `Failed to sync source: ${(error as Error).message}`,
      );
    }
  }
  ...
  async syncAllSources(options?: { silent?: boolean }): Promise<void> {
    ...
    try {
        ...
        for (let i = 0; i < enabledSources.length; i += concurrency) {
          ...
          for (let j = 0; j < results.length; j++) {
            ...
            if (result.status === "fulfilled") {
              ...
              failedCount++;
              this.logger.warn(
                `Failed to sync source "${batch[j].name}"`,
        ...
        await vscode.window.withProgress(
          ...
          async (progress) => {
            for (let i = 0; i < enabledSources.length; i += concurrency) {
              ...
              const results = await Promise.allSettled(
                batch.map((source) =>
                  this.registryManager.syncSource(source.id),
                ),
              );
      ...
      if (!silent) {
        vscode.window.showErrorMessage(
          `Failed to sync sources: ${(error as Error).message}`,
        );
      }
    }
  }
  ...
  async listSources(): Promise<void> {
    try {
          ...
          [
            {
              label: "$(sync) Sync",
              value: "sync",
              enabled: selected.source.enabled,
            },
      ...
      vscode.window.showErrorMessage(
        `Failed to list sources: ${(error as Error).message}`,
      );
    }
  }
  ...
  private async getSourceUrl(type: SourceType): Promise<string | undefined> {
    switch (type) {
      ...
      case "awesome-copilot":
        return await vscode.window.showInputBox({
          prompt:
            "Enter GitHub repository URL (or press Enter for official awesome-copilot)",
      ...
      case "skills":
        return await vscode.window.showInputBox({
          prompt:
            "Enter GitHub repository URL containing skills (e.g., anthropics/skills)",
  ...
  async toggleSource(sourceId?: string | any): Promise<void> {
    try {
      ...
      await this.registryManager.updateSource(finalId, {
      ...
      vscode.window.showErrorMessage(
        `Failed to toggle source: ${(error as Error).message}`,
      );
    }
  }
