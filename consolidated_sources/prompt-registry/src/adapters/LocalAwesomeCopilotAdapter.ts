...
export class LocalAwesomeCopilotAdapter extends RepositoryAdapter {
  ...
  constructor(source: RegistrySource) {
    ...
    this.logger.info(
      `LocalAwesomeCopilotAdapter initialized for: ${source.url}`,
    );
  }
  ...
  private async listCollectionFiles(): Promise<string[]> {
    ...
    if (!exists) {
      throw new Error(
        `Collections directory does not exist: ${collectionsPath}`,
      );
    }
    ...
    return entries
      .filter(
        (entry) => entry.isFile() && entry.name.endsWith(".collection.yml"),
      )
