...
export class GitHubAdapter extends RepositoryAdapter {
  ...
  private async getAuthenticationToken(): Promise<string | undefined> {
    ...
    if (this.authToken !== undefined) {
      this.logger.debug(
        `[GitHubAdapter] Using cached token (method: ${this.authMethod})`,
      );
    ...
    if (this.authPromise) {
      this.logger.debug(
        "[GitHubAdapter] Authentication in progress, waiting...",
      );
  ...
  private async performAuthentication(): Promise<string | undefined> {
    ...
    if (!this.attemptedMethods.has("explicit")) {
      ...
      if (explicitToken && explicitToken.trim().length > 0) {
        ...
        this.logger.debug(
          `[GitHubAdapter] Token preview: ${token.substring(0, 8)}...`,
        );
        return token;
      }
      ...
      this.logger.debug(
        "[GitHubAdapter] Skipping explicit token (already attempted)",
      );
    }
    ...
    if (!this.attemptedMethods.has("vscode")) {
      try {
        this.logger.debug(
          "[GitHubAdapter] Trying VSCode GitHub authentication...",
        );
      ...
      this.logger.debug(
        "[GitHubAdapter] Skipping VSCode auth (already attempted)",
      );
    }
    ...
    if (this.attemptedMethods.size > 0) {
      this.logger.error(
        "[GitHubAdapter] ✗ All authentication methods exhausted",
      );
      this.logger.error(
        `[GitHubAdapter] Attempted methods: ${Array.from(this.attemptedMethods).join(", ")}`,
      );
  ...
  private validateResponse(
    ...
    if (
      ...
      this.logger.warn(
        `[GitHubAdapter] Unexpected Content-Type: ${contentType}`,
      );
      return {
        isValid: false,
  ...
  private async makeRequest(
    ...
    if (redirectDepth >= MAX_REDIRECTS) {
      this.logger.error(
        `[GitHubAdapter] Maximum redirect depth (${MAX_REDIRECTS}) exceeded`,
      );
    ...
    this.logger.debug(
      `[GitHubAdapter] Request headers: ${JSON.stringify(sanitizedHeaders)}`,
    );
        ...
        .get(url, { headers }, (res) => {
          ...
          if (res.statusCode === 301 || res.statusCode === 302) {
            ...
            if (redirectUrl) {
              ...
              this.makeRequest(redirectUrl, retryCount, redirectDepth + 1)
                .then(resolve)
                .catch(reject);
              return;
            }
          ...
          res.on("end", async () => {
            if (res.statusCode && res.statusCode >= 400) {
              ...
              this.logger.error(
                `[GitHubAdapter] Auth method: ${this.authMethod}`,
              );
              this.logger.error(
            ...
            this.logger.debug(
              `[GitHubAdapter] Response OK (${res.statusCode})`,
            );
            try {
              resolve(JSON.parse(data));
            } catch (error) {
              this.logger.error(
                `[GitHubAdapter] Failed to parse response: ${error}`,
              );
              this.logger.error(
                `[GitHubAdapter] Response preview: ${data.substring(0, 200)}`,
              );
              reject(
                new Error(`Failed to parse GitHub response as JSON: ${error}`),
              );
            }
          });
        })
    ...
    try {
      ...
      for (let i = 0; i < validReleases.length; i += concurrency) {
        ...
        const batchResults = await Promise.allSettled(
          batch.map((release) =>
            this.processSingleRelease(release, owner, repo),
          ),
        );
  ...
  private async processSingleRelease(
    ...
    try {
      manifest = await this.fetchManifestWithCache(
        manifestAsset.url,
        manifestAsset.name,
      );
    } catch (manifestError) {
    ...
    const bundle: Bundle = {
      ...
      description:
        manifest?.description || this.extractDescription(release.body),
      author: manifest?.author || owner,
      sourceId: this.source.id,
      environments:
        manifest?.environments || this.extractEnvironments(release.body),
   ...
   */
  private async fetchManifestWithCache(
    url: string,
    filename: string,
  ): Promise<any> {
    // Check cache first
  ...
  async validate(): Promise<ValidationResult> {
    try {
      ...
      return {
        ...
        errors: [],
        warnings:
          releases.length === 0 ? ["No releases found in repository"] : [],
  ...
  public invalidateAuthCache(reason?: string): void {
    ...
    if (previousMethod !== "none") {
      this.logger.debug(
        `[GitHubAdapter] Previous auth method: ${previousMethod}`,
      );
