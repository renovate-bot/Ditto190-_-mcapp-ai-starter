...
export class LocalSkillsAdapter extends RepositoryAdapter {
  ...
  private async validateDirectoryStructure(): Promise<ValidationResult> {
    ...
    this.logger.debug(
      `[LocalSkillsAdapter] Validating directory structure: ${localPath}`,
    );
    ...
    if (isValid) {
      this.logger.debug(
        `[LocalSkillsAdapter] Directory structure validation passed`,
      );
    } else {
      this.logger.warn(
    ...
    try {
      ...
      try {
        ...
        if (skillCount === 0) {
          ...
          this.logger.info(
            `[LocalSkillsAdapter] Found ${skillCount} valid skill(s)`,
          );
        }
      } catch (scanError) {
  ...
  async fetchBundles(): Promise<Bundle[]> {
    this.logger.info(
      `[LocalSkillsAdapter] Fetching skills from: ${this.source.url}`,
    );
    ...
    try {
      ...
      for (const skill of skills) {
        try {
          ...
          bundles.push(bundle);
          this.logger.debug(
            `[LocalSkillsAdapter] Created bundle: ${bundle.id}`,
          );
        } catch (error) {
          this.logger.warn(
      ...
      this.logger.info(
        `[LocalSkillsAdapter] Successfully created ${bundles.length} bundles`,
      );
      return bundles;
    } catch (error) {
      this.logger.error(
        `[LocalSkillsAdapter] Failed to fetch skills: ${error}`,
      );
  ...
  private async scanSkillsDirectory(): Promise<SkillItem[]> {
    ...
    this.logger.debug(
      `[LocalSkillsAdapter] Scanning skills directory: ${skillsPath}`,
    );
    ...
    try {
      ...
      this.logger.error(
        `[LocalSkillsAdapter] Failed to scan skills directory: ${error}`,
      );
  ...
  private async processSkillDirectory(
    ...
    this.logger.debug(
      `[LocalSkillsAdapter] Processing skill directory: ${skillId}`,
    );
    ...
    try {
      ...
      this.logger.error(
        `[LocalSkillsAdapter] Error processing skill ${skillId}: ${error}`,
      );
      return null;
    }
  }
  ...
  private async parseSkillMd(skillMdPath: string): Promise<ParsedSkillFile> {
    ...
    try {
      ...
      if (!frontmatterMatch) {
        this.logger.warn(
          `[LocalSkillsAdapter] SKILL.md missing valid frontmatter`,
        );
        return {
      ...
      this.logger.error(
        `[LocalSkillsAdapter] Failed to parse SKILL.md: ${error}`,
      );
      throw error;
    }
  }
  ...
  async downloadBundle(bundle: Bundle): Promise<Buffer> {
    ...
    try {
      ...
      this.logger.error(
        `[LocalSkillsAdapter] Failed to download skill ${skillId}: ${error}`,
      );
  ...
  private async packageSkillAsZip(skill: SkillItem): Promise<Buffer> {
    ...
    this.logger.debug(
      `[LocalSkillsAdapter] Packaging skill as ZIP: ${skill.id}`,
    );
    ...
    try {
      ...
      this.logger.debug(
        `[LocalSkillsAdapter] Created ZIP bundle: ${zipBuffer.length} bytes`,
      );
      return zipBuffer;
    } catch (error) {
      this.logger.error(
        `[LocalSkillsAdapter] Failed to package skill ${skill.id}: ${error}`,
      );
    ...
    try {
      ...
      for (const entry of entries) {
        ...
        if (entry.isFile()) {
          ...
          this.logger.debug(
            `[LocalSkillsAdapter] Added file to ZIP: ${entryZipPath}`,
          );
