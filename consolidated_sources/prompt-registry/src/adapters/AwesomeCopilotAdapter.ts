...
export class AwesomeCopilotAdapter extends RepositoryAdapter {
  ...
  private collectionsCache: Map<
    string,
    { bundles: Bundle[]; timestamp: number }
  > = new Map();
  ...
  async downloadBundle(bundle: Bundle): Promise<Buffer> {
    ...
    try {
      ...
      const collectionFile =
        (bundle as any).collectionFile || `${bundle.id}.collection.yml`;
  ...
  private async parseCollection(
    ...
    try {
      ...
      const bundle: Bundle = {
        ...
        manifestUrl: this.buildRawUrl(
          `${this.config.collectionsPath}/${collectionFile}`,
        ),
        downloadUrl: this.buildRawUrl(
          `${this.config.collectionsPath}/${collectionFile}`,
        ),
        lastUpdated: new Date().toISOString(),
      ...
      this.logger.error(
        `Failed to parse collection ${collectionFile}`,
        error as Error,
      );
      return null;
    }
        ...
        try {
          ...
          for (const item of collection.items) {
            ...
            if (item.kind === "skill") {
              ...
              const skillDirPath = item.path.substring(
                0,
                item.path.lastIndexOf("/"),
              );
              this.logger.debug(
                `Fetching all files in skill directory: ${skillDirPath}`,
              );

              const skillFiles =
                await this.listDirectoryContentsRecursively(skillDirPath);
              this.logger.debug(
                `Found ${skillFiles.length} files in skill directory: ${skillFiles.join(", ")}`,
              );
              ...
              for (const filePath of skillFiles) {
                ...
                this.logger.debug(
                  `Added ${filePath} (${content.length} bytes)`,
                );
              }
            } else {
  ...
  private createDeploymentManifest(collection: CollectionManifest): any {
    const prompts = collection.items.map((item) => {
      ...
      const id = filename.replace(
        /\.(prompt|instructions|chatmode|agent)\.md$/,
        "",
      );
   ...
   */
  private async listDirectoryContentsRecursively(
    dirPath: string,
  ): Promise<string[]> {
    ...
    try {
      ...
      for (const item of contents) {
        if (item.type === "file") {
          ...
          const subFiles = await this.listDirectoryContentsRecursively(
            item.path,
          );
  ...
  async forceAuthentication(): Promise<void> {
    ...
    try {
      ...
      if (session) {
        ...
        this.logger.info(
          "[AwesomeCopilotAdapter] ✓ Re-authentication successful",
        );
      }
    } catch (error) {
      this.logger.error(
        `[AwesomeCopilotAdapter] Re-authentication failed: ${error}`,
      );
      throw error;
    }
  }
  ...
  private async getAuthenticationToken(): Promise<string | undefined> {
    ...
    try {
      this.logger.debug(
        "[AwesomeCopilotAdapter] Trying VSCode GitHub authentication...",
      );
      const session = await vscode.authentication.getSession(
        "github",
        ["repo"],
      ...
      if (session) {
        ...
        this.authMethod = "vscode";
        this.logger.info(
          "[AwesomeCopilotAdapter] ✓ Using VSCode GitHub authentication",
        );
        this.logger.debug(
      ...
      this.logger.debug(
        "[AwesomeCopilotAdapter] VSCode auth session not found",
      );
    } catch (error) {
    ...
    try {
      this.logger.debug(
        "[AwesomeCopilotAdapter] Trying gh CLI authentication...",
      );
      ...
      if (token && token.length > 0) {
        ...
        this.logger.info(
          "[AwesomeCopilotAdapter] ✓ Using gh CLI authentication",
        );
        this.logger.debug(
          `[AwesomeCopilotAdapter] Token preview: ${this.authToken.substring(0, 8)}...`,
        );
    ...
    if (explicitToken) {
      ...
      this.logger.info(
        "[AwesomeCopilotAdapter] ✓ Using explicit token from configuration",
      );
      this.logger.debug(
        `[AwesomeCopilotAdapter] Token preview: ${this.authToken.substring(0, 8)}...`,
      );
  ...
  private async fetchUrl(
    ...
    if (redirectDepth >= MAX_REDIRECTS) {
      this.logger.error(
        `[AwesomeCopilotAdapter] Maximum redirect depth (${MAX_REDIRECTS}) exceeded`,
      );
    ...
    if (token) {
      ...
      this.logger.debug(
        `[AwesomeCopilotAdapter] Request to ${url} WITHOUT auth`,
      );
    }
      ...
      https
        .get(url, { headers }, (res) => {
          ...
          if (res.statusCode === 301 || res.statusCode === 302) {
            ...
            if (redirectUrl) {
              this.logger.debug(
                `[AwesomeCopilotAdapter] Following redirect (depth ${redirectDepth + 1}) to: ${redirectUrl}`,
              );
              this.fetchUrl(redirectUrl, redirectDepth + 1)
                .then(resolve)
                .catch(reject);
              return;
            }
        ...
        .on("error", (error) => {
          this.logger.error(
            `[AwesomeCopilotAdapter] Network error: ${error.message}`,
          );
          reject(error);
        });
    });
