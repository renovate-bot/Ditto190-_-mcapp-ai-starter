...
export class OlafAdapter extends RepositoryAdapter {
  ...
  async fetchBundles(): Promise<Bundle[]> {
    this.logger.info(
      `[OlafAdapter] Fetching bundles from OLAF repository: ${this.source.url}`,
    );
  ...
  async validate(): Promise<ValidationResult> {
    this.logger.info(
      `[OlafAdapter] Validating OLAF repository: ${this.source.url}`,
    );
    ...
    try {
      ...
      try {
        ...
        if (error instanceof Error && error.message.includes("404")) {
          errors.push(
            `Missing required 'bundles' directory at repository root`,
          );
        } else {
      ...
      try {
        ...
        if (bundleCount === 0) {
          warnings.push(
            "No valid bundle definitions found in bundles/ directory",
          );
        } else {
          this.logger.info(
  ...
  private async scanBundleDefinitions(): Promise<BundleDefinitionInfo[]> {
    ...
    this.logger.debug(
      `[OlafAdapter] Scanning bundle definitions: ${bundlesUrl}`,
    );

    try {
      const contents: GitHubDirectoryContent[] =
        await this.makeGitHubRequest(bundlesUrl);
        ...
        try {
          ...
          try {
            const validatedSkills =
              await this.validateSkillReferences(definition);
      ...
      this.logger.error(
        `[OlafAdapter] Failed to scan bundle definitions: ${error}`,
      );
  ...
  private async parseBundleDefinition(
    ...
    this.logger.debug(
      `[OlafAdapter] Parsing bundle definition: ${jsonFile.name}`,
    );

    if (!jsonFile.download_url) {
      throw new Error(
        `No download URL for bundle definition: ${jsonFile.name}`,
      );
    }
    ...
    try {
      ...
      if (!data.metadata || typeof data.metadata !== "object") {
        throw new Error(
          `Missing or invalid metadata section in ${jsonFile.name}`,
        );
      }
      ...
      if (
        ...
        throw new Error(
          `Missing or invalid metadata.description in ${jsonFile.name}`,
        );
      }
      ...
      for (let i = 0; i < data.skills.length; i++) {
        ...
        if (!skill.description || typeof skill.description !== "string") {
          throw new Error(
            `Missing or invalid description field for ${skillContext}`,
          );
        }
        ...
        if (!skill.manifest || typeof skill.manifest !== "string") {
          throw new Error(
            `Missing or invalid manifest field for ${skillContext}`,
          );
        }
      }
      ...
      if (error instanceof SyntaxError) {
        throw new Error(
          `Invalid JSON syntax in ${jsonFile.name}: ${error.message}`,
        );
      }
      throw error;
    }
  ...
  private async validateSkillReferences(
      ...
      try {
        ...
        try {
          ...
          if (error instanceof Error && error.message.includes("404")) {
            ...
            errors.push(
              `${skillContext}: Failed to access directory - ${error}`,
            );
          }
          continue;
        }
        ...
        try {
          ...
          if (error instanceof Error && error.message.includes("404")) {
            ...
            errors.push(
              `${skillContext}: Failed to access manifest - ${error}`,
            );
          }
          continue;
        }
        ...
        try {
          ...
          errors.push(
            `${skillContext}: Invalid manifest file - ${manifestError}`,
          );
          continue;
        }
    ...
    if (validatedSkills.length === 0) {
      throw new Error(
        `No valid skills found in bundle "${bundleDefinition.metadata.name}"`,
      );
    }
  ...
  private async parseLocalSkillManifest(
    ...
    for (let i = 0; i < entryPoints.length; i++) {
      ...
      if (!entryPoint.protocol || typeof entryPoint.protocol !== "string") {
        throw new Error(
          `Missing or invalid protocol field for ${entryContext}`,
        );
      }
      ...
      if (!entryPoint.patterns || !Array.isArray(entryPoint.patterns)) {
        throw new Error(
          `Missing or invalid patterns array for ${entryContext}`,
        );
      }
  ...
  private async fetchSingleSkill(
    ...
    try {
      ...
      this.logger.error(
        `[OlafAdapter] Failed to fetch skill ${skillName}: ${error}`,
      );
      return null;
    }
  }
  ...
  private async scanSkillsDirectory(): Promise<SkillInfo[]> {
    ...
    try {
      ...
      for (const skillDir of skillDirectories) {
        try {
          const skillInfo = await this.processSkillDirectory(
            skillDir,
            owner,
            repo,
          );
      ...
      this.logger.info(
        `[OlafAdapter] Successfully discovered ${skills.length} valid skills`,
      );
      return skills;
    } catch (error) {
      this.logger.error(
        `[OlafAdapter] Failed to scan skills directory: ${error}`,
      );
  ...
  private async processSkillDirectory(
    ...
    try {
      ...
      const manifest = await this.parseSkillManifest(
        manifestFile.download_url!,
        skillName,
      );
      ...
      this.logger.error(
        `[OlafAdapter] Error processing skill ${skillName}: ${error}`,
      );
      throw error;
    }
  }
  ...
  private async parseSkillManifest(
    ...
    this.logger.debug(
      `[OlafAdapter] Parsing skill manifest from: ${manifestUrl}`,
    );
    ...
    try {
      ...
      const manifest: SkillManifest = {
        ...
        tags: Array.isArray(rawManifest.tags)
          ? rawManifest.tags
          : ["olaf", "skill"],
  ...
  private generateDeploymentManifest(skill: SkillInfo): any {
    ...
    this.logger.debug(
      `[OlafAdapter] Generated deployment manifest for skill: ${skill.id}`,
    );
  ...
  async downloadBundle(bundle: Bundle): Promise<Buffer> {
    ...
    try {
      ...
      const bundleDefinitions = await this.scanBundleDefinitions();
      const bundleInfo = bundleDefinitions.find(
        (info) => info.id === bundle.id,
      );
      ...
      throw new Error(
        `Failed to download OLAF bundle ${bundleFileName}: ${error}`,
      );
    }
  }
   ...
   */
  private async packageBundleAsZip(
    bundleInfo: BundleDefinitionInfo,
  ): Promise<Buffer> {
    const { owner, repo } = this.parseGitHubUrl();
    const AdmZip = require("adm-zip");

    this.logger.debug(
      `[OlafAdapter] Packaging bundle as ZIP: ${bundleInfo.fileName}`,
    );
    ...
    try {
      ...
      const deploymentManifest =
        this.generateBundleDeploymentManifest(bundleInfo);
      ...
      for (const skill of bundleInfo.validatedSkills) {
        this.logger.debug(
          `[OlafAdapter] Adding skill to bundle: ${skill.folderName}`,
        );
   ...
   */
  private generateBundleDeploymentManifest(
    bundleInfo: BundleDefinitionInfo,
  ): any {
    const { owner, repo } = this.parseGitHubUrl();
    ...
    const deploymentManifest = {
      ...
      prompts: validatedSkills.map((skill) => ({
        ...
        entry_points:
          (skill.manifest as LocalOlafSkillManifest).entry_points || [],
      })),
    };
  ...
  private async ensureRuntimeInstalled(): Promise<void> {
    try {
      ...
      if (workspacePath) {
        const hasLinks =
          await this.runtimeManager.hasWorkspaceLinks(workspacePath);
      ...
      this.logger.info(
        "[OlafAdapter] OLAF runtime setup completed successfully",
      );
    } catch (error) {
  ...
  private async packageSkillAsBundle(skill: SkillInfo): Promise<Buffer> {
    ...
    this.logger.debug(
      `[OlafAdapter] Packaging skill as bundle: ${skill.folderName}`,
    );
  ...
  private async addDirectoryToZip(
    ...
    try {
      ...
      for (const item of dirContents) {
        if (item.type === "file" && item.download_url) {
          try {
            ...
            this.logger.debug(
              `[OlafAdapter] Added nested file to ZIP: ${filePath}`,
            );
          } catch (error) {
            this.logger.warn(
      ...
      this.logger.warn(
        `[OlafAdapter] Failed to process directory ${dirPath}: ${error}`,
      );
    }
  }
  ...
  async postInstall(bundleId: string, installPath: string): Promise<void> {
    this.logger.info(
      `[OlafAdapter] Running post-installation for bundle: ${bundleId}`,
    );

    try {
      await this.registerBundleInCompetencyIndex(bundleId, installPath);
      this.logger.info(
        `[OlafAdapter] Post-installation completed successfully`,
      );
    } catch (error) {
  ...
  async postUninstall(bundleId: string, installPath: string): Promise<void> {
    this.logger.info(
      `[OlafAdapter] Running post-uninstallation for bundle: ${bundleId}`,
    );

    try {
      await this.unregisterBundleFromCompetencyIndex(bundleId, installPath);
      this.logger.info(
        `[OlafAdapter] Post-uninstallation completed successfully`,
      );
    } catch (error) {
  ...
  private async registerBundleInCompetencyIndex(
    ...
    try {
      ...
      this.logger.info(
        `[OlafAdapter] Competency index path: ${competencyIndexPath}`,
      );

      // Ensure the directory exists
      const competencyIndexDir = path.dirname(competencyIndexPath);
      this.logger.info(
        `[OlafAdapter] Creating directory if needed: ${competencyIndexDir}`,
      );
      ...
      if (fs.existsSync(competencyIndexPath)) {
        ...
        this.logger.error(
          `[OlafAdapter] Competency index file does not exist after write`,
        );
      }
  ...
  private async unregisterBundleFromCompetencyIndex(
    ...
    this.logger.info(
      `[OlafAdapter] Unregistering bundle from competency index: ${bundleId}`,
    );
    this.logger.info(`[OlafAdapter] Install path: ${installPath}`);
    ...
    try {
      ...
      if (bundleInfo) {
        ...
        competencyIndex = competencyIndex.filter(
          (s: any) => !s.file.startsWith(pattern),
        );
        this.logger.info(
          `[OlafAdapter] Removed all entries matching pattern: ${pattern}`,
        );
      }
      ...
      if (initialLength > finalLength) {
        ...
        this.logger.info(
          `[OlafAdapter] No skills found to remove from competency index`,
        );
      }

      this.logger.info(
        `[OlafAdapter] Successfully processed bundle unregistration`,
      );
    } catch (error) {
      this.logger.error(
  ...
  private async makeGitHubRequest(url: string): Promise<any> {
    ...
    if (explicitToken && explicitToken.trim().length > 0) {
      ...
      this.logger.debug(
        "[OlafAdapter] Using explicit token from configuration",
      );
    } else {
    ...
    return new Promise((resolve, reject) => {
      ...
      if (authToken) {
        ...
        this.logger.debug(
          `[OlafAdapter] Request to ${url} with authentication`,
        );
      } else {
        this.logger.debug(
          `[OlafAdapter] Request to ${url} without authentication`,
        );
      }
        ...
        .get(url, { headers }, (res: any) => {
          ...
          res.on("end", () => {
            if (res.statusCode >= 400) {
              ...
              this.logger.error(
                `[OlafAdapter] Response: ${data.substring(0, 500)}`,
              );
              reject(new Error(`HTTP ${res.statusCode}: ${res.statusMessage}`));
              return;
            }
