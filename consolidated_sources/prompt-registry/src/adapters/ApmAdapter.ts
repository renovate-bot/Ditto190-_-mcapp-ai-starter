...
export class ApmAdapter extends RepositoryAdapter {
  ...
  private async getAuthenticationToken(): Promise<string | undefined> {
    ...
    if (this.authToken !== undefined) {
      this.logger.debug(
        `[ApmAdapter] Using cached token (method: ${this.authMethod})`,
      );
    ...
    if (explicitToken) {
      ...
      this.logger.info(
        "[ApmAdapter] ✓ Using explicit token from configuration",
      );
      this.logger.debug(
        `[ApmAdapter] Token preview: ${this.authToken.substring(0, 8)}...`,
      );
    ...
    const manifestPaths = tree
      .filter((item) => {
        ...
        return (
          parts.length === 2 &&
          parts[1] === "apm.yml" &&
          !SKIP_DIRECTORIES.includes(parts[0])
        );
      })
      .map((item) => item.path);
  ...
  private async fetchGitTree(
    ...
    try {
      ...
      if (result.truncated) {
        this.logger.warn(
          `[ApmAdapter] Git tree for ${owner}/${repo} is truncated`,
        );
      }
   ...
   * Create deployment manifest
   */
  private async createDeploymentManifest(
    bundle: Bundle,
    installDir: string,
  ): Promise<any> {
    ...
    if (fs.existsSync(apmManifestPath)) {
      ...
      apmManifest = (yaml.load(content) as ApmManifest) || {
        name: bundle.name,
      };
    }
    ...
    const prompts = promptFiles.map((file) => {
      ...
      const id = filename.replace(
        /\.(prompt|instructions|agent|chatmode)\.md$/,
        "",
      );
      ...
      try {
        ...
        for (const entry of entries) {
          ...
          if (entry.isDirectory()) {
            ...
            PROMPT_EXTENSIONS.some((ext) => entry.name.endsWith(ext))
          ) {
            files.push(fullPath);
          }
        }
  ...
  private detectFileType(
    filename: string,
  ): "prompt" | "instructions" | "chatmode" | "agent" {
  ...
  async validate(): Promise<ValidationResult> {
    ...
    if (!runtimeStatus.installed) {
      errors.push(
        "APM CLI is not installed. Install with: pip install apm-cli",
      );
