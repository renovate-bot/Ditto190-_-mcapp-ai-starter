...
export class LocalAdapter extends RepositoryAdapter {
  ...
  private async getBundleDirectories(): Promise<string[]> {
    ...
    try {
      ...
      console.log(
        `[LocalAdapter] Found ${entries.length} entries in directory`,
      );
      ...
      console.log(
        `[LocalAdapter] Discovered ${bundleDirs.length} valid bundles`,
      );
      return bundleDirs;
    } catch (error) {
  ...
  async fetchBundles(): Promise<Bundle[]> {
    try {
      ...
      for (const bundleDir of bundleDirs) {
        ...
        try {
          const manifest = (await this.readYamlFile(
            manifestPath,
          )) as LocalManifest;
          console.log(
  ...
  async downloadBundle(bundle: Bundle): Promise<Buffer> {
    ...
    try {
      ...
      console.error(
        `[LocalAdapter] ✗ Cannot access bundle directory: ${error}`,
      );
      if ((error as NodeJS.ErrnoException).code === "ENOENT") {
        throw new Error(`Bundle directory not found: ${bundlePath}`);
      } else if ((error as NodeJS.ErrnoException).code === "EACCES") {
        throw new Error(
          `Permission denied accessing bundle directory: ${bundlePath}`,
        );
      }
      throw error;
    }
    ...
    return new Promise<Buffer>((resolve, reject) => {
      (async () => {
        try {
          ...
          console.error(
            `[LocalAdapter] ✗ Failed to create bundle archive: ${error}`,
          );
          reject(error);
        }
      })();
