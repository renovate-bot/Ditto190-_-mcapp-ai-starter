...
export class LocalApmAdapter extends RepositoryAdapter {
  ...
  private cache: Map<string, { bundles: LocalApmBundle[]; timestamp: number }> =
    new Map();
    ...
    try {
      ...
      for (const entry of entries) {
        ...
        if (manifest) {
          ...
          const nested = await this.scanSubdirectories(
            subdir,
            currentDepth + 1,
          );
          bundles.push(...nested);
  ...
  private async populateArchive(
    ...
    archive.append(yaml.dump(deploymentManifest), {
  ...
  private async createDeploymentManifest(
    ...
    const apmManifest: ApmManifest = (await this.readApmManifest(
      packageDir,
    )) || { name: bundle.name };
  ...
  async validate(): Promise<ValidationResult> {
    ...
    try {
      ...
      return {
        ...
        errors: [],
        warnings:
          bundles.length === 0 ? ["No apm.yml files found in directory"] : [],
