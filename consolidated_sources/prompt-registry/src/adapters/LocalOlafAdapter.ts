...
export class LocalOlafAdapter extends RepositoryAdapter {
  ...
  private async validateDirectoryStructure(): Promise<ValidationResult> {
    ...
    this.logger.debug(
      `[LocalOlafAdapter] Validating directory structure: ${localPath}`,
    );
  ...
  async validate(): Promise<ValidationResult> {
    ...
    try {
      this.logger.info(
        `[LocalOlafAdapter] Validating local OLAF source: ${this.source.url}`,
      );
  ...
  private async performAdditionalValidation(warnings: string[]): Promise<void> {
    ...
    try {
      ...
      this.logger.debug(
        `[LocalOlafAdapter] Additional validation checks failed: ${error}`,
      );
  ...
  async fetchMetadata(): Promise<SourceMetadata> {
    try {
      ...
      if (!validation.valid) {
        throw new Error(
          `Invalid directory structure: ${validation.errors.join(", ")}`,
        );
      }
  ...
  private async readJsonFile(filePath: string): Promise<any> {
    try {
      ...
      if (error instanceof SyntaxError) {
        ...
        throw new Error(
          `Permission denied reading file: ${path.basename(filePath)}`,
        );
      } else {
        throw new Error(
          `Failed to read file ${path.basename(filePath)}: ${error}`,
        );
      }
    }
  }
    ...
    try {
      ...
      if (data.metadata && data.bom) {
        ...
        if (!data.metadata.name || typeof data.metadata.name !== "string") {
          throw new Error(
            `Missing or invalid metadata.name field in ${fileName}`,
          );
        }

        if (!data.bom.entry_points || !Array.isArray(data.bom.entry_points)) {
          throw new Error(
            `Missing or invalid bom.entry_points array in ${fileName}`,
          );
        }
        ...
        description =
          data.metadata.description || data.metadata.shortDescription;
        ...
        if (!data.entry_points || !Array.isArray(data.entry_points)) {
          throw new Error(
            `Missing or invalid entry_points array in ${fileName}`,
          );
        }
    ...
    for (let i = 0; i < bundleDefinition.skills.length; i++) {
      ...
      try {
        ...
        try {
          ...
          errors.push(
            `${skillContext}: Failed to read skill files - ${filesError}`,
          );
          continue;
        }
  ...
  private async parseBundleDefinition(
    ...
    try {
      ...
      if (!data.metadata.name || typeof data.metadata.name !== "string") {
        throw new Error(
          `Missing or invalid metadata.name in ${path.basename(bundlePath)}`,
        );
      }
      ...
      if (!data.skills || !Array.isArray(data.skills)) {
        throw new Error(
          `Missing or invalid skills array in ${path.basename(bundlePath)}`,
        );
      }

      if (data.skills.length === 0) {
        throw new Error(
          `Bundle ${path.basename(bundlePath)} contains no skills`,
        );
      }
  ...
  private async packageBundleAsZip(
    ...
    this.logger.debug(
      `[LocalOlafAdapter] Packaging bundle as ZIP: ${bundleInfo.fileName}`,
    );
  ...
  async downloadBundle(bundle: Bundle): Promise<Buffer> {
    this.logger.info(
      `[LocalOlafAdapter] Preparing bundle for installation: ${bundle.name}`,
    );
    ...
    try {
      ...
      throw new Error(
        `Failed to prepare local OLAF bundle ${bundle.name}: ${error}`,
      );
    }
  }
  ...
  private async ensureRuntimeInstalled(): Promise<void> {
    ...
    try {
      ...
      if (!workspaceFolders || workspaceFolders.length === 0) {
        throw new Error(
          "No workspace found. OLAF runtime requires an active workspace.",
        );
      }
      ...
      const updatedInfo = await this.runtimeManager.getRuntimeInfo(
        "latest",
        true,
      );
      ...
      this.logger.info(
        `[LocalOlafAdapter] OLAF runtime installed successfully`,
      );
    } catch (error) {
      this.logger.error(
        `[LocalOlafAdapter] Runtime installation failed: ${error}`,
      );
  ...
  private async createWorkspaceLinks(): Promise<void> {
    ...
    try {
      ...
      this.logger.info(
        `[LocalOlafAdapter] Workspace symbolic links created successfully`,
      );
    } catch (error) {
      this.logger.error(
        `[LocalOlafAdapter] Failed to create workspace links: ${error}`,
      );
  ...
  async postInstall(bundleId: string, installPath: string): Promise<void> {
    this.logger.info(
      `[LocalOlafAdapter] Post-installation hook for bundle: ${bundleId}`,
    );
    ...
    try {
      ...
      this.logger.info(
        `[LocalOlafAdapter] Post-installation completed successfully`,
      );
    } catch (error) {
      this.logger.error(
        `[LocalOlafAdapter] Post-installation failed: ${error}`,
      );
      throw error;
    }
  }
  ...
  async postUninstall(bundleId: string, installPath: string): Promise<void> {
    this.logger.info(
      `[LocalOlafAdapter] Post-uninstallation hook for bundle: ${bundleId}`,
    );
    ...
    try {
      ...
      this.logger.info(
        `[LocalOlafAdapter] Post-uninstallation completed successfully`,
      );
    } catch (error) {
      this.logger.error(
        `[LocalOlafAdapter] Post-uninstallation failed: ${error}`,
      );
      throw error;
    }
  }
  ...
  private async registerBundleInCompetencyIndex(
    ...
    try {
      ...
      this.logger.info(
        `[LocalOlafAdapter] Competency index path: ${competencyIndexPath}`,
      );
      ...
      if (fs.existsSync(competencyIndexPath)) {
        this.logger.info(
          `[LocalOlafAdapter] Reading existing competency index`,
        );
      ...
      for (const skill of bundleInfo.validatedSkills) {
        await this.registerSkillInCompetencyIndex(
          skill,
          installPath,
          competencyIndex,
        );
   ...
   */
  private async createMinimalBundle(
    bundleInfo: BundleDefinitionInfo,
  ): Promise<Buffer> {
    try {
      const zip = new AdmZip();
  ...
  private async createSkillSymbolicLinks(bundleId: string): Promise<void> {
    this.logger.info(
      `[LocalOlafAdapter] Creating symbolic links for bundle: ${bundleId}`,
    );
    ...
    try {
      ...
      const externalSkillsPath = path.join(
        workspacePath,
        ".olaf",
        "external-skills",
      );
          ...
          if (existingEntry.exists) {
            if (existingEntry.isSymbolicLink) {
              ...
              if (existingEntry.isBroken) {
                this.logger.debug(
                  `[LocalOlafAdapter] Removed broken symbolic link: ${skillLinkPath}`,
                );
              } else {
                this.logger.info(
      ...
      this.logger.error(
        `[LocalOlafAdapter] Failed to create symbolic links: ${error}`,
      );
      throw error;
    }
  }
  ...
  private async removeSkillSymbolicLinks(bundleId: string): Promise<void> {
    this.logger.info(
      `[LocalOlafAdapter] Removing symbolic links for bundle: ${bundleId}`,
    );
    ...
    try {
      ...
      this.logger.error(
        `[LocalOlafAdapter] Failed to remove symbolic links: ${error}`,
      );
      throw error;
    }
  }
