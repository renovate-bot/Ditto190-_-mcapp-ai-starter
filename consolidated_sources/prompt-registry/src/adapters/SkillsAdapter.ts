...
export class SkillsAdapter extends RepositoryAdapter {
  ...
  async fetchBundles(): Promise<Bundle[]> {
    this.logger.info(
      `[SkillsAdapter] Fetching skills from repository: ${this.source.url}`,
    );

    try {
      const skills = await this.scanSkillsDirectory();
      this.logger.info(
        `[SkillsAdapter] Found ${skills.length} skills in repository`,
      );
      ...
      this.logger.info(
        `[SkillsAdapter] Successfully created ${bundles.length} bundles`,
      );
      return bundles;
    } catch (error) {
  ...
  private async scanSkillsDirectory(): Promise<SkillItem[]> {
    ...
    this.logger.debug(
      `[SkillsAdapter] Scanning skills directory: ${skillsUrl}`,
    );

    try {
      const contents: GitHubContentItem[] =
        await this.makeGitHubRequest(skillsUrl);
      const skills: SkillItem[] = [];

      const directories = contents.filter((item) => item.type === "dir");
      this.logger.debug(
        `[SkillsAdapter] Found ${directories.length} directories in skills/`,
      );
      ...
      for (let i = 0; i < directories.length; i += CONCURRENCY_LIMIT) {
        ...
        this.logger.debug(
          `[SkillsAdapter] Processing chunk ${Math.floor(i / CONCURRENCY_LIMIT) + 1}/${Math.ceil(directories.length / CONCURRENCY_LIMIT)}`,
        );
      ...
      this.logger.error(
        `[SkillsAdapter] Failed to scan skills directory: ${error}`,
      );
  ...
  private async processSkillDirectory(
    ...
    try {
      ...
      if (!skillMdFile) {
        this.logger.debug(
          `[SkillsAdapter] Skill ${skillId} missing SKILL.md, skipping`,
        );
        return null;
      }
      ...
      const files = allFiles.map((item) =>
        this.getRelativeSkillPath(item.path, skillPath),
      );
      ...
      this.logger.debug(
        `[SkillsAdapter] Successfully processed skill: ${skillItem.name}`,
      );
      return skillItem;
    } catch (error) {
      this.logger.error(
        `[SkillsAdapter] Error processing skill ${skillId}: ${error}`,
      );
      return null;
    }
  }
  ...
  private async parseSkillMd(downloadUrl: string): Promise<ParsedSkillFile> {
    ...
    try {
      ...
      try {
        ...
        this.logger.warn(
          `[SkillsAdapter] Failed to parse YAML frontmatter: ${yamlError}`,
        );
        frontmatter = { name: "", description: "" };
      }
  ...
  private async collectSkillFiles(
    owner: string,
    repo: string,
    initialEntries: GitHubContentItem[],
    ...
    while (queue.length > 0) {
      ...
      if (entry.type === "dir") {
        try {
          const nestedEntries: GitHubContentItem[] =
            await this.makeGitHubRequest(
              `${apiBase}/repos/${owner}/${repo}/contents/${entry.path}`,
            );
          queue.push(...nestedEntries);
        } catch (error) {
          this.logger.warn(
            `[SkillsAdapter] Failed to read nested directory ${entry.path}: ${error}`,
          );
        }
      }
    }
  ...
  async validate(): Promise<ValidationResult> {
    this.logger.info(
      `[SkillsAdapter] Validating skills repository: ${this.source.url}`,
    );
    ...
    try {
      ...
      try {
        ...
        if (skillCount === 0) {
          ...
          this.logger.info(
            `[SkillsAdapter] Found ${skillCount} valid skill(s)`,
          );
        }
      } catch (scanError) {
  ...
  async downloadBundle(bundle: Bundle): Promise<Buffer> {
    ...
    try {
      ...
      this.logger.error(
        `[SkillsAdapter] Failed to download skill ${skillId}: ${error}`,
      );
  ...
  private async fetchSingleSkill(skillId: string): Promise<SkillItem | null> {
    ...
    try {
      ...
      if (!skillMdFile || !skillMdFile.download_url) {
        this.logger.warn(
          `[SkillsAdapter] No SKILL.md found for skill: ${skillId}`,
        );
        return null;
      }
      ...
      this.logger.error(
        `[SkillsAdapter] Failed to fetch skill ${skillId}: ${error}`,
      );
      return null;
    }
  }
  ...
  private async packageSkillAsZip(skill: SkillItem): Promise<Buffer> {
    ...
    try {
      ...
      this.logger.debug(
        `[SkillsAdapter] Created ZIP bundle: ${zipBuffer.length} bytes`,
      );
      return zipBuffer;
    } catch (error) {
      this.logger.error(
        `[SkillsAdapter] Failed to package skill ${skill.id}: ${error}`,
      );
  ...
  private async addDirectoryToZip(
    ...
    try {
      ...
      const dirContents: GitHubContentItem[] =
      ...
      this.logger.warn(
        `[SkillsAdapter] Failed to add directory ${dirPath} to ZIP: ${error}`,
      );
    }
  }
  ...
  private async makeGitHubRequest(url: string): Promise<any> {
    ...
    if (explicitToken && explicitToken.trim().length > 0) {
      ...
      this.logger.debug(
        "[SkillsAdapter] Using explicit token from configuration",
      );
    } else {
      try {
        ...
        if (session) {
          ...
          this.logger.debug(
            "[SkillsAdapter] Using VSCode GitHub authentication",
          );
        }
      } catch (error) {
    ...
    return new Promise((resolve, reject) => {
      ...
      if (authToken) {
        ...
        this.logger.debug(
          `[SkillsAdapter] Request to ${url} with authentication`,
        );
      } else {
        this.logger.debug(
          `[SkillsAdapter] Request to ${url} without authentication`,
        );
      }
