...
export class MarketplaceViewProvider implements vscode.WebviewViewProvider {
  ...
  constructor(
    ...
    this.disposables.push(
      ...
      this.registryManager.onBundleUninstalled((bundleId) => {
        this.handleBundleEvent("uninstalled", bundleId, () =>
          this.loadBundles(),
        );
      }),
      this.registryManager.onBundleUpdated((installation) => {
        this.handleBundleEvent("updated", installation.bundleId, () =>
          this.loadBundles(),
        );
      }),
      this.registryManager.onBundlesInstalled((installations) => {
        this.handleBundleEvent(
          "installed",
          `${installations.length} bundles`,
          () => this.loadBundles(),
        );
      }),
      this.registryManager.onBundlesUninstalled((bundleIds) => {
        this.handleBundleEvent(
          "uninstalled",
          `${bundleIds.length} bundles`,
          () => this.loadBundles(),
        );
      }),
      // Source sync events with debouncing
      this.registryManager.onSourceSynced((event) =>
        this.handleSourceSynced(event),
      ),
      // Auto-update preference changes
      this.registryManager.onAutoUpdatePreferenceChanged(() =>
        this.loadBundles(),
      ),
  ...
  private handleBundleEvent(
    ...
    try {
      ...
      this.logger.error(
        `Error handling bundle ${eventType} event`,
        error as Error,
      );
   ...
   */
  private handleSourceSynced(event: {
    sourceId: string;
    bundleCount: number;
  }): void {
    this.logger.debug(
      `Source synced: ${event.sourceId} (${event.bundleCount} bundles)`,
    );
  ...
  private async loadBundles(): Promise<void> {
    ...
    try {
      ...
      const installedBundles =
        await this.registryManager.listInstalledBundles();
      ...
      const enhancedBundles = await Promise.all(
        bundles.map(async (bundle) => {
          ...
          const contentBreakdown = this.getContentBreakdown(
            bundle,
            installed?.manifest,
          );
          ...
          if (
            (bundle as any).isConsolidated &&
            (bundle as any).availableVersions
          ) {
            availableVersions = (bundle as any).availableVersions.map(
              (v: any) => ({
                version: v.version,
              }),
            );
      ...
      this.logger.debug(
        `Loaded ${enhancedBundles.length} bundles for marketplace`,
      );
    } catch (error) {
   ...
   */
  private countPromptsByType(
    prompts: any[],
    mcpServersCount: number = 0,
  ): ContentBreakdown {
   ...
   */
  private getContentBreakdown(
    bundle: Bundle,
    manifest?: any,
  ): ContentBreakdown {
    const bundleData = bundle as any;
    const mcpCount = manifest?.mcpServers
      ? Object.keys(manifest.mcpServers).length
      : this.countMcpServers(bundleData);
    ...
    if (bundleData.breakdown) {
      return {
        ...
        mcpServers:
          bundleData.breakdown.mcpServers || this.countMcpServers(bundleData),
      };
    }
    ...
    switch (message.type) {
      ...
      case "openPromptFile":
        if (message.installPath && message.filePath) {
          await this.openPromptFileInEditor(
            message.installPath,
            message.filePath,
          );
        }
  ...
  private async handleOpenSourceRepository(bundleId: string): Promise<void> {
    try {
      const { bundle, source } =
        await this.findInstalledBundleByMarketplaceId(bundleId);
      ...
      await vscode.commands.executeCommand(
        "promptregistry.openItemRepository",
        item,
      );
    } catch (error) {
  ...
  private async handleCompleteSetup(): Promise<void> {
    ...
    try {
      ...
      this.logger.error(
        "Failed to complete setup from marketplace",
        error as Error,
      );
   ...
   */
  private async openPromptFileInEditor(
    installPath: string,
    filePath: string,
  ): Promise<void> {
    try {
   ...
   */
  private async promptForScope(): Promise<
    import("../utils/scopeSelectionUI").ScopeSelectionResult | undefined
  > {
    const { showScopeSelectionDialog } =
      await import("../utils/scopeSelectionUI");
  ...
  private async handleInstall(bundleId: string): Promise<void> {
    try {
      ...
      this.logger.error(
        "Failed to install bundle from marketplace",
        error as Error,
      );
      vscode.window.showErrorMessage(
        `Failed to install bundle: ${(error as Error).message}`,
      );
    }
  }
  ...
  private async handleUninstall(bundleId: string): Promise<void> {
    try {
      ...
      vscode.window.showInformationMessage(
        `✅ Bundle uninstalled successfully!`,
      );
      ...
      this.logger.error(
        "Failed to uninstall bundle from marketplace",
        error as Error,
      );
      vscode.window.showErrorMessage(
  ...
  private async handleUpdate(bundleId: string): Promise<void> {
    try {
      ...
      if (!installed) {
        ...
        this.logger.warn(
          `Bundle ${bundleId} not installed, performing fresh install`,
        );
      ...
      this.logger.error(
        "Failed to update bundle from marketplace",
        error as Error,
      );
      vscode.window.showErrorMessage(
        `Failed to update bundle: ${(error as Error).message}`,
      );
    }
  }
   ...
   */
  private async handleInstallVersion(
    bundleId: string,
    version: string,
  ): Promise<void> {
    try {
      this.logger.info(
        `Installing specific version of bundle: ${bundleId} v${version}`,
      );
      ...
      vscode.window.showInformationMessage(
        `✅ Bundle v${version} installed successfully!`,
      );
  ...
  private async handleGetVersions(bundleId: string): Promise<void> {
    try {
      ...
      this.logger.debug(
        `Sent ${versions.length} versions for bundle ${bundleId}`,
      );
    } catch (error) {
   ...
   */
  private async handleToggleAutoUpdate(
    bundleId: string,
    enabled: boolean,
  ): Promise<void> {
    try {
      this.logger.info(
        `Toggling auto-update for bundle '${bundleId}' to ${enabled}`,
      );
      ...
      vscode.window.showInformationMessage(
        `Auto-update ${status} for ${bundleId}`,
      );
  ...
  async openBundleDetails(bundleId: string): Promise<void> {
    try {
      ...
      try {
        ...
        this.logger.error(
          `Failed to get bundle details for ${bundleId}`,
          error as Error,
        );
      ...
      panel.webview.onDidReceiveMessage(
        async (message) => {
          if (message.type === "openPromptFile") {
            ...
            await this.handleToggleAutoUpdate(
              message.bundleId,
              message.enabled,
            );
  ...
  private getBundleDetailsHtml(
    ...
    const escapedInstallPath = installPath
      .replace(/\\/g, "\\\\")
      .replace(/'/g, "\\'");
    ...
    const promptsSection = this.generatePromptsSection(
      installed,
      escapedInstallPath,
    );
    ...
    html = html
      ...
      .replace(
        "{{displayBundleId}}",
        isInstalled ? installed!.bundleId : bundle.id,
      )
      .replace(
        "{{displayVersion}}",
        isInstalled ? installed!.version : bundle.version,
      )
   ...
   */
  private generateMcpServersSection(
    installed: InstalledBundle | undefined,
  ): string {
    if (
      !installed?.manifest?.mcpServers ||
      Object.keys(installed.manifest.mcpServers).length === 0
    ) {
      return "";
    }

    const serverCount = Object.keys(installed.manifest.mcpServers).length;
    const serversHtml = Object.entries(installed.manifest.mcpServers)
        ...
        if (isStdio) {
          ...
          serverContent = `
                    ...
                    ${
                      ...
                      Object.keys(remoteConfig.headers).length > 0
                        ? `
                    <div class="mcp-env-vars">
   ...
   */
  private generatePromptsSection(
    installed: InstalledBundle | undefined,
    escapedInstallPath: string,
  ): string {
    if (!installed?.manifest?.prompts) {
      return "";
    }

    const promptsHtml = installed.manifest.prompts
      .map(
        (p) => `
