...
export class RegistryTreeItem extends vscode.TreeItem {
  ...
  private getIcon(): vscode.ThemeIcon | undefined {
    ...
    if (
      (this.type === TreeItemType.HUB_PROFILE ||
        this.type === TreeItemType.PROFILE) &&
      this.data?.icon
    ) {
      return undefined;
    }
...
export class RegistryTreeProvider implements vscode.TreeDataProvider<RegistryTreeItem> {
  ...
  private getContextValue(
    ...
    if (scope === "repository") {
      ...
      const commitModeSuffix =
        commitMode === "local-only" ? "local_only" : commitMode;
      ...
      case TreeItemType.HUB:
        if (this.viewMode === "favorites") {
          ...
          const favoriteProfiles = profiles.filter((p) =>
            hubFavorites.includes(p.id),
          );
          return this.organizeProfiles(
            hub.id,
            favoriteProfiles,
            [],
            hubFavorites,
  ...
  private getRootItems(): RegistryTreeItem[] {
    const profileRootLabel =
      this.viewMode === "all" ? "Shared Profiles" : "Favorites";
    const profileRootType =
      this.viewMode === "all"
        ? TreeItemType.HUBS_ROOT
        : TreeItemType.FAVORITES_ROOT;
  ...
  private async getFavoritesItems(): Promise<RegistryTreeItem[]> {
    ...
    for (const [hubId, profileIds] of Object.entries(favorites)) {
      ...
      try {
        ...
        this.logger.warn(
          `Failed to load hub ${hubId} for favorites view`,
          error as Error,
        );
      }
    }
    ...
    try {
      ...
      this.logger.error(
        "Failed to load local profiles for favorites view",
        error as Error,
      );
    }
  ...
  private async getHubChildren(hub: any): Promise<RegistryTreeItem[]> {
    ...
    const favoriteProfiles =
      (await this.hubManager.getFavoriteProfiles()) || {};
   ...
   */
  private async getProfileBundleItems(
    profile: Profile,
  ...
  private async getInstalledBundleItems(): Promise<RegistryTreeItem[]> {
    try {
      ...
      for (const bundle of installed) {
        try {
          const details = await this.registryManager.getBundleDetails(
            bundle.bundleId,
          );
          ...
          const autoUpdateEnabled =
            autoUpdatePreferences[bundle.bundleId] ?? false;
   ...
   */
  private async getCategoryBundles(
    category: string,
  ): Promise<RegistryTreeItem[]> {
