...
(function () {
  ...
  function updateFilterUI() {
    ...
    allItem.className =
      "source-item" + (selectedSource === "all" ? " active" : "");
    allItem.dataset.source = "all";
    allItem.innerHTML =
      '<input type="radio" name="source" id="source-all" value="all" ' +
      (selectedSource === "all" ? "checked" : "") +
      ">" +
    ...
    filterOptions.sources.forEach(function (source) {
      ...
      sourceItem.className =
        "source-item" + (selectedSource === source.id ? " active" : "");
      ...
      sourceItem.innerHTML =
        ...
        source.id +
        '" ' +
        (selectedSource === source.id ? "checked" : "") +
        ">" +
        '<label for="source-' +
        source.id +
        '">' +
      ...
      sourceItem.addEventListener("click", function () {
        ...
        document.getElementById("sourceSelectorText").textContent =
          source.name + " (" + source.bundleCount + ")";
  ...
  function updateSelectedTags() {
    var checkboxes = document.querySelectorAll(
      '#tagList input[type="checkbox"]:checked',
    );
  ...
  document
    .getElementById("sourceSelectorBtn")
  ...
  document
    .getElementById("installedCheckbox")
  ...
  document
    .getElementById("installedFilter")
    .addEventListener("click", function (e) {
  ...
  function formatUpdateLabel(installedVersion, latestVersion) {
    ...
    return (
      " (" +
      formatVersionLabel(installedVersion) +
      " -> " +
  ...
  function renderBundles() {
    ...
    marketplace.innerHTML = filteredBundles
      .map(function (bundle) {
        return (
          ...
          (bundle.installed && bundle.autoUpdateEnabled
            ? '<div class="installed-badge">🔄 Auto-Update</div>'
            : bundle.installed
          ...
          bundle.name +
          "</div>" +
          '<div class="bundle-author">by ' +
          (bundle.author || "Unknown") +
          " • " +
          formatVersionLabel(bundle.version) +
          "</div>" +
          "</div>" +
          ...
          renderContentItem(
            "💬",
            "Prompts",
            bundle.contentBreakdown ? bundle.contentBreakdown.prompts || 0 : 0,
          ) +
          renderContentItem(
            "📋",
            "Instructions",
            bundle.contentBreakdown
              ? bundle.contentBreakdown.instructions || 0
              : 0,
          ) +
          renderContentItem(
            "🤖",
            "Agents",
            bundle.contentBreakdown ? bundle.contentBreakdown.agents || 0 : 0,
          ) +
          renderContentItem(
            "🛠️",
            "Skills",
            bundle.contentBreakdown ? bundle.contentBreakdown.skills || 0 : 0,
          ) +
          renderContentItem(
            "🔌",
            "MCP Servers",
            bundle.contentBreakdown
              ? bundle.contentBreakdown.mcpServers || 0
              : 0,
          ) +
          "</div>" +
          ...
          '<button class="btn btn-secondary" data-action="openDetails" data-bundle-id="' +
          bundle.id +
          '">Details</button>' +
          '<button class="btn btn-link" data-action="openSourceRepo" data-bundle-id="' +
          bundle.id +
  ...
  function renderBundleButtons(bundle) {
      ...
      if (bundle.availableVersions && bundle.availableVersions.length > 1) {
        return (
          ...
          (bundle.installedVersion
            ? formatUpdateLabel(bundle.installedVersion, bundle.version)
            : "") +
          "</button>" +
          ...
          bundle.id +
          '-update">' +
          '<div class="version-item uninstall" data-action="uninstallBundle" data-bundle-id="' +
          bundle.id +
          '">' +
          "<span>Uninstall</span>" +
          "</div>" +
          '<div class="version-dropdown-header">Switch Version</div>' +
          (bundle.availableVersions || [])
            .map(function (versionObj, index) {
              return (
                '<div class="version-item ' +
                (versionObj.version === bundle.installedVersion
                  ? "current"
                  : "") +
                '" data-action="installBundleVersion" data-bundle-id="' +
                bundle.id +
                '" data-version="' +
                versionObj.version +
                '">' +
    ...
    if (bundle.availableVersions && bundle.availableVersions.length > 1) {
      return (
        '<div class="version-selector-group">' +
        '<button class="btn btn-primary" data-action="installBundle" data-bundle-id="' +
        bundle.id +
        '">Install</button>' +
        '<button class="version-selector-arrow" data-action="toggleVersionDropdown" data-dropdown-id="' +
        bundle.id +
        ...
        (bundle.availableVersions || [])
          .map(function (versionObj, index) {
            return (
              '<div class="version-item" data-action="installBundleVersion" data-bundle-id="' +
              bundle.id +
              '" data-version="' +
              versionObj.version +
              '">' +
  ...
  document.addEventListener("click", function (e) {
    ...
    if (actionElement) {
      ...
      var bundleId =
        actionElement.dataset.bundleId ||
        (actionElement.closest("[data-bundle-id]")
          ? actionElement.closest("[data-bundle-id]").dataset.bundleId
          : null);
