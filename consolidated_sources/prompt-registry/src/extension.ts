...
export class PromptRegistryExtension {
  ...
  public async activate(): Promise<void> {
    try {
      ...
      this.logger.error(
        "Failed to activate Prompt Registry extension",
        error as Error,
      );
  ...
  public deactivate(): void {
    try {
      ...
      console.error(
        "Error during Prompt Registry extension deactivation:",
        error,
      );
    }
  }
  ...
  private async runMigrations(): Promise<void> {
    try {
      ...
      await runSourceIdNormalizationMigration(
        registryStorage,
        migrationRegistry,
      );
    } catch (error) {
  ...
  private registerCommands(): void {
    ...
    this.setupStateManager = SetupStateManager.getInstance(
      this.context,
      this.hubManager,
    );
    ...
    this.hubIntegrationCommands = new HubIntegrationCommands(
      this.hubManager,
      this.context,
    );
    ...
    const commands = [
      ...
      vscode.commands.registerCommand(
        "promptRegistry.editProfile",
        (profileId?) => this.profileCommands!.editProfile(profileId),
      ),
      vscode.commands.registerCommand(
        "promptRegistry.activateProfile",
        (profileId?) => this.profileCommands!.activateProfile(profileId),
      ),
      vscode.commands.registerCommand(
        "promptRegistry.deactivateProfile",
        (profileId?) => this.profileCommands!.deactivateProfile(profileId),
      ),
      vscode.commands.registerCommand(
        "promptRegistry.deleteProfile",
        (profileId?) => this.profileCommands!.deleteProfile(profileId),
      ),
      vscode.commands.registerCommand(
        "promptRegistry.exportProfile",
        (profileId?) => this.profileCommands!.exportProfile(profileId),
      ),
      ...
      vscode.commands.registerCommand(
        "promptRegistry.editSource",
        (sourceId?) => this.sourceCommands!.editSource(sourceId),
      ),
      vscode.commands.registerCommand(
        "promptRegistry.removeSource",
        (sourceId?) => this.sourceCommands!.removeSource(sourceId),
      ),
      vscode.commands.registerCommand(
        "promptRegistry.syncSource",
        (sourceId?) => this.sourceCommands!.syncSource(sourceId),
      ),
      vscode.commands.registerCommand("promptRegistry.syncAllSources", () =>
        this.sourceCommands!.syncAllSources(),
      ),
      vscode.commands.registerCommand(
        "promptRegistry.toggleSource",
        (sourceId?) => this.sourceCommands!.toggleSource(sourceId),
      ),
      ...
      vscode.commands.registerCommand(
        ...
        (bundleId?) => this.bundleCommands!.installBundle(bundleId),
      ),
      vscode.commands.registerCommand(
      ...
      vscode.commands.registerCommand(
        "promptRegistry.checkBundleUpdates",
        (arg?) => {
      ...
      vscode.commands.registerCommand(
        "promptRegistry.manualCheckForUpdates",
        async () => {
      ...
      vscode.commands.registerCommand(
        "promptRegistry.enableAutoUpdate",
        (arg?) =>
          this.bundleCommands!.enableAutoUpdate(this.extractBundleId(arg)),
      ),
      vscode.commands.registerCommand(
        "promptRegistry.disableAutoUpdate",
        (arg?) =>
      ...
      // Cleanup Commands
      vscode.commands.registerCommand(
        "promptRegistry.cleanupStaleLockfileEntries",
        () => this.bundleCommands!.cleanupStaleLockfileEntries(),
      ),
      ...
      vscode.commands.registerCommand(
        "promptRegistry.listCollections",
        async () => {
      ...
      vscode.commands.registerCommand(
        "promptRegistry.createCollection",
        async () => {
      ...
      vscode.commands.registerCommand(
        "promptRegistry.showCommandMenu",
        async () => {
      ...
      vscode.commands.registerCommand(
        "promptRegistry.resetFirstRun",
        async () => {
          if (!this.setupStateManager) {
            this.logger.warn("SetupStateManager not initialized");
            vscode.window.showErrorMessage(
              "Cannot reset first run state: extension not fully initialized",
            );
            return;
          }
      ...
      vscode.commands.registerCommand(
        "promptRegistry.initializeHub",
        async (options?: { resetAuth?: boolean }) => {
      ...
      vscode.commands.registerCommand("promptregistry.update", () =>
  ...
  private async registerTreeView(): Promise<void> {
    ...
    this.treeProvider = new RegistryTreeProvider(
      this.registryManager,
      this.hubManager!,
    );
    ...
    vscode.commands.executeCommand(
      "setContext",
      "promptRegistry.favoritesViewActive",
      false,
    );
    ...
    const treeCommands = [
      ...
      vscode.commands.registerCommand(
        "promptRegistry.toggleProfileView",
        () => {
          this.treeProvider?.toggleViewMode();
        },
      ),
      vscode.commands.registerCommand(
        "promptRegistry.showFavoritesView",
        () => {
          this.treeProvider?.toggleViewMode();
        },
      ),
      vscode.commands.registerCommand(
        "promptRegistry.hideFavoritesView",
        () => {
          this.treeProvider?.toggleViewMode();
        },
      ),
  ...
  private async initializeCopilot(): Promise<void> {
    try {
      ...
      this.logger.warn(
        "Failed to initialize Copilot integration",
        error as Error,
      );
  ...
  private async initializeUpdateSystem(): Promise<void> {
    try {
      ...
      this.autoUpdateService = new AutoUpdateService(
        ...
        // Source operations
        {
          ...
          syncSource: (sourceId: string) =>
            this.registryManager.syncSource(sourceId),
        },
        bundleNotifications,
        registryStorage,
      );
      ...
      this.logger.warn(
        "Failed to initialize update notification system",
        error as Error,
      );
   ...
   */
  private subscribeLockfileChanges(
    lockfileManager: LockfileManager,
  ): vscode.Disposable {
    return lockfileManager.onLockfileUpdated(() => {
      this.logger.debug(
        "Lockfile changed externally, refreshing repository bundles",
      );
  ...
  private async initializeRepositoryServices(): Promise<void> {
    try {
      this.logger.info(
        "Initializing repository-level installation services...",
      );
      ...
      if (!workspaceFolders || workspaceFolders.length === 0) {
        this.logger.debug(
          "No workspace open, skipping repository services initialization",
        );
        return;
      }
      ...
      this.disposables.push(
        this.subscribeLockfileChanges(this.lockfileManager),
      );

      // Initialize RepositoryActivationService for this workspace
      const registryStorage = this.registryManager.getStorage();
      this.repositoryActivationService =
      ...
      if (repositoryScopeService) {
        ...
        this.logger.debug(
          "BundleScopeCommands not registered: no workspace open",
        );
      }

      this.logger.info(
        "Repository-level installation services initialized successfully",
      );
    } catch (error) {
      this.logger.warn(
        "Failed to initialize repository-level installation services",
        error as Error,
      );
   ...
   */
  private async initializeRepositoryServicesForWorkspace(
    workspaceFolder: vscode.WorkspaceFolder,
  ): Promise<void> {
    try {
      const repositoryPath = workspaceFolder.uri.fsPath;
      this.logger.info(
        `Initializing repository services for workspace: ${repositoryPath}`,
      );
      ...
      this.logger.info(
        `Repository services initialized for workspace: ${repositoryPath}`,
      );
    } catch (error) {
      this.logger.warn(
        `Failed to initialize repository services for workspace: ${workspaceFolder.uri.fsPath}`,
        error as Error,
      );
    }
  }
  ...
  private async ensureSingleActiveProfile(): Promise<void> {
    try {
      ...
      this.logger.info(
        `Profile cleanup complete, only ${activeProfiles[0].name} remains active`,
      );
    } catch (error) {
      this.logger.error(
        "Failed to ensure single active profile",
        error as Error,
      );
    }
  }
  ...
  private async checkForAutomaticUpdates(): Promise<void> {
    try {
      ...
      this.logger.debug(
        "Automatic update checking disabled - use command menu instead",
      );
    } catch (error) {
      this.logger.warn(
        "Failed to perform automatic update check",
        error as Error,
      );
    }
  } /**
  ...
  private async initializeDefaultSources(): Promise<void> {
    try {
      ...
      if (existingSources.length > 0) {
        this.logger.info(
          "Sources already exist, skipping default source initialization",
        );
        return;
      }
  ...
  private async handleIncompleteSetup(): Promise<boolean> {
    ...
    if (
      isIncomplete &&
      (await this.setupStateManager!.shouldShowResumePrompt())
    ) {
  ...
  private async handleFreshInstall(): Promise<void> {
    ...
    if (state === SetupState.NOT_STARTED) {
      ...
      const installedScopes =
        await this.installationManager.getInstalledScopes();
  ...
  private async checkFirstRun(): Promise<void> {
    if (!this.setupStateManager) {
      this.logger.warn(
        "SetupStateManager not initialized, skipping first-run check",
      );
      return;
    }
    ...
    try {
      ...
      if (this.shouldSkipFirstRun()) {
        this.logger.info(
          "Test environment detected, skipping first-run dialogs",
        );
  ...
  private showResumeSetupPrompt(): void {
    if (!this.setupStateManager) {
      this.logger.warn(
        "SetupStateManager not initialized, skipping resume prompt",
      );
      return;
    }
      ...
      .then(async (action) => {
        if (action === "Complete Setup") {
          ...
          if (currentState === SetupState.COMPLETE) {
            this.logger.info(
              "Setup already completed via another path, ignoring notification click",
            );
            return;
          }
          ...
          try {
            ...
            this.logger.info(
              "Setup resumed and completed successfully via notification",
            );
          } catch (error) {
            this.logger.error(
              "Failed to complete setup during resume",
              error as Error,
            );
            await this.setupStateManager!.markIncomplete();
          }
  ...
  private async syncActiveHub(): Promise<void> {
    try {
      ...
      if (!hubManager || !sourceCommands) {
        this.logger.warn(
          "HubManager or SourceCommands not initialized, skipping hub sync",
        );
        return;
      }
      ...
      if (activeProfiles.length === 0 || !activeProfiles[0].hubId) {
        this.logger.warn(
          "Active hub has no profiles or hub ID, skipping auto-sync",
        );
        return;
      }
      ...
      sourceCommands
        ...
        .then(() => {
          this.logger.info(
            "Active hub synchronized successfully on activation",
          );
      ...
      this.logger.warn(
        "Failed to auto-sync active hub on activation",
        error as Error,
      );
  ...
  private async showFirstRunHubSelector(): Promise<void> {
    ...
    const verificationResults = await Promise.all(
      defaultHubs.map(async (hub) => {
        const isAvailable = await hubManager.verifyHubAvailability(
          hub.reference,
        );
        this.logger.debug(
    ...
    const items = verifiedHubs
      ...
      .map((hub) => ({
        ...
        description: hub.recommended
          ? hub.description + " (recommended)"
          : hub.description,
    ...
    if (selected.hubConfig && selected.hubConfig.reference) {
      ...
      try {
        ...
        this.sourceCommands!.syncAllSources({ silent: true })
          .then(() => {
          ...
          .catch((syncError) => {
            this.logger.warn(
              "Failed to sync sources after hub import",
              syncError as Error,
            );
          });
        ...
        this.logger.info(
          "Hub imported successfully. User can manually activate a profile if desired.",
        );
      ...
      this.logger.info(
        "User chose custom hub URL, redirecting to import command",
      );
