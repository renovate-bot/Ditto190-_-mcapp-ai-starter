...
export class BundleUpdateNotifications extends BaseNotificationService {
  ...
  async showUpdateNotification(
    options: BundleUpdateNotificationOptions,
  ): Promise<void> {
    if (
  ...
  async showUpdateFailure(bundleId: string, error: string): Promise<void> {
    ...
    const action = await this.showErrorWithActions(message, [
      "Retry",
      "Show Logs",
      "Dismiss",
    ]);
  ...
  async showBatchUpdateSummary(
    ...
    if (successful.length > 0) {
      ...
      parts.push(
        `✅ ${successful.length} updated: ${successfulNames.join(", ")}`,
      );
    }
    ...
    const action =
      failed.length > 0
        ? await this.showWarningWithActions(message, [
            "Show Details",
            "Dismiss",
          ])
   ...
   */
  private async buildUpdateMessage(
    updates: UpdateCheckResult[],
  ): Promise<string> {
    if (updates.length === 1) {
    ...
    switch (action) {
      case "Update Now":
        ...
        if (updates.length === 1) {
          ...
          await vscode.commands.executeCommand(
            "promptRegistry.updateAllBundles",
          );
        }
        break;
      case "View Changes":
        ...
        if (updates.length > 0 && updates[0].releaseNotes) {
          await vscode.env.openExternal(
            vscode.Uri.parse(updates[0].releaseNotes),
          );
        } else {
