...
export class BundleInstaller {
  ...
  private getScopeService(scope: InstallationScope): IScopeService {
    if (scope === "repository") {
      ...
      if (!workspaceRoot) {
        throw new Error(
          "Repository scope requires an open workspace. Please open a workspace and try again.",
        );
      }
      return ScopeServiceFactory.create(
        scope,
        this.context,
        workspaceRoot,
        this.storage,
  ...
  async installFromBuffer(
    ...
    this.logger.info(
      `Installing bundle from buffer: ${bundle.name} v${bundle.version}`,
    );
    ...
    try {
      ...
      const isSkillsBundle =
        sourceType === "skills" || sourceType === "local-skills";
      ...
      const installSkillsToCopilotDir =
        isSkillsBundle && options.scope !== "repository";
      ...
      if (installSkillsToCopilotDir) {
        ...
        const copilotScope =
          options.scope === "workspace" ? "workspace" : "user";
        ...
        if (fs.existsSync(skillSourceDir)) {
          ...
          throw new Error(
            `Skill directory not found in bundle: skills/${skillName}`,
          );
        }
      } else {
      ...
      if (!installSkillsToCopilotDir) {
        ...
        if (options.scope === "repository") {
          await this.updateLockfileOnInstall(
            bundle,
            installed,
            options,
            sourceType,
      ...
      this.logger.info(
        `Bundle installed successfully from buffer: ${bundle.name}`,
      );
      return installed;
    } catch (error) {
      this.logger.error(
        "Bundle installation from buffer failed",
        error as Error,
      );
      throw error;
    }
  ...
  async uninstall(installed: InstalledBundle): Promise<void> {
    ...
    try {
      ...
      if (installed.installPath && fs.existsSync(installed.installPath)) {
        if (
          installed.scope === "repository" &&
          this.isGitHubDirectory(installed.installPath)
        ) {
          ...
          this.logger.debug(
            `Skipping removal of .github directory: ${installed.installPath}`,
          );

          // Remove the actual bundle cache from global storage instead
          const bundleCachePath = this.getInstallDirectory(
            installed.bundleId,
            "repository",
          );
          if (
            bundleCachePath &&
            fs.existsSync(bundleCachePath) &&
            bundleCachePath !== installed.installPath
          ) {
            await this.removeDirectory(bundleCachePath);
            this.logger.debug(
              `Removed bundle cache directory: ${bundleCachePath}`,
            );
          }
        } else {
  ...
  async update(
    ...
    this.logger.info(
      `Updating bundle: ${installed.bundleId} to v${bundle.version}`,
    );
  ...
  private async updateLockfileOnInstall(
    ...
    try {
      ...
      const files = await this.collectRepositoryFileEntries(
        workspaceRoot,
  ...
  private async collectRepositoryFileEntries(
    ...
    if (!fs.existsSync(manifestPath)) {
      this.logger.warn(
        "No deployment manifest found, falling back to bundle cache files",
      );
    ...
    try {
      ...
      this.logger.warn(
        "Failed to parse manifest, falling back to bundle cache files",
        error as Error,
      );
      return this.collectFileEntries(bundlePath, workspaceRoot);
    }
  ...
  private async updateLockfileOnUninstall(bundleId: string): Promise<void> {
    ...
    try {
      ...
      this.logger.error(
        "Failed to update lockfile on uninstall",
        error as Error,
      );
  ...
  private async validateBundle(
    ...
    if (!isManifestIdMatch(manifest.id, manifest.version, bundle.id)) {
      throw new Error(
        `Bundle ID mismatch: expected ${bundle.id}, got ${manifest.id}`,
      );
    }
  ...
  private getInstallDirectory(
    ...
    if (scope === "repository") {
      ...
      this.logger.info(
        `[BundleInstaller] Installing repository scope bundle to global storage: bundles/${bundleId}`,
      );
      return path.join(
   ...
   */
  private async copyOlafSkillFolders(
    sourceDir: string,
    targetDir: string,
  ): Promise<void> {
    const files = await readdir(sourceDir);
    ...
    for (const file of files) {
      ...
      if (stats.isDirectory()) {
        ...
        this.logger.debug(
          `[BundleInstaller] Skipping file (not a skill folder): ${file}`,
        );
      }
    }
  }
   ...
   */
  private async extractSkillNameFromBundle(
    extractDir: string,
  ): Promise<string> {
  ...
  private async installMcpServers(
    ...
    try {
      ...
      if (scope === "repository") {
        ...
        if (!workspaceRoot) {
          this.logger.warn(
            `Cannot install MCP servers for repository scope: no workspace root`,
          );
          return;
        }
        ...
        if (!result.success) {
          ...
          this.logger.info(
            `Successfully installed ${result.serversInstalled} MCP servers to workspace`,
          );
        }

        if (result.warnings && result.warnings.length > 0) {
          this.logger.warn(
            `MCP installation warnings: ${result.warnings.join(", ")}`,
          );
        }
        return;
      }
      ...
      if (!result.success) {
        ...
        this.logger.info(
          `Successfully installed ${result.serversInstalled} MCP servers`,
        );
      }
  ...
  private async uninstallMcpServers(
    ...
    try {
      ...
      if (scope === "repository") {
        ...
        if (!workspaceRoot) {
          this.logger.warn(
            `Cannot uninstall MCP servers for repository scope: no workspace root`,
          );
          return;
        }

        const result = await this.mcpManager.uninstallServersFromWorkspace(
          bundleId,
          workspaceRoot,
        );
        ...
        if (!result.success) {
          ...
          this.logger.info(
            `Successfully uninstalled ${result.serversRemoved} MCP servers from workspace`,
          );
        } else {
          this.logger.debug(
      ...
      if (!result.success) {
        ...
        this.logger.info(
          `Successfully uninstalled ${result.serversRemoved} MCP servers`,
        );
      } else {
  ...
  async installLocalSkillAsSymlink(
    ...
    try {
      ...
      try {
        ...
        this.logger.warn(
          `Symlink creation failed, falling back to copy: ${symlinkError}`,
        );
      ...
      this.logger.info(
        `Local skill installed successfully as symlink: ${skillName}`,
      );
      return installed;
    } catch (error) {
      this.logger.error(
  ...
  async uninstallSkillSymlink(installed: InstalledBundle): Promise<void> {
    ...
    try {
      ...
      if (!existingEntry.exists) {
        this.logger.debug(
          `Skill path does not exist: ${installed.installPath}`,
        );
        return;
      }
