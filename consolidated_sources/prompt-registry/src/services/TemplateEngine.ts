...
export class TemplateEngine {
  ...
  async loadManifest(): Promise<TemplateManifest> {
    ...
    const manifestUri = vscode.Uri.file(
      path.join(this.templateRoot, "manifest.json"),
    );
    try {
      ...
      if (error?.code === "FileNotFound" || error?.code === "ENOENT") {
        throw new Error(
          `Template manifest not found at: ${manifestUri.fsPath}`,
        );
      }
      throw new Error(
    ...
    this.logger.debug(
      `Loaded template manifest v${this.manifestCache!.version}`,
    );
    return this.manifestCache!;
  }
  ...
  async renderTemplate(
    ...
    const templateUri = vscode.Uri.file(
      path.join(this.templateRoot, template.path),
    );
    try {
  ...
  async scaffoldProject(
    ...
    const isSkillScaffold =
      ...
      Object.values(manifest.templates).some(
  ...
  private resolveRelativePath(name: string, templatePath: string): string {
    ...
    if (
      ...
      relativePath.includes("validate-collections.js")
    ) {
      const filename = path.basename(relativePath);
