...
export class HubManager {
  ...
  async validateHub(config: HubConfig): Promise<ValidationResult> {
    // Schema validation
    const schemaResult = await this.validator.validate(
      config,
      this.hubSchemaPath,
    );
    ...
    if (this.registryManager) {
      ...
      for (const source of sources) {
        if (source.hubId === hubId) {
          try {
            ...
            this.logger.info(
              `Removed source ${source.id} linked to hub ${hubId}`,
            );
          } catch (error) {
            this.logger.warn(
              `Failed to remove source ${source.id}`,
              error as Error,
            );
          }
        }
      ...
      for (const profile of profiles) {
        if (profile.hubId === hubId && profile.active) {
          try {
            await this.registryManager.updateProfile(profile.id, {
              active: false,
            });
            this.logger.info(
  ...
  async syncHub(hubId: string): Promise<void> {
    ...
    if (!validation.valid) {
      throw new Error(
        `Hub validation failed after sync: ${validation.errors.join(", ")}`,
      );
    }
  ...
  private async getAuthenticationToken(): Promise<string | undefined> {
    ...
    if (this.authToken !== undefined) {
      this.logger.debug(
        `[HubManager] Using cached token (method: ${this.authMethod})`,
      );
  ...
  private async fetchFromUrl(
    ...
    if (redirectDepth >= MAX_REDIRECTS) {
      this.logger.error(
        `[HubManager] Maximum redirect depth (${MAX_REDIRECTS}) exceeded`,
      );
    ...
    this.logger.debug(
      `[HubManager] Request headers: ${JSON.stringify(sanitizedHeaders)}`,
    );
      ...
      protocol
        .get(url, options, (res) => {
          ...
          if (res.statusCode === 301 || res.statusCode === 302) {
            ...
            if (redirectUrl) {
              this.logger.debug(
                `[HubManager] Following redirect (depth ${redirectDepth + 1}) to: ${redirectUrl}`,
              );
              this.fetchFromUrl(redirectUrl, redirectDepth + 1)
                .then(resolve)
                .catch(reject);
              return;
            }
          ...
          if (res.statusCode !== 200) {
            ...
            reject(
              new Error(`Failed to fetch hub config: HTTP ${res.statusCode}`),
            );
            return;
          }
        ...
        .on("error", (error) => {
          this.logger.error(
            `[HubManager] Network error fetching hub config`,
            error,
          );
  ...
  async loadHubSources(hubId: string): Promise<void> {
    if (!this.registryManager) {
      this.logger.warn(
        "RegistryManager not available, skipping source loading",
      );
      return;
    }
    ...
    try {
      ...
      for (const hubSource of hubSources) {
        ...
        const duplicateSource = this.findDuplicateSource(
          hubSource,
          existingSources,
        );
        ...
        // Add new source
        this.logger.info(
          `Adding new hub source: ${sourceId} (${hubSource.name})`,
        );
      ...
      this.logger.error(
        `Failed to load sources from hub ${hubId}`,
        error as Error,
      );
      throw error;
    }
    ...
    switch (source.type) {
      case "github":
        ...
        if (githubMatch) {
          ...
          const version =
            bundle.version === "latest" ? "latest" : `v${bundle.version}`;
  ...
  async resolveProfileBundles(
    ...
    this.logger.info(
      `Resolving bundles for profile ${profileId} in hub ${hubId}`,
    );
  ...
  async activateProfile(
    ...
    try {
      ...
      const resolvedBundles = await this.resolveProfileBundles(
        hubId,
        profileId,
      );
      ...
      await this.storage.saveProfileActivationState(
        hubId,
        profileId,
        activationState,
      );
      ...
      if (options.installBundles && this.registryManager) {
        ...
        this.logger.warn(
          "Bundle installation requested but RegistryManager not available",
        );
      }
  ...
  async deactivateProfile(
    ...
    try {
      ...
      const currentState = await this.storage.getProfileActivationState(
        hubId,
        profileId,
      );
  ...
  async hasProfileChanges(hubId: string, profileId: string): Promise<boolean> {
    ...
    return (
      ...
      (changes.bundlesRemoved !== undefined &&
        changes.bundlesRemoved.length > 0) ||
      (changes.bundlesUpdated !== undefined &&
  ...
  async getProfileChanges(
    ...
    const state = await this.storage.getProfileActivationState(
      hubId,
      profileId,
    );
    if (!state) {
    ...
    const bundlesAdded = currentProfile.bundles.filter(
      (b) => !syncedBundles.includes(b.id),
    );
    const bundlesRemoved = syncedBundles.filter(
      (id) => !currentBundleIds.includes(id),
    );

    // Check for version changes using stored bundle versions
    const bundlesUpdated: Array<{
      id: string;
      oldVersion: string;
      newVersion: string;
    }> = [];
    const profileUpdated =
      new Date(currentProfile.updatedAt) > new Date(state.activatedAt);
  ...
  async getProfilesWithUpdates(hubId: string): Promise<ProfileWithUpdates[]> {
    ...
    if (activeState) {
      const changes = await this.getProfileChanges(
        hubId,
  ...
  async cleanupOrphanedFavorites(): Promise<void> {
    ...
    for (const hubId of Object.keys(favorites)) {
      if (!existingHubIds.has(hubId)) {
        this.logger.info(
          `Removing orphaned favorites for non-existent hub: ${hubId}`,
        );
        delete favorites[hubId];
        changed = true;
      }
  ...
  formatChangeSummary(changes: ProfileChanges): string {
    ...
    if (changes.bundlesUpdated && changes.bundlesUpdated.length > 0) {
      ...
      for (const update of changes.bundlesUpdated) {
        lines.push(
          `  ~ ${update.id}: ${update.oldVersion} → ${update.newVersion}`,
        );
      }
    }
