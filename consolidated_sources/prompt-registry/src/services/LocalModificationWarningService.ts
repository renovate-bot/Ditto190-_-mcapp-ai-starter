 ...
 */
export type ModificationWarningResult =
  (typeof WARNING_RESULTS)[keyof typeof WARNING_RESULTS];
...
export class LocalModificationWarningService {
  ...
  async checkForModifications(bundleId: string): Promise<ModifiedFileInfo[]> {
    try {
      ...
      this.logger.error(
        `Failed to check modifications for bundle ${bundleId}:`,
    ...
    if (choice === "Contribute Changes") {
      ...
      if (bundleRepoUrl) {
        try {
          ...
          this.logger.warn(
            `Failed to open repository URL: ${bundleRepoUrl}`,
            error instanceof Error ? error : undefined,
          );
  ...
  private buildWarningMessage(
    ...
    for (const file of modifiedFiles) {
      const typeIndicator =
        file.modificationType === "missing" ? " (missing)" : "";
