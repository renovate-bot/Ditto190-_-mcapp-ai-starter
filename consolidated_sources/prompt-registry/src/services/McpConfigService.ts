...
export class McpConfigService {
    ...
    try {
      ...
      if (errors.length > 0) {
        const errorMessages = errors
          ...
          .join(", ");
        this.logger.warn(
          `JSONC parse warnings in ${location.configPath}: ${errorMessages}`,
        );
      }
      ...
      throw new Error(
        `Failed to read MCP configuration: ${(error as Error).message}`,
      );
    }
  }
  ...
  async writeMcpConfig(
    ...
    try {
      ...
      this.logger.error(
        `Failed to write mcp.json to ${location.configPath}`,
        error as Error,
      );
      throw new Error(
        `Failed to write MCP configuration: ${(error as Error).message}`,
      );
    }
  }

  async readTrackingMetadata(
    scope: "user" | "workspace",
    ...
    try {
      ...
      throw new Error(
        `Failed to read tracking metadata: ${(error as Error).message}`,
      );
    }
  }
  ...
  async writeTrackingMetadata(
    ...
    try {
      ...
      this.logger.debug(
        `Tracking metadata written to ${location.trackingPath}`,
      );
    } catch (error) {
      this.logger.error(
      ...
      throw new Error(
        `Failed to write tracking metadata: ${(error as Error).message}`,
      );
    }
  }
  ...
  parseServerPrefix(
    prefixedName: string,
  ): { bundleId: string; serverName: string } | null {
  ...
  processServerDefinition(
    ...
    if (isRemoteServerConfig(definition)) {
      ...
      return this.processStdioServerDefinition(
        definition as McpStdioServerConfig,
        context,
      );
    }
  }
  ...
  private processStdioServerDefinition(
    ...
    return {
      ...
      args: definition.args?.map(
        (arg) => this.substituteVariables(arg, context)!,
      ),
      env: definition.env
        ? Object.fromEntries(
  ...
  async detectAndDisableDuplicates(
    ...
    duplicatesDisabled: DuplicateInfo[];
    config: McpConfiguration;
  }> {
    ...
    const serverIdentities = new Map<
      string,
      { serverName: string; bundleId: string }
    >();
  ...
  async removeServersForBundle(
    ...
    if (removedServers.length > 0) {
      ...
      this.logger.info(
        `Removed ${removedServers.length} MCP servers for bundle ${bundleId}`,
      );
    }
