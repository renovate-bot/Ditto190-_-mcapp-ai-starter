 ...
 */
const PROMPT_REGISTRY_MANAGED_DIRS = [
  "prompts",
  "agents",
  "instructions",
  "skills",
...
export class RepositoryScopeService implements IScopeService {
  ...
  private async ensureDir(dir: string): Promise<void> {
    ...
    this.logger.debug(
      `[RepositoryScopeService] Ensured directory exists: ${dir}`,
    );
  }
  ...
  async syncBundle(
    ...
    try {
      ...
      this.logger.debug(
        `[RepositoryScopeService] Workspace root: ${this.workspaceRoot}`,
      );
      ...
      if (options?.commitMode) {
        ...
        this.logger.debug(
          `[RepositoryScopeService] Using commitMode from options: ${commitMode}`,
        );
      } else {
        const installedBundle = await this.storage.getInstalledBundle(
          bundleId,
          "repository",
        );
        commitMode = installedBundle?.commitMode ?? "commit";
        this.logger.debug(
          `[RepositoryScopeService] Using commitMode from storage: ${commitMode}`,
        );
      }
      ...
      this.logger.debug(
        `[RepositoryScopeService] Looking for manifest at: ${manifestPath}`,
      );
      if (!fs.existsSync(manifestPath)) {
        this.logger.warn(
          `[RepositoryScopeService] No manifest found for bundle: ${bundleId}`,
        );
        return;
      }
      this.logger.debug(
        `[RepositoryScopeService] Manifest found, reading content...`,
      );

      const manifestContent = await readFile(manifestPath, "utf-8");
      const manifest = yaml.load(manifestContent) as DeploymentManifest;
      this.logger.debug(
        `[RepositoryScopeService] Manifest parsed. Keys: ${Object.keys(manifest).join(", ")}`,
      );
      this.logger.debug(
        `[RepositoryScopeService] manifest.prompts exists: ${!!manifest.prompts}, length: ${manifest.prompts?.length ?? "N/A"}`,
      );

      if (!manifest.prompts || manifest.prompts.length === 0) {
        this.logger.info(
          `[RepositoryScopeService] Bundle ${bundleId} has no prompts to sync`,
        );
      } else {
        this.logger.info(
          `[RepositoryScopeService] Found ${manifest.prompts.length} prompts to sync`,
        );
        for (const p of manifest.prompts) {
          this.logger.info(
            `[RepositoryScopeService]   - Prompt: id=${p.id}, file=${p.file}, type=${p.type}`,
          );
        }
      }

      // Install files (handles empty prompts array gracefully)
      const installedPaths = await this.installFiles(
        bundlePath,
        manifest,
        commitMode,
      );
      ...
      this.logger.info(
        `[RepositoryScopeService] ✅ Synced ${installedPaths.length} files for bundle: ${bundleId}`,
      );
    } catch (error) {
      this.logger.error(
        `[RepositoryScopeService] Failed to sync bundle ${bundleId}`,
        error as Error,
      );
      throw error;
    }
  ...
  private async copyBundleFiles(
    ...
    for (const promptDef of manifest.prompts || []) {
      ...
      if (promptDef.type === "skill") {
        await this.installSkillAndTrack(
          bundlePath,
          promptDef.file,
          promptId,
          tracker,
        ...
        await this.installFileAndTrack(
          bundlePath,
          promptDef,
          promptId,
          tracker,
  ...
  private async installSkillAndTrack(
    ...
    const skillPaths = await this.installSkillDirectory(
      bundlePath,
      skillFile,
      skillId,
    );
    ...
    if (skillPaths.length > 0) {
      ...
      tracker.relativePaths.push(
        ...skillPaths.map((p) => this.getRelativePath(p)),
      );
  ...
  private async installFileAndTrack(
    ...
    this.logger.debug(
      `[RepositoryScopeService] installFileAndTrack: bundlePath=${bundlePath}, file=${promptDef.file}, promptId=${promptId}`,
    );
    const sourcePath = path.join(bundlePath, promptDef.file);
    this.logger.debug(`[RepositoryScopeService] Source path: ${sourcePath}`);
    this.logger.debug(
      `[RepositoryScopeService] Source exists: ${fs.existsSync(sourcePath)}`,
    );
    if (!fs.existsSync(sourcePath)) {
      this.logger.warn(
        `[RepositoryScopeService] Source file not found: ${sourcePath}`,
      );
      return;
    }

    const fileType =
      (promptDef.type as CopilotFileType) ||
      determineFileType(promptDef.file, promptDef.tags);
    const targetPath = this.getTargetPath(fileType, promptId);
    this.logger.info(
      `[RepositoryScopeService] File type: ${fileType}, Target path: ${targetPath}`,
    );

    await this.ensureDir(path.dirname(targetPath));
    await copyFile(sourcePath, targetPath);
    this.logger.info(
      `[RepositoryScopeService] ✅ Copied: ${sourcePath} → ${targetPath}`,
    );
   ...
   */
  private async updateGitExcludeForLocalOnly(
    relativePaths: string[],
  ): Promise<void> {
    const pathsForExclude =
      this.consolidateSkillPathsForGitExclude(relativePaths);
  ...
  private async rollbackInstallation(
    ...
    this.logger.error(
      `[RepositoryScopeService] Installation failed, rolling back...`,
    );
    ...
    for (const skillDir of tracker.skillDirs) {
      try {
        if (fs.existsSync(skillDir)) {
          ...
          this.logger.debug(
            `[RepositoryScopeService] Rolled back skill directory: ${skillDir}`,
          );
        }
      } catch (rollbackError) {
        this.logger.warn(
          `[RepositoryScopeService] Failed to rollback skill directory: ${skillDir}`,
        );
      }
    }
    ...
    for (const absolutePath of tracker.absolutePaths) {
      const isInSkillDir = tracker.skillDirs.some((dir) =>
      ...
      try {
        if (fs.existsSync(absolutePath)) {
          ...
          this.logger.debug(
            `[RepositoryScopeService] Rolled back: ${absolutePath}`,
          );
        }
      } catch (rollbackError) {
        this.logger.warn(
          `[RepositoryScopeService] Failed to rollback file: ${absolutePath}`,
        );
      }
    }
  }
  ...
  private async installSkillDirectory(
    ...
    if (!skillMatch) {
      this.logger.warn(
        `[RepositoryScopeService] Invalid skill path format: ${skillFile}`,
      );
      return [];
    }
    ...
    if (!fs.existsSync(sourcePath)) {
      this.logger.warn(
        `[RepositoryScopeService] Skill directory not found: ${sourcePath}`,
      );
      return [];
    }
    ...
    if (!sourceStats.isDirectory()) {
      this.logger.warn(
        `[RepositoryScopeService] Skill path is not a directory: ${sourcePath}`,
      );
      return [];
    }
    ...
    const installedFiles = await this.copyDirectoryRecursive(
      sourcePath,
      targetDir,
    );

    this.logger.debug(
      `[RepositoryScopeService] Installed skill ${skillId}: ${installedFiles.length} files`,
    );
  ...
  private async copyDirectoryRecursive(
    ...
    for (const entry of entries) {
      ...
      if (entry.isDirectory()) {
        ...
        const subFiles = await this.copyDirectoryRecursive(
          sourcePath,
          targetPath,
        );
        ...
        this.logger.debug(
          `[RepositoryScopeService] Copied skill file: ${entry.name}`,
        );
      }
    }
  ...
  async unsyncBundle(bundleId: string): Promise<void> {
    try {
      this.logger.debug(
        `[RepositoryScopeService] Removing files for bundle: ${bundleId}`,
      );
      ...
      const bundleEntry =
        mainLockfile?.bundles[bundleId] || localLockfile?.bundles[bundleId];

      if (!bundleEntry) {
        this.logger.debug(
          `[RepositoryScopeService] Bundle ${bundleId} not found in any lockfile`,
        );
        return;
      }
      ...
      if (bundleFiles.length === 0) {
        this.logger.debug(
          `[RepositoryScopeService] Bundle ${bundleId} has no tracked files in lockfile`,
        );
        return;
      }
      ...
      for (const fileEntry of bundleFiles) {
        ...
        if (!fs.existsSync(targetPath)) {
          this.logger.debug(
            `[RepositoryScopeService] File already removed: ${fileEntry.path}`,
          );
          continue;
        }
        ...
        if (filesUsedByOtherBundles.has(fileEntry.path)) {
          skippedPaths.push({
            path: fileEntry.path,
            reason: "used by another bundle",
          });
          this.logger.debug(
            `[RepositoryScopeService] Skipping file used by another bundle: ${fileEntry.path}`,
          );
          continue;
        }
        ...
        try {
          ...
          if (currentChecksum !== fileEntry.checksum) {
            ...
            this.logger.info(
              `[RepositoryScopeService] Preserving user-modified file: ${fileEntry.path}`,
            );
            continue;
          }
        } catch (error) {
          this.logger.warn(
            `[RepositoryScopeService] Failed to calculate checksum for: ${fileEntry.path}`,
          );
          continue;
        }
        ...
        try {
          ...
          this.logger.debug(
            `[RepositoryScopeService] Removed: ${fileEntry.path}`,
          );
        } catch (error) {
          this.logger.warn(
            `[RepositoryScopeService] Failed to remove file: ${fileEntry.path}`,
          );
        }
      }
      ...
      if (removedPaths.length > 0) {
        ...
        const pathsForExclude =
          this.consolidateSkillPathsForGitExclude(removedPaths);
      ...
      if (skippedPaths.length > 0) {
        this.logger.info(
          `[RepositoryScopeService] Preserved ${skippedPaths.length} files: ${skippedPaths.map((s) => `${s.path} (${s.reason})`).join(", ")}`,
        );
      }

      this.logger.info(
        `[RepositoryScopeService] ✅ Removed ${removedPaths.length} files for bundle: ${bundleId}`,
      );
    } catch (error) {
      this.logger.error(
        `[RepositoryScopeService] Failed to unsync bundle ${bundleId}`,
        error as Error,
      );
    }
  }
  ...
  private collectFilesUsedByOtherBundles(
    ...
    mainLockfile: {
      bundles?: Record<string, { files?: Array<{ path: string }> }>;
    } | null,
    localLockfile: {
      bundles?: Record<string, { files?: Array<{ path: string }> }>;
    } | null,
    ...
    for (const subdir of PROMPT_REGISTRY_MANAGED_DIRS) {
      ...
      try {
        ...
        if (files.length === 0) {
          ...
          this.logger.debug(
            `[RepositoryScopeService] Removed empty directory: ${this.getRelativePath(subdirPath)}`,
          );
        }
      } catch (error) {
        this.logger.warn(
          `[RepositoryScopeService] Failed to check/remove directory: ${subdirPath}`,
        );
      }
    }
  }
  ...
  private async cleanupEmptySkillDirectories(skillsDir: string): Promise<void> {
    ...
    try {
      ...
      this.logger.warn(
        `[RepositoryScopeService] Failed to clean up skill directories: ${skillsDir}`,
      );
    }
  }
  ...
  private async cleanupEmptyDirectoryRecursive(dir: string): Promise<boolean> {
    ...
    try {
      ...
      if (remainingEntries.length === 0) {
        ...
        this.logger.debug(
          `[RepositoryScopeService] Removed empty directory: ${this.getRelativePath(dir)}`,
        );
        return true;
      }
      ...
      this.logger.warn(
        `[RepositoryScopeService] Failed to clean up directory: ${dir}`,
      );
      return false;
    }
  }
    ...
    for (const subdir of PROMPT_REGISTRY_MANAGED_DIRS) {
      ...
      if (fs.existsSync(subdirPath)) {
        try {
          ...
          this.logger.debug(
            `[RepositoryScopeService] Could not read directory: ${subdirPath}`,
          );
        }
      }
    }
   ...
   */
  async switchCommitMode(
    bundleId: string,
    newMode: RepositoryCommitMode,
  ): Promise<void> {
    try {
      this.logger.debug(
        `[RepositoryScopeService] Switching commit mode for ${bundleId} to ${newMode}`,
      );
      ...
      if (currentMode === newMode) {
        this.logger.debug(
          `[RepositoryScopeService] Bundle ${bundleId} already in ${newMode} mode`,
        );
        return;
      }
      ...
      const githubPromptsDir = path.join(
        this.workspaceRoot,
        ".github",
        "prompts",
      );
      ...
      const githubAgentsDir = path.join(
        this.workspaceRoot,
        ".github",
        "agents",
      );
      ...
      this.logger.debug(
        `[RepositoryScopeService] Found ${filePaths.length} files to update git exclude for`,
      );
      ...
      this.logger.info(
        `[RepositoryScopeService] ✅ Switched ${bundleId} to ${newMode} mode`,
      );
    } catch (error) {
      this.logger.error(
        `[RepositoryScopeService] Failed to switch commit mode for ${bundleId}`,
        error as Error,
      );
    }
  }
  ...
  async addLocalLockfileToGitExclude(): Promise<void> {
    if (!this.hasGitDirectory()) {
      this.logger.debug(
        "[RepositoryScopeService] No .git directory, skipping git exclude",
      );
      return;
    }
  ...
  private async addToGitExclude(paths: string[]): Promise<void> {
    if (!this.hasGitDirectory()) {
      this.logger.warn(
        "[RepositoryScopeService] No .git directory found, skipping git exclude",
      );
      return;
    }
    ...
    try {
      ...
      if (sectionIndex !== -1) {
        ...
        const afterHeaderIndex =
          sectionIndex + GIT_EXCLUDE_SECTION_HEADER.length;
        ...
        if (nextSectionMatch && nextSectionMatch.index !== undefined) {
          sectionContent = remainingContent.substring(
            0,
      ...
      this.logger.debug(
        `[RepositoryScopeService] Added ${paths.length} paths to git exclude`,
      );
    } catch (error) {
      this.logger.warn(
        `[RepositoryScopeService] Failed to update git exclude: ${error}`,
      );
  ...
  private async removeFromGitExclude(paths: string[]): Promise<void> {
    ...
    try {
      ...
      await writeFile(excludePath, newContent.trim() + "\n", "utf-8");
      this.logger.debug(
        `[RepositoryScopeService] Removed ${paths.length} paths from git exclude`,
      );
    } catch (error) {
      this.logger.warn(
        `[RepositoryScopeService] Failed to update git exclude: ${error}`,
