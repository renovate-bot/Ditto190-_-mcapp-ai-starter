...
export class ExtensionUpdateManager {
  ...
  public async updateAll(
    ...
    try {
      ...
      const scopesToUpdate = updateChecks.filter(
        (u) => u.hasUpdate || options.force,
      );
   ...
   */
  public getUpdateNotificationMessage(
  ...
  private async checkScopeForUpdate(
    ...
    try {
      ...
      const installationInfo =
        await this.installationManager.getInstallationInfo(scope);
      ...
      const currentVersionInfo =
        this.githubService.parseVersion(currentVersion);
      ...
      // Compare versions
      const hasUpdate = this.githubService.isNewerVersion(
        latestVersion,
        currentVersion,
      );
      ...
      this.logger.error(
        `Failed to check update for scope: ${scope}`,
        error as Error,
      );
      return null;
    }
  ...
  private async restoreBackup(
    ...
    try {
      ...
      this.logger.info(
        `Restoring backup for scope: ${scope} from: ${backupPath}`,
      );
    } catch (error) {
