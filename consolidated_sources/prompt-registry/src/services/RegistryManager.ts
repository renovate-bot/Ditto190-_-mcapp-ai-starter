...
export class RegistryManager {
  ...
  readonly onAutoUpdatePreferenceChanged =
    this._onAutoUpdatePreferenceChanged.event;
  ...
  async enableAutoUpdate(bundleId: string): Promise<void> {
    if (!this._autoUpdateService) {
      throw new Error(
        "Auto-update service is not available. Please restart VS Code.",
      );
    }
  ...
  static getInstance(context?: vscode.ExtensionContext): RegistryManager {
    ...
    if (!RegistryManager.instance) {
      throw new Error(
        "RegistryManager not initialized. Provide context on first call.",
      );
    }
   ...
   */
  private async refreshLocalSkillInstallations(
    sourceId: string,
    latestBundles: Bundle[],
  ): Promise<void> {
    const installedBundles = await this.storage.getInstalledBundles();
    const installsForSource = installedBundles.filter(
      (bundle) => bundle.sourceId === sourceId && bundle.scope !== "repository",
    );
    ...
    for (const installed of installsForSource) {
      const latest = latestBundles.find(
        (bundle) => bundle.id === installed.bundleId,
      );
    ...
    if (updated.length > 0) {
      ...
      this.logger.info(
        `[local-skills] Refreshed ${updated.length} installed skill(s) to latest hash`,
      );
    }
  }
  ...
  async syncSource(sourceId: string): Promise<void> {
    ...
    this.logger.info(
      `Source '${sourceId}' synced. Found ${bundles.length} bundles.`,
    );

    // Apply source-type-specific sync behavior
    if (
      source.type === "awesome-copilot" ||
      source.type === "local-awesome-copilot"
    ) {
      ...
      this.logger.info(
        `[local-skills] Refreshing installed skill metadata for source '${sourceId}'`,
      );
  ...
  private async autoUpdateInstalledBundles(
    ...
    const bundlesToUpdate = await this.identifyBundlesForUpdate(
      sourceId,
      latestBundles,
    );
    const results = await this.performBundleUpdates(
      bundlesToUpdate,
      latestBundles,
    );
  ...
  private async identifyBundlesForUpdate(
    ...
    const bundlesFromSource = this.filterBundlesBySource(
      installed,
      sourceId,
      latestBundles,
    );
  ...
  private async performBundleUpdates(
    ...
    for (const installedBundle of bundlesToUpdate) {
      try {
        const latestBundle = this.findMatchingLatestBundle(
          installedBundle,
          latestBundles,
        );
        ...
        if (!latestBundle) {
          this.logger.warn(
            `Bundle '${installedBundle.bundleId}' no longer available`,
          );
  ...
  private filterBundlesBySource(
    ...
    return installed.filter((b) =>
      this.belongsToSource(b, sourceId, latestBundles),
    );
  }
  ...
  private bundlesMatch(
    ...
    const sourceType: SourceType =
      (installed.sourceType as SourceType) ?? "local";
  ...
  async searchBundles(query: SearchQuery): Promise<Bundle[]> {
    ...
    if (query.environment) {
      results = results.filter((b) =>
        b.environments.includes(query.environment!),
      );
    }
      ...
      if (hasVersionSuffix) {
        ...
        bundle = bundles.find((b) => {
          ...
          if (source?.type === "github") {
            ...
            const sourceIdentity = VersionManager.extractBundleIdentity(
              b.id,
              "github",
            );
  ...
  async installBundle(
    ...
    const installOptions = await this.checkExistingInstallation(
      bundleId,
      bundle,
      options,
    );
    ...
    const installation = await this.downloadAndInstall(
      bundle,
      source,
      installOptions,
    );
   ...
   */
  async installBundles(
    bundles: { bundleId: string; options: InstallOptions }[],
  ): Promise<void> {
      ...
      const results = await Promise.all(
        chunk.map(async (b) => {
          try {
            ...
            this.logger.error(
              `Failed to install bundle ${b.bundleId}`,
              error as Error,
            );
            return null;
          }
  ...
  private async cleanupOldVersions(
    ...
    try {
      ...
      const baseIdentity = VersionManager.extractBundleIdentity(
        bundle.id,
        sourceType,
      );
      ...
      const oldInstallations = candidateBundles.filter((installed) => {
        const installedSourceType =
          (installed.sourceType as SourceType) || "github";
      ...
      this.logger.warn(
        `Failed to cleanup old versions: ${(error as Error).message}`,
      );
    }
  }
  ...
  private async checkExistingInstallation(
    ...
    const existing = await this.storage.getInstalledBundle(
      bundleId,
      options.scope,
    );
    ...
    if (existing.version !== bundle.version) {
      this.logger.info(
        `Version change detected: ${existing.version} → ${bundle.version}`,
      );
      return { ...options, force: true };
    }

    throw new Error(
      `Bundle '${bundleId}' is already installed. Use force=true to reinstall.`,
    );
  }
  ...
  private async resolveInstallationBundle(
    ...
    if (options.version && BundleIdentityMatcher.hasVersionSuffix(bundleId)) {
      const exactBundle = await this.tryGetExactVersionedBundle(
        bundleId,
        options.version,
      );
      if (exactBundle) {
  ...
  private async tryGetExactVersionedBundle(
    ...
    try {
      ...
      this.logger.debug(
        `Exact bundle ${bundleId} not found, trying identity-based search`,
      );
      return null;
    }
  }
  ...
  private async resolveByIdentity(
    ...
    if (options.version) {
      bundle = await this.applyVersionOverride(
        bundle,
        bundleId,
        options.version,
      );
   ...
   */
  private async determineSearchId(
    bundleId: string,
    options: InstallOptions,
  ): Promise<string> {
    if (!options.version) {
    ...
    const sources = await this.storage.getSources();
    for (const source of sources) {
      const cachedBundles = await this.storage.getCachedSourceBundles(
        source.id,
      );
  ...
  private async downloadAndInstall(
    ...
    if (source.type === "local-skills") {
      this.logger.debug(
        `Installing local skill as symlink from ${source.type} adapter`,
      );
    ...
    const installation: InstalledBundle =
      await this.installer.installFromBuffer(
        bundle,
        bundleBuffer,
        options,
        source.type,
    ...
    if (source.type === "olaf" || source.type === "local-olaf") {
      this.logger.debug(
        `Checking for post-installation hook on ${source.type} adapter`,
      );
      this.logger.debug(
      ...
      if (typeof (adapter as any).postInstall === "function") {
        this.logger.info(
          `Calling post-installation hook for ${source.type} adapter`,
        );
        try {
          await (adapter as any).postInstall(
            bundle.id,
        ...
        this.logger.warn(
          `Post-installation hook not found on ${source.type} adapter`,
        );
      }
    }
  ...
  async uninstallBundle(
    ...
    if (scope === "repository") {
      ...
      if (!workspaceRoot) {
        throw new Error(
          "Cannot uninstall repository-scoped bundle: no workspace is open",
        );
      }
    ...
    if (!installed) {
      throw new Error(
        `Bundle '${bundleId}' is not installed in ${scope} scope`,
      );
    }
    ...
    if (
      installed.sourceType === "local-skills" ||
      source?.type === "local-skills"
    ) {
    ...
    if (
      (source?.type === "olaf" || source?.type === "local-olaf") &&
      installed.installPath
    ) {
      this.logger.debug(
        `Checking for post-uninstallation hook on ${source.type} adapter`,
      );
      ...
      try {
        ...
        if (typeof (adapter as any).postUninstall === "function") {
          this.logger.info(
            `Calling post-uninstallation hook for ${source.type} adapter`,
          );
          try {
        ...
        this.logger.warn(
          `Failed to get adapter for post-uninstallation hook: ${error}`,
        );
  ...
  async updateBundle(bundleId: string, version?: string): Promise<void> {
    this.logger.info(
      `Updating bundle: ${bundleId} to version: ${version || "latest"}`,
    );
    ...
    if (updated.bundleId !== bundleId && current.scope !== "repository") {
      ...
      this.logger.debug(
        `BundleId unchanged ('${bundleId}'), old record already overwritten`,
      );
    }
  ...
  private async checkLocalModificationsBeforeUpdate(
    ...
    const bundleRepoUrl =
      current.manifest?.metadata?.repository?.url ||
      current.manifest?.metadata?.homepage;

    const warningResult = await warningService.checkAndWarn(
      bundleId,
      bundleRepoUrl,
    );
    ...
    switch (warningResult) {
      case WARNING_RESULTS.CONTRIBUTE:
        this.logger.info(
          `Update aborted for '${bundleId}': user chose to contribute changes`,
        );
      ...
      case WARNING_RESULTS.OVERRIDE:
        this.logger.info(
          `User chose to override local modifications for '${bundleId}'`,
        );
        return;
    ...
    if (scope === "repository" || scope === undefined) {
      ...
      if (workspaceRoot) {
        try {
          ...
          this.logger.warn(
            "Failed to query repository bundles from lockfile:",
      ...
      try {
        ...
        try {
          ...
          const source = sources.find(
            (s) => s.id === installedBundle?.sourceId,
          );
    ...
    if (this.hubManager) {
      try {
        ...
        const activeProfileIds = new Set(
          activeProfiles.map((ap) => ap.profileId),
        );
  ...
  async activateProfile(profileId: string): Promise<void> {
    return await vscode.window.withProgress(
      ...
      async (progress) => {
        ...
        if (this.hubManager) {
          try {
            const activeHubProfiles =
              await this.hubManager.listAllActiveProfiles();
            ...
            this.logger.error(
              "Failed to deactivate hub profiles",
              error as Error,
            );
          }
        }
        ...
        if (this.hubManager) {
          ...
          if (isHub) {
            ...
            const hubProfiles = await this.hubManager.listActiveHubProfiles();
            const hubProfile = hubProfiles.find(
              (p) => p.id === validatedProfileId,
            );
        ...
        this.logger.info(
          `Profile '${validatedProfileId}' activated successfully`,
        );
  ...
  private validateProfileId(profileId: any): string {
    if (typeof profileId !== "string") {
      ...
      if (profileObj && typeof profileObj === "object" && profileObj.id) {
        this.logger.warn(
          "Profile object passed to activateProfile, extracting ID",
        );
        return profileObj.id;
      }
      throw new Error(
        `Invalid profile identifier: expected string, got ${typeof profileId}`,
      );
    }
    return profileId;
  }
  ...
  private async installProfileBundles(
    ...
    progress.report({
      message: `Installing ${profile.bundles.length} bundle(s)...`,
    });
    this.logger.info(
      `Installing ${profile.bundles.length} bundles for profile '${profileId}'`,
    );
    ...
    this.logger.info(
      `Profile bundle installation complete: ${installed.length} installed`,
    );
  }
  ...
  private async installProfileBundle(
    ...
    const installedBundles = await this.storage.getInstalledBundles();
    const alreadyInstalled = installedBundles.find(
      (b) => b.bundleId === bundleRef.id,
    );
    ...
    this.logger.info(
      `Searching for bundle: ${bundleRef.id} v${bundleRef.version}`,
    );
    ...
    this.logger.info(
      `Found ${searchResults.length} matching bundles.`,
      searchResults,
    );
    ...
    const matchingBundle = searchResults.find((b) => {
      const idMatch =
        b.id === bundleRef.id ||
        b.name.toLowerCase().includes(bundleRef.id.toLowerCase());
    ...
    if (!source) {
      this.logger.warn(
        `Source not found for bundle: ${matchingBundle.sourceId}`,
      );
      return null;
    }
  ...
  async deactivateProfile(profileId: string): Promise<void> {
    ...
    if (this.hubManager) {
      ...
      if (isHub) {
        this.logger.info(
          `Profile ${profileId} is from hub, delegating to HubManager`,
        );
        const hubProfiles = await this.hubManager.listActiveHubProfiles();
        const hubProfile = hubProfiles.find((p) => p.id === profileId);
        if (hubProfile && hubProfile.hubId) {
          const result = await this.hubManager.deactivateProfile(
            hubProfile.hubId,
            profileId,
          );
  ...
  async importSettings(
    ...
    try {
      ...
      throw new Error(
        `Invalid ${format.toUpperCase()} format: ${error.message}`,
      );
    }
    ...
    if (!Array.isArray(settings.sources) || !Array.isArray(settings.profiles)) {
      throw new Error(
        "Invalid settings format: sources and profiles must be arrays",
      );
    }
    ...
    if (settings.configuration) {
      ...
      if (settings.configuration.defaultVersion !== undefined) {
        await config.update(
          "defaultVersion",
          settings.configuration.defaultVersion,
          true,
        );
      }
      if (settings.configuration.enableLogging !== undefined) {
        await config.update(
          "enableLogging",
          settings.configuration.enableLogging,
          true,
        );
      }
    }
  ...
  async getBundleName(bundleId: string): Promise<string> {
    try {
      ...
      installed ??= await this.storage.getInstalledBundle(
        bundleId,
        "workspace",
      );
      ...
      this.logger.debug(
        `Could not resolve bundle name for '${bundleId}': ${error}`,
      );
      return bundleId;
    }
  }
  ...
  handleWorkspaceFoldersChanged(): void {
    this.logger.info(
      "Workspace folders changed, refreshing repository bundles",
    );
