...
export class InstallationManager {
  ...
  public async installBundle(
    ...
    try {
      ...
      await this.updatePlatformConfiguration(
        platform.platform,
        scope,
        extractionPath,
      );
      ...
      this.logger.info(
        `Installation completed successfully at: ${extractionPath}`,
      );
      return result;
    } catch (error) {
  ...
  public async pruneEmptyDirs(scope: InstallationScope): Promise<void> {
    ...
    try {
      ...
      this.logger.info(
        `Files that were installed: ${installedFiles.length} files`,
      );
    } catch {
    ...
    this.logger.debug(
      `Checking ${sortedDirectories.length} directories for emptiness`,
    );
  ...
  private async removeIfEmpty(dirPath: string): Promise<void> {
    try {
      ...
      if ((error as any).code === "ENOENT") {
        ...
        this.logger.debug(
          `Failed to check/remove directory: ${dirPath}`,
          error as Error,
        );
      }
    }
   ...
   */
  private async removeEmptyParentsRecursively(
    dirPath: string,
    rootPath: string,
  ): Promise<void> {
    ...
    try {
      ...
      this.logger.debug(
        `Stopped recursive removal at: ${dirPath}`,
        error as Error,
      );
    }
  }
  ...
  public async uninstall(scope: InstallationScope): Promise<boolean> {
    try {
      ...
      try {
        ...
        this.logger.info(
          `Extraction path from metadata: ${metadata.extractionPath}`,
        );
        extractionPath = metadata.extractionPath ?? extractionPath;
      } catch {
        this.logger.warn(
          "Could not read installation metadata, removing entire directory",
        );
      }
      ...
      this.logger.info(
        `Uninstallation completed successfully from: ${installationPath}`,
      );
      return true;
    } catch (error) {
  ...
  private async readInstallationMetadata(
  ...
  private async updatePlatformConfiguration(
    ...
    try {
      ...
      switch (platform) {
        ...
        default:
          this.logger.debug(
            "No platform-specific configuration updates needed",
          );
      }
    } catch (error) {
      this.logger.warn(
