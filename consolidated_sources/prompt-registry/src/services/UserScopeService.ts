...
export class UserScopeService implements IScopeService {
  ...
  private getCopilotPromptsDirectory(): string {
    ...
    const userIndex = globalStoragePath.lastIndexOf(
      path.sep + "User" + path.sep,
    );
    ...
    const userDir = globalStoragePath.substring(
      0,
      userIndex + path.sep.length + "User".length,
    );
    ...
    if (detectedProfile) {
      this.logger.info(
        `[UserScopeService] Using profile: ${detectedProfile.name}`,
      );
    ...
    try {
      ...
      if (fs.existsSync(storageJsonPath)) {
        const storageData = JSON.parse(
          fs.readFileSync(storageJsonPath, "utf-8"),
        );
        const items =
          storageData?.lastKnownMenubarData?.menus?.Preferences?.items;

        if (Array.isArray(items)) {
          const profilesMenu = items.find(
            (i: any) => i?.id === "submenuitem.Profiles",
          );
        ...
        if (profileId) {
          this.logger.debug(
            `[UserScopeService] Profile detected from storage.json: ${profileId}`,
          );
      ...
      for (const candidateId of profiles) {
        ...
        if (fs.existsSync(profileGlobalStorage)) {
          ...
          if (ageMinutes < 5) {
            this.logger.debug(
              `[UserScopeService] Profile detected from filesystem heuristic: ${candidateId}`,
            );
  ...
  async syncBundle(
    ...
    try {
      ...
      for (const promptDef of manifest.prompts) {
        ...
        const copilotFile = this.determineCopilotFileType(
          promptDef,
          sourcePath,
          bundleId,
        );
  ...
  private async syncSkillFromBundle(
    ...
    try {
      ...
      this.logger.error(
        `Failed to sync skill from bundle ${bundleId}`,
        error as Error,
      );
    }
  }
    ...
    try {
      ...
      if (existingEntry.exists) {
        if (existingEntry.isSymbolicLink) {
          ...
          this.logger.warn(
            `File already exists (not managed): ${file.targetPath}`,
          );
          return;
        }
      }
  ...
  async unsyncBundle(bundleId: string): Promise<void> {
    try {
        ...
        if (existingEntry.exists) {
          ...
          if (existingEntry.isSymbolicLink) {
            ...
            if (existingEntry.isBroken) {
              this.logger.debug(
                `Removed broken symlink: ${path.basename(copilotFile.targetPath)}`,
              );
            } else {
              this.logger.debug(
                `Removed: ${path.basename(copilotFile.targetPath)}`,
              );
            }
            removedCount++;
          } else {
            ...
            try {
              ...
              this.logger.warn(
                `Failed to compare/remove file ${path.basename(copilotFile.targetPath)}: ${err}`,
              );
            }
          }
        }
      ...
      this.logger.info(
        `✅ Removed ${removedCount} Copilot file(s) for bundle: ${bundleId}`,
      );
    } catch (error) {
    ...
    try {
      ...
      if (removeFromClaude) {
        ...
        if (fs.existsSync(claudeTargetDir)) {
          await this.removeSkillDirectory(claudeTargetDir);
          this.logger.info(
            `✅ Also removed skill from Claude: ${claudeTargetDir}`,
          );
        }
      }
    } catch (error) {
