...
export class RepositoryActivationService {
  private static instances: Map<string, RepositoryActivationService> =
    new Map();
  ...
  static getInstance(
    ...
    if (!workspaceRoot) {
      throw new Error(
        "Workspace root path required for RepositoryActivationService.getInstance()",
      );
    }
    ...
    if (!RepositoryActivationService.instances.has(normalizedPath)) {
      if (!lockfileManager || !hubManager || !storage) {
        throw new Error(
          "Dependencies required on first call to RepositoryActivationService.getInstance() for a workspace",
        );
      }
      RepositoryActivationService.instances.set(
        normalizedPath,
        new RepositoryActivationService(
          lockfileManager,
          hubManager,
          storage,
          normalizedPath,
  ...
  static getExistingInstance(
    workspaceRoot: string,
  ): RepositoryActivationService | undefined {
    const normalizedPath = path.normalize(workspaceRoot);
    return RepositoryActivationService.instances.get(normalizedPath);
  }
  ...
  async checkAndPromptActivation(): Promise<void> {
    try {
      ...
      if (!(await this.isSetupComplete())) {
        this.logger.info(
          "First-run setup not complete, deferring source/hub detection",
        );
        return;
      }
      ...
      if (await this.wasDeclined(repositoryPath)) {
        this.logger.debug(
          `Repository ${repositoryPath} was previously declined`,
        );
        return;
      }
      ...
      this.logger.error(
        "Failed to check and detect sources:",
   ...
   */
  async installMissingBundles(
    lockfile: Lockfile,
    missingBundleIds: string[],
  ): Promise<MissingBundleInstallResult> {
    ...
    if (!this.bundleInstaller) {
      this.logger.warn(
        "Bundle installer not available, cannot install missing bundles",
      );
    ...
    return await vscode.window.withProgress(
      ...
      async (progress, token) => {
        ...
        for (const bundleId of missingBundleIds) {
          ...
          if (!bundleEntry) {
            this.logger.warn(
              `Bundle ${bundleId} not found in lockfile, skipping`,
            );
          ...
          try {
            ...
            this.logger.error(
              `Failed to install bundle ${bundleId}:`,
              error instanceof Error ? error : undefined,
            );
          }
   ...
   */
  async checkAndOfferMissingSources(
    lockfile: Lockfile,
  ): Promise<MissingSourcesResult> {
    ...
    try {
      ...
      result.missingSources = lockfileSourceIds.filter(
        (id) => !allKnownSourceIds.has(id),
      );
      ...
      if (lockfile.hubs) {
        ...
        result.missingHubs = lockfileHubIds.filter(
          (id) => !configuredHubIds.has(id),
        );
      }
      ...
      if (result.missingSources.length > 0 || result.missingHubs.length > 0) {
        const totalMissing =
          result.missingSources.length + result.missingHubs.length;
        const itemType =
          result.missingHubs.length > 0 ? "sources and hubs" : "sources";
        ...
        if (choice === "Add Sources") {
          this.logger.info(
            `User chose to add ${totalMissing} missing ${itemType}`,
          );
      ...
      this.logger.error(
        "Failed to check for missing sources/hubs:",
        error instanceof Error ? error : undefined,
      );
    }
  ...
  private async isSetupComplete(): Promise<boolean> {
    if (!this.setupStateManager) {
      this.logger.debug(
        "SetupStateManager not available, proceeding with detection",
      );
      return true;
    }
    try {
      ...
      this.logger.warn(
        "Failed to check setup completion, proceeding with detection",
        error as Error,
      );
      return true;
    }
