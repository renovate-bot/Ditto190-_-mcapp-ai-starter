...
export class AutoUpdateService {
  ...
  async autoUpdateBundle(options: AutoUpdateOptions): Promise<void> {
    ...
    try {
      ...
      this.logger.info(
        `Auto-update completed successfully for bundle '${bundleId}'`,
      );
    } catch (error) {
      const errorObj = toError(error);
      this.logger.error(
        `Auto-update failed for bundle '${bundleId}'`,
        errorObj,
      );
  ...
  async autoUpdateBundles(updates: UpdateCheckResult[]): Promise<void> {
    ...
    this.logger.info(
      `Starting batch auto-update for ${updates.length} bundles`,
    );
    ...
    const toUpdate = updates.filter((u) => {
      if (!u.autoUpdateEnabled) {
        this.logger.debug(
          `Skipping bundle '${u.bundleId}' - auto-update not enabled`,
        );
        return false;
      }
      return true;
    ...
    for (
      let i = 0;
      i < toUpdate.length;
      i += CONCURRENCY_CONSTANTS.BATCH_SIZE
    ) {
  ...
  async setAutoUpdate(bundleId: string, enabled: boolean): Promise<void> {
    this.logger.info(
      `Setting auto-update for bundle '${bundleId}' to ${enabled}`,
    );
   ...
   */
  private async captureCurrentVersion(
    bundleId: string,
  ): Promise<string | null> {
    const installedBefore = await this.bundleOps.listInstalledBundles();
    return (
      installedBefore.find((b) => b.bundleId === bundleId)?.version ?? null
    );
  }
  ...
  private async handleUpdateFailure(
    ...
    if (previousVersion) {
      try {
        ...
        this.logger.error(
          `Rollback failed for bundle '${bundleId}'`,
          rollbackErrorObj,
        );
        await this.bundleNotifications.showUpdateFailure(
          bundleId,
