...
export class UpdateScheduler {
  ...
  private readonly _onUpdatesDetected = new vscode.EventEmitter<
    UpdateCheckResult[]
  >();
  ...
  constructor(
    ...
    this.bundleNotifications = new BundleUpdateNotifications(
      bundleNameResolver,
    );
  ...
  async initialize(): Promise<void> {
    ...
    if (this.config.enabled && !this.isTestEnvironment) {
      ...
      this.logger.debug(
        "UpdateScheduler disabled by configuration, skipping timers",
      );
  ...
  private scheduleStartupCheck(): void {
    this.logger.debug(
      `Scheduling startup check in ${this.config.startupCheckDelay}ms`,
    );
  ...
  schedulePeriodicChecks(): void {
    if (this.isTestEnvironment) {
      this.logger.debug(
        "Test environment detected, skipping periodic check timers",
      );
      return;
    }
    ...
    this.scheduledCheckTimer = setTimeout(async () => {
      ...
      if (this.isCheckInProgress) {
        this.logger.warn(
          "Previous check still in progress, skipping this cycle",
        );
        this.schedulePeriodicChecks();
        return;
      }
  ...
  private async performUpdateCheck(
    ...
    try {
      ...
      this.logger.info(
        `Update check complete: ${updates.length} updates found`,
      );
      ...
      if (updates.length > 0) {
        ...
        const globalAutoUpdateEnabled = config.get<boolean>(
          "autoUpdate",
          false,
        );
        ...
        if (globalAutoUpdateEnabled && this.autoUpdateService) {
          ...
          this.logger.debug(
            "Global auto-update disabled, skipping background updates",
          );
        }
