...
export class McpServerManager {
  ...
  async installServers(
    ...
    try {
      ...
      const existingConfig = await this.configService.readMcpConfig(
        options.scope,
      );
      const tracking = await this.configService.readTrackingMetadata(
        options.scope,
      );
      ...
      if (
        ...
        !options.skipOnConflict &&
        !options.overwrite
      ) {
        result.errors?.push(
          `Conflicts detected: ${mergeResult.conflicts.join(", ")}`,
        );
      ...
      const { duplicatesDisabled, config: deduplicatedConfig } =
        await this.configService.detectAndDisableDuplicates(options.scope);
      if (duplicatesDisabled.length > 0) {
        ...
        const duplicateNames = duplicatesDisabled
          .map((d) => d.serverName)
          .join(", ");
        result.warnings?.push(
          `Disabled ${duplicatesDisabled.length} duplicate server(s): ${duplicateNames}`,
        );
        this.logger.info(
          `Disabled ${duplicatesDisabled.length} duplicate MCP servers: ${duplicateNames}`,
        );
      }
  ...
  async uninstallServers(
    ...
    try {
      ...
      const removedServers = await this.configService.removeServersForBundle(
        bundleId,
        scope,
      );
    ...
    try {
      ...
      for (const serverConfig of Object.values(config.servers)) {
        if (
          ...
          serverConfig.description?.includes("Duplicate of")
        ) {
          const match = serverConfig.description.match(
            /Duplicate of ([^\s(]+)/,
          );
      ...
      if (needsUpdate) {
        ...
        const { duplicatesDisabled, config: deduplicatedConfig } =
          await this.configService.detectAndDisableDuplicates(scope);
        await this.configService.writeMcpConfig(
          deduplicatedConfig,
          scope,
          false,
        ...
        if (duplicatesDisabled.length > 0) {
          this.logger.info(
            `Re-enabled and re-evaluated duplicates: ${duplicatesDisabled.length} still disabled`,
          );
        } else {
      ...
      this.logger.warn(
        `Failed to re-enable duplicates after removal: ${(error as Error).message}`,
      );
    }
  }
  ...
  > {
    try {
      ...
      return Object.entries(tracking.managedServers).map(
        ([serverName, metadata]) => ({
          serverName,
  ...
  async getServersForBundle(
    ...
    try {
      ...
      this.logger.error(
        `Failed to get servers for bundle ${bundleId}`,
        error as Error,
      );
      return [];
    }
   ...
   */
  private static readonly GIT_EXCLUDE_SECTION_HEADER =
  ...
  private getWorkspaceTrackingPath(workspaceRoot: string): string {
    return path.join(
      workspaceRoot,
      ".vscode",
      "prompt-registry-mcp-tracking.json",
    );
  }
   ...
   */
  private async readWorkspaceMcpConfig(
    ...
    try {
      ...
      this.logger.error(
        `Failed to read workspace mcp.json from ${configPath}`,
        error as Error,
      );
      throw new Error(
        `Failed to read workspace MCP configuration: ${(error as Error).message}`,
      );
    }
  }
  ...
  private async writeWorkspaceMcpConfig(
    ...
    try {
      ...
      this.logger.error(
        `Failed to write workspace mcp.json to ${configPath}`,
        error as Error,
      );
      throw new Error(
        `Failed to write workspace MCP configuration: ${(error as Error).message}`,
      );
    }
  }
   ...
   */
  private async readWorkspaceTrackingMetadata(
    workspaceRoot: string,
  ): Promise<McpTrackingMetadata> {
    ...
    try {
      ...
      this.logger.error(
        `Failed to read workspace tracking metadata from ${trackingPath}`,
        error as Error,
      );
      throw new Error(
        `Failed to read workspace tracking metadata: ${(error as Error).message}`,
      );
    }
  }
   ...
   */
  private async writeWorkspaceTrackingMetadata(
    workspaceRoot: string,
    ...
    try {
      ...
      this.logger.debug(
        `Workspace tracking metadata written to ${trackingPath}`,
      );
    } catch (error) {
      this.logger.error(
        `Failed to write workspace tracking metadata to ${trackingPath}`,
        error as Error,
      );
      throw new Error(
        `Failed to write workspace tracking metadata: ${(error as Error).message}`,
      );
    }
  }
  ...
  private async addToGitExclude(
    ...
    if (!this.hasGitDirectory(workspaceRoot)) {
      this.logger.warn(
        "[McpServerManager] No .git directory found, skipping git exclude",
      );
      return;
    }
    ...
    try {
      ...
      if (content.includes(pathToExclude)) {
        this.logger.debug(
          `[McpServerManager] Path already in git exclude: ${pathToExclude}`,
        );
        return;
      }
      ...
      this.logger.debug(
        `[McpServerManager] Added ${pathToExclude} to git exclude`,
      );
    } catch (error) {
      this.logger.warn(
        `[McpServerManager] Failed to update git exclude: ${error}`,
      );
   ...
   */
  private async removeFromGitExclude(
    workspaceRoot: string,
    pathToRemove: string,
  ): Promise<void> {
    if (!this.hasGitDirectory(workspaceRoot)) {
    ...
    try {
      ...
      await fs.writeFile(excludePath, newContent.trim() + "\n", "utf-8");
      this.logger.debug(
        `[McpServerManager] Removed ${pathToRemove} from git exclude`,
      );
    } catch (error) {
      this.logger.warn(
        `[McpServerManager] Failed to update git exclude: ${error}`,
  ...
  async installServersToWorkspace(
    ...
    try {
      ...
      this.logger.info(
        `Installing ${Object.keys(serversManifest).length} MCP servers for bundle ${bundleId} to workspace`,
      );
      ...
      for (const [serverName, definition] of Object.entries(serversManifest)) {
        ...
        if (existingConfig.servers[prefixedName]) {
          if (options.overwrite) {
            result.warnings?.push(
              `Overwriting existing server: ${prefixedName}`,
            );
          } else if (options.skipOnConflict) {
            result.warnings?.push(
              `Skipping conflicting server: ${prefixedName}`,
            );
            continue;
          } else {
      ...
      await this.writeWorkspaceMcpConfig(
        workspaceRoot,
        mergedConfig,
        options.createBackup !== false,
      );
      ...
      this.logger.info(
        `Successfully installed ${result.serversInstalled} MCP servers for bundle ${bundleId} to workspace`,
      );
    } catch (error) {
      this.logger.error(
        `Failed to install MCP servers for bundle ${bundleId} to workspace`,
        error as Error,
      );
      result.errors?.push((error as Error).message);
      result.success = false;
  ...
  async uninstallServersFromWorkspace(
    ...
    try {
      this.logger.info(
        `Uninstalling MCP servers for bundle ${bundleId} from workspace`,
      );
      ...
      for (const [serverName, metadata] of Object.entries(
        tracking.managedServers,
      )) {
      ...
      if (removedServers.length > 0) {
        ...
        await this.writeWorkspaceTrackingMetadata(workspaceRoot, tracking);
        this.logger.info(
          `Removed ${removedServers.length} MCP servers for bundle ${bundleId} from workspace`,
        );
      } else {
        this.logger.debug(
          `No MCP servers found for bundle ${bundleId} in workspace`,
        );
      }

      // Check if we should clean up git exclude
      // Only remove from git exclude if no more managed servers exist
      const hasRemainingManagedServers =
        Object.keys(tracking.managedServers).length > 0;
      ...
      this.logger.error(
        `Failed to uninstall MCP servers for bundle ${bundleId} from workspace`,
        error as Error,
      );
      result.errors?.push((error as Error).message);
      result.success = false;
   ...
   * @param workspaceRoot - Path to workspace root
   */
  async getServersForBundleInWorkspace(
    bundleId: string,
    workspaceRoot: string,
  ): Promise<string[]> {
    try {
      ...
      this.logger.error(
        `Failed to get servers for bundle ${bundleId} in workspace`,
        error as Error,
      );
      return [];
    }
