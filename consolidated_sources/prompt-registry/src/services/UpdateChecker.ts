...
export class UpdateChecker {
  ...
  async checkForUpdates(
    ...
    if (!isBundleUpdateArray(updates)) {
      this.logger.error(
        "RegistryManager.checkUpdates() returned invalid data structure",
      );
  ...
  private async enrichSingleUpdate(update: {
    ...
    try {
      ...
      const bundleDetails = await this.registryManager.getBundleDetails(
        update.bundleId,
      );

      // Get auto-update preference
      const autoUpdateEnabled = await this.storage.getUpdatePreference(
        update.bundleId,
      );
      ...
      switch (errorType) {
        case "network":
          ...
          return {
            skipped: { bundleId: update.bundleId, reason: "network error" },
          };
        case "notfound":
          this.logger.debug(
          ...
          return {
            skipped: { bundleId: update.bundleId, reason: "not found" },
          };
        case "authentication":
          this.logger.debug(
            `Authentication error enriching '${update.bundleId}'`,
            err,
          );
          return {
            skipped: {
        ...
        default:
          ...
          this.logger.error(
            `Unexpected error enriching '${update.bundleId}'`,
            err,
          );
  ...
  private async syncGitHubReleaseSources(): Promise<void> {
    ...
    try {
      ...
      for (const source of githubSources) {
        try {
          this.logger.debug(
            `Syncing GitHub source: ${source.id} (${source.name})`,
          );
