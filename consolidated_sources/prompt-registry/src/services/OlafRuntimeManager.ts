...
export class OlafRuntimeManager {
  ...
  private runtimeStatusCache: Map<
    string,
    { info: OlafRuntimeInfo; timestamp: number }
  > = new Map();
  ...
  private detectIDE(): "vscode" | "kiro" | "windsurf" {
    ...
    if (executablePath.includes("windsurf")) {
      this.logger.debug(
        "[OlafRuntime] IDE detected via executable path: Windsurf",
      );
      return "windsurf";
    }
    ...
    try {
      ...
      if (appName?.includes("windsurf")) {
        this.logger.debug(
          "[OlafRuntime] IDE detected via VSCode API: Windsurf",
        );
        return "windsurf";
      }
    } catch (error) {
      this.logger.warn(
        "[OlafRuntime] Failed to detect IDE via VSCode API",
        error as Error,
      );
    }
  ...
  private getUserRuntimePath(version: string): string {
    if (!this.context) {
      throw new Error(
        "OlafRuntimeManager not initialized. Call initialize() first.",
      );
    }
  ...
  isRuntimeInstalled(version: string = "latest"): boolean {
    try {
      ...
      this.logger.error(
        `[OlafRuntime] Error checking runtime installation: ${error}`,
      );
      return false;
    }
  }
  ...
  async getRuntimeInfo(
    ...
    if (
      !forceRefresh &&
      cached &&
      Date.now() - cached.timestamp < RUNTIME_STATUS_CACHE_TTL
    ) {
      return cached.info;
    }
    ...
    const info: OlafRuntimeInfo = {
      ...
      isInstalled,
      ideType,
      installedAt: isInstalled
        ? await this.getInstallationTimestamp(installPath)
        : undefined,
    };
   ...
   */
  private async getInstallationTimestamp(
    installPath: string,
  ...
  async ensureRuntimeInstalled(workspacePath?: string): Promise<boolean> {
    ...
    this.logger.info(
      `[OlafRuntime] Runtime v${version} not found, installing...`,
    );
    ...
    try {
      ...
      if (updatedInfo.isInstalled) {
        this.logger.info(
          `[OlafRuntime] Runtime v${version} installed successfully`,
        );
        return true;
      } else {
  ...
  async installRuntime(version: string = "latest"): Promise<void> {
    ...
    const ideType = this.detectIDE();
    this.logger.info(
      `[OlafRuntime] Installing runtime v${version} for ${ideType}`,
    );
    ...
    this.logger.info(
      `[OlafRuntime] Runtime v${version} installed to ${runtimePath}`,
    );
  }
   ...
   */
  private async downloadRuntimeBundle(
    ide: string,
    version: string,
  ): Promise<Buffer> {
    ...
    try {
      ...
      this.logger.error(
        `[OlafRuntime] Failed to fetch release information: ${error}`,
      );
      throw error;
    }
    ...
    this.logger.info(
      `[OlafRuntime] Looking for IDE-specific asset: ${assetName}`,
    );
    ...
    if (!asset) {
      ...
      const commonAsset = releaseInfo.assets?.find(
        (a: any) => a.name === commonAssetName,
      );
        ...
        .get(url, (response) => {
          ...
          response.on("data", (chunk: Buffer) => {
            ...
            if (downloadedBytes > MAX_DOWNLOAD_SIZE) {
              ...
              reject(
                new Error(
                  `Download size exceeded limit: ${downloadedBytes} bytes`,
                ),
              );
              return;
            }
          ...
          response.on("end", () => {
            ...
            this.logger.debug(
              `[OlafRuntime] Downloaded ${buffer.length} bytes`,
            );
            resolve(buffer);
          });
  ...
  private async extractRuntime(
    ...
    try {
      ...
      if (!fs.existsSync(idePath)) {
        this.logger.warn(
          `[OlafRuntime] IDE-specific directory not found: ${idePath}`,
        );
  ...
  async createWorkspaceLinks(
    ...
    if (!runtimeInfo.isInstalled) {
      throw new Error(
        `Runtime v${version} is not installed. Install it first.`,
      );
    }

    const runtimePath = this.getUserRuntimePath(version);
    const workspaceOlafPath = path.join(workspacePath, ".olaf");
    const workspaceIdePath = path.join(
      workspacePath,
      this.getIdeSpecificFolderName(),
    );

    this.logger.info(
      `[OlafRuntime] Creating workspace links in ${workspacePath}`,
    );
    ...
    try {
      ...
      this.logger.error(
        `[OlafRuntime] Failed to create workspace links: ${error}`,
      );
      throw error;
    }
  }
   ...
   */
  private async createSymbolicLink(
    source: string,
    target: string,
  ): Promise<void> {
    try {
      ...
      if (existingEntry.exists) {
        if (existingEntry.isSymbolicLink) {
          ...
          this.logger.debug(
            `[OlafRuntime] Removing existing symbolic link: ${target}`,
          );
      ...
      this.logger.debug(
        `[OlafRuntime] Created symbolic link: ${target} -> ${source}`,
      );
    } catch (error) {
  ...
  private async handleExistingPath(target: string): Promise<void> {
    ...
    if (stats.isDirectory()) {
      ...
      await fs.promises.rename(target, backupPath);
      this.logger.warn(
        `[OlafRuntime] Backed up existing file: ${target} -> ${backupPath}`,
      );
    }
  }
  ...
  private async fallbackToCopy(source: string, target: string): Promise<void> {
    try {
      ...
      throw new Error(
        `Fallback directory copy failed: ${(error as Error).message}`,
      );
    }
  }
  ...
  async removeWorkspaceLinks(workspacePath: string): Promise<void> {
    ...
    try {
      ...
      if (olafEntry.exists) {
        if (olafEntry.isSymbolicLink) {
          ...
          if (olafEntry.isBroken) {
            this.logger.debug(
              `[OlafRuntime] Removed broken symbolic link: ${olafPath}`,
            );
          } else {
            this.logger.debug(
              `[OlafRuntime] Removed symbolic link: ${olafPath}`,
            );
          }
        } else {
          ...
          this.logger.debug(
            `[OlafRuntime] Removed copied directory: ${olafPath}`,
          );
        }
      }
      ...
      if (ideEntry.exists) {
        if (ideEntry.isSymbolicLink) {
          ...
          if (ideEntry.isBroken) {
            this.logger.debug(
              `[OlafRuntime] Removed broken symbolic link: ${idePath}`,
            );
          } else {
            this.logger.debug(
              `[OlafRuntime] Removed symbolic link: ${idePath}`,
            );
          }
        } else {
          // If it's a copied directory, remove it
          ...
          this.logger.debug(
            `[OlafRuntime] Removed copied directory: ${idePath}`,
          );
        }
      }
      ...
      this.logger.info(
        `[OlafRuntime] Workspace links removed from ${workspacePath}`,
      );
    } catch (error) {
      this.logger.error(
        `[OlafRuntime] Failed to remove workspace links: ${error}`,
      );
      throw error;
    }
  }
