...
export class GitHubService {
  ...
  private constructor() {
    ...
    this.baseUrl =
      config.get<string>("githubApiUrl") || "https://api.github.com";
  ...
  private async getAuthHeaders(): Promise<Record<string, string>> {
    ...
    if (shouldUseAuth) {
      ...
      if (token) {
        ...
        throw new Error(
          "No GitHub token available for private repository access",
        );
      }
    }
  ...
  public async validateAccess(): Promise<{ valid: boolean; message: string }> {
    try {
      ...
      if (status === 401) {
        ...
        message +=
          "3. Repository is not private (or you have appropriate permissions)";
      } else {
  ...
  public findPlatformBundle(
    ...
    const bundlePattern = new RegExp(
      `^${escapedPrefix}-installation-bundle-(.*)\\.zip$`,
      "i",
    );
    ...
    this.logger.warn(
      `No bundle found for platform ${platform} in release ${release.tag_name}`,
    );
    return null;
  }
  ...
  public getAllBundles(release: GitHubRelease): BundleInfo[] {
    ...
    this.logger.debug(
      `Found ${bundles.length} bundles in release ${release.tag_name}`,
    );
    return bundles;
  }
  ...
  public async downloadBundle(
    ...
    try {
      ...
      const downloadUrl = await this.getAssetDirectDownloadUrl(
        bundleInfo.asset.id,
      );
      ...
      this.logger.error(
        `Failed to download bundle ${bundleInfo.filename}`,
        error as Error,
      );
   ...
   */
  public async getReleaseByVersionPreference(
    version?: string,
   ...
   */
  private async getReleaseByTagWithFallback(
    version: string,
  ): Promise<GitHubRelease> {
