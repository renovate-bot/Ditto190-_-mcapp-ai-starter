...
export class ScopeConflictResolver {
  ...
  private readonly ALL_SCOPES: InstallationScope[] = [
    "user",
    "workspace",
    "repository",
  ];
   ...
   */
  private async getInstalledBundleForScope(
    bundleId: string,
  ...
  async checkConflict(
    ...
    this.logger.debug(
      `[ScopeConflictResolver] Checking conflict for bundle ${bundleId} at scope ${targetScope}`,
    );
    ...
    for (const scope of this.ALL_SCOPES) {
      ...
      const installedBundle = await this.getInstalledBundleForScope(
        bundleId,
        scope,
      );
      ...
      if (installedBundle) {
        this.logger.info(
          `[ScopeConflictResolver] Conflict detected: bundle ${bundleId} exists at ${scope}, target is ${targetScope}`,
        );
    ...
    this.logger.debug(
      `[ScopeConflictResolver] No conflict for bundle ${bundleId}`,
    );
    return null;
  }
  ...
  async migrateBundle(
    ...
    this.logger.info(
      `[ScopeConflictResolver] Migrating bundle ${bundleId} from ${fromScope} to ${toScope}`,
    );
    ...
    try {
      ...
      this.logger.debug(
        `[ScopeConflictResolver] Uninstalling bundle ${bundleId} from ${fromScope}`,
      );
      try {
      ...
      this.logger.debug(
        `[ScopeConflictResolver] Installing bundle ${bundleId} at ${toScope}`,
      );
      try {
        ...
        this.logger.error(
          `[ScopeConflictResolver] Failed to install at ${toScope}: ${installErrorMessage}`,
        );

        // Step 4: Attempt rollback - re-install at original scope
        result.rollbackAttempted = true;
        this.logger.info(
          `[ScopeConflictResolver] Attempting rollback: re-installing bundle ${bundleId} at ${fromScope}`,
        );
        ...
        try {
          ...
          this.logger.info(
            `[ScopeConflictResolver] Rollback successful: bundle ${bundleId} restored at ${fromScope}`,
          );
        } catch (rollbackError) {
          ...
          this.logger.error(
            `[ScopeConflictResolver] Rollback failed for bundle ${bundleId}: ${rollbackErrorMessage}`,
          );
        }
      ...
      result.success = true;
      this.logger.info(
        `[ScopeConflictResolver] Successfully migrated bundle ${bundleId} from ${fromScope} to ${toScope}`,
      );
      return result;
    } catch (error) {
