...
export class UpdateCache {
  ...
  constructor(private storage: vscode.Memento) {
    ...
    this.defaultTTL = config.get<number>(
      "cacheTTL",
      CACHE_CONSTANTS.DEFAULT_TTL_MS,
    );
  }
  ...
  async get(): Promise<UpdateCheckResult[] | null> {
    const cached = this.storage.get<CachedUpdateResult>(
      CACHE_CONSTANTS.CACHE_KEY,
    );
  ...
  isValid(cached?: CachedUpdateResult): boolean {
    const entry =
      cached ?? this.storage.get<CachedUpdateResult>(CACHE_CONSTANTS.CACHE_KEY);
    if (!entry) {
      return false;
    }
