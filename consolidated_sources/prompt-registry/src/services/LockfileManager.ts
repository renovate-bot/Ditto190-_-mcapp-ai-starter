...
export class LockfileManager {
  ...
  static getInstance(repositoryPath?: string): LockfileManager {
    if (!repositoryPath) {
      throw new Error(
        "Repository path required for LockfileManager.getInstance()",
      );
    }
    ...
    if (!LockfileManager.instances.has(normalizedPath)) {
      LockfileManager.instances.set(
        normalizedPath,
        new LockfileManager(normalizedPath),
      );
    }
  ...
  private setupFileWatcher(): void {
    try {
      ...
      this.logger.warn(
        "Failed to set up lockfile watcher:",
  ...
  private getLockfilePathForMode(commitMode: RepositoryCommitMode): string {
    const filename =
      commitMode === "local-only" ? LOCAL_LOCKFILE_NAME : LOCKFILE_NAME;
   ...
   */
  private async readLockfileByMode(
    commitMode: RepositoryCommitMode,
    ...
    try {
      ...
      this.logger.error(
        `Failed to read ${commitMode} lockfile:`,
        error instanceof Error ? error : undefined,
      );
      return null;
    }
  ...
  async createOrUpdate(options: CreateOrUpdateOptions): Promise<void> {
    ...
    if (
      !options.commitMode ||
      !["commit", "local-only"].includes(options.commitMode)
    ) {
    ...
    const localLockfileExistedBefore =
      isLocalLockfile && fs.existsSync(targetLockfilePath);
  ...
  async remove(bundleId: string): Promise<void> {
    ...
    if (localLockfile && localLockfile.bundles[bundleId]) {
      await this.removeFromLockfileByMode(
        bundleId,
        localLockfile,
        "local-only",
      );
  ...
  private async deleteLockfileAtPath(lockfilePath: string): Promise<void> {
    try {
      ...
      this.logger.error(
        `Failed to delete lockfile at ${lockfilePath}:`,
        error instanceof Error ? error : undefined,
      );
    }
  }
  ...
  private async removeLocalLockfileFromGitExclude(): Promise<void> {
    try {
      ...
      if (!fs.existsSync(gitDir)) {
        this.logger.debug(
          "No .git directory found, skipping git exclude removal for local lockfile",
        );
        return;
      }
      ...
      if (!fs.existsSync(excludePath)) {
        this.logger.debug(
          ".git/info/exclude does not exist, nothing to remove",
        );
        return;
      }
      ...
      this.logger.warn(
        "Failed to remove local lockfile from git exclude:",
        error instanceof Error ? error : undefined,
      );
    }
  }
   ...
   */
  private cleanupOrphanedSources(
    lockfile: Lockfile,
    removedSourceId: string,
  ): void {
    // Check if any other bundle references this source
    const isSourceReferenced = Object.values(lockfile.bundles).some(
      (bundle) => bundle.sourceId === removedSourceId,
    );
  ...
  async detectModifiedFiles(bundleId: string): Promise<ModifiedFileInfo[]> {
    ...
    for (const fileEntry of bundleEntry.files) {
      ...
      try {
        ...
        this.logger.warn(
          `Failed to check file ${fileEntry.path}:`,
          error instanceof Error ? error : undefined,
        );
        modifiedFiles.push({
          path: fileEntry.path,
   ...
   */
  private async writeAtomicToPath(
    lockfile: Lockfile,
    targetPath: string,
  ): Promise<void> {
  ...
  private async ensureLocalLockfileExcluded(): Promise<void> {
    try {
      ...
      if (!fs.existsSync(gitDir)) {
        this.logger.debug(
          "No .git directory found, skipping git exclude for local lockfile",
        );
        return;
      }
      ...
      // Log but don't throw - git exclude is optional
      this.logger.warn(
        "Failed to add local lockfile to git exclude:",
        error instanceof Error ? error : undefined,
      );
    }
  }
   ...
   */
  async updateCommitMode(
    bundleId: string,
    newMode: RepositoryCommitMode,
  ): Promise<void> {
    // Determine source lockfile (opposite of newMode)
    const currentMode: RepositoryCommitMode =
      newMode === "commit" ? "local-only" : "commit";
    ...
    if (!sourceLockfile) {
      throw new Error(
        `Bundle ${bundleId} not found in ${currentMode} lockfile`,
      );
    }
  ...
  async getInstalledBundles(): Promise<InstalledBundle[]> {
    ...
    if (mainLockfile) {
      for (const [bundleId, entry] of Object.entries(mainLockfile.bundles)) {
        ...
        const installedBundle = createInstalledBundleFromLockfile(
          bundleId,
          entry,
          {
    ...
    if (localLockfile) {
      for (const [bundleId, entry] of Object.entries(localLockfile.bundles)) {
        if (seenIds.has(bundleId)) {
          ...
          this.logger.error(
            `Bundle ${bundleId} exists in both lockfiles - manual resolution required`,
          );
   ...
   */
  private async checkFilesMissing(
