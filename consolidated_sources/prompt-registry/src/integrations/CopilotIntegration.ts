...
export class CopilotIntegration implements vscode.Disposable {
  ...
  async activate(): Promise<void> {
    try {
      ...
      this.logger.info(
        "Copilot integration activated: @prompts participant registered",
      );
    } catch (error) {
      this.logger.error(
        "Failed to activate Copilot integration",
        error as Error,
      );
  ...
  private async handleRequest(
    ...
    try {
      ...
      this.logger.debug(
        `Chat request: command=${command}, prompt="${userInput}"`,
      );
  ...
  private async executePrompt(
    ...
    stream.markdown(
      `- First 200 chars: ${promptContent.content.substring(0, 200)}...\n\n`,
    );
