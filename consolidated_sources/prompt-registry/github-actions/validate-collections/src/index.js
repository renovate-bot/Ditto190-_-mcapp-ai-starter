...
function validateCollection(filePath, projectRoot) {
  ...
  try {
    ...
    return {
      valid: false,
      errors: [
        { file: fileName, message: `Unexpected error: ${error.message}` },
      ],
      warnings: [],
    };
  }
...
function main() {
  console.log(
    `${colors.cyan}${colors.bold}📋 Collection Validation${colors.reset}\n`,
  );
  console.log(
    `${colors.cyan}Attribution: Inspired by github/awesome-copilot${colors.reset}`,
  );
  console.log(
    `${colors.cyan}https://github.com/github/awesome-copilot${colors.reset}\n`,
  );
  ...
  if (!fs.existsSync(collectionsDir)) {
    console.error(
      `${colors.red}❌ Error: Collections directory not found: ${collectionsDir}${colors.reset}`,
    );
    process.exit(1);
  }
  ...
  if (files.length === 0) {
    console.log(
      `${colors.yellow}⚠️  No collection files found in ${collectionsDir}${colors.reset}`,
    );
    process.exit(0);
  }
    ...
    if (result.errors.length === 0 && result.warnings.length === 0) {
      ...
      if (result.warnings.length > 0) {
        result.warnings.forEach((warn) => {
          console.log(
            `  ${colors.yellow}⚠ Warning: ${warn.message}${colors.reset}`,
          );
        });
  ...
  if (totalWarnings > 0) {
    ...
    console.log(
      `${colors.green}Total Warnings: ${totalWarnings}${colors.reset}`,
    );
  }
