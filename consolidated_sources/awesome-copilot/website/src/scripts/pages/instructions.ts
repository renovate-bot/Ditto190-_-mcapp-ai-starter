...
function renderItems(items: Instruction[], query = ""): void {
  ...
  if (items.length === 0) {
    list.innerHTML =
      '<div class="empty-state"><h3>No instructions found</h3><p>Try a different search term or adjust filters</p></div>';
    return;
  }
        ...
        <div class="resource-meta">
          ...
          ${
            item.extensions
              ...
              .map(
                (e) =>
                  `<span class="resource-tag tag-extension">${escapeHtml(e)}</span>`,
              )
              .join("") || ""
          }
